...
namespace RTi.GR
{
	...
	/// </summary>
	internal class JpegEncoder_DCT
	{
	/// <summary>
	...
	/// </summary>
	/// <param name="input"> the input data on which to have the DCT performed. </param>
	public virtual double[][] forwardDCT(float[][] input)
	{
//JAVA TO C# CONVERTER NOTE: The following call to the 'RectangularArrays' helper class reproduces the rectangular array initialization that is automatic in Java:
//ORIGINAL LINE: double[][] output = new double[N][N];
		double[][] output = RectangularArrays.RectangularDoubleArray(N, N);
...
//ORIGINAL LINE: double[][] output = new double[N][N];
	...
	/// </summary>
	/// <param name="inputData"> the data to be quantized </param>
	...
	public virtual int[] quantizeBlock(double[][] inputData, int code)
	{
		...
		for (i = 0; i < 8; i++)
		{
			...
			// The second line results in significantly better compression.
				outputData[index] = (int)((long)Math.Round(inputData[i][j] * (((double[])(divisors[code]))[index]), MidpointRounding.AwayFromZero));
	...
	public virtual int[] quantizeBlockExtreme(double[][] inputData, int code)
	{
		...
		for (i = 0; i < 8; i++)
		{
			for (j = 0; j < 8; j++)
			{
				outputData[index] = (int)((long)Math.Round(inputData[i][j] / (double)(((int[])(quantum[code]))[index]), MidpointRounding.AwayFromZero));
				index++;
			}
		}
