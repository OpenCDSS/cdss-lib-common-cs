...
// StringFormatterSpecifiersJPanel - panel to provide editing capabilities to construct a format specifier string
...
//ORIGINAL LINE: @SuppressWarnings("serial") public class StringFormatterSpecifiersJPanel extends javax.swing.JPanel implements java.awt.event.ItemListener
	public class StringFormatterSpecifiersJPanel : JPanel, ItemListener
	{
	/// <summary>
	/// Hint to aid user.
	/// </summary>
	...
	/// </summary>
	internal SimpleJComboBox __formatterTypeJComboBox = null;

	/// <summary>
	/// Choices for the list of format specifiers.
	/// </summary>
	internal SimpleJComboBox __specifierJComboBox = null;

	/// <summary>
	/// Default formatter type for use with blank formatter type choice.
	/// </summary>
	internal StringFormatterType __defaultFormatter = StringFormatterType.C;
	...
	public StringFormatterSpecifiersJPanel(int width, bool includeFormatterType, bool includeBlankFormatterType, StringFormatterType defaultFormatter)
	{
		if (defaultFormatter == null)
		{
			defaultFormatter = StringFormatterType.C;
		}
		...
		if (includeFormatterType)
		{
			...
			__formatterTypeJComboBox.setPrototypeDisplayValue("" + StringFormatterType.C);
			IList<string> choicesList = new List<string>();
			if (includeBlankFormatterType)
			{
				choicesList.Add("");
			}
			choicesList.Add("" + StringFormatterType.C);
			// TODO SAM 2013-05-13 Need to add other formatter types
	...
	/// <summary>
	/// Return the StringFormatterType that is in effect for the format string.  This can be used, for example, to
	...
	public virtual StringFormatterType getStringFormatterType(bool onlyIfVisible)
	{
		if (onlyIfVisible)
		{
			...
			else
			{
				...
				return StringFormatterType.valueOfIgnoreCase(__formatterTypeJComboBox.getSelected());
			}
		}
		else
		{
			...
			if ((__formatterTypeJComboBox == null) || __formatterTypeJComboBox.getSelected().Equals(""))
			{
				return StringFormatterType.C; // Default when not visible
			}
			else
			{
				// Get formatter that corresponds to what is shown
	...
	public virtual string getText(bool includeFormatterType, bool onlyIfVisible)
	{
		if (includeFormatterType)
		{
			...
			StringFormatterType t = getStringFormatterType(onlyIfVisible);
			if (t == null)
			{
	...
	private void populateFormatSpecifiers()
	{
		...
		StringFormatterType formatterType = null;
		try
		{
			formatterType = StringFormatterType.valueOfIgnoreCase(selectedFormatterType);
		}
		...
		else if (formatterType == StringFormatterType.C)
		{
			// Only want formats that can be parsed...
			choicesList.Add(__hint);
			((IList<string>)choicesList).AddRange(Arrays.asList(StringUtil.getStringFormatSpecifiers(true,false)));
		}
		__specifierJComboBox.setData(choicesList);
		__specifierJComboBox.setMaximumRowCount(choicesList.Count);
	}

	/// <summary>
	/// Select the formatter type.  Select the empty string if formatterType=null.
	/// </summary>
	public virtual void selectFormatterType(StringFormatterType formatterType)
	{
		if (formatterType == null)
		{
