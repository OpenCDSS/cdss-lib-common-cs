...
{
	using BinConverter = Blowfish.BinConverter;
	using BlowfishCBC = Blowfish.BlowfishCBC;
	...
	/// </summary>
	/// <param name="prefix"> two-character code that specifies the cipher package and the
	/// seeds and passwords to use. </param>
	...
	/// </para>
	/// with Blowfish will have lengths that are multiples of 8.<para>
	...
	/// that tells which passwords and seeds to use for decryption. </param>
	/// <returns> a string decrypted from the hex-value string passed in </returns>
	public virtual string decrypt(string val)
	{
		switch (__cipherPackage)
		{
			case BLOWFISH:
				...
				length = BinConverter.binHexToBytes(val, bval, 0, 0, size);
				...
				if (rest > 0)
				{
					...
					messBuff = new sbyte[length + (8 - rest)];
					Array.Copy(bval, 0, messBuff, 0, length);

					for (int nI = length; nI < messBuff.Length ; nI++)
					{
						messBuff[nI] = 0x20;
					}
				}
				...
				__bfcbc.setCBCIV(RTiCryptoData.getBlowfishCBCIV(prefix));
	...
	/// </para>
	/// </summary>
	/// <param name="val"> the string to be encrypted </param>
	/// <returns> a hex-value string representing the encrypted string.  This string
	/// is a way of representing a byte array of values, and each element of the
	/// byte array is represented by a 2-character string that is the hex value
	/// for that element.  Thus, byte[]=[09][16][255] becomes String="090FFF"
	/// Returns "Invalid Cipher" if the 2 character cipher code isn't supported. </returns>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public String encrypt(String val) throws java.io.UnsupportedEncodingException
	public virtual string encrypt(string val)
	{
		switch (__cipherPackage)
		{
			case BLOWFISH:
				...
				sbyte[] bval = val.GetBytes(Encoding.UTF8);
				...
				// used for getting the current CBC Cipher seed 
				sbyte[] showIV = new sbyte[BlowfishCBC.BLOCKSIZE];
				...
				return (__prefix + BinConverter.bytesToBinHex(messBuff));
		}
...
//		base.finalize();
	...
	{
		__bfcbc = new BlowfishCBC(RTiCryptoData.getBlowfishPassword(prefix), RTiCryptoData.getBlowfishCBCIV(prefix));
	}
	...
	/// equal to the encrypted String. </summary>
	/// <param name="u"> the unencrypted string to check. </param>
	/// <param name="e"> the encrypted string to check. </param>
	/// <returns> true if the two match, false if not. </returns>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public boolean validateString(String u, String e) throws Exception
	public virtual bool validateString(string u, string e)
	{
		string r = encrypt(u);
