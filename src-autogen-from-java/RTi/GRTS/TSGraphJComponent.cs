...
//ORIGINAL LINE: @SuppressWarnings("serial") public class TSGraphJComponent extends RTi.GR.GRJComponentDevice implements java.awt.event.KeyListener, java.awt.event.MouseListener, java.awt.event.MouseMotionListener, java.awt.print.Printable, TSViewListener
	public class TSGraphJComponent : GRJComponentDevice, KeyListener, MouseListener, MouseMotionListener, Printable, TSViewListener
	{
	...
	/// </summary>
	public const int INTERACTION_EDIT = 3;
	/// <summary>
	/// Interaction modes.
	...
	/// </summary>
	private bool _first_paint = true;
	...
	/// </summary>
	private bool _need_to_close = false;
	...
	/// </summary>
	private bool _zoom_keep_y_limits = false;
	...
	/// </summary>
	private IList<TSGraph> _tsgraphs = new List<TSGraph>();
	/// <summary>
	...
	/// </summary>
	private bool _force_redraw = true;
	/// <summary>
	...
	/// </summary>
	private bool _force_redraw_clean = true;
	/// <summary>
	...
	/// </summary>
	private bool _displayCrossHairCursor = false;

	/// <summary>
	/// Indicate whether the paint is to an SVG file.  See the saveAsSVG() method.
	/// </summary>
	private bool __paintForSVG = false;
	...
	/// </summary>
	public static readonly bool svgEnabled;

	static TSGraphJComponent()
	{
			// by default, its not enabled - if the classes are present, then its enabled.
			bool enabled = false;
			try
			{
				Type.GetType("org.apache.batik.svggen.SVGGeneratorContext");
				enabled = true;
			}
	...
	/// <summary>
	/// Construct a TSGraphJComponet and display the time series. </summary>
	/// <param name="parent"> Parent Frame object (often a TSViewGraphJFrame). </param>
	/// <param name="tslist"> list of time series to graph. </param>
	/// <param name="props"> Properties to control display of time series.  Most of these
	...
	/// series with the longest overall period). </param>
	public TSGraphJComponent(TSViewGraphJFrame parent, IList<TS> tslist, PropList props) : base("TSGraphJComponent")
	{
		_force_redraw = true;
		_parent = parent;
		_tslist = tslist;
		...
		if ((!string.ReferenceEquals(propVal, null)) && StringUtil.isDouble(propVal))
		{
		...
		_tsgraphs = createTSGraphsFromTSProduct(_tsproduct, _displayProps, tslist, new GRLimits(0.0,0.0,width,height));
	...
	/// <summary>
	/// Construct a TSGraphJComponent and display the time series. </summary>
	/// <param name="parent"> Parent JFrame object (often a TSViewGraphGUI). </param>
	/// <param name="tsproduct"> TSProduct describing the graph(s).
	/// The following override properties can be set in the TSProduct to affect this graph component. </param>
	/// <param name="displayProps"> Display properties that control the display and which are
	/// separate from the TSProduct.  Valid properties include those described below.
	/// ReferenceGraph can be set to "true" or "false" to indicate whether the graph is
	/// a reference graph.  ReferenceTSIndex can be set to a Vector index to indicate
	/// the reference time series for the reference graph (the default is the time
	/// series with the longest overall period). </param>
	public TSGraphJComponent(TSViewGraphJFrame parent, TSProduct tsproduct, PropList displayProps) : base("TSGraphJComponent")
	{
		...
		if ((_tslist == null) || (_tslist.Count == 0))
		{
		...
		if ((!string.ReferenceEquals(prop_val, null)) && StringUtil.isDouble(prop_val))
		{
		...
		_tsgraphs = createTSGraphsFromTSProduct(_tsproduct, _displayProps, _tslist, new GRLimits(0.0,0.0,width,height));
		checkTSProductGraphs(_tsproduct, _tsgraphs);
	...
	/// in registered listeners being called. </summary>
	/// <param name="listener"> TSViewListener to add. </param>
	public virtual void addTSViewListener(TSViewListener listener)
	...
	/// display a reference graph.  This method does not currently evaluate the zoom group property. </summary>
	/// <returns> true if at least one graph allows zooming. </returns>
	public virtual bool canUseReferenceGraph()
	{
		return canUseZoom();
	}
	...
	/// This is called by TSViewGraphJFrame to let it decide whether to enable the zoom button. </summary>
	/// <returns> true if at least one graph allows zooming. </returns>
	public virtual bool canUseZoom()
	{
		int size = _tsgraphs.Count;
		string prop_value;
		for (int i = 0; i < size; i++)
		{
	...
	/// display properties and sets internal variables that are commonly used. </summary>
	/// <param name="displayProps"> display properties to provide additional runtime information above and beyond the
	/// time series product properties </param>
	private void checkDisplayProperties(TSProduct tsproduct, PropList displayProps)
	{
//JAVA TO C# CONVERTER WARNING: The .NET Type.FullName property will not always yield results identical to the Java Class.getName method:
		string routine = this.GetType().FullName + ".checkDisplayProperties";
	...
	/// time series data are checked to determine some properties that impact other properties.</b> </summary>
	/// <param name="tsproduct"> TSProduct to check. </param>
	/// <param name="tsgraphs"> list of TSGraph describing the graphs. </param>
	private void checkTSProductGraphs(TSProduct tsproduct, IList<TSGraph> tsgraphs)
	{ // Put here any checks that cannot be performed in the initial call
		...
		IList<TS> tslistLeftYAxis = null;
		IList<TS> tslistRightYAxis = null;
		TSGraph tsgraph = null;
		int nts = 0, ntsLeftYAxis = 0, ntsRightYAxis = 0;
		string prop_val;
		for (int isub = 0; isub < nsubs; isub++)
		{
			tsgraph = tsgraphs[isub];
			...
			{
				ntsLeftYAxis = tslistLeftYAxis.Count;
			}
			ntsRightYAxis = 0;
			if (tslistRightYAxis != null)
			{
				ntsRightYAxis = tslistRightYAxis.Count;
			}
			...
			if (string.ReferenceEquals(tsproduct.getLayeredPropValue("LeftYAxisIgnoreUnits", isub, -1, false), null))
			{
			...
			if (string.ReferenceEquals(tsproduct.getLayeredPropValue("LeftYAxisTitleString", isub, -1, false), null))
			{
			...
			if (string.ReferenceEquals(tsproduct.getLayeredPropValue("LeftYAxisUnits", isub, -1, false), null))
			{
				if (graphType == TSGraphType.XY_SCATTER)
				{
					// Units will be 1st (dependent) time series -
			...
			if (string.ReferenceEquals(tsproduct.getLayeredPropValue("LeftYAxisLabelPrecision", isub, -1, false), null))
			{
			...
			if (string.ReferenceEquals(tsproduct.getLayeredPropValue("BottomXAxisTitleString", isub, -1, false), null))
			{
				tsgraph = _tsgraphs[isub];
				if (tsgraph == null)
				{
					continue;
				}
			...
			// code with 1 == 1?  Was this a work-around for some other
			// problem.  Need to evaluate further when regression tests are in place.
			if (string.ReferenceEquals(tsproduct.getLayeredPropValue("RightYAxisTitleString", isub, -1, false), null))
			{
			...
			if (string.ReferenceEquals(tsproduct.getLayeredPropValue("RightYAxisUnits", isub, -1, false), null))
			{
				if (graphType == TSGraphType.XY_SCATTER)
				{
					// Units will be 1st (dependent) time series -
			...
			if (string.ReferenceEquals(tsproduct.getLayeredPropValue("RightYAxisLabelPrecision", isub, -1, false), null))
			{
				if ((graphType == TSGraphType.PERIOD) || (graphType == TSGraphType.RASTER))
				{
	...
	/// This typically would be called in the first paint where the component size is known. </summary>
	/// <param name="tsproduct"> TSProduct describing what to graph. </param>
	/// <param name="display_props"> Display properties (e.g., whether a reference graph). </param>
	/// <param name="tsproduct_tslist"> List of time series to graph for the full TSProduct. </param>
	/// <param name="drawlim_graphs"> Drawing limits of the area set aside for graphs.  This
	/// area is divided among the individual graphs. </param>
	/// <returns> List of TSGraph to use when drawing.  The list is
	/// guaranteed to be non-null but may contain zero graphs. </returns>
	private IList<TSGraph> createTSGraphsFromTSProduct(TSProduct tsproduct, PropList display_props, IList<TS> tsproduct_tslist, GRLimits drawlim_graphs)
	{
		...
		if (nsubs == 0)
		{
			...
			if (Message.isDebugOn)
			{
				Message.printDebug(1, routine, _gtype + "Created 0 graphs from TSProduct (no enabled subproducts defined)");
			}
			return new List<TSGraph> (1);
		}
		...
		TS ts, tsfound;
		string prop_val;
		IList<TSGraph> tsgraphs = new List<TSGraph> (nsubs);
		...
		bool showDrawingAreaOutline = false;
		...
		bool check_input; // Indicates whether to check the input
		...
		for (int isub = 0; isub < nsubs; isub++)
		{
			...
			drawlim = new GRLimits(drawlim_graphs.getLeftX(), drawlim_graphs.getTopY() - (isub + 1) * height, drawlim_graphs.getRightX(), drawlim_graphs.getTopY() - isub * height);
			/*
			...
			*/
			...
			{ // Loop for TSIDs in graph
				...
				if ((string.ReferenceEquals(TSID_prop_val, null)) && (string.ReferenceEquals(TSAlias_prop_val, null)))
				{
					// Done with data for the graph...
					break;
				}
				if (string.ReferenceEquals(TSID_prop_val, null))
				{
					// Assign blank...
					TSID_prop_val = "";
				}
				if (string.ReferenceEquals(TSAlias_prop_val, null))
				{
					// Assign blank...
					TSAlias_prop_val = "";
				}
				if (TSID_prop_val.IndexOf("~", StringComparison.Ordinal) >= 0)
				{
				...
				if (Message.isDebugOn)
				{
					Message.printDebug(2, routine, _gtype + "Looking for time series needed for graph:  TSID_prop_val=\"" + TSID_prop_val + "\" TSAlias_prop_val = \"" + TSAlias_prop_val + "\".");
				}
				...
				for (int kts = 0; kts < nts; kts++)
				{
					...
					if (Message.isDebugOn)
					{
						Message.printDebug(2, routine, _gtype + "Comparing to TS in available data: " + "TSID=\"" + ts.getIdentifier().ToString(true) + "\" Alias=\"" + ts.getAlias() + "\"");
					}
					//if ( Message.isDebugOn ) {
					...
					if (!TSAlias_prop_val.Equals(""))
					{
						...
						if (ts.getAlias().Equals(TSAlias_prop_val, StringComparison.OrdinalIgnoreCase))
						{
							if (Message.isDebugOn)
							{
				...
				if ((nsubs == 1) && (!string.ReferenceEquals(prop_val, null)) && StringUtil.isInteger(prop_val))
				{
					tsfound = tsproduct_tslist[StringUtil.atoi(prop_val)];
				}
				...
				else
				{
					if (Message.isDebugOn)
					{
						Message.printDebug(2, routine, "Found time series \"" + tsfound.getIdentifierString() + "\" alias=\"" + tsfound.getAlias() + "\" for graph.");
					}
				}
				tslist.Add(tsfound);
			}
	...
	/// expanded TSProduct properties are transferred as is. </summary>
	/// <param name="proplist"> PropList passed in during construction. </param>
	/// <param name="a"> TSProduct containing values from the PropList.  If the list is null, 
	/// an empty TSProduct is created.  The checkTSProduct() method assigns appropriate
	/// default values for the resulting TSProduct. </param>
	/// <param name="tslist"> List of time series for the product. </param>
	private TSProduct createTSProductFromPropList(PropList proplist, IList<TS> tslist)
	...
	*/
		...
		IList<Prop> v = proplist.getPropsMatchingRegExp("Product.*");
		int size = 0;
		if (v != null)
		{
			size = v.Count;
		...
		if (!string.ReferenceEquals(prop_val, null))
		{
			...
			if (prop_val.Equals("BarsLeftOfDate", StringComparison.OrdinalIgnoreCase))
			{
				tsproduct.setPropValue("BarPosition", "LeftOfDate", 0, -1);
			}
			else if (prop_val.Equals("BarsRightOfDate", StringComparison.OrdinalIgnoreCase))
			{
	...
	/// </summary>
	private IList<TSGraphDataLimits> determineDataLimits()
	{
		IList<TSGraphDataLimits> dataLimits = new List<object>();
		IList<string> ids = null;
		IList<TS> tslist = null;

		for (int i = 0; i < _tsgraphs.Count; i++)
		{
			tslist = _tsgraphs[i].getTSList();
			...
			dataLimits.Add(new TSGraphDataLimits(tslist.Count,ids,_tsgraphs[i].getDataLimits()));
		}
	...
	public virtual void drawDrawingAreas()
	{
		...
		// Reference and main...
		GRDrawingAreaUtil.drawRectangle(_da_graphs, _datalim_graphs.getLeftX(), _datalim_graphs.getBottomY(), _datalim_graphs.getWidth(), _datalim_graphs.getHeight());
		if (_is_reference_graph)
		{
	...
	/// Currently the focus is to draw the glass pane tracker here but may put in the tracker data and draw elsewhere. </summary>
	/// <param name="trackerData"> mouse tracker data to pass back to calling code,
	/// typically a TSGraphJComponentGlassPaneMouseListener </param>
	/// <param name="devx"> device (JComponent) pixel coordinate (0=left edge) of mouse motion </param>
	/// <param name="devy"> device (JComponent) pixel coordinate (0=top edge) of mouse motion </param>
	protected internal virtual void drawMouseTracker(TSGraphJComponentGlassPane glassPane, Graphics2D g, int devx, int devy)
	{
		...
		for (int itsgraph = 1; itsgraph <= _tsgraphs.Count; itsgraph++)
		{
			TSGraph tsgraph = _tsgraphs[itsgraph - 1];
			// Left y-axis drawing area
			GRJComponentDrawingArea da = new GRJComponentDrawingArea(glassPane, "" + itsgraph + "." + tsgraph.getLeftYAxisGraphDrawingArea().getName(), GRAspect.FILL, null, GRUnits.DEVICE, GRLimits.DEVICE, null);
			...
			{
				da = new GRJComponentDrawingArea(glassPane, "" + itsgraph + "." + tsgraph.getRightYAxisGraphDrawingArea().getName(), GRAspect.FILL, null, GRUnits.DEVICE, GRLimits.DEVICE, null);
		...
		// If so, draw it according to mouse tracker rules.
		int itsgraph = -1;
		...
		bool drawTrackerLine = false; // Only draw when in graph type that supports tracker
		foreach (TSGraph tsgraph in _tsgraphs)
		{
			++itsgraph;
			...
			IList<GRDrawingArea> daGraphList = new List<GRDrawingArea>();
			...
			{
				if ((leftYAxisGraphType == TSGraphType.AREA) || (leftYAxisGraphType == TSGraphType.BAR) || (leftYAxisGraphType == TSGraphType.LINE) || (leftYAxisGraphType == TSGraphType.POINT))
				{
			...
			{
				if ((rightYAxisGraphType == TSGraphType.AREA) || (rightYAxisGraphType == TSGraphType.BAR) || (rightYAxisGraphType == TSGraphType.LINE) || (rightYAxisGraphType == TSGraphType.POINT))
				{
			...
			IList<double> distNearestList = null;
			IList<TSData> tsdataNearestList = null;
			IList<TS> tsNearestList = null;
			IList<DateTime> dtNearestList = null;
			IList<GRDrawingArea> daGraphNearestList = null;
			IList<TSGraph> graphNearestList = null;
			IList<int> itsgraphNearestList = null;
			foreach (GRDrawingArea daGraph in daGraphList)
			{
				IList<TS> tsForAxis = new List<TS>();
				...
				if ((trackerType == TSGraphMouseTrackerType.NEAREST_TIME) || (trackerType == TSGraphMouseTrackerType.NEAREST_TIME_SELECTED))
				{
					// Reset the lists because time series are tracked on each axis.
					distNearestList = new List<double>(tsForAxis.Count);
					tsdataNearestList = new List<TSData>(tsForAxis.Count);
					tsNearestList = new List<TS>(tsForAxis.Count);
					dtNearestList = new List<DateTime>(tsForAxis.Count);
					daGraphNearestList = new List<GRDrawingArea>(tsForAxis.Count);
					graphNearestList = new List<TSGraph>(tsForAxis.Count);
					itsgraphNearestList = new List<int>(tsForAxis.Count);
					for (int i = 0; i < tsForAxis.Count; i++)
					{
				...
				bool doDraw = false;
				if ((trackerType == TSGraphMouseTrackerType.NEAREST) || (trackerType == TSGraphMouseTrackerType.NEAREST_SELECTED))
				{
				...
				else if ((trackerType == TSGraphMouseTrackerType.NEAREST_TIME) || (trackerType == TSGraphMouseTrackerType.NEAREST_TIME_SELECTED))
				{
				...
				if (drawTrackerLine && doDraw)
				{
					...
					foreach (TS ts in tsForAxis)
					{
						...
						if (((trackerType == TSGraphMouseTrackerType.NEAREST_SELECTED) || (trackerType == TSGraphMouseTrackerType.NEAREST_TIME_SELECTED)) && !tsgraph.isTimeSeriesSelected(ts))
						{
							...
							if (!ts.isDataMissing(tsvalue) && daDataLimits.containsX(xSearchTime))
							{
								...
								if ((trackerType == TSGraphMouseTrackerType.NEAREST) || (trackerType == TSGraphMouseTrackerType.NEAREST_SELECTED))
								{
									...
									if (distNearestList == null)
									{
										...
										distNearestList = new List<double>();
										distNearestList.Add(new double?(dist));
										tsdataNearestList = new List<TSData>();
										tsdataNearestList.Add(new TSData(tsdata));
										tsNearestList = new List<TS>();
										tsNearestList.Add(ts);
										dtNearestList = new List<DateTime>();
										dtNearestList.Add(new DateTime(searchTime));
										daGraphNearestList = new List<GRDrawingArea>();
										daGraphNearestList.Add(daGraph);
										graphNearestList = new List<TSGraph>();
										graphNearestList.Add(tsgraph);
										itsgraphNearestList = new List<int>();
										itsgraphNearestList.Add(itsgraph);
									}
									else if (dist < distNearestList[0])
									{
										...
										distNearestList[0] = new double?(dist);
										tsdataNearestList[0] = new TSData(tsdata);
										tsNearestList[0] = ts;
										dtNearestList[0] = new DateTime(searchTime);
										daGraphNearestList[0] = daGraph;
										graphNearestList[0] = tsgraph;
										itsgraphNearestList[0] = new int?(itsgraph);
								...
								else if ((trackerType == TSGraphMouseTrackerType.NEAREST_TIME) || (trackerType == TSGraphMouseTrackerType.NEAREST_TIME_SELECTED))
								{
									...
									if ((distNearestList[its] == null) || (dist < (distNearestList[its])))
									{
										// Found a closer point for the specific time series so save its data
										distNearestList[its] = new double?(dist);
										tsdataNearestList[its] = new TSData(tsdata);
										tsNearestList[its] = ts;
										dtNearestList[its] = new DateTime(searchTime);
										daGraphNearestList[its] = daGraph;
										graphNearestList[its] = tsgraph;
										itsgraphNearestList[its] = new int?(itsgraph);
									}
								}
							}
						}
				...
				if ((trackerType == TSGraphMouseTrackerType.NEAREST_TIME) || (trackerType == TSGraphMouseTrackerType.NEAREST_TIME_SELECTED))
				{
					...
					drawMouseTrackerPoints(glassPane, distNearestList, tsNearestList, dtNearestList, tsdataNearestList, daGraphNearestList, itsgraphNearestList);
				}
			}
			if ((trackerType == TSGraphMouseTrackerType.NEAREST) || (trackerType == TSGraphMouseTrackerType.NEAREST_SELECTED))
			{
	...
	private void drawMouseTrackerPoints(TSGraphJComponentGlassPane glassPane, IList<double> distNearestList, IList<TS> tsNearestList, IList<DateTime> dtNearestList, IList<TSData> tsdataNearestList, IList<GRDrawingArea> daGraphNearestList, IList<int> itsgraphNearestList)
	{
		if (distNearestList != null)
		{
			// Have something to draw
			//System.out.println("Have " + distNearestList.size() + " points to draw");
			for (int i = 0; i < distNearestList.Count; i++)
			{
				double? distNearest = distNearestList[i];
				if (distNearest == null)
				{
				...
				TS tsNearest = tsNearestList[i];
				DateTime dtNearest = dtNearestList[i];
				TSData tsdataNearest = tsdataNearestList[i];
				GRDrawingArea daGraphNearest = daGraphNearestList[i];
				int? itsgraphNearest = itsgraphNearestList[i];
				...
				int symbolSize = 8;
				if ((string.ReferenceEquals(symbolSizeProp, null)) || symbolSizeProp.Length == 0)
				{
					symbolSize = 8;
				}
				else
				{
					symbolSize = int.Parse(symbolSizeProp);
					if (symbolSize == 0)
					{
						symbolSize = 8;
					}
				...
				else
				{
					...
					if ((tsdataNearest.getDataValue() + textExtents.getHeight() / 2) > da.getDataLimits().getTopY())
					{
						...
						GRDrawingAreaUtil.drawSymbolText(da, GRSymbol.SYM_FCIR, dtNearest.toDouble(), tsdataNearest.getDataValue(), symbolSize, trackerLabel, GRColor.black, 0.0, textAlign | GRText.TOP, GRUnits.DEVICE, GRSymbol.SYM_CENTER_X | GRSymbol.SYM_CENTER_Y);
					}
					else
					{
						// Normally, draw the text center-left of the point.
						GRDrawingAreaUtil.drawSymbolText(da, GRSymbol.SYM_FCIR, dtNearest.toDouble(), tsdataNearest.getDataValue(), symbolSize, trackerLabel, GRColor.black, 0.0, textAlign | GRText.CENTER_Y, GRUnits.DEVICE, GRSymbol.SYM_CENTER_X | GRSymbol.SYM_CENTER_Y);
					}
				}
			}
		}
	...
	private void drawTitles()
	{
		...
		string maintitle_string = _tsproduct.expandPropertyValue(_tsproduct.getLayeredPropValue("MainTitleString", -1, -1, false));
		...
		string subtitle_string = _tsproduct.expandPropertyValue(_tsproduct.getLayeredPropValue("SubTitleString", -1, -1, false));
...
//		base.finalize();
	...
	/// <summary>
	/// Determine the TSGraph that an event occurred in, checking only the graph drawing areas in the TSGraph. </summary>
	/// <returns> the TSGraph that an event occurred in or null if not within the bounds of a TSGraph. </returns>
	/// <param name="pt"> point for the raw device units where the event occurred. </param>
	...
	/// Determine the TSGraph that an event occurred in, checking only the graph drawing areas in the TSGraph. </summary>
	/// <returns> the TSGraph that an event occurred in or null if not within the bounds of a TSGraph. </returns>
	/// <param name="pt"> point for the raw device units where the event occurred, such as mouse click. </param>
	/// <param name="includeGraphArea"> if true, evaluate whether the click was in the main graph drawing area. </param>
	/// <param name="includePage"> if true, evaluate whether the click was anywhere on the device (page). </param>
	private TSGraph getEventTSGraph(GRPoint pt, bool includeGraphArea, bool includePage)
	{
		int size = _tsgraphs.Count;
		TSGraph tsgraph = null;
		if (includeGraphArea)
	...
	/// <summary>
	/// Return the graph interaction mode. </summary>
	/// <returns> the interaction mode (see INTERACTION_*). </returns>
	...
	/// to display a dialog in lower-level code. </summary>
	/// <returns> the JFrame for the component. </returns>
	protected internal virtual JFrame getJFrame()
	{
		return _parent;
	}
	...
	/// </summary>
	protected internal virtual TSViewZoomHistory getReferenceGraphZoomHistory()
	{
	...
	/// </summary>
	public virtual IList<TSGraph> getTSGraphs()
	{
		return _tsgraphs;
	}
	...
	/// PropList (old convention) during construction. </summary>
	/// <returns> TSProduct associated with the component. </returns>
	public virtual TSProduct getTSProduct()
	{
		return _tsproduct;
	}

	/// <summary>
	/// Indicate whether this component is currently printing. </summary>
	/// <returns> true if the component is currently being printed, false if not. </returns>
	...
	/// <summary>
	/// Indicate whether this component is a reference graph. </summary>
	/// <returns> true if the component contains a reference graph, false if not. </returns>
	...
	/// <summary>
	/// Look up the color to use for a time series. </summary>
	/// <param name="index"> Zero-referenced index in time series vector. </param>
	/// <returns> the color for a time series, as a string color name. </returns>
	...
	/// <summary>
	/// Look up the symbol to use for a time series.  This is only used for point graphs. </summary>
	/// <param name="index"> Zero-referenced index in time series vector. </param>
	/// <returns> the symbol for a time series, as a string symbol name. </returns>
	...
	/// <summary>
	/// Handle mouse clicked event. </summary>
	/// <param name="event"> MouseEvent. </param>
	...
	/// </summary>
	// TODO:dre refactor
	private bool isInside(MouseEvent @event, GRLimits grLimits)
	{
	  return (@event.getX() > (int)grLimits.getLeftX() && @event.getX() < (int)grLimits.getRightX() && @event.getY() > (int)grLimits.getTopY() && @event.getY() < (int)grLimits.getBottomY()) ?true:false;
	}

	/// <summary>
	/// Handle mouse drag event.  If in zoom mode, redraw the rubber-band line.
	/// If a mouse tracker is enabled, call the TSViewListener.mouseMotion() method. </summary>
	...
	{ //event.consume();
		...
		if (dotrack)
		{
			...
			if (tsgraph.getLeftYAxisGraphType() == TSGraphType.RASTER)
			{
				...
				bool includeLeftYAxis = true;
				bool includeRightYAxis = false;
				IList<TS> tslist = tsgraph.getEnabledTSList(includeLeftYAxis,includeRightYAxis);
				if (tslist.Count > 0)
				{
					datapt.associated_object = tslist[0];
					devpt.associated_object = tslist[0];
				}
			}
			int size = _listeners.Length;
			for (int i = 0; i < size; i++)
	...
	/// <summary>
	/// Handle mouse enter event.  Currently does not do anything. </summary>
	/// <param name="event"> MouseEvent to handle. </param>
	public virtual void mouseEntered(MouseEvent @event)
	{
	}

	/// <summary>
	/// Handle mouse exit event.  Currently does not do anything. </summary>
	/// <param name="event"> MouseEvent to handle. </param>
	...
	public virtual void mouseMoved(MouseEvent @event)
	{
		...
		if (getInteractionMode() == INTERACTION_EDIT)
		{
			_cursorDecorator.mouseMoved(@event,tsgraph.getLeftYAxisGraphDrawingArea().getPlotLimits(GRDrawingArea.COORD_DEVICE));
			//  refresh(false);
		}
	...
	/// Otherwise, start a select or zoom.  The event is completed when the mouse is released. </summary>
	/// <param name="event"> MouseEvent to handle. </param>
	...
	{ //event.consume();
		...
		if (Message.isDebugOn)
		{
			Message.printDebug(1, _gtype + "TSGraphJComponent.mousePressed", "Mouse pressed at device coordinates " + @event.getX() + "," + @event.getY());
		}
		...
		bool forceRepaint = false; // In cases where need to redraw, for example, when time series is selected in legend
		if (tsgraphForPage != null)
		{
			...
			if (eventTS != null)
			{
				...
				bool isSelected = tsgraphForPage.toggleTimeSeriesSelection(eventTS);
		...
		if (tsgraphForGraph != null)
		{
			...
			else if ((_interaction_mode == INTERACTION_SELECT) || (_interaction_mode == INTERACTION_ZOOM))
			{
		...
		if ((_interaction_mode == INTERACTION_SELECT) || (_interaction_mode == INTERACTION_ZOOM))
		{
			...
			if ((deltax <= delta_min) || (deltay <= delta_min))
			{
				if (_interaction_mode == INTERACTION_SELECT)
				{
					...
					GRPoint datapt = tsgraph.getLeftYAxisGraphDrawingArea().getDataXY(_mouse_x1, _mouse_y1, GRDrawingArea.COORD_DEVICE);
					if (_listeners != null)
					{
						int size = _listeners.Length;
						...
						for (int i = 0; i < size; i++)
						{
							_listeners[i].tsViewSelect(tsgraph, devpt, datapt, (System.Collections.IList)null);
						}
					}
					_rubber_banding = false;
	...
	*/
			...
			if (_interaction_mode == INTERACTION_SELECT)
			{
				...
				if (_listeners != null)
				{
					...
					for (int i = 0; i < size; i++)
					{
						_listeners[i].tsViewSelect(tsgraph, mouseLimits, newDataLimits, (IList<object>)null);
					}
				}
			}
			else if (_interaction_mode == INTERACTION_ZOOM)
	...
	/// after construction in higher-level code. </summary>
	/// <returns> true if the TSGraphJComponent (and the graph) should be closed due to a user confirmation. </returns>
	...
	/// should not continue and should be closed.  This should be called immediately
	/// when a problem occurs in TSGraph constructors and should be checked when constructing this class. </summary>
	/// <param name="need_to_close"> True if the TSGraphJComponent should be closed because of
	/// user verification not to go on. </param>
	/// <returns> true if the TSGraphJComponent (and the graph) should be closed due to a
	/// user confirmation. </returns>
	public virtual bool needToClose(bool need_to_close)
	{
	...
	{ // Full page...
		...
		_da_page = new GRJComponentDrawingArea(this, "TSGraphJComponent.Page", GRAspect.FILL, null, GRUnits.DEVICE, GRLimits.DEVICE, null);
		...
		_da_maintitle = new GRJComponentDrawingArea(this, "TSGraphJComponent.MainTitle", GRAspect.FILL, null, GRUnits.DEVICE, GRLimits.DEVICE, null);
		...
		_da_subtitle = new GRJComponentDrawingArea(this, "TSGraphJComponent.SubTitle", GRAspect.FILL, null, GRUnits.DEVICE, GRLimits.DEVICE, null);
		_datalim_subtitle = new GRLimits(0.0, 0.0, 1.0, 1.0);
		_da_subtitle.setDataLimits(_datalim_subtitle);

		// Drawing area for graphing...

		_da_graphs = new GRJComponentDrawingArea(this, "TSGraphJComponent.Graphs", GRAspect.FILL, null, GRUnits.DEVICE, GRLimits.DEVICE, null);
		...
		_da_leftfoot = new GRJComponentDrawingArea(this, "TSGraphJComponent.LeftFooter", GRAspect.FILL, null, GRUnits.DEVICE, GRLimits.DEVICE, null);
		...
		_da_centerfoot = new GRJComponentDrawingArea(this, "TSGraphJComponent.CenterFooter", GRAspect.FILL, null, GRUnits.DEVICE, GRLimits.DEVICE, null);
		...
		_da_rightfoot = new GRJComponentDrawingArea(this, "TSGraphJComponent.RightFooter", GRAspect.FILL, null, GRUnits.DEVICE, GRLimits.DEVICE, null);
	...
	/// or render to SVG file (set __printForSVG=true before calling). </summary>
	/// <param name="g"> Graphics instance either from the component event handling or from an
	...
	public virtual void paint(Graphics g)
	{
		...
		if (_da_page == null)
		{
			...
			if (Message.isDebugOn)
			{
				Message.printDebug(1, routine, _gtype + "Set initial graph drawing limits to " + _drawlim_graphs.ToString());
			}
		}
		...
		if (_force_redraw || _force_redraw_clean || _printing || __paintForSVG || !_double_buffering || resizing)
		{
			if (Message.isDebugOn)
			{
				Message.printDebug(1, routine, _gtype + "Drawing graph (_force_redraw=" + _force_redraw + " _force_redraw_clean=" + _force_redraw_clean + " resizing=" + resizing + ")...");
			}
			try
			{
				...
				if (_first_paint)
				{
					...
					if (Message.isDebugOn)
					{
						Message.printDebug(1, routine, _gtype + "After call to zoomOut() __visibleStart=" + __visibleStart + " __visibleEnd=" + __visibleEnd);
					}
				...
				for (int isub = 0; isub < size; isub++)
				{
					...
					if (prop_val.Equals("false", StringComparison.OrdinalIgnoreCase))
					{
						if (Message.isDebugOn)
						{
			...
			catch (Exception e)
			{
				...
				Message.printWarning(2, routine, "Error drawing graph(s) uniquetempvar.");
	...
	/// <summary>
	/// Print method from the Printable interface. </summary>
	/// <param name="g"> the Graphics context to use for drawing the printed graphics. </param>
	/// <param name="pageFormat"> the PageFormat to use for printing. </param>
	/// <param name="the"> pageIndex (0-based) of the page to print. </param>
	/// <returns> whether a page could be printed (PAGE_EXISTS) or not (NO_SUCH_PAGE). </returns>
	...
	*/
			...
			_printBounds = new GRLimits((int)pageFormat.getImageableX(), (int)(pageFormat.getHeight() - (pageFormat.getImageableY() + pageFormat.getImageableHeight())), (int)(pageFormat.getImageableWidth() + pageFormat.getImageableX()), (int)(pageFormat.getHeight() - pageFormat.getImageableY()));
	...
	*/
	...
	{
//JAVA TO C# CONVERTER WARNING: The .NET Type.FullName property will not always yield results identical to the Java Class.getName method:
		string routine = this.GetType().FullName + ".printGraph";
		try
		{
			...
			{
				new GraphicsPrinterJob(this, "Graph", null, "na-letter", null, "Landscape", .5, .5, .5, .5, null, true); // show print configuration dialog
			}
		}
		catch (Exception e)
		{
	...
	/// <summary>
	/// Refresh the plot based on changes in properties.  This is typically called
	/// from outside code, e.g., when time series have been enabled/disabled in the TSViewPropertiesGUI. </summary>
	/// <param name="recompute_drawing_limits"> If true, the drawing areas will be recomputed.
	...
	/// maximum data value has been used to set the y-axis label widths). </param>
	public virtual void refresh(bool recompute_drawing_limits)
	{
		clearView();
	...
	/// the internal numbering changes, or axis labels and legend properties are changed. </summary>
	/// <param name="product"> the TSProduct from which to read TSGraph information.  This will
	/// most likely be the product that's already resident in memory, but not necessarily.  </param>
	public virtual void reinitializeGraphs(TSProduct product)
	{
		string routine = this.GetType().Name + ".reinitializeGraphs";
		...
		foreach (TSGraph g in _tsgraphs)
		{
		...
		IList<TSGraphDataLimits> v = determineDataLimits();
		...
		IList<IList<TS>> selectedTimeSeriesListOld = new List<IList<TS>>();
		IList<TSGraph> tsgraphListOld = _tsgraphs;
		foreach (TSGraph tsgraph in tsgraphListOld)
		{
			selectedTimeSeriesListOld.Add(tsgraph.getSelectedTimeSeriesList());
		}

		_tsgraphs = createTSGraphsFromTSProduct(_tsproduct, _displayProps, _tslist, new GRLimits(0.0,0.0, getWidth(), getHeight()));
		checkTSProductGraphs(_tsproduct, _tsgraphs);
		// Add the saved information
		if (tsgraphListOld.Count == _tsgraphs.Count)
		{
			for (int igraph = 0; igraph < _tsgraphs.Count; igraph++)
			{
				_tsgraphs[igraph].setSelectedTimeSeriesList(selectedTimeSeriesListOld[igraph]);
			}
		}
	...
	/// <summary>
	/// Resets the data limits for each graph during a call to reinitializeGraphs(). </summary>
	/// <param name="graphDataLimitList"> a list of TSGraphDataLimits to control the graph limits </param>
	private void resetGraphDataLimits(IList<TSGraphDataLimits> graphDataLimitsList)
	{
		int tsCount = 0;
		int vCount = 0;
		IList<string> ids = null;
		IList<TS> tslist = null;
		foreach (TSGraph graph in _tsgraphs)
		{
			...
			ids = StringUtil.sortStringList(ids);
			tsCount = tslist.Count;
			foreach (TSGraphDataLimits graphDataLimits in graphDataLimitsList)
			{
				vCount = graphDataLimits.getNumTimeSeries();
				if (vCount == tsCount)
				{
					IList<string> vids = graphDataLimits.getTimeSeriesIds();
	...
	/// but use an SVG graphics driver instead. </summary>
	/// <param name="path"> Path to SVG file to save.  The path is not adjusted and therefore
	/// should generally be specified as absolute and with the *.svg extension. </param>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public void saveAsSVG(String path) throws java.io.FileNotFoundException, java.io.IOException
	public virtual void saveAsSVG(string path)
	{
	...
	/// <summary>
	/// Scroll the current zoom group a multiple of the visible page. </summary>
	/// <param name="pages"> Number of pages to scroll (-1.0 is one page left, 1.0 is one page right). </param>
	/// <param name="notifyListeners"> If true, the tsViewZoom() method is called for the new data limits. </param>
	public virtual void scroll(double pages, bool notifyListeners)
	...
	/// <summary>
	/// Scroll the current zoom group to the end of the available data.  If zoomed out, no action is taken. </summary>
	/// <param name="notifyListeners"> If true, the tsViewZoom() method is called for the new data limits. </param>
	public virtual void scrollToEnd(bool notifyListeners)
	{ // Loop through the zoom levels and zoom each group of graphs to the
	...
	/// <summary>
	/// Scroll the current zoom group to the start of the available data.  If zoomed out, no action is taken. </summary>
	/// <param name="notifyListeners"> If true, the tsViewZoom() method is called for the new data limits. </param>
	public virtual void scrollToStart(bool notifyListeners)
	{ // Loop through the zoom levels and zoom each group of graphs to the
		// maximum data extent within the zoom group.  Need a concept of an active zoom group...
	...
	/// </summary>
	internal virtual void setDrawingLimits()
	{
		...
		if ((!string.ReferenceEquals(maintitle_string, null)) && !maintitle_string.Equals(""))
		{
		...
		if ((!string.ReferenceEquals(subtitle_string, null)) && !subtitle_string.Equals(""))
		{
			// Get the text extents and set the height based on that...
			string subtitle_font = _tsproduct.getLayeredPropValue("SubTitleFontName", -1, -1, false);
			string subtitle_fontsize = _tsproduct.getLayeredPropValue("SubTitleFontSize", -1, -1, false);
		...
		if (maintitle_height == 0.0)
		{
			...
			_drawlim_maintitle = new GRLimits((_drawlim_page.getLeftX() + buffer), _drawlim_page.getTopY(), (_drawlim_page.getRightX() - buffer), _drawlim_page.getTopY());
		}
		else
		{
			_drawlim_maintitle = new GRLimits((_drawlim_page.getLeftX() + buffer), (_drawlim_page.getTopY() - buffer - maintitle_height), (_drawlim_page.getRightX() - buffer), (_drawlim_page.getTopY() - buffer));
		}
		if (Message.isDebugOn)
		{
			Message.printDebug(1, "TSGraphJComponent.setDrawingLimits", _gtype + "Main title drawing limits are: " + _drawlim_maintitle);
		}

		if (subtitle_height == 0.0)
		{
			// Zero height drawing area place holder...
			_drawlim_subtitle = new GRLimits((_drawlim_page.getLeftX() + buffer), _drawlim_maintitle.getBottomY(), (_drawlim_page.getRightX() - buffer), _drawlim_maintitle.getBottomY());
		}
		else
		{
			_drawlim_subtitle = new GRLimits((_drawlim_page.getLeftX() + buffer), (_drawlim_maintitle.getBottomY() - buffer - subtitle_height), (_drawlim_page.getRightX() - buffer), (_drawlim_maintitle.getBottomY() - buffer));
		}
		if (Message.isDebugOn)
		{
			Message.printDebug(1, "TSGraphJComponent.setDrawingLimits", _gtype + "Sub title drawing limits are: " + _drawlim_subtitle);
		}
		...
		if (_is_reference_graph)
		{
			_drawlim_graphs = new GRLimits(_drawlim_page.getLeftX(), _drawlim_page.getBottomY(), _drawlim_page.getRightX(), _drawlim_page.getTopY());
		}
		else
		{
			_drawlim_graphs = new GRLimits((_drawlim_page.getLeftX() + buffer), (_drawlim_page.getBottomY() + buffer), (_drawlim_page.getRightX() - buffer), (_drawlim_subtitle.getBottomY() - buffer));
		}
		if (Message.isDebugOn)
		{
			Message.printDebug(1, "TSGraphJComponent.setDrawingLimits", _gtype + "Graph drawing limits are: " + _drawlim_graphs.ToString());
		}
	...
	/// rather than repaint.  For example, call from mousePressed() when a legend hotspot is pressed. </summary>
	/// <param name="forceRedraw"> if true, redraw the graph (but don't clear it first), suitable for mouse zoom box. </param>
	/// <param name="forceRedrawClean"> if true, redraw the graph (and force clear), suitable for legend time series selection clicks </param>
	internal virtual void setForceRedraw(bool forceRedraw, bool forceRedrawClean)
	{
	...
	/// </summary>
	public virtual void setGraphDrawingLimits()
	{
		int nsubs = _tsgraphs.Count;
		TSGraph tsgraph;
		GRLimits drawlim;
	...
	//	System.out.println("");
		...
		for (i = 0; i < nsubs; i++)
		{
			...
			else
			{
				...
				drawlim = new GRLimits(_drawlim_graphs.getLeftX(), _drawlim_graphs.getTopY() - runningTotal, _drawlim_graphs.getRightX(), _drawlim_graphs.getTopY() - runningTotal + heights[i]);
	...
	/// INTERACTION_ZOOM, or INTERACTION_NONE. </summary>
	/// <param name="mode"> Interaction mode. </param>
	public virtual void setInteractionMode(int mode)
	{
		if ((mode == INTERACTION_NONE) || (mode == INTERACTION_SELECT) || (mode == INTERACTION_ZOOM) || (mode == INTERACTION_EDIT))
		{
			if (Message.isDebugOn)
			{
				Message.printDebug(1, _gtype + "TSGraphJComponent.setInteractionMode", "Set interaction mode to " + mode);
			}
	...
	/// <summary>
	/// Show an information dialog indicating the closest time series and value to the click. </summary>
	/// <param name="tsgraph"> TSGraph where the select occurred. </param>
	/// <param name="x"> X-coordinate of mouse click, from event handler. </param>
	/// <param name="y"> Y-coordinate of mouse click, from event handler. </param>
	public virtual void showInfoDialog(TSGraph tsgraph, int x, int y)
	{
		GRPoint datapt = tsgraph.getLeftYAxisGraphDrawingArea().getDataXY(x, y, GRDrawingArea.COORD_DEVICE);
		if (datapt == null)
		{
		...
		int size = 0;
		IList<TS> tslist = tsgraph.getTSList();
		if (tslist != null)
		{
			size = tslist.Count;
		}
		...
		if (mints == null)
		{
			new ResponseJDialog(_parent, "Time Series Information", "Unable to find nearest time series.", ResponseJDialog.OK);
		}
		else
		{
			d.setPrecision(mints.getDataIntervalBase());
			new ResponseJDialog(_parent, "Time Series Information", "Nearest time series is:\n" + "Time Series Identifier:  " + mints.getIdentifier() + "\n" + "Time Series Description:  " + mints.getDescription() + "\n" + "Date:  " + d.ToString() + "\n" + "Data value:  " + StringUtil.formatString(mints.getDataValue(d), DataUnits.getOutputFormatString(mints.getDataUnits(),0,2)) + " " + mints.getDataUnits(), ResponseJDialog.OK);
		}
	}

	/// <summary>
	/// Tests to see whether two sorted lists, each of which contain Strings, contain
	/// the same strings in the same order. </summary>
	/// <param name="v1"> the first list to check. </param>
	/// <param name="v2"> the second list to check. </param>
	/// <returns> true if the lists contain the same strings (compared by 
	...
	/// the same strings, element-by-element. </returns>
	private bool stringListsAreEqual(IList<string> v1, IList<string> v2)
	{
		if (v1 == null && v2 == null)
		{
			return true;
	...
	/// <summary>
	/// Return a property=value list of properties, separated by newline character. </summary>
	/// <returns> property list string for instance. </returns>
	...
	/// <param name="g"> TSGraph that mouse motion occurred in. </param>
	...
	public virtual void tsViewMouseMotion(TSGraph g, GRPoint devpt, GRPoint datapt)
	{
	}
	...
	/// Currently this does nothing. </summary>
	/// <param name="g"> TSGraph that select occurred in. </param>
	/// <param name="dev_shape"> Coordinates of mouse in device coordinates (pixels). </param>
	/// <param name="data_shape"> Coordinates of mouse in data coordinates. </param>
	/// <param name="selected"> list of selected TS.  Currently ignored. </param>
	public virtual void tsViewSelect(TSGraph g, GRShape dev_shape, GRShape data_shape, System.Collections.IList selected)
	{
	}
	...
	/// occur in the reference window. </summary>
	/// <param name="g"> TSGraph where the zoom event occurred. </param>
	/// <param name="dev_shape"> Limits of zoom in device coordinates (pixels). </param>
	/// <param name="data_shape"> Limits of zoom in data coordinates. </param>
	public virtual void tsViewZoom(TSGraph g, GRShape dev_shape, GRShape data_shape)
	{
		if (data_shape == null)
		{
			// Do nothing...
	...
	/// to previous/next zoom actions in the TSViewGraphJFrame interface. </summary>
	/// <param name="newdataLimits"> new zoom limits, in data units </param>
	public virtual void zoom(GRLimits newdataLimits)
	{
		// Call the other method with necessary data
		GRLimits mouseLimits = null;
		if (_is_reference_graph)
		{
			if (_tsgraphs.Count > 0)
			{
				TSGraph tsgraph = _tsgraphs[0];
				zoom(tsgraph, mouseLimits, newdataLimits);
			}
		}
	}
	...
	/// limits appropriately (e.g., use the new X axis date limits but handle its own Y axis limits). </summary>
	/// <param name="tsgraph"> TSGraph that generated the zoom event. </param>
	/// <param name="mouse_limits"> Component limits for zoom extent (currently not used). </param>
	/// <param name="newdata_limits"> Data limits for zoom extent in the original graph. </param>
	public virtual void zoom(TSGraph tsgraph, GRLimits mouse_limits, GRLimits newdata_limits)
	{
		...
		for (int isub = 0; isub < size; isub++)
		{
			tsgraph2 = _tsgraphs[isub];
			if (tsgraph2.canZoom() && zoom_group.Equals(_tsproduct.getLayeredPropValue("ZoomGroup", isub, -1, false), StringComparison.OrdinalIgnoreCase))
			{
	...
	/// one time in the first paint to force the different graphs to synchronize their zoom levels. </summary>
	/// <param name="re_draw"> If true, redraw the component after zooming.  If false, just set
	/// the data limits in the graphs but do not redraw (a redraw may occur elsewhere
	/// after this method is called). </param>
	public virtual void zoomOut(bool re_draw)
	{
		int size = _tsgraphs.Count;
		// Loop through the zoom levels and zoom each group of graphs to the
	...
	/// already been called to compute the maximum limits. </summary>
	/// <param name="re_draw"> If true, redraw the component after zooming.  If false, just set
	/// the data limits in the graphs but do not redraw (a redraw may occur elsewhere
	/// after this method is called). </param>
	public virtual void zoomToVisiblePeriod(DateTime visibleStart, DateTime visibleEnd, bool re_draw)
	{
		int size = _tsgraphs.Count;
		// Loop through the zoom levels and zoom each group of graphs to the
		// maximum data extent within the zoom group.
	...
	/// <summary>
	/// Controls display of cross hair cursor on graph. </summary>
	/// <param name="display"> If true displays cross hair </param>
	public virtual void setDisplayCursor(bool display)
	{
		_displayCrossHairCursor = display;
	}

	public virtual void setEditor(TSGraphEditor tsGraphEditor)
	{
