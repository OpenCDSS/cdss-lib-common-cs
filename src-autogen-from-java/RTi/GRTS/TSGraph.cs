...
namespace RTi.GRTS
{
	...
	using GRColorTable = RTi.GR.GRColorTable;
	...
	using TSDurationAnalysis = RTi.TS.TSDurationAnalysis;
	using TSIterator = RTi.TS.TSIterator;
	using TSLimits = RTi.TS.TSLimits;
	using TSRegression = RTi.TS.TSRegression;
	...
	using FDistribution = RTi.Util.Math.FDistribution;
	...
	using DataTableList_JFrame = RTi.Util.Table.DataTableList_JFrame;
	...
	/// </pre>
	/// </summary>
	public class TSGraph : ActionListener
	{
		...
		/// </summary>
		protected internal static string YAXIS_LIMITS_USE_PRODUCT_PROPERTIES = "UseProductProperties";
		/// <summary>
		...
		/// </summary>
		protected internal static string YAXIS_LIMITS_AUTOFILL_AND_KEEP = "AutoFillAndKeep";

	/// <summary>
	/// Popup menu options.
	/// </summary>
	private readonly string __MENU_ANALYSIS_DETAILS = "Analysis Details", __MENU_PROPERTIES = "Properties", __MENU_REFRESH = "Refresh", __MENU_Y_SET_Y_LIMITS = "Set Y Limits", __MENU_Y_MAXIMUM_AUTO = "Set Y Maximum to Auto (round maximum data value for full period)", __MENU_Y_MINIMUM_AUTO = "Set Y Minimum to Auto (round minimum data value for full period)", __MENU_Y_MAXIMUM_FILL_WINDOW = "Set Y Maximum to Fill Window", __MENU_Y_MINIMUM_FILL_WINDOW = "Set Y Minimum to Fill Window";

	/// <summary>
	/// If the graph type is Duration, this holds the results of the duration data for each time series.
	/// </summary>
	private IList<TSDurationAnalysis> _duration_data = null;
	...
	/// </summary>
	private IList<TS> __derivedTSList = new List<TS>();
	...
	/// </summary>
	private IList<TS> _selectedTimeSeriesList = new List<TS>();
	...
	/// </summary>
	private Dictionary<TS, GRLimits> __legendTimeSeriesDrawMap = new Dictionary<TS, GRLimits>();
	...
	/// </summary>
	private IList<string> __errorMessageList = new List<string>();
	...
	/// </summary>
	private IList<TS> __left_tslist = null;
	...
	/// This is needed to keep the product property index lined up.
	/// </summary>
	private IList<TS> __right_tslist = null;
	...
	/// </summary>
	private bool _is_reference_graph = false;
	...
	/// </summary>
	private bool _zoomKeepFullPeriodYLimits = false;

	/// <summary>
	/// Regression data used by scatter plot.
	/// </summary>
	private IList<TSRegression> _regression_data = null;
	...
	/// </summary>
	private bool __useSetDates = false;

	/// <summary>
	/// Construct a TSGraph and display the time series. </summary>
	/// <param name="dev"> TSGraphJComponent that is managing this graph. </param>
	/// <param name="drawlim_page"> Initial device limits that should be used for this graph,
	/// determined in the managing TSGraphJComponent (the limits will change as the component is resized. </param>
	/// <param name="tsproduct"> TSProduct containing information to control display of time
	/// series.  Most of these properties are documented in TSViewFrame, with the following additions: </param>
	/// <param name="display_props"> Additional properties used for displays.
	...
	/// tslist list that is passed in. </param>
	/// <param name="subproduct"> The sub-product from the main product.  This is used to look
	/// up properties specific to this graph product.  The first product is 1. </param>
	/// <param name="tslist"> list of time series to graph.  Only the time series for this
	/// graph are expected but time series for other graphs can be shared (access them in the TSProduct). </param>
	/// <param name="reference_ts_index"> Index in the "tslist" for the reference time series.
	/// This may be different from the "ReferenceTSIndex" property in display_props,
	/// which was for the original time series list (not the subset used just for this graph). </param>
	public TSGraph(TSGraphJComponent dev, GRLimits drawlim_page, TSProduct tsproduct, PropList display_props, int subproduct, IList<TS> tslist, int reference_ts_index)
	{
		...
		else if (__leftYAxisGraphType == TSGraphType.RASTER)
		{
			...
			if (tslist.Count > 0)
			{
				__tslist.Add(tslist[0]);
			}
		}
		// Get time series lists for each axis
		__left_tslist = new List<TS>();
		__right_tslist = new List<TS>();
		string propVal = null;
		int lefttsCount = 0;
		int righttsCount = 0;
		for (int its = 0; its < __tslist.Count; its++)
		{
			propVal = _tsproduct.getLayeredPropValue("YAxis", _subproduct, its, false);
			// Include time series associated with the left y-axis
			if ((string.ReferenceEquals(propVal, null)) || propVal.Length == 0 || propVal.Equals("Left", StringComparison.OrdinalIgnoreCase))
			{
				__left_tslist.Add(__tslist[its]);
				++lefttsCount;
			}
			else
			{
			...
			if ((!string.ReferenceEquals(propVal, null)) && propVal.Equals("Right", StringComparison.OrdinalIgnoreCase))
			{
				__right_tslist.Add(__tslist[its]);
				++righttsCount;
			}
			else
			{
		...
		for (int ii = 0; ii < ssize; ii++)
		{
			...
			{
				Message.printStatus(3, routine, _gtype + "TS[" + ii + "] is " + sts.getIdentifierString() + "period " + sts.getDate1() + " to " + sts.getDate2());
			}
		}
	...
	public virtual void actionPerformed(ActionEvent @event)
	{
		...
		else if (command.Equals(__MENU_ANALYSIS_DETAILS))
		{
			...
			if ((_duration_data != null) && (_duration_data.Count > 0))
			{
				// Create a DataTable and then use generic table view component
				DataTable table = new DataTable();
				IList<TSDurationAnalysis> durationList = _duration_data;
				IList<DataTable> tableList = new List<DataTable>(durationList.Count);
				string[] viewLabels = new string[durationList.Count];
				int its = -1;
				foreach (TSDurationAnalysis a in durationList)
				{
					...
					bool useExtendedLegend = false; // For now disable this to see how following works
			...
			if (_regression_data != null)
			{
				...
				if (_regression_data != null)
				{
					size = _regression_data.Count;
				}
				TSRegression r = null;
				IList<string> v = new List<string>();
				for (int i = 0; i < size; i++)
				{
					r = _regression_data[i];
					if (r == null)
					{
						continue;
					}
					// Split by newlines so that report has separate lines of information...
					IList<string> lines = StringUtil.breakStringList(r.ToString(), "\n", 0);
					int size2 = 0;
					if (lines != null)
					{
						size2 = lines.Count;
		...
		else if (command.Equals(__MENU_Y_SET_Y_LIMITS))
		{
			...
			if (c is JFrame)
			{
				...
				if (this.leftYAxisViewMinY.Equals(YAXIS_LIMITS_AUTOFILL_AND_KEEP, StringComparison.OrdinalIgnoreCase) || this.leftYAxisViewMaxY.Equals(YAXIS_LIMITS_AUTOFILL_AND_KEEP, StringComparison.OrdinalIgnoreCase))
				{
					...
					if (__leftYAxisGraphType == TSGraphType.PREDICTED_VALUE_RESIDUAL)
					{
						...
						for (int i = 0; i < nreg; i++)
						{
							...
							regressionData = _regression_data[i];
					...
					{
						bool includeLeftYAxis = true;
						bool includeRightYAxis = false;
					...
					try
					{
						...
						{
							if (this.leftYAxisViewMinY.Equals(YAXIS_LIMITS_AUTOFILL_AND_KEEP, StringComparison.OrdinalIgnoreCase))
							{
								setLeftYAxisViewMinYFromData(limits.getMinValue());
							}
							if (this.leftYAxisViewMaxY.Equals(YAXIS_LIMITS_AUTOFILL_AND_KEEP, StringComparison.OrdinalIgnoreCase))
							{
	...
	/// makes sure we have cleanly handled the end points without having to adjust the array lengths. </summary>
	/// <param name="x"> X-coordinates for confidence curve. </param>
	/// <param name="y"> Y-coordinates for confidence curve. </param>
	/// <param name="npts"> Number of points to process (may be less than array size). </param>
	...
	/// Later it may be tied to a property. </summary>
	/// <returns> true if the graph can zoom, false otherwise. </returns>
	public virtual bool canZoom()
	{
		if ((__leftYAxisGraphType == TSGraphType.XY_SCATTER) || (__leftYAxisGraphType == TSGraphType.DURATION) || (__leftYAxisGraphType == TSGraphType.RASTER))
		{
			return false;
		}
		else
	...
	{ // "BottomXAxisLabelFormat" = _bottomx_date_format;
		...
		if (!string.ReferenceEquals(prop_val, null))
		{
			...
			if (prop_val.Equals("MM-DD", StringComparison.OrdinalIgnoreCase))
			{
		...
		if (!string.ReferenceEquals(prop_val, null))
		{
			try
			{
				_lefty_precision = int.Parse(prop_val);
			}
		...
		if (!string.ReferenceEquals(prop_val, null))
		{
			try
			{
				_righty_precision = int.Parse(prop_val);
			}
			catch (System.FormatException)
			{
				_righty_precision = 2;
	...
	/// The x-axis limits (time) for left and right y-axes must be set to the same so that time will be accurate. </summary>
	/// <param name="graphType"> the type of graph being produced. </param>
	/// <param name="computeFromMaxPeriod"> whether to compute data limits from the max dates or not.
	...
	/// For other graphs, see setComputeWithSetDates(). </param>
	protected internal virtual void computeDataLimits(bool computeFromMaxPeriod)
	{
		...
		try
		{
			...
			bool includeLeftYAxis = false;
			bool includeRightYAxis = true;
			IList<TS> enabledRightYAxisTSList = getEnabledTSList(includeLeftYAxis,includeRightYAxis);
			TSLimits rightYLimits0 = null;
			if (enabledRightYAxisTSList.Count > 0)
			{
			...
			IList<TS> enabledLeftYAxisTSList = getEnabledTSList(includeLeftYAxis,includeRightYAxis);
			if (enabledLeftYAxisTSList.Count == 0)
			{
			...
			if (enabledLeftYAxisTSList.Count > 0)
			{
				...
				// First set defaults...
				if ((__leftYAxisGraphType == TSGraphType.DOUBLE_MASS) || (__leftYAxisGraphType == TSGraphType.PERIOD) || (__leftYAxisGraphType == TSGraphType.XY_SCATTER))
				{
				...
				bool ignoreLeftAxisUnits = false;
				if ((!string.ReferenceEquals(ignoreLeftAxisUnitsProp, null)) && ignoreLeftAxisUnitsProp.Equals("true", StringComparison.OrdinalIgnoreCase))
				{
					_ignoreLeftAxisUnits = true;
				...
				bool ignoreRightAxisUnits = false;
				if ((!string.ReferenceEquals(ignoreRightAxisUnitsProp, null)) && ignoreRightAxisUnitsProp.Equals("true", StringComparison.OrdinalIgnoreCase))
				{
					_ignoreRightAxisUnits = true;
					if (TSUtil.areUnitsCompatible(getTSListToRender(true,includeLeftYAxis,includeRightYAxis), true))
					{
				...
				try
				{
					...
					else
					{
						...
						bool enabledOnly = true; // Want enabled time series only
						IList<TS> tslistToRender = getTSListToRender(enabledOnly,includeLeftYAxis,includeRightYAxis);
						if (!_is_reference_graph)
						{
							if (Message.isDebugOn)
							{
								Message.printStatus(2, routine, "Calling areUnitsCompatible for left y-axis, have " + tslistToRender.Count + " time series");
							}
						}
						if (!TSUtil.areUnitsCompatible(tslistToRender, true))
						{
							...
							else
							{
								...
								{
									x = (new ResponseJDialog(_dev.getJFrame(), "Continue Graph?", "The left y-axis data units are incompatible\n" + "Continue graphing anyway?", ResponseJDialog.YES | ResponseJDialog.NO)).response();
								}
								else
								{
					...
					if (graphType == TSGraphType.PERIOD)
					{
						...
						_max_tslimits_lefty.setMinValue(getEnabledTSList(includeLeftYAxis,includeRightYAxis).Count + 1);
					}
				}
				catch (Exception e)
				{
				...
				if (_is_reference_graph && (_reference_ts_index >= 0))
				{
					...
					IList<TS> ref_tslist = new List<TS>(1);
					ref_tslist.Add(__tslist[_reference_ts_index]);
				...
				else
				{
					...
					else if (!string.ReferenceEquals(prop_value, null) && prop_value.Equals("Auto", StringComparison.OrdinalIgnoreCase))
					{
						if (__leftYAxisGraphType == TSGraphType.PREDICTED_VALUE_RESIDUAL)
						{
							int nreg = 0;
						...
						{
							TSLimits tempLimits = TSUtil.getDataLimits(getTSListToRender(true,includeLeftYAxis,includeRightYAxis), _max_start_date, _max_end_date, "", false, _ignoreLeftAxisUnits);
				...
				{
					bool xlimits_found = false;
					bool ylimits_found = false;
					TSRegression regressionData = null;
					int nregression = 0;
					if (_regression_data != null)
					{
						nregression = _regression_data.Count;
					}
					double xmin = 0.0, ymin = 0.0, xmax = 1.0, ymax = 1.0;
					for (int ir = 0; ir < nregression; ir++)
					{
						regressionData = _regression_data[ir];
						...
						{
							bool[] analyze_month = regressionData.getAnalyzeMonth();
							// Monthly analysis...
							//
							// Get the limits from the regression data since some data may have been ignored...
							// Y is the 2nd+ time series picked.
				...
				else if ((__leftYAxisGraphType == TSGraphType.PREDICTED_VALUE_RESIDUAL) || (__leftYAxisGraphType == TSGraphType.PREDICTED_VALUE))
				{
					bool residual = true;
				...
				{
					_data_lefty_limits = new GRLimits(_start_date.toDouble(), _tslimits_lefty.getMinValue(), _end_date.toDouble(),_tslimits_lefty.getMaxValue());
				}
				...
				if (_data_lefty_limits != null)
				{
					...
					else
					{
						...
						{
							GRLimits tempLimits = new GRLimits(_max_start_date.toDouble(), _tslimits_lefty.getMinValue(), _max_end_date.toDouble(),_tslimits_lefty.getMaxValue());
					...
					if (Message.isDebugOn)
					{
						Message.printDebug(1, routine, _gtype + "Initial computed _max_lefty_data_limits (including limit properties) are " + _max_lefty_data_limits);
					}
				}
			}
			...
			if (!_is_reference_graph && (enabledRightYAxisTSList.Count > 0))
			{
				...
				try
				{
					...
					else
					{
						...
						// TODO - need to add on the fly conversion of units (slower but changing original data is
						// a worse alternative).
						Message.printStatus(2, routine, "Calling areUnitsCompatible for right y-axis, have " + enabledRightYAxisTSList.Count + " time series");
						if (!TSUtil.areUnitsCompatible(enabledRightYAxisTSList, true))
						{
							...
							else
							{
								...
								if (_dev.getJFrame() != null)
								{
									x = (new ResponseJDialog(_dev.getJFrame(), "Continue Graph?", "The right y-axis data units are incompatible\n" + "Continue graphing anyway?", ResponseJDialog.YES | ResponseJDialog.NO)).response();
								}
								else
								{
									// No frame so default to ignore units...
					...
					if ((!string.ReferenceEquals(prop_value, null)) && StringUtil.isDouble(prop_value))
					{
						double ymax = double.Parse(prop_value);
					...
					if ((!string.ReferenceEquals(prop_value, null)) && StringUtil.isDouble(prop_value))
					{
						double ymin = double.Parse(prop_value);
				...
				if (_tslimits_righty != null)
				{
					...
					_data_righty_limits = new GRLimits(_start_date.toDouble(), _tslimits_righty.getMinValue(), _end_date.toDouble(),_tslimits_righty.getMaxValue());
				}
				...
				if (_data_righty_limits != null)
				{
					...
					else
					{
						...
						else
						{
							if (_tslimits_righty != null)
							{
								GRLimits tempLimits = new GRLimits(_max_start_date.toDouble(), _tslimits_righty.getMinValue(), _max_end_date.toDouble(),_tslimits_righty.getMaxValue());
					...
					if (Message.isDebugOn)
					{
						Message.printDebug(1, routine, _gtype + "Initial computed _max_righty_data_limits (including limit properties) are " + _max_righty_data_limits);
					}
				}
			}
		}
	...
	/// Any errors in processing will result in the properties not being used. </summary>
	/// <param name="tslist"> list of time series to process. </param>
	/// <param name="limits"> the limits of the graph based on time series. </param>
	private void computeDataLimits_CheckDisplayLimitProperties(IList<TS> tslist, TSLimits limits)
	{
		...
		foreach (TS ts in tslist)
		{
			...
			if (propMinValueO != null)
			{
				...
				if (propMinValueO is double?)
				{
					propMinValue = (double?)propMinValueO;
				}
				else if (propMinValueO is string)
				{
					try
					{
						propMinValue = double.Parse((string)propMinValueO);
					}
					catch (System.FormatException)
					{
						Message.printWarning(3, "", "Time series \"" + ts.getIdentifierString() + "\" property DataValueDisplayMin \"" + propMinValueO + "\" is invalid number.");
					}
				}
			}
			...
			{
				// The property can be a string or a Double
				if (propMaxValueO is double?)
				{
					propMaxValue = (double?)propMaxValueO;
				}
				else if (propMaxValueO is string)
				{
					try
					{
						propMaxValue = double.Parse((string)propMaxValueO);
					}
					catch (System.FormatException)
					{
						Message.printWarning(3, "", "Time series \"" + ts.getIdentifierString() + "\" property DataValueDisplayMax \"" + propMaxValueO + "\" is invalid number.");
					}
				}
			}
			// If have the display limit values, set the limits from the properties
			// - ensure that the max and min values are not the same
			if ((propMinValue != null) && (propMaxValue != null) && !propMinValue.Equals(propMaxValue) && (propMinValue < propMaxValue))
			{
				...
				if (propMinValue != null)
				{
					...
					if (limits.areLimitsFound())
					{
						...
						if (limits.getMinValue() < propMinValue.Value)
						{
							// The value computed from the time series is out of range
							limits.setMinValue(propMinValue.Value);
				...
				if (propMaxValue != null)
				{
					...
					{
						// Limits are found so can check
						if (limits.getMaxValue() > propMaxValue.Value)
						{
							// The value computed from the time series is out of range
							limits.setMaxValue(propMaxValue.Value);
	...
	/// This resets _datalim_lefty_graph and _datalim_righty_graph to be nicer bounding limits. </summary>
	/// <param name="limitsLeftYAxis"> For data that is being used (generally the max or current
	/// limits - whatever the graph is supposed to display).  <b>This is time series
	/// data so for scatter plots, etc., it does not contain all that is needed.</b> </param>
	private void computeLabels(TSLimits limitsLeftYAxis, TSLimits limitsRightYAxis)
	{
		...
		else
		{
			...
			if ((!string.ReferenceEquals(propValue, null)) && !propValue.Equals("None", StringComparison.OrdinalIgnoreCase))
			{
		...
		bool log_y_lefty = false;
		bool log_xy_scatter = false;
		propValue = _tsproduct.getLayeredPropValue("LeftYAxisType", _subproduct, -1, false);
		if ((!string.ReferenceEquals(propValue, null)) && propValue.Equals("Log", StringComparison.OrdinalIgnoreCase))
		{
			log_y_lefty = true;
		}
		...
		else if (__leftYAxisGraphType == TSGraphType.PERIOD)
		{
			...
			bool includeLeftYAxis = true;
			bool includeRightYAxis = false;
			IList<TS> enabledTSList = getEnabledTSList(includeLeftYAxis,includeRightYAxis);
			_ylabels_lefty = new double[enabledTSList.Count];
			for (int i = 0; i < enabledTSList.Count; i++)
			{
		...
		else
		{
			...
			if ((__leftYAxisGraphType == TSGraphType.XY_SCATTER) && (_regression_data != null))
			{
				while (minlabels >= 3)
				{
					...
					_ylabels_lefty = GRAxis.findNLabels(_data_lefty_limits.getMinY(), _data_lefty_limits.getMaxY(), false, minlabels, maxlabels);
					if (_ylabels_lefty != null)
					{
						break;
					}
		...
		if ((_ylabels_lefty == null) || (_ylabels_lefty.Length == 0))
		{
			if (Message.isDebugOn)
			{
				Message.printDebug(1, routine, _gtype + "Unable to find left y-axis labels using " + minlabels + " to " + maxlabels + " labels.  Using end-point data values.");
			}
			...
			if (__leftYAxisGraphType == TSGraphType.XY_SCATTER)
			{
				...
				_data_lefty_limits = new GRLimits(_max_lefty_data_limits.getMinX(), _ylabels_lefty[0], _max_lefty_data_limits.getMaxX(), _ylabels_lefty[1]);
			}
			else
			{
				if (log_y_lefty)
		...
		else
		{
			if (__leftYAxisGraphType == TSGraphType.PERIOD)
			{
				bool includeLeftYAxis = true;
				bool includeRightYAxis = false;
				_data_lefty_limits = new GRLimits(_start_date.toDouble(), (getEnabledTSList(includeLeftYAxis,includeRightYAxis).Count + 1), _end_date.toDouble(), 0.0);
			}
			else if (__leftYAxisGraphType == TSGraphType.XY_SCATTER)
			{
				_data_lefty_limits = new GRLimits(_max_lefty_data_limits.getMinX(), _ylabels_lefty[0], _max_lefty_data_limits.getMaxX(), _ylabels_lefty[_ylabels_lefty.Length - 1]);
			}
			else
			{
				_data_lefty_limits = new GRLimits(_start_date.toDouble(), _ylabels_lefty[0], _end_date.toDouble(), _ylabels_lefty[_ylabels_lefty.Length - 1]);
			}

			if (Message.isDebugOn)
			{
				Message.printDebug(1, routine, _gtype + "Found " + _ylabels_lefty.Length + " labels requesting " + minlabels + " to " + maxlabels);
			}
		}
		if (!_is_reference_graph)
		{
			...
			if (Message.isDebugOn)
			{
				Message.printDebug(1, routine, _gtype + "Y-axis labels (LeftYAxisDirection=" + __leftyDirection + ") resulted in data limits " + dataLimits);
			}
		...
		else if (__rightYAxisGraphType == TSGraphType.PERIOD)
		{
			...
			bool includeLeftYAxis = false;
			bool includeRightYAxis = true;
			IList<TS> enabledTSList = getEnabledTSList(includeRightYAxis,includeRightYAxis);
			_ylabels_righty = new double[enabledTSList.Count];
			for (int i = 0; i < enabledTSList.Count; i++)
			{
		...
		if ((_ylabels_righty == null) || (_ylabels_righty.Length == 0))
		{
			if (Message.isDebugOn)
			{
				Message.printDebug(1, routine, _gtype + "Unable to find right y-axis labels using " + minlabels + " to " + maxlabels + " labels.  Using end-point data values.");
			}
		...
		else
		{
			if (__rightYAxisGraphType == TSGraphType.PERIOD)
			{
				bool includeLeftYAxis = true;
				bool includeRightYAxis = false;
				_data_righty_limits = new GRLimits(_start_date.toDouble(), (getEnabledTSList(includeRightYAxis,includeRightYAxis).Count + 1), _end_date.toDouble(), 0.0);
			}
			else
			{
				_data_righty_limits = new GRLimits(_start_date.toDouble(), _ylabels_righty[0], _end_date.toDouble(), _ylabels_righty[_ylabels_righty.Length - 1]);
			}

			if (Message.isDebugOn)
			{
				Message.printDebug(1, routine, _gtype + "Found " + _ylabels_righty.Length + " labels requesting " + minlabels + " to " + maxlabels);
			}
		}
		if (!_is_reference_graph)
		{
			...
			if (Message.isDebugOn)
			{
				Message.printDebug(1, routine, _gtype + "Y-axis labels (RightYAxisDirection=" + __rightyDirection + ") resulted in data limits " + dataLimits);
			}
		...
		if (__leftYAxisGraphType == TSGraphType.DURATION)
		{
			...
			if (_xlabels == null)
			{
				if (Message.isDebugOn)
				{
					Message.printDebug(1, routine, _gtype + "Unable to find X labels using " + minlabels + " to " + maxlabels + " labels.  Using data values.");
				}
				_xlabels = new double [2];
				_xlabels[0] = _data_lefty_limits.getMinX();
				_xlabels[1] = _data_lefty_limits.getMaxX();
			}
			_data_lefty_limits = new GRLimits(_xlabels[0], _ylabels_lefty[0], _xlabels[_xlabels.Length - 1], _ylabels_lefty[_ylabels_lefty.Length - 1]);
			if (!_is_reference_graph)
			{
				if (__leftyDirection == GRAxisDirectionType.REVERSE)
				{
		...
		else if (__leftYAxisGraphType == TSGraphType.RASTER)
		{
			...
			bool includeLeftYAxis = true;
			bool includeRightYAxis = false;
			IList<TS> tslist = getEnabledTSList(includeLeftYAxis,includeRightYAxis);
			if (tslist.Count == 0)
			{
				return;
			}
		...
		else if (__leftYAxisGraphType == TSGraphType.XY_SCATTER)
		{
			...
			while (minlabels >= 3)
			{
				if (_regression_data != null)
				{
					...
					if (asLog)
					{
						_xlabels = GRAxis.findNLabels(_data_lefty_limits.getMinX(), _data_lefty_limits.getMaxX(), false, minlabels, maxlabels);
					}
					else
					{
						_xlabels = GRAxis.findNLabels(_data_lefty_limits.getMinX(), _data_lefty_limits.getMaxX(), false, minlabels, maxlabels);
				...
				else
				{
					...
					_xlabels = GRAxis.findNLabels(limitsLeftYAxis.getMinValue(),limitsLeftYAxis.getMaxValue(), false, minlabels, maxlabels);
				}
				if (_xlabels != null)
				{
					break;
		...
		IList<double> x_axis_labels_temp = new List<double>(10);
		...
		else if (_xaxis_date_precision == DateTime.PRECISION_HOUR)
		{
			...
			{
				nhours = (int)(TimeUtil.absoluteMinute(_end_date.getYear(),_end_date.getMonth(), _end_date.getDay(), _end_date.getHour(),_end_date.getMinute()) - TimeUtil.absoluteMinute(_start_date.getYear(),_start_date.getMonth(), _start_date.getDay(),_start_date.getHour(), _start_date.getMinute())) / 60;
			}
		...
		else if (_xaxis_date_precision == DateTime.PRECISION_MINUTE)
		{
			...
			if (dataIntervalBase == TimeInterval.IRREGULAR)
			{
				nminutes = (int)(TimeUtil.absoluteMinute(_end_date.getYear(),_end_date.getMonth(), _end_date.getDay(), _end_date.getHour(), _end_date.getMinute()) - TimeUtil.absoluteMinute(_start_date.getYear(), _start_date.getMonth(), _start_date.getDay(), _start_date.getHour(), _start_date.getMinute()));
			}
			else if (dataIntervalBase == TimeInterval.HOUR)
			{
				// Not likely but could happen
		...
		int size = x_axis_labels_temp.Count;
		_xlabels = new double[size];
		for (int i = 0; i < size; i++)
		{
			_xlabels[i] = x_axis_labels_temp[i];
			if (Message.isDebugOn)
			{
	...
	/// After the analysis, the data limits are recomputed (this is done for simple data also). </summary>
	...
	private void doAnalysis(TSGraphType graphType)
	{
		...
		if (Message.isDebugOn)
		{
			Message.printDebug(1, routine, "Analyzing time series for " + __leftYAxisGraphType + " graph - will produced derived data if necessary for output.");
		}
		...
		if (graphType == TSGraphType.XY_SCATTER)
		{
			...
			TS ts0 = null;
			TS ts = null;
			int nreg = tslist.Count - 1;
			if (nreg > 0)
			{
				_regression_data = new List<TSRegression> (nreg);
			}
			// Set the regression properties.  These may be changed by the properties interface.
			bool analyzeForFilling = false;
			if (_tsproduct.getLayeredPropValue("XYScatterAnalyzeForFilling", _subproduct, -1, false).Equals("True", StringComparison.OrdinalIgnoreCase))
			{
			...
			{
				intercept = double.Parse(propValue);
			}
			RegressionType analysisMethod = RegressionType.valueOfIgnoreCase(_tsproduct.getLayeredPropValue("XYScatterMethod", _subproduct, -1, false));
			int[] analysisMonths = StringUtil.parseIntegerSequenceArray(_tsproduct.getLayeredPropValue("XYScatterMonth", _subproduct, -1, false), ", ", StringUtil.DELIM_SKIP_BLANKS);
			NumberOfEquationsType numberOfEquations = NumberOfEquationsType.valueOfIgnoreCase(_tsproduct.getLayeredPropValue("XYScatterNumberOfEquations", _subproduct, -1, false));
			DataTransformationType transformation = DataTransformationType.valueOfIgnoreCase(_tsproduct.getLayeredPropValue("XYScatterTransformation", _subproduct, -1, false));
			...
			for (int i = 1; i <= nreg; i++)
			{
				...
				try
				{
					regressionData = new TSRegression(ts, ts0, analyzeForFilling, analysisMethod, intercept, numberOfEquations, analysisMonths, transformation, null, null, dependentAnalysisStart, dependentAnalysisEnd, independentAnalysisStart, independentAnalysisEnd, fillStart, fillEnd);
				}
				catch (Exception e)
				{
	...
	*/
		...
		else if ((graphType == TSGraphType.DURATION) && (tslist.Count != 0))
		{
			// Generate TSDurationAnalysis for each time series...
			int size = tslist.Count;
			_duration_data = new List<TSDurationAnalysis> (size);
			TSDurationAnalysis da = null;
			for (int i = 0; i < size; i++)
			{
				try
				{
					da = new TSDurationAnalysis(tslist[i]);
		...
		else if (graphType == TSGraphType.PREDICTED_VALUE || graphType == TSGraphType.PREDICTED_VALUE_RESIDUAL)
		{
			...
			for (int i = 1; i <= nreg; i++)
			{
				...
				try
				{
					...
					regressionData = new TSRegression(ts0, ts, false, RegressionType.MOVE2, null, NumberOfEquationsType.ONE_EQUATION, null, DataTransformationType.NONE, null, null, null, null, null, null, null, null); // FillEnd
	...
	{
		string routine = this.GetType().Name + ".doAnalysisAreaStacked";
		...
		IList<TS> derivedTSList = new List<TS>();
		// Get the overall period for the time series - all stacked time series will have the same period
		TSLimits limits = null;
		try
		{
			limits = TSUtil.getPeriodFromTS(tslist[0], tslist, TSUtil.MAX_POR);
		}
		catch (Exception)
		{
		...
		foreach (TS ts in tslist)
		{
			...
			{
				Message.printStatus(2,routine,"Time series [" + its + "] graph type (" + tsGraphType + ") is not stacked area type...set to null in derived list.");
				derivedTSList.Add(null);
				continue;
			}
			...
			if (newtsPrev == null)
			{
				...
				{
					Message.printWarning(3,routine, "Error changing period of record for stacked area time series [" + its + "]..." + "setting to null in derived list (" + e + ").");
					derivedTSList.Add(null);
					continue;
				}
			}
			else
			{
				...
				{
					Message.printWarning(3, routine, "Error adding time series for stacked area graph [" + its + "]..." + "setting to null in derived list (" + e + ").");
					derivedTSList.Add(null);
					continue;
				}
			}
		...
		Message.printStatus(2, routine, "Created " + derivedTSList.Count + " time series in derived list.");
		setDerivedTSList(derivedTSList);
	}

	/// <summary>
	/// Draws any annotations on the graph.  This method can be called multiple times, 
	/// once with false before drawing data and then with true after data have been drawn. </summary>
	/// <param name="drawingStepType"> indicates the step during drawing that should be matched.
	/// If the annotation "Order" property does not match the drawing step then the annotation will not be drawn.   </param>
	private void drawAnnotations(TSProduct tsproduct, int subproduct, GRDrawingArea daLeftYAxisGraph, GRDrawingArea daRightYAxisGraph, TSGraphDrawingStepType drawingStepType)
	{
		if (_is_reference_graph)
		{
			return;
		}
		string routine = this.GetType().Name + ".drawAnnotations(" + drawingStepType + ")";
		int na = tsproduct.getNumAnnotations(subproduct);
		PropList annotation = null;
		string s = null;
		string type = null;
		string points = null;
		IList<string> pointsV = null;
		string point = null;
		IList<string> pointV = null;
		bool valid = false;
		bool niceSymbols = true;
		bool isSymbol = false;
		...
		bool drawLeftYAxis = true;
		bool drawRightYAxis = false;
		for (int iatt = 0; iatt < na; iatt++)
		{
			...
			else if (type.Equals("Line", StringComparison.OrdinalIgnoreCase))
			{
				...
				if (string.ReferenceEquals(points, null))
				{
					valid = false;
				...
				else
				{
					...
					if (pointsV == null || pointsV.Count != 4)
					{
						valid = false;
			...
			else if (type.Equals("Symbol", StringComparison.OrdinalIgnoreCase))
			{
				point = tsproduct.getLayeredPropValue("Point", subproduct, iatt, false, true);
				if (string.ReferenceEquals(point, null))
				{
					valid = false;
			...
			if ((drawingStepType == TSGraphDrawingStepType.BEFORE_DATA) && !s.Equals("BehindData", StringComparison.OrdinalIgnoreCase))
			{
				// Current drawing step is before data but that does not match the annotation
				continue;
			}
			if ((drawingStepType == TSGraphDrawingStepType.AFTER_DATA) && !s.Equals("OnTopOfData", StringComparison.OrdinalIgnoreCase))
			{
				// Current drawing step is after data but that does not match the annotation
				continue;
			}
			if ((drawingStepType == TSGraphDrawingStepType.BEFORE_BACK_AXES) && !s.Equals("BehindAxes", StringComparison.OrdinalIgnoreCase))
			{
			...
			if ((!string.ReferenceEquals(annotationTableID, null)) && annotationTableID.Length > 0)
			{
			...
			else
			{
				...
				if ((!string.ReferenceEquals(yAxis, null)) && yAxis.Length > 0 && yAxis.Equals("Right", StringComparison.OrdinalIgnoreCase))
				{
					drawLeftYAxis = false;
					drawRightYAxis = true;
				}
	...
	/// includes the axes tic marks, titles, and labels.  The tic marks are currently always drawn in black. </summary>
	/// <param name="tsproduct"> time series product containing properties </param>
	/// <param name="subproduct"> subproduct (graph) number (0+) </param>
	private void drawAxesFront(TSProduct tsproduct, int subproduct, GRDrawingArea daMainTitle, bool drawLeftYAxisLabels, TSGraphType leftYAxisGraphType, GRDrawingArea daLeftYAxisGraph, GRDrawingArea daLeftYAxisTitle, GRDrawingArea daLeftYAxisLabel, GRLimits datalimLeftYAxisGraph, GRLimits datalimLeftYAxisTitle, GRLimits datalimLeftYAxisLabel, double[] ylabelsLeftYAxis, int leftYAxisPrecision, bool drawRightYAxisLabels, TSGraphType rightYAxisGraphType, GRDrawingArea daRightYAxisGraph, GRDrawingArea daRightYAxisTitle, GRDrawingArea daRightYAxisLabel, GRLimits datalimRightYAxisGraph, GRLimits datalimRightYAxisTitle, GRLimits datalimRightYAxisLabel, double[] ylabelsRightYAxis, int rightYAxisPrecision, GRDrawingArea daBottomXAxisTitle, GRDrawingArea daBottomXAxisLabel, GRLimits datalimBottomXAxisTitle, GRLimits datalimBottomXAxisLabel, double[] xlabelsBottomXAxis)
	{
		if (_is_reference_graph)
		{
			return;
		...
		bool leftYAxisLogY = false;
		prop_value = tsproduct.getLayeredPropValue("LeftYAxisType", subproduct, -1, false);
		if ((!string.ReferenceEquals(prop_value, null)) && prop_value.Equals("Log", StringComparison.OrdinalIgnoreCase))
		{
			leftYAxisLogY = true;
		}
		...
		bool yaxisDirReverse = false;
		if ((!string.ReferenceEquals(yaxisDir, null)) && yaxisDir.Equals("" + GRAxisDirectionType.REVERSE, StringComparison.OrdinalIgnoreCase))
		{
		...
		if (leftYAxisLogY)
		{
			...
			GRAxis.drawLabels(daLeftYAxisLabel, ylabels_log.Length, ylabels_log, datalimLeftYAxisLabel.getRightX(), GRAxis.Y, "%.1f", GRText.RIGHT | GRText.CENTER_Y);
			ylabels_log = null;
		}
		else
		{
			if (drawLeftYAxisLabels)
			{
				if (leftYAxisGraphType == TSGraphType.PERIOD)
				{
					...
					GRAxis.drawLabels(daLeftYAxisLabel, ylabelsLeftYAxis.Length, ylabelsLeftYAxis, datalimLeftYAxisLabel.getRightX(), GRAxis.Y, "%." + leftYAxisPrecision + "f", GRText.RIGHT | GRText.CENTER_Y);
				}
				else
				{
					GRAxis.drawLabels(daLeftYAxisLabel, ylabelsLeftYAxis.Length, ylabelsLeftYAxis, datalimLeftYAxisLabel.getRightX(), GRAxis.Y, "%." + leftYAxisPrecision + "f", GRText.RIGHT | GRText.CENTER_Y);
		...
		try
		{
			rotationDeg = double.Parse(rotation);
		}
		catch (System.FormatException)
		{
		...
		GRDrawingAreaUtil.drawText(daLeftYAxisTitle, title, datalimLeftYAxisTitle.getCenterX(), datalimLeftYAxisTitle.getCenterY(), 0.0, GRText.CENTER_X | GRText.CENTER_Y, rotationDeg);
		...
		if (drawRightYAxisLabels)
		{
			...
			bool yaxisDirRightReverse = false;
			if ((!string.ReferenceEquals(yaxisDirRight, null)) && yaxisDirRight.Equals("" + GRAxisDirectionType.REVERSE, StringComparison.OrdinalIgnoreCase))
			{
			...
			if (rightYAxisLogY)
			{
				...
				GRAxis.drawLabels(daRightYAxisLabel, ylabels_log.Length, ylabels_log, datalimRightYAxisLabel.getRightX(), GRAxis.Y, "%.1f", GRText.RIGHT | GRText.CENTER_Y);
				ylabels_log = null;
			}
			else if ((daRightYAxisLabel != null) && (ylabelsRightYAxis != null) && (datalimRightYAxisLabel != null))
			{ // Can be null if no right y-axis
				if (rightYAxisGraphType == TSGraphType.PERIOD)
				{
					// Only want to label with whole numbers that are > 0 and <= __tslist.size()...
					// FIXME SAM 2013-07-21 This is no different than below.  Precision is being set elsewhere for PERIOD graph
					GRAxis.drawLabels(daRightYAxisLabel, ylabelsRightYAxis.Length, ylabelsRightYAxis, datalimRightYAxisLabel.getLeftX(), GRAxis.Y, "%." + rightYAxisPrecision + "f", GRText.LEFT | GRText.CENTER_Y);
				}
				else
				{
					GRAxis.drawLabels(daRightYAxisLabel, ylabelsRightYAxis.Length, ylabelsRightYAxis, datalimRightYAxisLabel.getLeftX(), GRAxis.Y, "%." + rightYAxisPrecision + "f", GRText.LEFT | GRText.CENTER_Y);
			...
			if ((!string.ReferenceEquals(position, null)) && !position.Equals("None", StringComparison.OrdinalIgnoreCase))
			{
				...
				GRDrawingAreaUtil.drawText(daRightYAxisTitle, title, datalimRightYAxisTitle.getCenterX(), datalimRightYAxisTitle.getCenterY(), 0.0, GRText.CENTER_X | GRText.CENTER_Y, rotationDeg);
			}
		}
		...
		GRDrawingAreaUtil.drawText(daBottomXAxisTitle, title, datalimBottomXAxisTitle.getCenterX(), datalimBottomXAxisTitle.getCenterY(), 0.0, GRText.CENTER_X | GRText.CENTER_Y);
		...
		if ((leftYAxisGraphType == TSGraphType.XY_SCATTER) || (leftYAxisGraphType == TSGraphType.DURATION))
		{
			...
			GRAxis.drawLabels(daBottomXAxisLabel, xlabelsBottomXAxis.Length, xlabelsBottomXAxis, datalimBottomXAxisLabel.getTopY(), GRAxis.X, "%.1f", GRText.TOP | GRText.CENTER_X);
	...
	private void drawCurrentDateTime()
	{
		...
		if ((string.ReferenceEquals(prop_value, null)) || (prop_value.Trim().Length == 0) || prop_value.Equals("None", StringComparison.OrdinalIgnoreCase))
		{
			return;
		}
		try
		{
			...
			if (prop_value.Equals("Auto", StringComparison.OrdinalIgnoreCase))
			{
	...
	/// </summary>
	public virtual void drawDrawingAreas()
	{ // This method is used by developers so OK to use global object data extensively.
		//boolean do_names = false;
		bool do_names = true; // Display drawing area names
		_da_page.setColor(GRColor.magenta); // Sets color for all
		GRDrawingAreaUtil.setFont(_da_page, "Helvetica", "Plain", 8);
		// Reference and main...
		GRDrawingAreaUtil.drawRectangle(_da_lefty_graph, _data_lefty_limits.getLeftX(), _data_lefty_limits.getBottomY(), _data_lefty_limits.getWidth(), _data_lefty_limits.getHeight());
		if (do_names)
		{
			GRDrawingAreaUtil.drawText(_da_lefty_graph, _da_lefty_graph.getName(), _data_lefty_limits.getCenterX(), _data_lefty_limits.getCenterY(), 0.0, GRText.CENTER_X | GRText.CENTER_Y);
		}
		if ((_da_righty_graph != null) && (_data_righty_limits != null))
		{
			GRDrawingAreaUtil.drawRectangle(_da_righty_graph, _data_righty_limits.getLeftX(), _data_righty_limits.getBottomY(), _data_righty_limits.getWidth(), _data_righty_limits.getHeight());
			if (do_names)
			{
				// Add a little bit to the height so does not overlap the left y-axis text
				GRDrawingAreaUtil.drawText(_da_righty_graph, _da_righty_graph.getName(), _data_righty_limits.getCenterX(), (_data_righty_limits.getCenterY() + _data_righty_limits.getHeight() * .05), 0.0, GRText.CENTER_X | GRText.CENTER_Y);
			}
		}
		if (_is_reference_graph)
		{
		...
		GRDrawingAreaUtil.drawRectangle(_da_page, _datalim_page.getLeftX(), _datalim_page.getBottomY(), _datalim_page.getWidth(), _datalim_page.getHeight());
		if (do_names)
		{
			GRDrawingAreaUtil.drawText(_da_page, _da_page.getName(), _datalim_page.getCenterX(), _datalim_page.getCenterY(), 0.0, GRText.CENTER_X | GRText.CENTER_Y);
		}

		GRDrawingAreaUtil.drawRectangle(_da_maintitle, _datalim_maintitle.getLeftX(), _datalim_maintitle.getBottomY(), _datalim_maintitle.getWidth(), _datalim_maintitle.getHeight());
		if (do_names)
		{
			GRDrawingAreaUtil.drawText(_da_maintitle, _da_maintitle.getName(), _datalim_maintitle.getCenterX(), _datalim_maintitle.getCenterY(), 0.0, GRText.CENTER_X | GRText.CENTER_Y);
		}

		GRDrawingAreaUtil.drawRectangle(_da_subtitle, _datalim_subtitle.getLeftX(), _datalim_subtitle.getBottomY(), _datalim_subtitle.getWidth(), _datalim_subtitle.getHeight());
		if (do_names)
		{
			GRDrawingAreaUtil.drawText(_da_subtitle, _da_subtitle.getName(), _datalim_subtitle.getCenterX(), _datalim_subtitle.getCenterY(), 0.0, GRText.CENTER_X | GRText.CENTER_Y);
		}
	...
	*/
		...
		GRDrawingAreaUtil.drawRectangle(_da_lefty_title, _datalim_lefty_title.getLeftX(), _datalim_lefty_title.getBottomY(), _datalim_lefty_title.getWidth(), _datalim_lefty_title.getHeight());
		if (do_names)
		{
			GRDrawingAreaUtil.drawText(_da_lefty_title, _da_lefty_title.getName(), _datalim_lefty_title.getCenterX(), _datalim_lefty_title.getCenterY(), 0.0, GRText.CENTER_X | GRText.CENTER_Y);
		}

		GRDrawingAreaUtil.drawRectangle(_da_lefty_label, _datalim_lefty_label.getLeftX(), _datalim_lefty_label.getBottomY(), _datalim_lefty_label.getWidth(), _datalim_lefty_label.getHeight());
		if (do_names)
		{
			GRDrawingAreaUtil.drawText(_da_lefty_label, _da_lefty_label.getName(), _datalim_lefty_label.getCenterX(), _datalim_lefty_label.getCenterY(), 0.0, GRText.CENTER_X | GRText.CENTER_Y);
		}

		GRDrawingAreaUtil.drawRectangle(_da_righty_title, _datalim_righty_title.getLeftX(), _datalim_righty_title.getBottomY(), _datalim_righty_title.getWidth(), _datalim_righty_title.getHeight());
		if (do_names)
		{
			GRDrawingAreaUtil.drawText(_da_righty_title, _da_righty_title.getName(), _datalim_righty_title.getCenterX(), _datalim_righty_title.getCenterY(), 0.0, GRText.CENTER_X | GRText.CENTER_Y);
		}

		GRDrawingAreaUtil.drawRectangle(_da_righty_label, _datalim_righty_label.getLeftX(), _datalim_righty_label.getBottomY(), _datalim_righty_label.getWidth(), _datalim_righty_label.getHeight());
		if (do_names)
		{
			GRDrawingAreaUtil.drawText(_da_righty_label, _da_righty_label.getName(), _datalim_righty_label.getCenterX(), _datalim_righty_label.getCenterY(), 0.0, GRText.CENTER_X | GRText.CENTER_Y);
		}

		GRDrawingAreaUtil.drawRectangle(_da_bottomx_label, _datalim_bottomx_label.getLeftX(), _datalim_bottomx_label.getBottomY(), _datalim_bottomx_label.getWidth(), _datalim_bottomx_label.getHeight());
		if (do_names)
		{
			GRDrawingAreaUtil.drawText(_da_bottomx_label, _da_bottomx_label.getName(), _datalim_bottomx_label.getCenterX(), _datalim_bottomx_label.getCenterY(), 0.0, GRText.CENTER_X | GRText.CENTER_Y);
		}

		GRDrawingAreaUtil.drawRectangle(_da_bottomx_title, _datalim_bottomx_title.getLeftX(), _datalim_bottomx_title.getBottomY(), _datalim_bottomx_title.getWidth(), _datalim_bottomx_title.getHeight());
		if (do_names)
		{
			GRDrawingAreaUtil.drawText(_da_bottomx_title, _da_bottomx_title.getName(), _datalim_bottomx_title.getCenterX(), _datalim_bottomx_title.getCenterY(), 0.0, GRText.CENTER_X | GRText.CENTER_Y);
		}

		GRDrawingAreaUtil.drawRectangle(_da_lefty_bottom_legend, _datalim_lefty_bottom_legend.getLeftX(), _datalim_lefty_bottom_legend.getBottomY(), _datalim_lefty_bottom_legend.getWidth(), _datalim_lefty_bottom_legend.getHeight());
		if (do_names)
		{
			GRDrawingAreaUtil.drawText(_da_lefty_bottom_legend, _da_lefty_bottom_legend.getName(), _datalim_lefty_bottom_legend.getCenterX(), _datalim_lefty_bottom_legend.getCenterY(), 0.0, GRText.CENTER_X | GRText.CENTER_Y);
		}

		GRDrawingAreaUtil.drawRectangle(_da_lefty_left_legend, _datalim_lefty_left_legend.getLeftX(), _datalim_lefty_left_legend.getBottomY(), _datalim_lefty_left_legend.getWidth(), _datalim_lefty_left_legend.getHeight());
		if (do_names)
		{
			GRDrawingAreaUtil.drawText(_da_lefty_left_legend, _da_lefty_left_legend.getName(), _datalim_lefty_left_legend.getCenterX(), _datalim_lefty_left_legend.getCenterY(), 0.0, GRText.CENTER_X | GRText.CENTER_Y);
		}

		GRDrawingAreaUtil.drawRectangle(_da_lefty_right_legend, _datalim_lefty_right_legend.getLeftX(), _datalim_lefty_right_legend.getBottomY(), _datalim_lefty_right_legend.getWidth(), _datalim_lefty_right_legend.getHeight());
		if (do_names)
		{
			GRDrawingAreaUtil.drawText(_da_lefty_right_legend, _da_lefty_right_legend.getName(), _datalim_lefty_right_legend.getCenterX(), _datalim_lefty_right_legend.getCenterY(), 0.0, GRText.CENTER_X | GRText.CENTER_Y);
		}
	}

	/// <summary>
	/// Draw a duration plot. </summary>
	/// <param name="tslist"> the list of time series to render </param>
	private void drawDurationPlot(GRDrawingArea daGraph, TSProduct tsproduct, int subproduct, IList<TS> tslist, IList<TSDurationAnalysis> durationAnalysisList, bool isReferenceGraph)
	{
		...
		for (int i = 0; i < size; i++)
		{
			...
			if (!string.ReferenceEquals(prop_value, null))
			{
				...
				{
					GRDrawingAreaUtil.setLineWidth(daGraph, int.Parse(prop_value));
				}
			}
			...
			if (!isReferenceGraph && (symbol != GRSymbol.SYM_NONE) && (symbol_size > 0))
			{
				...
				GRDrawingAreaUtil.drawSymbols(daGraph, symbol, values.Length, percents, values, symbol_size, GRUnits.DEVICE, GRSymbol.SYM_CENTER_X | GRSymbol.SYM_CENTER_Y);
				if (niceSymbols)
				{
	...
	{
		if (this.__errorMessageList.Count > 0)
		{
			...
			string error = this.__errorMessageList[0];
			bool doFill = false; // For now don't fill
			if (doFill)
			{
				// Use black text in filled drawing area
				GRDrawingAreaUtil.setColor(daError, GRColor.lightGray);
				GRDrawingAreaUtil.fillRectangle(daError, daError.getDataLimits().getLeftX(), daError.getDataLimits().getBottomY(), daError.getDataLimits().getWidth(), daError.getDataLimits().getHeight());
			...
			GRDrawingAreaUtil.drawText(daError, " " + error, daError.getDataLimits().getLeftX(), daError.getDataLimits().getTopY(), 0.0, GRText.LEFT | GRText.TOP);
	...
	/// other time series drawing methods. </summary>
	/// <param name="graphType"> graph type to draw, for the axis. </param>
	/// <param name="daGraph"> drawing area for the graph, could be aligned with left y-axis or right y-axis. </param>
	/// <param name="tslist"> list of time series to render, might be all time series or only one axis, depending on graph type. </param>
	/// <param name="dataLimits"> data limits for drawing area, consistent with nice labels, and zoom. </param>
	private void drawGraph(TSGraphType graphType, GRDrawingArea daGraph, TSProduct tsproduct, int subproduct, IList<TS> tslist, GRLimits dataLimits)
	{
		...
		if (Message.isDebugOn)
		{
			Message.printDebug(1, routine, _gtype + "Drawing graph type " + graphType + ", " + size + " time series total (some may be other graph types).");
		}
		if (size == 0)
		{
			return;
		...
		*/
		...
		if (graphType == TSGraphType.DURATION)
		{
			...
			drawDurationPlot(daGraph, tsproduct, subproduct, tslist, _duration_data, _is_reference_graph);
		}
		...
		else
		{
			...
			if (Message.isDebugOn)
			{
				Message.printStatus(2, routine, _gtype + "In drawGraph _is_reference_graph=" + _is_reference_graph + " tslist.size()=" + tslist.Count);
			}
	...
	/// <summary>
	/// Draw the time series graph for an "AreaStacked" graph. </summary>
	/// <param name="tslist"> list of time series to draw, all time series for the axis. </param>
	/// <param name="derivedTSList"> list of derived time series needed for stacked area graph, time series cumulative values. </param>
	private void drawGraphAreaStacked(TSProduct tsproduct, int subproduct, IList<TS> tslist, IList<TS> derivedTSList)
	{
		...
		for (int its = 0; its < size; its++)
		{
			...
			if (tsGraphType != TSGraphType.AREA_STACKED)
			{
				drawTS(tsproduct, subproduct, its, tslist[its], tsGraphType);
			}
		}
	}

	/// <summary>
	/// Draw the time series graph for a "Raster" graph. </summary>
	/// <param name="run">-time overrideProps override properties for the graph </param>
	private void drawGraphRaster(TSProduct tsproduct, int subproduct, IList<TS> tslist)
	{
		string routine = this.GetType().Name + ".drawGraphRaster";
		...
		TS ts = null;
		int its;
		for (its = 0; its < tslist.Count; its++)
		{
			ts = tslist[its];
	...
	/// right, and bottom legends.  Additional logic may need to be implemented when the top legend is supported. </summary>
	/// <param name="axis"> GRAxis.LEFT or GRAxis.RIGHT, indicating which y-axis legend to draw. </param>
	private void drawLegend(int axis)
	{
		string routine = this.GetType().Name + ".drawLegend";
		if (_is_reference_graph)
		{
		...
		IList<TS> tslistForAxis = new List<TS>(); // List of time series in the legend, depends on whether left or right
		IList<TS> tslist = __tslist; // All the time series for the graph, needed to get the right color, based on color defaults
		...
		if (axis == GRAxis.LEFT)
		{
			...
			legendPosition = _tsproduct.getLayeredPropValue("LeftYAxisLegendPosition", _subproduct, -1, false);
			if (string.ReferenceEquals(legendPosition, null))
			{
				// Legacy value being transitioned to "LeftYAxisLegendPosition" because of addition of right y-axis
				legendPosition = _tsproduct.getLayeredPropValue("LegendPosition", _subproduct, -1, false);
			}
		...
		if (legendPosition.ToUpper().StartsWith("BOTTOM", StringComparison.Ordinal))
		{
			if (axis == GRAxis.LEFT)
			{
				da_legend = _da_lefty_bottom_legend;
				datalim_legend = _datalim_lefty_bottom_legend;
			}
			else if (axis == GRAxis.RIGHT)
			{
				da_legend = _da_righty_bottom_legend;
				datalim_legend = _datalim_righty_bottom_legend;
			}
		}
		else if (legendPosition.Equals("Left", StringComparison.OrdinalIgnoreCase))
		{
			if (axis == GRAxis.LEFT)
			{
				da_legend = _da_lefty_left_legend;
				datalim_legend = _datalim_lefty_left_legend;
			}
			else if (axis == GRAxis.RIGHT)
			{
				da_legend = _da_righty_left_legend;
				datalim_legend = _datalim_righty_left_legend;
			}
		}
		else if (legendPosition.Equals("Right", StringComparison.OrdinalIgnoreCase))
		{
			if (axis == GRAxis.LEFT)
			{
				da_legend = _da_lefty_right_legend;
				datalim_legend = _datalim_lefty_right_legend;
			}
			else if (axis == GRAxis.RIGHT)
			{
				da_legend = _da_righty_right_legend;
				datalim_legend = _datalim_righty_right_legend;
			}
		}
		else if (StringUtil.startsWithIgnoreCase(legendPosition, "Inside"))
		{
			if (axis == GRAxis.LEFT)
			{
				da_legend = _da_lefty_inside_legend;
				datalim_legend = _datalim_lefty_inside_legend;
			}
			else if (axis == GRAxis.RIGHT)
			{
				da_legend = _da_righty_inside_legend;
				datalim_legend = _datalim_righty_inside_legend;
			}
		...
		if ((axis == GRAxis.LEFT) && (__leftYAxisGraphType == TSGraphType.PREDICTED_VALUE_RESIDUAL))
		{
			size = size - 1;
		}
		...
		double legendLineLength = 25.0; // Pixels width of the legend line
		...
		for (int i = iStart; i != iEnd; i = i + iIncrement)
		{
			...
			if ((axis == GRAxis.LEFT) && (__leftYAxisGraphType == TSGraphType.PREDICTED_VALUE))
			{
				...
				if (predicted)
				{
					...
					regressionData = _regression_data[tsNum - 1];
					ts = regressionData.getPredictedTS();

				}
				else
				{
					ts = tslistForAxis[tsNum];
				}
				...
				if (string.ReferenceEquals(line_style, null))
				{
					line_style = "None";
				}
			...
			else if ((axis == GRAxis.LEFT) && (__leftYAxisGraphType == TSGraphType.PREDICTED_VALUE_RESIDUAL))
			{
				if (!isTSEnabled(i + 1))
				{
					continue;
			...
			else
			{
				...
				if ((i == 0) && legendPosition.Equals("BottomRight", StringComparison.OrdinalIgnoreCase))
				{
					...
					for (int its = 0; its < size; its++)
					{
						if (isTSEnabled(its))
						{
							...
							if (string.ReferenceEquals(legend2, null))
							{
								continue;
							}
							else
					...
					{
						// Have layout issues and have gone past the left edge of the window
						// so draw with offset zero - use will have to resize window or adjust legend format
						xOffset = 0.0;
					}
					else
					{
				...
				line_style = _tsproduct.getLayeredPropValue("LineStyle", _subproduct, imatch, false);
			...
			else if ((tsGraphType == TSGraphType.AREA) || (tsGraphType == TSGraphType.AREA_STACKED) || (tsGraphType == TSGraphType.BAR) || (tsGraphType == TSGraphType.PREDICTED_VALUE_RESIDUAL))
			{
			...
			else
			{
				...
				{
					prop_value = getLayeredPropValue("LineWidth", _subproduct, imatch, false, null);
				}
				if (!string.ReferenceEquals(prop_value, null))
				{
					if (StringUtil.isInteger(prop_value))
					{
						...
						int lineWidth = int.Parse(prop_value);
				...
				if (axisGraphType != TSGraphType.POINT)
				{
					...
					if (line_style.Equals("Solid", StringComparison.OrdinalIgnoreCase))
					{
						GRDrawingAreaUtil.drawLine(da_legend, x, y);
					}
					else if (line_style.Equals("Dashed", StringComparison.OrdinalIgnoreCase))
					{
				...
				if ((symbol != GRSymbol.SYM_NONE) && (symbol_size > 0))
				{
					...
					GRDrawingAreaUtil.drawSymbol(da_legend, symbol, (x[0] + x[1]) / 2.0, y[0], symbol_size, GRUnits.DEVICE, GRSymbol.SYM_CENTER_X | GRSymbol.SYM_CENTER_Y);
					if (niceSymbols)
					{
			...
			if (ts != null)
			{
				...
				GRLimits legendDrawLimits = new GRLimits(xll,yll, (xll + textDrawLim.getWidth()), (yll - textDrawLim.getHeight()));
				__legendTimeSeriesDrawMap[ts] = legendDrawLimits;
	...
	private void drawOutlineBox()
	{
		...
		{ // Normal drawing area...
			GRDrawingAreaUtil.drawRectangle(_da_lefty_graph, _data_lefty_limits.getMinX(), _data_lefty_limits.getMinY(), _data_lefty_limits.getWidth(), _data_lefty_limits.getHeight());
		}
	}
	...
	private void drawTitles()
	{
		...
		GRDrawingAreaUtil.setFont(_da_maintitle, maintitle_font, maintitle_fontstyle, StringUtil.atod(maintitle_fontsize));
		string maintitle_string = _tsproduct.expandPropertyValue(_tsproduct.getLayeredPropValue("MainTitleString", _subproduct, -1, false));
		GRDrawingAreaUtil.drawText(_da_maintitle, maintitle_string, _datalim_maintitle.getCenterX(), _datalim_maintitle.getCenterY(), 0.0, GRText.CENTER_X | GRText.CENTER_Y);
		...
		string subtitle_string = _tsproduct.expandPropertyValue(_tsproduct.getLayeredPropValue("SubTitleString", _subproduct, -1, false));
		GRDrawingAreaUtil.drawText(_da_subtitle, subtitle_string, _datalim_subtitle.getCenterX(), _datalim_subtitle.getCenterY(), 0.0, GRText.CENTER_X | GRText.CENTER_Y);
	}

	/// <summary>
	/// Draw (render) a single time series on the graph with no override properties. </summary>
	/// <param name="its"> the time series list position (0+, for retrieving properties and messaging) </param>
	/// <param name="ts"> the time series to render </param>
	/// <param name="graphType"> the graph type to use for the time series </param>
	...
	/// <summary>
	/// Draw a single time series. </summary>
	/// <param name="its"> the time series list position (0+, for retrieving properties and messaging) </param>
	/// <param name="ts"> Single time series to draw. </param>
	/// <param name="tsGraphType"> the graph type to use for the time series, typically the same as the
	/// graph type, but can be different if overlaying lines on area graph, for example. </param>
	/// <param name="overrideProps"> run-time override properties to consider when getting graph properties </param>
	private void drawTS(TSProduct tsproduct, int subproduct, int its, TS ts, TSGraphType tsGraphType, PropList overrideProps)
	{
		...
		bool drawUsingLeftYAxis = true; // Simplify logic to know when drawing using left Y-axis
		bool drawUsingRightYAxis = false; // Simplify logic to know when drawing using right Y-axis
		...
		bool ignoreYAxisUnits = _ignoreLeftAxisUnits;
		string yAxisUnitsProperty = "LeftYAxisUnits"; // Used to look up units for time series
		if ((!string.ReferenceEquals(yAxis, null)) && yAxis.Equals("right", StringComparison.OrdinalIgnoreCase))
		{
		...
		*/
		...
		if (Message.isDebugOn)
		{
			Message.printDebug(1, routine, _gtype + "Drawing time series " + start + " to " + end + " global period is: " + _start_date + " to " + _end_date + " yaxis=" + yAxis);
		}
		...
		bool graphLabelFormatSet = false;
		...
		if (string.ReferenceEquals(label_format, null) || label_format.Equals(""))
		{
		...
		if (labelPointWithFlag)
		{
			...
			if (graphLabelFormatSet || label_position_string.Equals("") || label_position_string.Equals("Auto", StringComparison.OrdinalIgnoreCase))
			{
				// Try to get from the graph properties.
				label_position_string = getLayeredPropValue("DataLabelPosition", subproduct, -1, false, overrideProps);
				if (label_position_string.Equals("") || label_position_string.Equals("Auto", StringComparison.OrdinalIgnoreCase))
				{
					// Default position
		...
		if (!string.ReferenceEquals(prop_value, null))
		{
			if (prop_value.Equals("LeftOfDate", StringComparison.OrdinalIgnoreCase))
			{
				bar_position = -1;
			}
			else if (prop_value.Equals("RightOfDate", StringComparison.OrdinalIgnoreCase))
			{
				bar_position = 1;
			}
		}

		bool barOverlap = false; // whether bars are overlapped
		...
		int nts = getEnabledTSList(drawUsingLeftYAxis,drawUsingRightYAxis).Count;

		if (drawUsingLeftYAxis && (yAxisGraphType == TSGraphType.PREDICTED_VALUE_RESIDUAL) && (__tslist != null))
		{
			int numReg = __tslist.Count - 1;
			nts = 0;
		...
		if (!string.ReferenceEquals(prop_value, null) && prop_value.Equals("None", StringComparison.OrdinalIgnoreCase))
		{
			draw_line = false;
		}

		prop_value = getLayeredPropValue("GraphType", subproduct, -1, false, overrideProps);
		if (!string.ReferenceEquals(prop_value, null) && prop_value.Equals("Point", StringComparison.OrdinalIgnoreCase))
		{
			draw_line = false;
		}
		...
		else
		{
			...
			if (!string.ReferenceEquals(prop_value, null))
			{
				if (StringUtil.isInteger(prop_value))
				{
					lineWidth = int.Parse(prop_value);
					if (lineWidth < 0)
					{
						lineWidth = 1;
					}
		...
		prop_value = getLayeredPropValue("LineStyle", subproduct, its, false, overrideProps);
		if (!string.ReferenceEquals(prop_value, null) && prop_value.Equals("Dashed", StringComparison.OrdinalIgnoreCase))
		{
			dashedLine = true;
		...
		if (interval_base == TimeInterval.IRREGULAR)
		{
			...
			for (int i = 0; i < nalltsdata; i++)
			{
				...
				if (date.greaterThanOrEqualTo(start))
				{
					...
					if (((drawcount == 0) || ts.isDataMissing(lasty)) && (yAxisGraphType != TSGraphType.BAR && yAxisGraphType != TSGraphType.PREDICTED_VALUE_RESIDUAL))
					{
						...
						{
							// Text only
							GRDrawingAreaUtil.drawText(daGraph, TSData.ToString(label_format,label_value_format, date, y, 0.0, tsdata.getDataFlag().Trim(),label_units), x, y, 0.0, label_position);
						}
						else if (labelPointWithFlag)
						{
							...
							// Text and symbol
							GRDrawingAreaUtil.drawSymbolText(daGraph, symbol, x, y, symbol_size, TSData.ToString(label_format,label_value_format, date, y, 0.0, tsdata.getDataFlag().Trim(), label_units), 0.0, label_position, GRUnits.DEVICE, GRSymbol.SYM_CENTER_X | GRSymbol.SYM_CENTER_Y);
						...
						else
						{
							...
							GRDrawingAreaUtil.drawSymbol(daGraph, symbol, x, y, symbol_size, GRUnits.DEVICE, GRSymbol.SYM_CENTER_X | GRSymbol.SYM_CENTER_Y);
							...
							if (draw_line)
							{
								...
								if (this.useXYCache)
								{
									...
									if (pointsInSegment == this.xCacheArray.Length)
									{
										...
										Array.Copy(this.xCacheArray, 0, xTemp, 0, this.xCacheArray.Length);
										this.xCacheArray = xTemp;
										double[] yTemp = new double[newLength];
										Array.Copy(this.yCacheArray, 0, yTemp, 0, this.yCacheArray.Length);
			...
			if (this.useXYCache && ((yAxisGraphType != TSGraphType.BAR) && (yAxisGraphType != TSGraphType.PREDICTED_VALUE_RESIDUAL)) && (pointsInSegment > 0) && !yIsMissing)
			{
		...
		else
		{
			...
			bool doDataPoint = false; // Symbol will be determined from "symbolNoFlag" value
	...
	/// <summary>
	/// Determine the end date/time for rendering the time series </summary>
	/// <param name="ts"> the time series being rendered </param>
	/// <returns> the end date/time for iterating time series data </returns>
	...
	/// <summary>
	/// Determine the start date/time for drawing the time series. </summary>
	/// <param name="ts"> the time series being rendered </param>
	/// <returns> the start date/time for iterating time series data </returns>
	...
	/// <summary>
	/// Determine the symbol size used for drawing a time series. </summary>
	/// <param name="its"> the time series list position (0+, for retrieving properties and messaging) </param>
	/// <param name="overrideProps"> run-time override properties to consider when getting graph properties </param>
	/// <param name="return"> the symbol style from product properties </param>
	...
	/// <summary>
	/// Determine the symbol style used for drawing a time series. </summary>
	/// <param name="its"> the time series list position (0+, for retrieving properties and messaging) </param>
	/// <param name="overrideProps"> run-time override properties to consider when getting graph properties </param>
	/// <param name="flaggedData"> if true then the symbol is determined from the "FlaggedDataSymbolStyle" property (if not set
	/// then "SymbolStyle" is used); if false the symbol is determined from the "SymbolStyle" property </param>
	/// <returns> the symbol style from product properties </returns>
	private int drawTSHelperGetSymbolStyle(int its, PropList overrideProps, bool flaggedData)
	{
		...
		else
		{
			...
			if (flaggedData)
			{
				...
				if ((string.ReferenceEquals(propValue, null)) || propValue.Equals("") || propValue.Equals("None", StringComparison.OrdinalIgnoreCase))
				{
	...
	/// data for the graph to preserve positioning. </summary>
	/// <param name="its"> the time series list position (0+, for retrieving properties and messaging) </param>
	/// <param name="overrideProps"> run-time override properties to consider when getting graph properties </param>
	/// <param name="return"> the color that is set </param>
	...
	/// <param name="its"> Counter for time series (starting at 0). </param>
	/// <param name="ts"> Single time series to draw. </param>
	/// <param name="graphType"> the graph type to use for the time series (may be needed for other calls). </param>
	/// <param name="overrideProps"> override run-time properties to consider when getting graph properties </param>
	...
	{ //String routine = "TSGraph.drawTSRenderAreaGraph";
		...
		bool anchorToZero = true; // If true, always anchor around zero.  If false, draw from bottom of graph
		while (true)
		{
			tsdata = tsi.next();
			yIsMissing = false;
	...
	/// <summary>
	/// Draw a single time series as a raster.  The time series values are used to create rectangles that are color-coded. </summary>
	/// <param name="ts"> Single time series to draw. </param>
	/// <param name="graphType"> the graph type to use for the time series (may be needed for other calls). </param>
	/// <param name="overrideProps"> override run-time properties to consider when getting graph properties </param>
	...
	{ //String routine = "TSGraph.drawTSRenderRasterGraph";
		...
		GRColor[] scaleColors = (GRColor [])GRColorTable.createColorTable(GRColorTable.BLUE_TO_RED, nScaleColors, true).ToArray(typeof(GRColor));
		...
		for (int i = 0; i < nScaleColors; i++)
		{
			...
			Message.printStatus(2, "", "Scale " + value + " color [" + i + "] = " + scaleColors[i].getRed() + "," + scaleColors[i].getGreen() + "," + scaleColors[i].getBlue());
		}
		...
		Message.printStatus(2,"","Drawing raster graph for start=" + start + " end=" + end + " ts.date1=" + ts.getDate1() + " ts.date2=" + ts.getDate2() + " data limits = " + _da_lefty_graph.getDataLimits());
	...
	private void drawXAxisGrid()
	{
		if ((__leftYAxisGraphType == TSGraphType.XY_SCATTER) || (__leftYAxisGraphType == TSGraphType.DURATION) || (__leftYAxisGraphType == TSGraphType.RASTER))
		{
			...
			if ((string.ReferenceEquals(color_prop, null)) || color_prop.Equals("None", StringComparison.OrdinalIgnoreCase))
			{
				return;
			}
	...
	/// set as true to draw the background grid, or "draw_grid" set to false to draw the labels. </summary>
	/// <param name="graphType"> the graph type being drawn </param>
	/// <param name="drawGrid"> if true, draw the x-axis grid lines </param>
	private void drawXAxisDateLabels(TSGraphType graphType, bool drawGrid)
	{
		...
		{
			// Use 8 pixels converted back to data units
		...
		{
			// Reversed axes...
			IList<TS> tslist = getEnabledTSList(true,false);
			yt[0] = tslist.Count + 1;
			yt2[0] = tslist.Count + 1;
			tic_height = _data_lefty_limits.getHeight() * .02;
			yt[1] = yt[0] - tic_height;
			yt2[1] = yt2[0] - tic_height / 2.0;
		}
		...
		if (graphType == TSGraphType.RASTER)
		{
			...
			for (int i = 1; i <= 12; i++)
			{
				...
				GRDrawingAreaUtil.drawText(_da_bottomx_label, TimeUtil.monthAbbreviation(i), x, _datalim_bottomx_label.getTopY(), 0.0, GRText.CENTER_X | GRText.TOP);
			}
			return;
		}
		...
		if ((_xaxis_date_precision == DateTime.PRECISION_YEAR) || ((_end_date.getAbsoluteMonth() - _start_date.getAbsoluteMonth()) > 36))
		{
			...
			int[] year_increments = new int[] {1, 2, 5, 10, 20, 25, 50, 100};
			...
			for (; ; date.addYear(year_increment))
			{
				...
				else
				{
					...
					GRDrawingAreaUtil.drawText(_da_bottomx_label, date.ToString(), x, _datalim_bottomx_label.getTopY(), 0.0, GRText.CENTER_X | GRText.TOP);
		...
		else if ((_xaxis_date_precision == DateTime.PRECISION_MONTH) || ((_end_date.getAbsoluteDay() - _start_date.getAbsoluteDay() > 90)))
		{
			...
			// First try with the visible start date and see if all years
			// can be shown.  Determine by seeing if the first two overlap..
			int[] month_increments = new int[] {1, 2, 3, 4, 6};
			...
			for (; ; date.addMonth(month_increment))
			{
				...
				{
					GRDrawingAreaUtil.drawText(_da_bottomx_label, "" + date.getMonth(),x, _datalim_bottomx_label.getTopY(), 0.0, GRText.CENTER_X | GRText.TOP);
					if (date.getMonth() <= month_increment)
					{
						// Label the year...
						GRDrawingAreaUtil.drawText(_da_bottomx_label, "" + date.getYear(), x, _datalim_bottomx_label.getBottomY(), 0.0, GRText.CENTER_X | GRText.BOTTOM);
						++nlabel2;
					}
					// Draw tick marks at the labels...
					xt[0] = x;
		...
		else if ((_xaxis_date_precision == DateTime.PRECISION_DAY) || (TSUtil.calculateDataSize(_start_date,_end_date, TimeInterval.HOUR, 1) > 168))
		{
			...
			// First try with the visible start date and see if all years
			// can be shown.  Determine by seeing if the first two overlap..
			int[] day_increments = new int[] {1, 2, 7};
			int day_increment = 1;
			bool found = false;
			...
			for (; ; date.addDay(day_increment))
			{
				...
				{
					GRDrawingAreaUtil.drawText(_da_bottomx_label, "" + date.getDay(), x, _datalim_bottomx_label.getTopY(), 0.0, GRText.CENTER_X | GRText.TOP);
					if (date.getDay() <= day_increment)
					{
						// Label the year and month...
						GRDrawingAreaUtil.drawText(_da_bottomx_label, date.ToString(DateTime.FORMAT_YYYY_MM), x, _datalim_bottomx_label.getBottomY(), 0.0, GRText.CENTER_X | GRText.BOTTOM);
						++nlabel2;
					}
					// Draw tick marks at the labels...
					xt[0] = x;
		...
		else if ((_xaxis_date_precision == DateTime.PRECISION_HOUR) || (TSUtil.calculateDataSize(_start_date,_end_date, TimeInterval.MINUTE,1) > 1440))
		{
			...
			int[] hour_increments = new int[] {1, 2, 3, 4, 6, 12};
			...
			for (; ; date.addHour(hour_increment))
			{
				...
				{
					GRDrawingAreaUtil.drawText(_da_bottomx_label, "" + date.getHour(), x, _datalim_bottomx_label.getTopY(), 0.0, GRText.CENTER_X | GRText.TOP);
					if (date.getHour() == 0)
					{
						if (nlabel2 == 0)
						{
							// Label YYYY-MM-DD...
							GRDrawingAreaUtil.drawText(_da_bottomx_label, date.ToString(DateTime.FORMAT_YYYY_MM_DD), x, _datalim_bottomx_label.getBottomY(), 0.0, GRText.CENTER_X | GRText.BOTTOM);
						}
						else
						{
							// Label MM-DD...
							GRDrawingAreaUtil.drawText(_da_bottomx_label, date.ToString(DateTime.FORMAT_MM_DD), x, _datalim_bottomx_label.getBottomY(), 0.0, GRText.CENTER_X | GRText.BOTTOM);
						}
						++nlabel2;
					}
					// Draw tick marks at the labels...
		...
		else
		{
			...
			int[] minute_increments = new int[] {1, 2, 5, 10, 15, 20, 30};
			...
			for (; ; date.addMinute(minute_increment))
			{
				...
				{
					GRDrawingAreaUtil.drawText(_da_bottomx_label, "" + date.getMinute(), x, _datalim_bottomx_label.getTopY(), 0.0, GRText.CENTER_X | GRText.TOP);
					if (date.getMinute() == 0)
					{
						if (nlabel2 == 0)
						{
							// Label the YYYY-MM-DD:HH...
							GRDrawingAreaUtil.drawText(_da_bottomx_label, date.ToString(DateTime.FORMAT_YYYY_MM_DD) + ":" + StringUtil.formatString(date.getHour(),"%02d"), x, _datalim_bottomx_label.getBottomY(), 0.0, GRText.CENTER_X | GRText.BOTTOM);
						}
						else
						{
							// Label the HH...
							GRDrawingAreaUtil.drawText(_da_bottomx_label, "" + StringUtil.formatString(date.getHour(),"%02d"), x, _datalim_bottomx_label.getBottomY(), 0.0, GRText.CENTER_X | GRText.BOTTOM);
						}
						++nlabel2;
					}
					// Draw tick marks at the labels...
	...
	/// <summary>
	/// Draw a scatter plot.  One X-axis time series is drawn against multiple Y-axis time series. </summary>
	/// <param name="daGraph"> drawing area to draw the graph </param>
	/// <param name="tsproduct"> product being processed </param>
	/// <param name="subproduct"> subproduct number on the product (0+) </param>
	/// <param name="dataLimits"> data limits for the drawing area, adjusted for nice axis limits, zoom, etc. </param>
	/// <param name="tslist"> list of time series to process - all will be drawn so filtering for left/right axis should already be done. </param>
	/// <param name="regressionDataList"> regression data for the time series, precomputed at graph initialization so drawing is fast </param>
	private void drawXYScatterPlot(GRDrawingArea daGraph, TSProduct tsproduct, int subproduct, GRLimits dataLimits, IList<TS> tslist, IList<TSRegression> regressionDataList)
	{
		...
		bool draw_line = true;
		bool label_symbol = false; // Default...
		string label_position_string;
		string label_units = "";
		string label_format = "";
		string label_value_format = "";
		...
		for (int i = 0, its = 1; i < nreg; i++, its++)
		{
			...
			draw_line = true;
			regressionData = regressionDataList[i];
			if (regressionData == null)
			{
				...
				for (int il = 1; il <= nlines; il++)
				{
					...
					else
					{
						...
						if ((!string.ReferenceEquals(label, null)) && !label.Equals(""))
						{
							xlabel = xp[1];
							ylabel = yp[1];
							GRDrawingAreaUtil.drawText(daGraph, label, xlabel, ylabel, 0.0, GRText.CENTER_X | GRText.CENTER_Y);
						}
					}
					...
					if ((!string.ReferenceEquals(prop_value, null)) && (prop_value.Equals("95") || prop_value.Equals("99")) && (n1 > 0))
					{
						...
						try
						{
						F = FDistribution.getCumulativeFDistribution(2, (n1 - 2), (100 - int.Parse(prop_value)));
						if (analyze_monthly)
						{
						...
						for (; date.lessThanOrEqualTo(end); date.addInterval(interval_base, interval_mult))
						{
							...
							left = ybar + B * (x - xbar);
							right = Math.Sqrt(2.0 * F) * Math.Sqrt((1.0 / (dn1 - 2.0)) * yhatsum) * Math.Sqrt(1 / dn1 + ((x - xbar) * (x - xbar)) / xsum);
							xp[iyci] = x;
							yp[iyci] = left + right;
							yp2[iyci++] = left - right;
						}
			...
			for (; date.lessThanOrEqualTo(end); date.addInterval(interval_base, interval_mult))
			{
				...
				if (label_symbol)
				{
					GRDrawingAreaUtil.drawSymbolText(daGraph, symbol, x, y, symbol_size, TSData.ToString(label_format,label_value_format, date, x, y, "", label_units), 0.0, label_position, GRUnits.DEVICE,GRSymbol.SYM_CENTER_X | GRSymbol.SYM_CENTER_Y);
				}
				else
				{
					GRDrawingAreaUtil.drawSymbol(daGraph, symbol, x, y, symbol_size, GRUnits.DEVICE, GRSymbol.SYM_CENTER_X | GRSymbol.SYM_CENTER_Y);
...
//ORIGINAL LINE: protected void finalize() throws Throwable
	~TSGraph()
	{
		...
		_da_lefty_bottom_legend = null;
		_da_lefty_left_legend = null;
		_da_lefty_right_legend = null;
		_datalim_page = null;
		_drawlim_page = null;
		...
		_datalim_lefty_bottom_legend = null;
		_drawlim_lefty_bottom_legend = null;
		_datalim_lefty_left_legend = null;
		_drawlim_lefty_left_legend = null;
		_datalim_lefty_right_legend = null;
		_drawlim_lefty_right_legend = null;
		_gtype = null;
...
//		base.finalize();
	...
	/// If right y-axis is used, format as:  "LEFT X: xxxxx,  Y: yyyyy / RIGHT X: xxxxx,  Y:  yyyyy". </summary>
	/// <param name="devpt"> Device point to format (needed to lookup right y-axis value). </param>
	/// <param name="datapt"> Data point to format. </param>
	public virtual string formatMouseTrackerDataPoint(GRPoint devpt, GRPoint datapt)
	{
		if (datapt == null)
		{
			return "";
		}
		else if ((__leftYAxisGraphType == TSGraphType.DOUBLE_MASS) || (__leftYAxisGraphType == TSGraphType.DURATION) || (__leftYAxisGraphType == TSGraphType.XY_SCATTER))
		{
		...
		else if (__leftYAxisGraphType == TSGraphType.RASTER)
		{
			...
			{
				if (datapt.associated_object is TS)
				{
			...
			if (_data_lefty_limits.getMaxX() <= 12.0)
			{
				...
				if (ts != null)
				{
					...
					if ((!string.ReferenceEquals(flag, null)) && !flag.Equals(""))
					{
						flagString = " (" + flag + ")";
					}
					if (ts.isDataMissing(value))
			...
			else if (_data_lefty_limits.getMaxX() <= 366.0)
			{
				...
				bool isLeapYear = TimeUtil.isLeapYear(year);
	...
	/// limits from this method are the same as for the drawing area. </summary>
	/// <returns> The current data limits. </returns>
	...
	/// <summary>
	/// Return the derived time series list used for graphing. </summary>
	/// <param name="enabledOnly"> if true return only time series that are enabled.  If false, return all. </param>
	/// <returns> the derived time series list being graphed. </returns>
	public virtual IList<TS> getDerivedTSList()
	{
	...
	/// return null.  If no time series are enabled, an empty list will be returned. </summary>
	/// <param name="includeLeftYAxis"> if true, include left y-axis time series. </param>
	/// <param name="includeRightYAxis"> if true, include right y-axis time series. </param>
	/// <returns> a list of all the time series that are enabled. </returns>
	public virtual IList<TS> getEnabledTSList(bool includeLeftYAxis, bool includeRightYAxis)
	{
		if (__tslist == null || __tslist.Count == 0)
		{
		...
		for (int its = 0; its < size; its++)
		{
			...
			else
			{
				...
				if ((string.ReferenceEquals(propValue, null)) || propValue.Length == 0)
				{
					// Default is time series is associated with left axis
					propValue = "Left";
				}
				if (includeLeftYAxis && propValue.Equals("Left", StringComparison.OrdinalIgnoreCase))
				{
					tslist.Add(__tslist[its]);
				}
				if (includeRightYAxis && propValue.Equals("Right", StringComparison.OrdinalIgnoreCase))
				{
					tslist.Add(__tslist[its]);
				}
			}
	...
	/// <summary>
	/// Returns the end date. </summary>
	/// <returns> the end date. </returns>
	protected internal virtual DateTime getEndDate()
	{
		return _end_date;
	}
	...
	/// event was not in a legend hotspot. </summary>
	/// <param name="eventPoint"> location of mouse click in original device units (y is zero at top). </param>
	/// <returns> the matching time series if clicked on time series legend text, or null if not a hit. </returns>
	protected internal virtual TS getEventLegendTimeSeries(GRPoint eventPoint)
	{
		...
		foreach (KeyValuePair<TS, GRLimits> entry in __legendTimeSeriesDrawMap.SetOfKeyValuePairs())
		{
	...
	/// <summary>
	/// Returns the first time series in the time series list that is enabled. </summary>
	/// <returns> the first time series in the time series list that is enabled. </returns>
	public virtual TS getFirstEnabledTS(bool includeLeftYAxis, bool includeRightYAxis)
	{
		IList<TS> v = getEnabledTSList(includeLeftYAxis, includeRightYAxis);
		if (v.Count == 0)
		{
			return null;
		}
	...
	/// properties that may be set in the graph.  This method is used when drawing time series. </summary>
	/// <param name="key"> the key of the property to return. </param>
	/// <param name="subproduct"> the subproduct of the property to return </param>
	/// <param name="its"> the number of the time series of the property to return </param>
	/// <param name="annotation"> if true, then the property is an annotation </param>
	/// <param name="overrideProps"> if not null, this is the proplist that will be checked for the property. </param>
	/// <returns> the prop value, or null if the property does not exist </returns>
	protected internal virtual string getLayeredPropValue(string key, int subproduct, int its, bool annotation, PropList overrideProps)
	{
	...
	/// <summary>
	/// Return the graph type for the left y-axis. </summary>
	/// <returns> the graph type for the left y-axis. </returns>
	public virtual TSGraphType getLeftYAxisGraphType()
	{
	...
	/// This is most often used by code that handles mouse events. </summary>
	/// <returns> The drawing area used for the left y-axis graph. </returns>
	public virtual GRJComponentDrawingArea getLeftYAxisGraphDrawingArea()
	{
	...
	/// </summary>
	protected internal virtual string getLeftYAxisViewMaxY()
	{
	...
	/// </summary>
	protected internal virtual string getLeftYAxisViewMinY()
	{
	...
	/// </summary>
	protected internal virtual string getRightYAxisViewMaxY()
	{
	...
	/// </summary>
	protected internal virtual string getRightYAxisViewMinY()
	{
		return this.rightYAxisViewMinY;
	}

	/// <summary>
	/// Return the legend string for a time series. </summary>
	/// <returns> a legend string.  If null is returned, the legend should not be drawn. </returns>
	/// <param name="ts"> Time series to get legend. </param>
	/// <param name="i"> Loop counter for time series (0-index). </param>
	private string getLegendString(TS ts, int i)
	{
		...
		if (string.ReferenceEquals(legend_format, null))
		{
			// Try the legend format for the subproduct...
			legend_format = subproduct_legend_format;
		}
		if ((string.ReferenceEquals(legend_format, null)) || (legend_format.Length == 0))
		{
		...
		else if (!legend_format.Equals("Auto", StringComparison.OrdinalIgnoreCase))
		{
			// A specific legend has been specified...
			legend = ts.formatLegend(legend_format);
		}
		// Below here "Auto" is in effect...
		else if ((!string.ReferenceEquals(ts.getLegend(), null)) && (ts.getLegend().Length != 0))
		{
		...
		else if (!subproduct_legend_format.Equals("Auto", StringComparison.OrdinalIgnoreCase))
		{
		...
		else
		{
			...
			if (_ignoreLeftAxisUnits && !ts.getDataUnits().Equals(""))
			{
				...
				if (ts.getAlias().Equals(""))
				{
					legend = ts.getDescription() + ", " + ts.getIdentifierString() + ", " + ts.getDataUnits() + " (" + ts.getDate1() + " to " + ts.getDate2() + ")";
				}
				else
				{
					legend = ts.getAlias() + " - " + ts.getDescription() + ", " + ts.getIdentifierString() + ", " + ts.getDataUnits() + " (" + ts.getDate1() + " to " + ts.getDate2() + ")";
				}
			}
			else
			{
				...
				if (ts.getAlias().Equals(""))
				{
					legend = ts.getDescription() + ", " + ts.getIdentifierString() + " (" + ts.getDate1() + " to " + ts.getDate2() + ")";
				}
				else
				{
					legend = ts.getAlias() + " - " + ts.getDescription() + ", " + ts.getIdentifierString() + " (" + ts.getDate1() + " to " + ts.getDate2() + ")";
				}
			}
		}
		if (__leftYAxisGraphType == TSGraphType.XY_SCATTER)
	...
	/// </summary>
	public virtual GRLimits getMaxDataLimits()
	{
		return _max_lefty_data_limits;
	}

	/// <summary>
	/// Returns the max end date. </summary>
	/// <returns> the max end date. </returns>
	protected internal virtual DateTime getMaxEndDate()
	{
		return _max_end_date;
	}

	/// <summary>
	/// Returns the max start date. </summary>
	/// <returns> the max start date. </returns>
	protected internal virtual DateTime getMaxStartDate()
	{
		return _max_start_date;
	}

	/// <summary>
	/// Return the number of time series. </summary>
	/// <returns> the number of time series (useful for automated selection of colors, symbols, etc.) </returns>
	public virtual int getNumTS()
	{
		return __tslist.Count;
	}

	/// <summary>
	/// Return the drawing area used for the full page.  The value may be null. </summary>
	/// <returns> The drawing area used for the full page. </returns>
	public virtual GRJComponentDrawingArea getPageDrawingArea()
	{
		return _da_page;
	}
	...
	/// graph.  The PopupMenu can then be added to the and shown. </summary>
	/// <returns> a PopupMenu instance appropriate for the graph, or null if no PopupMenu
	/// choices are available (also return null if a reference graph). </returns>
	public virtual JPopupMenu getJPopupMenu()
	{
		if (_is_reference_graph)
		{
	...
	/// This can be displayed in a details window for the graph. </summary>
	/// <returns> TSRegression data for graph (use with scatter plot) or null if no analysis has been performed. </returns>
	public virtual IList<TSRegression> getRegressionData()
	{
	...
	/// This is most often used by code that handles mouse events. </summary>
	/// <returns> The drawing area used for the right y-axis graph. </returns>
	public virtual GRJComponentDrawingArea getRightYAxisGraphDrawingArea()
	{
		return _da_righty_graph;
	}

	/// <summary>
	/// Return the graph type for the right y-axis. </summary>
	/// <returns> the graph type for the right y-axis. </returns>
	public virtual TSGraphType getRightYAxisGraphType()
	{
		return __rightYAxisGraphType;
	}

	/// <summary>
	/// Return the list of time series that are selected via legend interactions. </summary>
	/// <returns> the list of time series that are selected via legend interactions,
	/// guaranteed to be non-null but may be empty. </returns>
	protected internal virtual IList<TS> getSelectedTimeSeriesList()
	{
		return _selectedTimeSeriesList;
	}

	/// <summary>
	/// Returns the start date. </summary>
	/// <returns> the start date. </returns>
	protected internal virtual DateTime getStartDate()
	{
		return _start_date;
	}
	...
	/// </summary>
	public virtual int getSubProductNumber()
	{
		return _subproduct;
	}
	...
	/// graph type for the graph, based on the left or right y-axis graph type. </summary>
	/// <param name="mainGraphType"> the graph type for the graph, used for time series if time series graph type is not specified </param>
	/// <param name="its"> time series in the graph, 0+. </param>
	private TSGraphType getTimeSeriesGraphType(TSGraphType mainGraphType, int its)
	{
		string routine = this.GetType().Name + ".getTimeSeriesGraphType";
	...
	/// <summary>
	/// Return the time series list used for graphing (all the time series). </summary>
	/// <returns> the time series list being graphed (all the time series). </returns>
	...
	/// <summary>
	/// Return the time series list used for graphing (time series for the left y-axis). </summary>
	/// <returns> the time series list being graphed (time series for the left y-axis). </returns>
	public virtual IList<TS> getTSListForLeftYAxis()
	{
		return __left_tslist;
	}

	/// <summary>
	/// Return the time series list used for graphing (time series for the right y-axis). </summary>
	/// <returns> the time series list being graphed (time series for the right y-axis). </returns>
	public virtual IList<TS> getTSListForRightYAxis()
	{
	...
	/// determines graph limits, although actually rendering code may take more care. </summary>
	/// <param name="enabledOnly"> if true, only return the list of enabled time series </param>
	/// <param name="includeLeftYAxis"> if true, include left y-axis time series. </param>
	/// <param name="includeRightYAxis"> if true, include right y-axis time series. </param>
	/// <returns> the time series list being rendered. </returns>
	private IList<TS> getTSListToRender(bool enabledOnly, bool includeLeftYAxis, bool includeRightYAxis)
	{
		...
		if (enabledOnly)
		{
			...
			if (Message.isDebugOn)
			{
				Message.printStatus(2, "getTSListToRender", "Got " + tslist.Count + " time series that are enabled, includeLeftYAxis=" + includeLeftYAxis + ", includeRightYAxis=" + includeRightYAxis);
			}
		}
		else
		{
		...
		if (includeLeftYAxis && (leftYAxisGraphType == TSGraphType.AREA_STACKED))
		{
			...
			IList<TS> tsToRender = new List<TS>();
			((IList<TS>)tsToRender).AddRange(getDerivedTSList());
	...
	/// <summary>
	/// Get the x-axis date precision. </summary>
	/// <returns> x-axis date precision, useful for formatting mouse-tracker, etc. </returns>
	protected internal virtual int getXAxisDateTimePrecision()
	{
	...
	/// event should impact this TSGraph. </summary>
	/// <param name="devpt"> a point of interest, in raw device units (not GR plotting units). </param>
	/// <returns> true if devpt is in the graph drawing area. </returns>
	public virtual bool graphContains(GRPoint devpt)
	...
	/// </summary>
	public virtual bool ignoreLeftYAxisUnits()
	{
	...
	/// </summary>
	public virtual bool ignoreRightYAxisUnits()
	{
	...
	/// </summary>
	public virtual bool isReferenceGraph()
	{
		return _is_reference_graph;
	}

	/// <summary>
	/// Indicate whether the time series is selected (highlighted in legend). </summary>
	/// <param name="ts"> time series to evaluate. </param>
	/// <returns> true if the time series is selected, false if not. </returns>
	protected internal bool isTimeSeriesSelected(TS ts)
	{
		foreach (TS ts2 in _selectedTimeSeriesList)
		{
			if (ts2 == ts)
			{
				return true;
	...
	/// <summary>
	/// Returns whether the time series of the graph's subproduct is enabled. </summary>
	/// <param name="its"> the time series to check. </param>
	/// <returns> true if the time series is enabled, false if not. </returns>
	public virtual bool isTSEnabled(int its)
	{
	...
	/// <summary>
	/// Modify the line with for the time series because it is selected. </summary>
	/// <param name="lineWidth"> original line width. </param>
	/// <returns> modified line width to use for drawing. </returns>
	private int modifyLineWidthForSelectedTimeSeries(int lineWidth)
	{
		...
		if (!string.ReferenceEquals(propValue, null))
		{
			...
			if (StringUtil.isDouble(propValue))
			{
				lineWidth = (int)double.Parse(propValue);
			}
			else if (propValue.StartsWith("x", StringComparison.Ordinal) && (propValue.Length > 1) && StringUtil.isDouble(propValue.Substring(1).Trim()))
			{
				// x5 would multiple width by 5
				lineWidth = (int)(lineWidth * double.Parse(propValue.Substring(1).Trim()));
			}
			else if (propValue.StartsWith("+", StringComparison.Ordinal) && (propValue.Length > 1) && StringUtil.isDouble(propValue.Substring(1).Trim()))
			{
				// +5 would add 5 to width
				lineWidth = lineWidth + (int)double.Parse(propValue.Substring(1).Trim());
			}
		}
		return lineWidth;
	}
	...
	{
		bool log_y_left = false;
		bool log_y_right = false;
		bool log_xy_scatter = false;
		string prop_val = _tsproduct.getLayeredPropValue("LeftYAxisType", _subproduct, -1, false);
		if (prop_val.Equals("Log", StringComparison.OrdinalIgnoreCase))
		{
			log_y_left = true;
		}
		...
		if ((!string.ReferenceEquals(prop_val, null)) && prop_val.Equals("Log", StringComparison.OrdinalIgnoreCase))
		{
			log_y_left = false;
			log_xy_scatter = true;
		}
		...
		_da_page = new GRJComponentDrawingArea(_dev, "TSGraph.Page", GRAspect.FILL, null, GRUnits.DEVICE, GRLimits.DEVICE, null);
		...
		_da_error = new GRJComponentDrawingArea(_dev, "TSGraph.Error", GRAspect.FILL, null, GRUnits.DEVICE, GRLimits.DEVICE, null);
		_datalim_error = new GRLimits(0.0, 0.0, 1.0, 1.0);
		_da_error.setDataLimits(_datalim_error);

		// Drawing area for main title...

		_da_maintitle = new GRJComponentDrawingArea(_dev, "TSGraph.MainTitle", GRAspect.FILL, null, GRUnits.DEVICE, GRLimits.DEVICE, null);
		_datalim_maintitle = new GRLimits(0.0, 0.0, 1.0, 1.0);
		_da_maintitle.setDataLimits(_datalim_maintitle);

		// Drawing area for sub title...

		_da_subtitle = new GRJComponentDrawingArea(_dev, "TSGraph.SubTitle", GRAspect.FILL, null, GRUnits.DEVICE, GRLimits.DEVICE, null);
		_datalim_subtitle = new GRLimits(0.0, 0.0, 1.0, 1.0);
		_da_subtitle.setDataLimits(_datalim_subtitle);

		// Top X axis...

		_da_topx_title = new GRJComponentDrawingArea(_dev, "TSGraph.TopXTitle", GRAspect.FILL, null, GRUnits.DEVICE, GRLimits.DEVICE, null);
		_datalim_topx_title = new GRLimits(0.0, 0.0, 1.0, 1.0);
		_da_topx_title.setDataLimits(_datalim_topx_title);

		_da_topx_label = new GRJComponentDrawingArea(_dev, "TSGraph.TopXLabels", GRAspect.FILL, null, GRUnits.DEVICE, GRLimits.DEVICE, null);
	...
	//		GRDrawingAreaUtil.setAxes(_da_topx_label, GRAxis.LOG, GRAxis.LINEAR);
		...
		_da_lefty_title = new GRJComponentDrawingArea(_dev, "TSGraph.LeftYTitle", GRAspect.FILL, null, GRUnits.DEVICE, GRLimits.DEVICE, null);
		_datalim_lefty_title = new GRLimits(0.0, 0.0, 1.0, 1.0);
		_da_lefty_title.setDataLimits(_datalim_lefty_title);

		_da_righty_title = new GRJComponentDrawingArea(_dev, "TSGraph.RightYTitle", GRAspect.FILL, null, GRUnits.DEVICE, GRLimits.DEVICE, null);
		_datalim_righty_title = new GRLimits(0.0, 0.0, 1.0, 1.0);
		_da_righty_title.setDataLimits(_datalim_righty_title);

		// Left Y axis labels...

		_da_lefty_label = new GRJComponentDrawingArea(_dev, "TSGraph.LeftYLabel", GRAspect.FILL, null, GRUnits.DEVICE, GRLimits.DEVICE, null);
		...
		_da_righty_label = new GRJComponentDrawingArea(_dev, "TSGraph.RightYLabel", GRAspect.FILL, null, GRUnits.DEVICE, GRLimits.DEVICE, null);
		_datalim_righty_label = new GRLimits(0.0, 0.0, 1.0, 1.0);
		_da_righty_label.setDataLimits(_datalim_righty_label);

		// Drawing area for bottom X axis...

		_da_bottomx_label = new GRJComponentDrawingArea(_dev, "TSGraph.BottomXLabel", GRAspect.FILL, null, GRUnits.DEVICE, GRLimits.DEVICE, null);
	...
	//		GRDrawingAreaUtil.setAxes(_da_bottomx_label, GRAxis.LOG, GRAxis.LINEAR);
		...
		_da_bottomx_title = new GRJComponentDrawingArea(_dev, "TSGraph.BottomXTitle", GRAspect.FILL, null, GRUnits.DEVICE, GRLimits.DEVICE, null);
		...
		_da_lefty_bottom_legend = new GRJComponentDrawingArea(_dev, "TSGraph.LeftyYAxisBottomLegend", GRAspect.FILL, null, GRUnits.DEVICE, GRLimits.DEVICE, null);
		_datalim_lefty_bottom_legend = new GRLimits(0.0, 0.0, 1.0, 1.0);
		_da_lefty_bottom_legend.setDataLimits(_datalim_lefty_bottom_legend);

		_da_lefty_left_legend = new GRJComponentDrawingArea(_dev, "TSGraph.LeftYAxisLeftLegend", GRAspect.FILL, null, GRUnits.DEVICE, GRLimits.DEVICE, null);
		_datalim_lefty_left_legend = new GRLimits(0.0, 0.0, 1.0, 1.0);
		_da_lefty_left_legend.setDataLimits(_datalim_lefty_left_legend);

		_da_lefty_right_legend = new GRJComponentDrawingArea(_dev, "TSGraph.LeftYAxisRightLegend", GRAspect.FILL, null, GRUnits.DEVICE, GRLimits.DEVICE, null);
		_datalim_lefty_right_legend = new GRLimits(0.0, 0.0, 1.0, 1.0);
		_da_lefty_right_legend.setDataLimits(_datalim_lefty_right_legend);

		_da_lefty_inside_legend = new GRJComponentDrawingArea(_dev, "TSGraph.LeftYAxisInsideLegend", GRAspect.FILL, null, GRUnits.DEVICE, GRLimits.DEVICE, null);
		...
		_da_righty_bottom_legend = new GRJComponentDrawingArea(_dev, "TSGraph.RightYAxisBottomLegend", GRAspect.FILL, null, GRUnits.DEVICE, GRLimits.DEVICE, null);
		...
		_da_righty_left_legend = new GRJComponentDrawingArea(_dev, "TSGraph.RightYAxisLeftLegend", GRAspect.FILL, null, GRUnits.DEVICE, GRLimits.DEVICE, null);
		_datalim_righty_left_legend = new GRLimits(0.0, 0.0, 1.0, 1.0);
		_da_righty_left_legend.setDataLimits(_datalim_righty_left_legend);

		_da_righty_right_legend = new GRJComponentDrawingArea(_dev, "TSGraph.RightYAxisRightLegend", GRAspect.FILL, null, GRUnits.DEVICE, GRLimits.DEVICE, null);
		...
		_da_righty_inside_legend = new GRJComponentDrawingArea(_dev, "TSGraph.RightYAxisInsideLegend", GRAspect.FILL, null, GRUnits.DEVICE, GRLimits.DEVICE, null);
	...
	/// TSGraph that corresponds to the reference time series. </summary>
	...
	public virtual void paint(Graphics g)
	{
		...
		try
		{
			...
			if (__leftYAxisGraphType == TSGraphType.XY_SCATTER)
			{
				...
				if ((_regression_data != null) && (_regression_data.Count > 0))
				{
					if (!_tsproduct.getLayeredPropValue("XYScatterMethod", _subproduct, -1, false).Equals("" + _regression_data[0].getAnalysisMethod(), StringComparison.OrdinalIgnoreCase) || !_tsproduct.getLayeredPropValue("XYScatterMonth", _subproduct, -1, false).Equals(StringUtil.formatNumberSequence(_regression_data[0].getAnalysisMonths(),","), StringComparison.OrdinalIgnoreCase) || !_tsproduct.getLayeredPropValue("XYScatterNumberOfEquations", _subproduct, -1, false).Equals("" + _regression_data[0].getNumberOfEquations(), StringComparison.OrdinalIgnoreCase) || !_tsproduct.getLayeredPropValue("XYScatterTransformation", _subproduct, -1, false).Equals("" + _regression_data[0].getTransformation(), StringComparison.OrdinalIgnoreCase))
					{
						needToAnalyze = true;
					}
				}
			...
			if (_is_reference_graph)
			{
				...
				GRDrawingAreaUtil.fillRectangle(_da_lefty_graph, _max_lefty_data_limits.getLeftX(), _max_lefty_data_limits.getBottomY(), _max_lefty_data_limits.getWidth(), _max_lefty_data_limits.getHeight());
				...
				GRDrawingAreaUtil.fillRectangle(_da_lefty_graph, _data_lefty_limits.getLeftX(), _data_lefty_limits.getBottomY(), _data_lefty_limits.getWidth(), _max_lefty_data_limits.getHeight());
				// Also draw a line in case we are zoomed in so far that the rectangle does not draw...
				GRDrawingAreaUtil.drawLine(_da_lefty_graph, _data_lefty_limits.getLeftX(), _data_lefty_limits.getBottomY(), _data_lefty_limits.getLeftX(), _data_lefty_limits.getTopY());
			...
			drawAxesFront(_tsproduct, _subproduct, _da_maintitle, __drawLeftyLabels, __leftYAxisGraphType, _da_lefty_graph, _da_lefty_title, _da_lefty_label, _data_lefty_limits, _datalim_lefty_title, _datalim_lefty_label, _ylabels_lefty, _lefty_precision, __drawRightyLabels, __rightYAxisGraphType, _da_righty_graph, _da_righty_title, _da_righty_label, _data_righty_limits, _datalim_righty_title, _datalim_righty_label, _ylabels_righty, _righty_precision, _da_bottomx_title, _da_bottomx_label, _datalim_bottomx_title, _datalim_bottomx_label, _xlabels); // Draw axes components that are in front of data
			...
			drawLegend(GRAxis.LEFT); // Draw the left y-axis legend, which may be on top of the graph
			drawLegend(GRAxis.RIGHT); // Draw the right y-axis legend, which may be on top of the graph
	...
	/// setStartDate methods and should be used for calculating the date limits. </summary>
	/// <param name="useSetDates"> if true, use the set dates to limit the maximum period,
	/// generally passed in, for example, when time series product file constrains the period. </param>
	public virtual void setComputeWithSetDates(bool useSetDates)
	{
	...
	/// limits are the data limits from the second time series. </summary>
	/// <param name="datalim_lefty_graph"> Data limits for the left y-axis graph.
	/// The data limits are either the initial values or the values from a zoom. </param>
	public virtual void setDataLimitsForDrawing(GRLimits datalim_lefty_graph)
	{
		...
		if (Message.isDebugOn)
		{
			Message.printDebug(1, routine, _gtype + "Setting [" + _subproduct + "] _data_lefty_limits to " + datalim_lefty_graph.ToString());
		}
		...
		else
		{
			...
			if (Message.isDebugOn)
			{
				Message.printDebug(1, routine, _gtype + "Set _start_date to " + _start_date + " _end_date to " + _end_date);
			}
			...
			try
			{
				...
				IList<TS> tslistToRender = getTSListToRender(true,includeLeftYAxis,includeRightYAxis);
				if (tslistToRender.Count == 0)
				{
					_tslimits_lefty = null;
					return;
				}
				else
				{
					...
					if (__leftYAxisGraphType == TSGraphType.PERIOD)
					{
						...
						_tslimits_lefty.setMinValue(getEnabledTSList(includeLeftYAxis,includeRightYAxis).Count + 1);
					}
					else if (__leftYAxisGraphType == TSGraphType.RASTER)
					{
					...
					else
					{
						...
						if (!_zoomKeepFullPeriodYLimits)
						{
							...
							{
								_tslimits_lefty.setMinValue(double.Parse(leftYAxisViewMinY));
							...
							{
								_tslimits_lefty.setMaxValue(double.Parse(leftYAxisViewMaxY));
							...
							if (leftYAxisViewMinY.Equals(YAXIS_LIMITS_AUTOFILL_AND_KEEP, StringComparison.OrdinalIgnoreCase))
							{
							...
							if (leftYAxisViewMaxY.Equals(YAXIS_LIMITS_AUTOFILL_AND_KEEP, StringComparison.OrdinalIgnoreCase))
							{
			...
			try
			{
				...
				else
				{
					...
					if (__rightYAxisGraphType == TSGraphType.PERIOD)
					{
						...
						_tslimits_righty.setMinValue(tslistToRender.Count + 1);
					}
					...
					if (!_zoomKeepFullPeriodYLimits)
					{
						...
						{
							_tslimits_righty.setMinValue(double.Parse(rightYAxisViewMinY));
						...
						{
							_tslimits_righty.setMaxValue(double.Parse(rightYAxisViewMaxY));
							Message.printStatus(2, routine, "Setting max time series value to user maximum " + rightYAxisViewMaxY);
						}
						// Also check whether the value should have been taken from a data view
						if (rightYAxisViewMinY.Equals(YAXIS_LIMITS_AUTOFILL_AND_KEEP, StringComparison.OrdinalIgnoreCase))
						{
						...
						if (rightYAxisViewMaxY.Equals(YAXIS_LIMITS_AUTOFILL_AND_KEEP, StringComparison.OrdinalIgnoreCase))
						{
			...
			*/
			...
			includeLeftYAxis = true;
			includeRightYAxis = true;
			if (getTSListToRender(true,includeLeftYAxis,includeRightYAxis).Count > 0)
			{
	...
	/// </summary>
	private void setDerivedTSList(IList<TS> derivedTSList)
	{
	...
	/// resizes).  Axes are set to log if the properties indicate to do so. </summary>
	/// <param name="drawlim_page"> Drawing limits for the full extent of the graph within a GRJComponentDevice,
	/// corresponding to canvas/component width and height in pixels (or part of a canvas/component). </param>
	public virtual void setDrawingLimits(GRLimits drawlim_page)
	{
		...
		IList<TS> tslistLeftYAxis = this.__left_tslist; // Left y-axis time series
		IList<TS> tslistRightYAxis = this.__right_tslist; // Right y-axis time series

		bool log_y_lefty = false;
		bool log_y_righty = false;
		bool log_xy_scatter_lefty = false;
		...
		if ((!string.ReferenceEquals(mainTitleString, null)) && !mainTitleString.Equals(""))
		{
		...
		if ((!string.ReferenceEquals(subTitleString, null)) && !subTitleString.Equals(""))
		{
			// Get the text extents and set the height based on that...
		...
		if ((!string.ReferenceEquals(leftYAxisTitleString, null)) && !leftYAxisTitleString.Equals(""))
		{
			// Get the text extents and set the height based on that...
			...
			if ((!string.ReferenceEquals(leftYAxisTitleRotation, null)) && leftYAxisTitleRotation.Length > 0)
			{
				try
				{
					rotation = double.Parse(leftYAxisTitleRotation);
				}
				catch (System.FormatException)
				{
					// Just use 0
			...
			if (leftYAxisTitlePosition.Equals("AboveAxis", StringComparison.OrdinalIgnoreCase))
			{
		...
		if ((!string.ReferenceEquals(rightYAxisTitleString, null)) && !rightYAxisTitleString.Equals("") && (!string.ReferenceEquals(rightYAxisTitlePosition, null)) && !rightYAxisTitlePosition.Equals("None", StringComparison.OrdinalIgnoreCase))
		{
			// Get the text extents and set the height based on that...
			...
			if ((!string.ReferenceEquals(rightYAxisTitleRotation, null)) && rightYAxisTitleRotation.Length > 0)
			{
				try
				{
					rotation = double.Parse(rightYAxisTitleRotation);
				}
				catch (System.FormatException)
				{
					// Just use 0
			...
			if (rightYAxisTitlePosition.Equals("AboveAxis", StringComparison.OrdinalIgnoreCase))
			{
		...
		if ((!string.ReferenceEquals(topXAxisTitleString, null)) && !topXAxisTitleString.Equals(""))
		{
			// Get the text extents and set the height based on that...
		...
		if ((!string.ReferenceEquals(bottomXAxisTitleString, null)) && !bottomXAxisTitleString.Equals(""))
		{
			// Get the text extents and set the height based on that...
			...
			GRDrawingAreaUtil.setFont(_da_bottomx_title, bottomXAxisTitleFontName, bottomXAxisTitleFontStyle, StringUtil.atod(bottomXAxisTitleFontSize));
		...
		// Make an initial determination of the left y-axis legend height and width, based
		...
		string leftYAxisLegendPosition = tsproduct.getLayeredPropValue("LeftYAxisLegendPosition", subproduct, -1, false);
		if (string.ReferenceEquals(leftYAxisLegendPosition, null))
		{
			// Check legacy value
			leftYAxisLegendPosition = tsproduct.getLayeredPropValue("LegendPosition", subproduct, -1, false);
		}
		...
		if ((tslistLeftYAxis == null) || (tslistLeftYAxis.Count == 0) || (getEnabledTSList(true,false).Count == 0) || leftYAxisLegendPosition.Equals("None", StringComparison.OrdinalIgnoreCase))
		{
		...
		else
		{
			...
			GRDrawingAreaUtil.setFont(_da_lefty_bottom_legend, legendFontName, legendFontStyle, StringUtil.atod(legendFontSize));
			int nts = 0;
			int size = tslistLeftYAxis.Count;
			TS ts = null;
			string legend = null;

			for (int i = 0; i < size; i++)
			{
				ts = tslistLeftYAxis[i];
				if (ts == null || !isTSEnabled(i))
				{
					continue;
				}
				if (ts.getEnabled())
				{
					if (__leftYAxisGraphType == TSGraphType.PREDICTED_VALUE_RESIDUAL)
					{
						...
						if (!string.ReferenceEquals(legend, null))
						{
							textLimits = GRDrawingAreaUtil.getTextExtents(_da_lefty_bottom_legend, legend, GRUnits.DEVICE);
			...
			textLimits = GRDrawingAreaUtil.getTextExtents(_da_lefty_bottom_legend, "TEST STRING", GRUnits.DEVICE);
		...
		// Make an initial determination of the right y-axis legend height and width, based
		// on the font height and string width.  The dynamic nature of the plot
		// size really should only impact how many legend items are shown.  Try
		// to limit to a reasonable number.  This logic works for any legend position.
		// TODO SAM 2017-02-07 put the following and above into reused code once it tests out

		string rightYAxisLegendPosition = tsproduct.getLayeredPropValue("RightYAxisLegendPosition", subproduct, -1, false);
		...
		if ((tslistRightYAxis == null) || (tslistRightYAxis.Count == 0) || rightYAxisLegendPosition.Equals("None", StringComparison.OrdinalIgnoreCase))
		{
			// Default to no legend...
		...
		else
		{
			...
			string legendFontSize = tsproduct.getLayeredPropValue("LegendFontSize", subproduct, -1, false);
			string legendFontStyle = tsproduct.getLayeredPropValue("LegendFontStyle", subproduct, -1, false);
			GRDrawingAreaUtil.setFont(_da_righty_bottom_legend, legendFontName, legendFontStyle, StringUtil.atod(legendFontSize));
			int nts = 0;
			int size = tslistRightYAxis.Count;
			TS ts = null;
			string legend = null;

			for (int i = 0; i < size; i++)
			{
				ts = tslistRightYAxis[i];
				...
				if (ts.getEnabled())
				{
					if (__rightYAxisGraphType == TSGraphType.PREDICTED_VALUE_RESIDUAL)
					{
						if (i == 0)
						{
							// ignore the zeroth
						...
						if (!string.ReferenceEquals(legend, null))
						{
							textLimits = GRDrawingAreaUtil.getTextExtents(_da_righty_bottom_legend, legend, GRUnits.DEVICE);
							rightYAxisLegendWidth = MathUtil.max(rightYAxisLegendWidth, textLimits.getWidth());
						}
						++nts;
					}
					else if (__rightYAxisGraphType == TSGraphType.PREDICTED_VALUE)
					{
						// The time series will be plotted and will be shown in the legend...
						legend = getLegendString(ts, i);
						if (!string.ReferenceEquals(legend, null))
			...
			ts = null;
			// Estimate the overall height...
			textLimits = GRDrawingAreaUtil.getTextExtents(_da_righty_bottom_legend, "TEST STRING", GRUnits.DEVICE);
			rightYAxisLegendHeight = nts * textLimits.getHeight();
			// The legend width is increased by the width of the symbol (currently always 25 pixels)...
			rightYAxisLegendWidth += 25;
			textLimits = null;
		}
		// Calculate legend limits for each potential legend position, which is the maximum of
		// the left and right y-axis legend dimensions.  Under normal cases, there should not be conflict
		...
		if (leftYAxisLegendPosition.ToUpper().StartsWith("BOTTOM", StringComparison.Ordinal) || leftYAxisLegendPosition.Equals("Top", StringComparison.OrdinalIgnoreCase))
		{
		...
		if (rightYAxisLegendPosition.ToUpper().StartsWith("BOTTOM", StringComparison.Ordinal) || rightYAxisLegendPosition.Equals("Top", StringComparison.OrdinalIgnoreCase))
		{
			// Limit legend to half the full page height
		...
		if (leftYAxisLegendPosition.ToUpper().StartsWith("BOTTOM", StringComparison.Ordinal))
		{
			bottom_legend_height = leftYAxisLegendHeight;
		}
		else if (leftYAxisLegendPosition.Equals("Left", StringComparison.OrdinalIgnoreCase))
		{
			left_legend_width = leftYAxisLegendWidth;
			left_legend_buffer = buffer;
		}
		else if (leftYAxisLegendPosition.Equals("Right", StringComparison.OrdinalIgnoreCase))
		{
			right_legend_width = leftYAxisLegendWidth;
			right_legend_buffer = buffer;
		}
		else if (leftYAxisLegendPosition.Equals("Top", StringComparison.OrdinalIgnoreCase))
		{
		}
		...
		if (rightYAxisLegendPosition.ToUpper().StartsWith("BOTTOM", StringComparison.Ordinal))
		{
			if (leftYAxisLegendPosition.ToUpper().StartsWith("BOTTOM", StringComparison.Ordinal))
			{
		...
		else if (rightYAxisLegendPosition.Equals("Left", StringComparison.OrdinalIgnoreCase))
		{
			left_legend_width = Math.Max(leftYAxisLegendWidth,rightYAxisLegendWidth);
			left_legend_buffer = buffer;
		}
		else if (rightYAxisLegendPosition.Equals("Right", StringComparison.OrdinalIgnoreCase))
		{
			right_legend_width = Math.Max(leftYAxisLegendWidth,rightYAxisLegendWidth);
			right_legend_buffer = buffer;
		}
		else if (rightYAxisLegendPosition.Equals("Top", StringComparison.OrdinalIgnoreCase))
		{
		}
		...
		if (mainTitleHeight == 0.0)
		{
			...
			_drawlim_maintitle = new GRLimits((drawlim_page.getLeftX() + buffer), drawlim_page.getTopY(), (drawlim_page.getRightX() - buffer), drawlim_page.getTopY());
		}
		else
		{
			_drawlim_maintitle = new GRLimits((drawlim_page.getLeftX() + buffer), (drawlim_page.getTopY() - buffer - mainTitleHeight), (drawlim_page.getRightX() - buffer), (drawlim_page.getTopY() - buffer));
		}
		if (Message.isDebugOn)
		{
		...
		{
			// Zero height drawing area place holder...
			_drawlim_subtitle = new GRLimits((drawlim_page.getLeftX() + buffer), _drawlim_maintitle.getBottomY(), (drawlim_page.getRightX() - buffer), _drawlim_maintitle.getBottomY());
		}
		else
		{
			_drawlim_subtitle = new GRLimits((drawlim_page.getLeftX() + buffer), (_drawlim_maintitle.getBottomY() - buffer - subTitleHeight), (drawlim_page.getRightX() - buffer), (_drawlim_maintitle.getBottomY() - buffer));
		}
		if (Message.isDebugOn)
		{
		...
		if (topXAxisTitleHeight == 0.0)
		{
			...
			_drawlim_topx_title = new GRLimits((drawlim_page.getLeftX() + left_legend_buffer + left_legend_width + buffer + leftYAxisLabelWidth + buffer), _drawlim_subtitle.getBottomY(), (drawlim_page.getRightX() - buffer - right_legend_width - right_legend_buffer - rightYAxisLabelWidth - buffer), _drawlim_subtitle.getBottomY());
		}
		else
		{
			_drawlim_topx_title = new GRLimits((drawlim_page.getLeftX() + left_legend_buffer + left_legend_width + buffer + leftYAxisLabelWidth + buffer), _drawlim_subtitle.getBottomY() - buffer - topXAxisTitleHeight, (drawlim_page.getRightX() - right_legend_buffer - right_legend_width - buffer - rightYAxisLabelWidth - buffer), _drawlim_subtitle.getBottomY() - buffer);
		}
		if (Message.isDebugOn)
		{
		...
		if (leftYAxisTitlePosition.Equals("LeftOfAxis", StringComparison.OrdinalIgnoreCase))
		{
			...
			_drawlim_lefty_title = new GRLimits(leftYAxisTitleLeft, yAxisLabelBottom, (leftYAxisTitleLeft + leftYAxisTitleWidth), yAxisLabelTop);
		}
		else
		{
			// Default is "AboveAxis" to match legacy behavior
			double leftEdge = (drawlim_page.getLeftX() + left_legend_buffer + left_legend_width + buffer + leftYAxisLabelWidth + buffer - leftYAxisTitleWidth / 2.0);
			// Center above left edge of graph
			_drawlim_lefty_title = new GRLimits(leftEdge, (yAxisLabelTop + buffer), (leftEdge + leftYAxisTitleWidth), (yAxisLabelTop + buffer + leftYAxisTitleHeight));
		}
		if (Message.isDebugOn)
		{
		...
		if (leftYAxisTitlePosition.Equals("LeftOfAxis", StringComparison.OrdinalIgnoreCase))
		{
			...
			_drawlim_lefty_label = new GRLimits(leftYAxisLabelLeft, yAxisLabelBottom, (leftYAxisLabelLeft + leftYAxisLabelWidth), yAxisLabelTop);
		}
		else
		{
		...
		_drawlim_righty_label = new GRLimits(rightYLabelLeftEdge, yAxisLabelBottom, (rightYLabelLeftEdge + rightYAxisLabelWidth), yAxisLabelTop);
		if (Message.isDebugOn)
		{
		...
		if (rightYAxisTitlePosition.Equals("RightOfAxis", StringComparison.OrdinalIgnoreCase))
		{
			...
			_drawlim_righty_title = new GRLimits((rightEdge - rightYAxisTitleWidth), yAxisLabelBottom, rightEdge, yAxisLabelTop);
		}
		else
		{
			// Default is "AboveAxis"
			double leftEdge = (drawlim_page.getRightX() - right_legend_buffer - right_legend_width - buffer - rightYAxisLabelWidth - buffer - rightYAxisTitleWidth / 2.0);
			// Center above right edge of graph
			_drawlim_righty_title = new GRLimits(leftEdge, (yAxisLabelTop + buffer), (leftEdge + rightYAxisTitleWidth), (yAxisLabelTop + buffer + rightYAxisTitleHeight));
		}
		if (Message.isDebugOn)
		{
		...
		if (bottom_legend_height == 0.0)
		{
			...
			_drawlim_lefty_bottom_legend = new GRLimits((_drawlim_page.getLeftX() + buffer), _drawlim_page.getBottomY(), (_drawlim_page.getRightX() - buffer), _drawlim_page.getBottomY());
		}
		else
		{
			_drawlim_lefty_bottom_legend = new GRLimits((_drawlim_page.getLeftX() + buffer), (_drawlim_page.getBottomY() + buffer), (_drawlim_page.getRightX() - buffer), (_drawlim_page.getBottomY() + buffer + bottom_legend_height));
		}
		// Set the data limits for the legend to use device units...
		_datalim_lefty_bottom_legend = new GRLimits(0.0, 0.0, _drawlim_lefty_bottom_legend.getWidth(), _drawlim_lefty_bottom_legend.getHeight());

		// Bottom right y-axis legend
		if (bottom_legend_height == 0.0)
		{
			// Make zero-height same Y as the bottom of the page area...
			_drawlim_righty_bottom_legend = new GRLimits((_drawlim_page.getLeftX() + buffer), _drawlim_page.getBottomY(), (_drawlim_page.getRightX() - buffer), _drawlim_page.getBottomY());
		}
		else
		{
			_drawlim_righty_bottom_legend = new GRLimits((_drawlim_page.getLeftX() + buffer), (_drawlim_page.getBottomY() + buffer), (_drawlim_page.getRightX() - buffer), (_drawlim_page.getBottomY() + buffer + bottom_legend_height));
		}
		// Set the data limits for the legend to use device units...
		_datalim_righty_bottom_legend = new GRLimits(0.0, 0.0, _drawlim_righty_bottom_legend.getWidth(), _drawlim_righty_bottom_legend.getHeight());
		...
		if (bottomXAxisTitleHeight == 0.0)
		{
			...
			_drawlim_bottomx_title = new GRLimits((_drawlim_lefty_label.getRightX() + buffer), (_drawlim_lefty_bottom_legend.getTopY() + buffer), (_drawlim_righty_label.getLeftX() - buffer), (_drawlim_lefty_bottom_legend.getTopY() + buffer + buffer));
		}
		else
		{
			_drawlim_bottomx_title = new GRLimits((_drawlim_lefty_label.getRightX() + buffer), (_drawlim_lefty_bottom_legend.getTopY() + buffer), (_drawlim_righty_label.getLeftX() - buffer), (_drawlim_lefty_bottom_legend.getTopY() + buffer + bottomXAxisTitleHeight + buffer));
		}

		if (bottomXAxisLabelHeight == 0.0)
		{
			// Make zero-height same Y as the top of the X title...
			_drawlim_bottomx_label = new GRLimits((_drawlim_lefty_label.getRightX() + buffer), _drawlim_bottomx_title.getTopY(), (_drawlim_righty_label.getLeftX() - buffer), _drawlim_bottomx_title.getTopY());
		}
		else
		{
			_drawlim_bottomx_label = new GRLimits((_drawlim_lefty_label.getRightX() + buffer), (_drawlim_bottomx_title.getTopY() + buffer), (_drawlim_righty_label.getLeftX() - buffer), (_drawlim_bottomx_title.getTopY() + buffer + bottomXAxisLabelHeight));
		}
		...
		if (_is_reference_graph)
		{
			_drawlim_lefty_graph = new GRLimits(drawlim_page.getLeftX(), drawlim_page.getBottomY(), drawlim_page.getRightX(), drawlim_page.getTopY());
		}
		else
		{
			// Left y-axis graph
			if (leftYAxisTitlePosition.Equals("LeftOfAxis", StringComparison.OrdinalIgnoreCase))
			{
				_drawlim_lefty_graph = new GRLimits((_drawlim_lefty_label.getRightX() + buffer), (_drawlim_bottomx_label.getTopY() + buffer), (_drawlim_righty_label.getLeftX() - buffer), _drawlim_lefty_title.getTopY());
			}
			else
			{
				// "AboveAxis" (legacy)
				_drawlim_lefty_graph = new GRLimits((_drawlim_lefty_label.getRightX() + buffer), (_drawlim_bottomx_label.getTopY() + buffer), (_drawlim_righty_label.getLeftX() - buffer), (_drawlim_lefty_title.getBottomY() - buffer));
			}
		}
		...
		if (leftYAxisLegendPosition.Equals("Left", StringComparison.OrdinalIgnoreCase))
		{
			_drawlim_lefty_left_legend = new GRLimits((_drawlim_page.getLeftX() + left_legend_buffer), _drawlim_lefty_graph.getBottomY(), (_drawlim_page.getLeftX() + left_legend_buffer + left_legend_width), _drawlim_lefty_graph.getTopY());
		}
		else
		{
			// Set to zero width...
			_drawlim_lefty_left_legend = new GRLimits(_drawlim_page.getLeftX(), _drawlim_lefty_graph.getBottomY(), _drawlim_page.getLeftX(), _drawlim_lefty_graph.getTopY());
		}
		// Set the data limits for the legend to use device units...
		_datalim_lefty_left_legend = new GRLimits(0.0, 0.0, _drawlim_lefty_left_legend.getWidth(), _drawlim_lefty_left_legend.getHeight());

		// Left y-axis right legend

		if (leftYAxisLegendPosition.Equals("Right", StringComparison.OrdinalIgnoreCase))
		{
			_drawlim_lefty_right_legend = new GRLimits((_drawlim_page.getRightX() - right_legend_buffer - right_legend_width), _drawlim_lefty_graph.getBottomY(), (_drawlim_page.getRightX() - right_legend_buffer), _drawlim_lefty_graph.getTopY());
		}
		else
		{
			// Set to zero width...
			_drawlim_lefty_right_legend = new GRLimits(_drawlim_page.getRightX(), _drawlim_lefty_graph.getBottomY(), _drawlim_page.getRightX(), _drawlim_lefty_graph.getTopY());
		}
		// Set the data limits for the legend to use device units...
		_datalim_lefty_right_legend = new GRLimits(0.0, 0.0, _drawlim_lefty_right_legend.getWidth(), _drawlim_lefty_right_legend.getHeight());

		// Right y-axis left legend

		if (rightYAxisLegendPosition.Equals("Left", StringComparison.OrdinalIgnoreCase))
		{
			_drawlim_righty_left_legend = new GRLimits((_drawlim_page.getLeftX() + left_legend_buffer), _drawlim_righty_graph.getBottomY(), (_drawlim_page.getLeftX() + left_legend_buffer + left_legend_width), _drawlim_righty_graph.getTopY());
		}
		else
		{
			// Set to zero width...
			_drawlim_righty_left_legend = new GRLimits(_drawlim_page.getLeftX(), _drawlim_righty_graph.getBottomY(), _drawlim_page.getLeftX(), _drawlim_righty_graph.getTopY());
		}
		// Set the data limits for the legend to use device units...
		_datalim_righty_left_legend = new GRLimits(0.0, 0.0, _drawlim_righty_left_legend.getWidth(), _drawlim_righty_left_legend.getHeight());

		// Right y-axis right legend

		if (rightYAxisLegendPosition.Equals("Right", StringComparison.OrdinalIgnoreCase))
		{
			_drawlim_righty_right_legend = new GRLimits((_drawlim_page.getRightX() - right_legend_buffer - right_legend_width), _drawlim_righty_graph.getBottomY(), (_drawlim_page.getRightX() - right_legend_buffer), _drawlim_righty_graph.getTopY());
		}
		else
		{
			// Set to zero width...
			_drawlim_righty_right_legend = new GRLimits(_drawlim_page.getRightX(), _drawlim_righty_graph.getBottomY(), _drawlim_page.getRightX(), _drawlim_righty_graph.getTopY());
		}
		// Set the data limits for the legend to use device units...
		_datalim_righty_right_legend = new GRLimits(0.0, 0.0, _drawlim_righty_right_legend.getWidth(), _drawlim_righty_right_legend.getHeight());

		// Left y-axis inner legend positions

		if (leftYAxisLegendPosition.Equals("InsideUpperLeft", StringComparison.OrdinalIgnoreCase))
		{
			_drawlim_lefty_inside_legend = new GRLimits(_drawlim_lefty_graph.getLeftX() + inside_lefty_legend_buffer, _drawlim_lefty_graph.getTopY() - inside_lefty_legend_buffer - inside_lefty_legend_height, _drawlim_lefty_graph.getLeftX() + inside_lefty_legend_buffer + inside_lefty_legend_width, _drawlim_lefty_graph.getTopY() - inside_lefty_legend_buffer);
		}
		else if (leftYAxisLegendPosition.Equals("InsideUpperRight", StringComparison.OrdinalIgnoreCase))
		{
			_drawlim_lefty_inside_legend = new GRLimits(_drawlim_lefty_graph.getRightX() - inside_lefty_legend_buffer - inside_lefty_legend_width, _drawlim_lefty_graph.getTopY() - inside_lefty_legend_buffer - inside_lefty_legend_height, _drawlim_lefty_graph.getRightX() - inside_lefty_legend_buffer, _drawlim_lefty_graph.getTopY() - inside_lefty_legend_buffer);
		}
		else if (leftYAxisLegendPosition.Equals("InsideLowerLeft", StringComparison.OrdinalIgnoreCase))
		{
			_drawlim_lefty_inside_legend = new GRLimits(_drawlim_lefty_graph.getLeftX() + inside_lefty_legend_buffer, _drawlim_lefty_graph.getBottomY() + inside_lefty_legend_buffer, _drawlim_lefty_graph.getLeftX() + inside_lefty_legend_buffer + inside_lefty_legend_width, _drawlim_lefty_graph.getBottomY() + inside_lefty_legend_buffer + inside_lefty_legend_height);
		}
		else if (leftYAxisLegendPosition.Equals("InsideLowerRight", StringComparison.OrdinalIgnoreCase))
		{
			_drawlim_lefty_inside_legend = new GRLimits(_drawlim_lefty_graph.getRightX() - inside_lefty_legend_buffer - inside_lefty_legend_width, _drawlim_lefty_graph.getBottomY() + inside_lefty_legend_buffer, _drawlim_lefty_graph.getRightX() - inside_lefty_legend_buffer, _drawlim_lefty_graph.getBottomY() + inside_lefty_legend_buffer + inside_lefty_legend_height);
		}
		else
		{
			_drawlim_lefty_inside_legend = new GRLimits(_drawlim_page.getLeftX(), _drawlim_page.getBottomY(), _drawlim_page.getRightX(), _drawlim_page.getTopY());
		}
		_datalim_lefty_inside_legend = new GRLimits(0.0, 0.0, _drawlim_lefty_inside_legend.getWidth(), _drawlim_lefty_inside_legend.getHeight());

		// Right y-axis inner legend positions - same as left y-axis other than the drawing limits name

		if (rightYAxisLegendPosition.Equals("InsideUpperLeft", StringComparison.OrdinalIgnoreCase))
		{
			_drawlim_righty_inside_legend = new GRLimits(_drawlim_righty_graph.getLeftX() + inside_righty_legend_buffer, _drawlim_righty_graph.getTopY() - inside_righty_legend_buffer - inside_righty_legend_height, _drawlim_righty_graph.getLeftX() + inside_righty_legend_buffer + inside_righty_legend_width, _drawlim_righty_graph.getTopY() - inside_righty_legend_buffer);
		}
		else if (rightYAxisLegendPosition.Equals("InsideUpperRight", StringComparison.OrdinalIgnoreCase))
		{
			_drawlim_righty_inside_legend = new GRLimits(_drawlim_righty_graph.getRightX() - inside_righty_legend_buffer - inside_righty_legend_width, _drawlim_righty_graph.getTopY() - inside_righty_legend_buffer - inside_righty_legend_height, _drawlim_righty_graph.getRightX() - inside_righty_legend_buffer, _drawlim_righty_graph.getTopY() - inside_righty_legend_buffer);
		}
		else if (rightYAxisLegendPosition.Equals("InsideLowerLeft", StringComparison.OrdinalIgnoreCase))
		{
			_drawlim_righty_inside_legend = new GRLimits(_drawlim_righty_graph.getLeftX() + inside_righty_legend_buffer, _drawlim_righty_graph.getBottomY() + inside_righty_legend_buffer, _drawlim_righty_graph.getLeftX() + inside_righty_legend_buffer + inside_righty_legend_width, _drawlim_righty_graph.getBottomY() + inside_righty_legend_buffer + inside_righty_legend_height);
		}
		else if (rightYAxisLegendPosition.Equals("InsideLowerRight", StringComparison.OrdinalIgnoreCase))
		{
			_drawlim_righty_inside_legend = new GRLimits(_drawlim_righty_graph.getRightX() - inside_righty_legend_buffer - inside_righty_legend_width, _drawlim_righty_graph.getBottomY() + inside_righty_legend_buffer, _drawlim_righty_graph.getRightX() - inside_righty_legend_buffer, _drawlim_righty_graph.getBottomY() + inside_righty_legend_buffer + inside_righty_legend_height);
		}
		else
		{
			_drawlim_righty_inside_legend = new GRLimits(_drawlim_page.getLeftX(), _drawlim_page.getBottomY(), _drawlim_page.getRightX(), _drawlim_page.getTopY());
		}
		_datalim_righty_inside_legend = new GRLimits(0.0, 0.0, _drawlim_righty_inside_legend.getWidth(), _drawlim_righty_inside_legend.getHeight());
		...
		if ((_da_lefty_bottom_legend != null) && (_drawlim_lefty_bottom_legend != null))
		{
			_da_lefty_bottom_legend.setDrawingLimits(_drawlim_lefty_bottom_legend, GRUnits.DEVICE, GRLimits.DEVICE);
			_da_lefty_bottom_legend.setDataLimits(_datalim_lefty_bottom_legend);
		}
		if ((_da_lefty_left_legend != null) && (_drawlim_lefty_left_legend != null))
		{
			_da_lefty_left_legend.setDrawingLimits(_drawlim_lefty_left_legend, GRUnits.DEVICE, GRLimits.DEVICE);
			_da_lefty_left_legend.setDataLimits(_datalim_lefty_left_legend);
		}
		if ((_da_lefty_right_legend != null) && (_drawlim_lefty_right_legend != null))
		{
			_da_lefty_right_legend.setDrawingLimits(_drawlim_lefty_right_legend, GRUnits.DEVICE, GRLimits.DEVICE);
			_da_lefty_right_legend.setDataLimits(_datalim_lefty_right_legend);
		}
		if ((_da_lefty_inside_legend != null) && (_drawlim_lefty_inside_legend != null))
		{
			_da_lefty_inside_legend.setDrawingLimits(_drawlim_lefty_inside_legend, GRUnits.DEVICE, GRLimits.DEVICE);
			_da_lefty_inside_legend.setDataLimits(_datalim_lefty_inside_legend);
		}
		// Right y-axis legend options
		if ((_da_righty_bottom_legend != null) && (_drawlim_righty_bottom_legend != null))
		{
			_da_righty_bottom_legend.setDrawingLimits(_drawlim_righty_bottom_legend, GRUnits.DEVICE, GRLimits.DEVICE);
			_da_righty_bottom_legend.setDataLimits(_datalim_righty_bottom_legend);
		}
		if ((_da_righty_left_legend != null) && (_drawlim_righty_left_legend != null))
		{
			_da_righty_left_legend.setDrawingLimits(_drawlim_righty_left_legend, GRUnits.DEVICE, GRLimits.DEVICE);
			_da_righty_left_legend.setDataLimits(_datalim_righty_left_legend);
		}
		if ((_da_righty_right_legend != null) && (_drawlim_righty_right_legend != null))
		{
			_da_righty_right_legend.setDrawingLimits(_drawlim_righty_right_legend, GRUnits.DEVICE, GRLimits.DEVICE);
			_da_righty_right_legend.setDataLimits(_datalim_righty_right_legend);
		}
		if ((_da_righty_inside_legend != null) && (_drawlim_righty_inside_legend != null))
		{
			_da_righty_inside_legend.setDrawingLimits(_drawlim_righty_inside_legend, GRUnits.DEVICE, GRLimits.DEVICE);
			_da_righty_inside_legend.setDataLimits(_datalim_righty_inside_legend);
		}
	}

	/// <summary>
	/// Sets the end date. </summary>
	/// <param name="endDate"> value to set the end date to. </param>
	protected internal virtual void setEndDate(DateTime endDate)
	{
		_end_date = endDate;
	}
	...
	/// </summary>
	protected internal virtual void setLeftYAxisViewMaxY(string leftYAxisViewMaxY)
	{
		this.leftYAxisViewMaxY = leftYAxisViewMaxY;
		if (leftYAxisViewMaxY.Equals(YAXIS_LIMITS_USE_PRODUCT_PROPERTIES, StringComparison.OrdinalIgnoreCase))
		{
	...
	/// </summary>
	protected internal virtual void setLeftYAxisViewMaxYFromData(double leftYAxisViewMaxYFromData)
	{
	...
	/// </summary>
	protected internal virtual void setLeftYAxisViewMinY(string leftYAxisViewMinY)
	{
		this.leftYAxisViewMinY = leftYAxisViewMinY;
		if (leftYAxisViewMinY.Equals(YAXIS_LIMITS_USE_PRODUCT_PROPERTIES, StringComparison.OrdinalIgnoreCase))
		{
			// Clear so it is not used
	...
	/// </summary>
	protected internal virtual void setLeftYAxisViewMinYFromData(double leftYAxisViewMinYFromData)
	{
	...
	/// </summary>
	protected internal virtual void setRightYAxisViewMaxY(string rightYAxisViewMaxY)
	{
		this.rightYAxisViewMaxY = rightYAxisViewMaxY;
		if (rightYAxisViewMaxY.Equals(YAXIS_LIMITS_USE_PRODUCT_PROPERTIES, StringComparison.OrdinalIgnoreCase))
		{
			// Clear so it is not used
	...
	/// </summary>
	protected internal virtual void setRightYAxisViewMaxYFromData(double rightYAxisViewMaxYFromData)
	{
	...
	/// </summary>
	protected internal virtual void setRightYAxisViewMinY(string rightYAxisViewMinY)
	{
		this.rightYAxisViewMinY = rightYAxisViewMinY;
		if (rightYAxisViewMinY.Equals(YAXIS_LIMITS_USE_PRODUCT_PROPERTIES, StringComparison.OrdinalIgnoreCase))
		{
			// Clear so it is not used
	...
	/// </summary>
	protected internal virtual void setRightYAxisViewMinYFromData(double rightYAxisViewMinYFromData)
	{
		this.rightYAxisViewMinYFromData = rightYAxisViewMinYFromData;
	}

	/// <summary>
	/// Sets the maximum end date. </summary>
	/// <param name="d"> the end date. </param>
	protected internal virtual void setMaxEndDate(DateTime d)
	{
		_max_end_date = new DateTime(d);
	}

	/// <summary>
	/// Sets the maximum start date. </summary>
	/// <param name="d"> the start date. </param>
	protected internal virtual void setMaxStartDate(DateTime d)
	{
	...
	/// properties, the selects must be transferred manually. </summary>
	/// <param name="selectedTimeSeriesList"> list of time series that should be considered as selected,
	/// as if they were clicked on in the legend. </param>
	protected internal virtual void setSelectedTimeSeriesList(IList<TS> selectedTimeSeriesList)
	{
	...
	/// </summary>
	protected internal virtual void setShowDrawingAreaOutline(bool showDrawingAreaOutline)
	{
		_showDrawingAreaOutline = showDrawingAreaOutline;
	}

	/// <summary>
	/// Sets the start date. </summary>
	/// <param name="startDate"> value to set the start date to. </param>
	protected internal virtual void setStartDate(DateTime startDate)
	{
	...
	/// chosen, regardless of whether the candidate's hour aligns with the time series data. </summary>
	/// <param name="candidate_DateTime"> The candidate DateTime, which may have a precision
	/// greater than or less than those used by the indicated time series. </param>
	/// <param name="ts"> Time series to examine, which may be a regular or irregular time series. </param>
	/// <returns> DateTime matching the precision of DateTimes used in the specified time
	/// series that is equal to or less than the candidate DateTime.  The returned
	/// DateTime will align with the time series data and may NOT align evenly with the candidate DateTime. </returns>
	...
	/// This keeps the list short for lookups. </summary>
	/// <param name="ts"> time series to be (un)selected. </param>
	/// <returns> true if the time series is selected after call, false if it is unselected after call </returns>
	public virtual bool toggleTimeSeriesSelection(TS ts)
	{
		bool found = false;
		bool isSelected = false;
		TS ts2;
		for (int i = (_selectedTimeSeriesList.Count - 1); i >= 0; i--)
		{
			ts2 = _selectedTimeSeriesList[i];
			if (ts2 == ts)
			{
