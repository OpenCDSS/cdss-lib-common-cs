...
namespace RTi.TS
{
	...
	/// <summary>
	/// Constructor. </summary>
	/// <param name="oldTS"> Time series used as data to generate the new time series. </param>
	/// <param name="newInterval"> New interval as a string that can be parsed by RTi.Util.Time.TimeInterval.parseInterval(),
	/// indicating the new interval for the time series. </param>
	/// <param name="oldTimeScale"> The old time scale indicates whether values in the old time series are accumulated over
	/// the interval ("ACCM"), an average over the interval ("MEAN"), or instantaneous for the point in time ("INST")). </param>
	/// <param name="newTimeScale"> The new time scale indicates whether values in the new time series are accumulated over
	/// the interval ("ACCM"), an average over the interval ("MEAN"), or instantaneous for the point in time ("INST")). </param>
	/// <param name="statisticType"> the output statistic type, support for some calculations (currently only INST-small
	/// to INST-large), for example compute daily max from instantaneous small interval.  Specify null to use the
	/// default statistic for the time scale and interval combination. </param>
	/// <param name="outputYearType"> output year type used when the new interval is year (if null use calendar). </param>
	/// <param name="newDataType"> the data type for the new time series.  If null, the data type from the old time series
	/// will be used (Warning: for some systems, the meaning of the data type changes depending on the interval.
	/// For example "Streamflow" may be instantaneous for irregular data and mean for daily data.) </param>
	/// <param name="newUnits"> Units for the new time series.  If null, use the units from the original time series. </param>
	/// <param name="tolerance"> Used when converting large interval MEAN data to small interval INST data - see TSTool documentation. </param>
	/// <param name="handleEndpointsHow"> when changing from INST to MEAN, small interval to large interval (but only daily or
	/// smaller), indicate how the end-points should be handled. </param>
	/// <param name="allowMissingCount"> Indicate the number of missing values that can be missing in the input data and still
	...
	/// the input data will result in missing data in the result. </param>
	/// <param name="allowMissingPercent"> Indicate the percent of missing values that can be missing in the input data and
	...
	/// allowMissingCount is specified, it will override this default. </param>
	/// <param name="allowMissingConsecutive"> the number of consecutive missing values allowed and still compute the result,
	/// considered at the same time as allowMissingCount </param>
	/// <param name="outputFillMethod"> This property is used only by conversions from INST to MEAN when the conversion
	...
	/// depending on the time interval (<Day or >=Day) data point. The default if specified as null is REPEAT. </param>
	/// <param name="handleMissingImputHow"> Indicate how to handle the missing data. The options are:
	...
	/// TODO SAM 2009-11-02 need to evaluate the references to ALERT versus general behavior. </param>
	public TSUtil_ChangeInterval(TS oldTS, TimeInterval newInterval, TimeScaleType oldTimeScale, TimeScaleType newTimeScale, TSStatisticType statisticType, YearType outputYearType, string newDataType, string newUnits, double? tolerance, TSUtil_ChangeInterval_HandleEndpointsHowType handleEndpointsHow, TSUtil_ChangeInterval_OutputFillMethodType outputFillMethod, TSUtil_ChangeInterval_HandleMissingInputHowType handleMissingInputHow, int? allowMissingCount, double? allowMissingPercent, int? allowMissingConsecutive)
	{
		// Check controlling information
		// OldTS - Make sure it is not null and has a not zero length period of record.
		if (oldTS == null)
		{
			throw new System.ArgumentException("Input time series is null.  Cannot change interval.");
		}
		...
		if (newInterval == null)
		{
			throw new System.ArgumentException("New interval is null.  Cannot change interval.");
		}
		...
		{
			throw new System.ArgumentException("Old time scale is null.  Cannot change interval.");
		}
		...
		{
			throw new System.ArgumentException("New time scale is null.  Cannot change interval.");
		}
		...
		if (statisticType != null)
		{
			...
			{
				throw new System.ArgumentException("Statistic can only be specified with INST(small) to INST(large).");
			}
			...
			IList<TSStatisticType> statistics = TSUtil_CalculateTimeSeriesStatistic.getStatisticChoices();
			foreach (TSStatisticType statistic in statistics)
			{
			...
			if (!supported)
			{
				throw new System.ArgumentException("The statistic (" + statisticType + ") is not supported by this command.");
			}
		}
		...
		if ((string.ReferenceEquals(newDataType, null)) || newDataType.Equals(""))
		{
		...
		if ((string.ReferenceEquals(newUnits, null)) || newUnits.Equals(""))
		{
		...
		if ((tolerance.Value < 0.0) || (tolerance.Value > 1.0))
		{
			throw new System.ArgumentException("Tolerance_double (" + tolerance + ") is not between 0 and 1.  " + "Cannot change interval.");
		}
		setTolerance(tolerance.Value);

		// HandleEndpointsHow - Used when changing from INST to MEAN, small interval to large (daily or less) interval.
		if ((oldTimeScale == TimeScaleType.INST) && (newTimeScale == TimeScaleType.MEAN) && (newInterval.getBase() <= TimeInterval.DAY) && (oldIntervalBase < TimeInterval.DAY))
		{
		...
		else
		{
			...
			{
				throw new System.ArgumentException("Specifying HandleEndpointsHow is only valid when changing " + "from INST to MEAN small interval to large (daily or less).");
			}
		}
		...
		if (((newTimeScale == TimeScaleType.MEAN) || (newTimeScale == TimeScaleType.ACCM)) && (newInterval.getBase() == TimeInterval.YEAR) && (handleMissingInputHow != TSUtil_ChangeInterval_HandleMissingInputHowType.KEEP_MISSING))
		{
			throw new System.ArgumentException("HandleMissingInputHow cannot be specified for MEAN or ACCM, Year interval output.");
		}
		...
		{
			throw new System.ArgumentException("Only one of these parameters should be provided as non-null: " + "AllowMissingCount or AllowMissingPercent.  Cannot change interval.");
		}

		// AllowMissingCount
		if (allowMissingCount == null)
		{
			allowMissingCount = new int?(0); // Default is don't allow missing
		}
		// If the given value is less than 0, throw exception.
		if (allowMissingCount.Value < 0)
		{
			throw new System.ArgumentException("AllowMissingCount (" + allowMissingCount + ") is negative - must be >= 0.  Cannot change interval.");
		}
		...
		{
			// If the given value is less than 0, throw exception.
			if (allowMissingPercent.Value < 0.0)
			{
				throw new System.ArgumentException("AllowMissingPercent (" + allowMissingPercent + ") is negative - must be between 0 and 100.  Cannot change interval.");
			}
			else if (allowMissingPercent.Value > 100.0)
			{
				throw new System.ArgumentException("AllowMissingPercent (" + allowMissingPercent + ") is > 100 - must be between 0 and 100.  Cannot change interval.");
			}
			setAllowMissingPercent(allowMissingPercent.Value);
		}
		...
		if (allowMissingConsecutive == null)
		{
			if (allowMissingCount > 0)
			{
				...
				allowMissingConsecutive = new int?(allowMissingCount);
			}
			else
			{
				allowMissingConsecutive = new int?(0); // Default is don't allow missing
			}
		}
		// If the given value is less than 0, throw exception.
		if (allowMissingConsecutive.Value < 0)
		{
			throw new System.ArgumentException("AllowMissingConsecutive (" + allowMissingConsecutive + ") is negative - must be >= 0.  Cannot change interval.");
		}
		setAllowMissingConsecutive(allowMissingConsecutive.Value);
	}
		...
		/// </summary>
		/// <param name="TSMean"> - old Mean TS we are trying to change to instantaneous </param>
		/// <param name="TSInst"> - newly created instantaneous TS </param>
		/// <param name="tolerance"> - a fraction 0 to 1 </param>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: private void adjustInstantaneousBasedOnMean(TS oldTS, TS newTS, double tolerance) throws Exception
		private void adjustInstantaneousBasedOnMean(TS oldTS, TS newTS, double tolerance)
		{
		...
		/// intervals that are involved. </summary>
		/// <returns> A new time series of the requested data interval. All of the original time series header
		/// information will be essentially the same, except for the interval and possibly the data type. </returns>
		/// <param name="createData"> if true, calculate values for the data array; if false, only assign metadata </param>
		/// <exception cref="Exception"> if an error occurs (e.g., bad new interval string). </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public TS changeInterval(boolean createData) throws Exception
		public virtual TS changeInterval(bool createData)
		{
			...
			if (newTS == null)
			{
				throw new Exception("Could not create the new time series - cannot change interval.");
			}
			...
			{
				if ((outputYearType.getStartYearOffset() < 0) && (oldTS.getDate1().getMonth() >= outputYearType.getStartMonth()))
				{
					...
					Message.printStatus(2, routine, "Adjusting output time series start to " + date1 + " to align with " + outputYearType + " year type.");
				}
				// Similarly shift the end of the year...
				if ((outputYearType.getStartYearOffset() < 0) && (oldTS.getDate2().getMonth() >= outputYearType.getStartMonth()))
				{
					DateTime date2 = newTS.getDate2();
					date2.addYear(1);
					newTS.setDate2(date2);
					Message.printStatus(2, routine, "Adjusting output time series start to " + date2 + " to align with " + outputYearType + " year type.");
				}
			}
			...
			if (newTS.getDataIntervalBase() == TimeInterval.IRREGULAR)
			{
				...
				throw new System.ArgumentException(warning);
			}
			...
			if ((newtsBase == TimeInterval.YEAR) && (outputYearType != YearType.CALENDAR))
			{
				...
				if ((oldTS.getDataIntervalBase() != TimeInterval.DAY) && (oldTS.getDataIntervalBase() != TimeInterval.MONTH) && (oldTS.getDataIntervalMult() != 1))
				{
				...
				changeIntervalToDifferentYearType(oldTS, newTS, newTimeScale, outputYearType, allowMissingCount, allowMissingConsecutive);
				returnTS = true;
			}
			else if (oldTS.getDataIntervalBase() == TimeInterval.IRREGULAR)
			{
				...
				if (((oldTimeScale == TimeScaleType.ACCM) && (newTimeScale == TimeScaleType.ACCM)) || ((oldTimeScale == TimeScaleType.MEAN) && (newTimeScale == TimeScaleType.MEAN)) || ((oldTimeScale == TimeScaleType.INST) && (newTimeScale == TimeScaleType.MEAN)) || ((oldTimeScale == TimeScaleType.INST) && (newTimeScale == TimeScaleType.INST)))
				{
					if (changeInterval_fromIRREGULAR(oldTSi, newTSi, oldTimeScale, newTimeScale, handleMissingInputHow))
					{
						returnTS = true;
					}
				}
			...
			else
			{
				...
				if ((oldTimeScale == TimeScaleType.MEAN) || (oldTimeScale == TimeScaleType.ACCM))
				{
					...
					{
						if (changeInterval_toMEANorACCM(oldTSi, newTSi, intervalRelation, oldTimeScale, newTimeScale, handleMissingInputHow, allowMissingCount, allowMissingConsecutive, outputFillMethod, handleEndpointsHow))
						{
							returnTS = true;
						}
					}
					...
					{
						if (changeInterval_toINST(oldTSi, newTSi, intervalRelation, oldTimeScale, newTimeScale, handleMissingInputHow, tolerance))
						{
							returnTS = true;
						}
					}
				...
				else if (oldTimeScale == TimeScaleType.INST)
				{
					if (newTimeScale == TimeScaleType.INST)
					{
						...
						// ---------------------------------------------
						if (changeInterval_fromINST(oldTSi, newTSi, intervalRelation, oldTimeScale, newTimeScale, statistic, handleMissingInputHow, allowMissingCount, allowMissingConsecutive))
						{
							returnTS = true;
						}
					}
		...
		/// <summary>
		/// Change intervals from INST to INST for regular interval time series. </summary>
		/// <param name="oldTSi"> Reference to the iterator object for the old time series. </param>
		/// <param name="newTSi"> Reference to the iterator object for the new time series. </param>
		/// <param name="intervalRelation"> ratio of intervals, negative if newTS interval is longer than the oldTS. </param>
		/// <param name="oldTimeScale"> time scale of the old time series. </param>
		/// <param name="newTimeScale"> time scale of the new time series. </param>
		/// <param name="handleMissingInputHow"> Indicates how to treat missing values in the input time series </param>
		/// <param name="allowMissingCount"> the number of missing values allowed (only used with statistic). </param>
		/// <param name="allowMissingConsecutive"> only compute the new data value if the number of consecutive missing in the
		/// input interval is <= the specified count. </param>
		/// <returns> true if successful or false if an error. </returns>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: private boolean changeInterval_fromINST(TSIterator oldTSi, TSIterator newTSi, int intervalRelation, RTi.Util.Time.TimeScaleType oldTimeScale, RTi.Util.Time.TimeScaleType newTimeScale, TSStatisticType statistic, TSUtil_ChangeInterval_HandleMissingInputHowType handleMissingInputHow, int allowMissingCount, int allowMissingConsecutive) throws Exception
		private bool changeInterval_fromINST(TSIterator oldTSi, TSIterator newTSi, int intervalRelation, TimeScaleType oldTimeScale, TimeScaleType newTimeScale, TSStatisticType statistic, TSUtil_ChangeInterval_HandleMissingInputHowType handleMissingInputHow, int allowMissingCount, int allowMissingConsecutive)
		{
			...
			if (intervalRelation < 0)
			{
				...
				bool previousIsMissing = false; // Whether previous value in input interval was missing
				bool okToCompute = true; // Whether an output interval value can be computed
						...
						if (statistic != null)
						{
							...
							else
							{
								...
								if (statisticSampleSize == statisticSample.Length)
								{
									...
									Array.Copy(statisticSample, 0, statisticSampleNew, 0, statisticSample.Length);
			...
			else
			{
				...
				return changeIntervalFromInstByInterpolation(oldTSi, newTSi, intervalRelation, oldTimeScale, newTimeScale, handleMissingInputHow, true);
			}
			return true;
		}
		...
		/// <param name="oldTSi"> Reference to the iterator object for the old time series. </param>
		/// <param name="newTSi"> Reference to the iterator object for the new time series. </param>
		/// <param name="intervalRelation"> ratio of intervals, negative if newTS interval is longer oldTS.
		/// This parameter is used only for verification. </param>
		/// <param name="oldTimeScale"> - The time scale of the old time series.
		/// This parameter is used only for verification. </param>
		/// <param name="newTimeScale"> - The time scale of the new time series.
		/// This parameter is used only for verification. </param>
		/// <param name="handleMissingInputHow"> Indicates how to treat missing values in the input TS </param>
		/// <param name="timeStampedAtEnd"> - This argument is used to time stamp the data at the beginning of the period.
		/// If passed as false, the data will be time stamped at end of the interval, otherwise at the beginning. </param>
		/// <returns> true if successful or false if an error. </returns>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: private boolean changeIntervalFromInstByInterpolation(TSIterator oldTSi, TSIterator newTSi, int intervalRelation, RTi.Util.Time.TimeScaleType oldTimeScale, RTi.Util.Time.TimeScaleType newTimeScale, TSUtil_ChangeInterval_HandleMissingInputHowType handleMissingInputHow, boolean timeStampedAtEnd) throws Exception
		private bool changeIntervalFromInstByInterpolation(TSIterator oldTSi, TSIterator newTSi, int intervalRelation, TimeScaleType oldTimeScale, TimeScaleType newTimeScale, TSUtil_ChangeInterval_HandleMissingInputHowType handleMissingInputHow, bool timeStampedAtEnd)
		{
			string warning;

			// Make sure the required conversion is supported
			if (((oldTimeScale == TimeScaleType.INST) && (newTimeScale == TimeScaleType.INST)) || ((oldTimeScale == TimeScaleType.INST) && (newTimeScale == TimeScaleType.MEAN)))
			{
			...
			bool previousValueMissing = true;
			bool currentValueMissing = true;
		...
		/// <param name="oldTSi"> Reference to the iterator object for the old time series. </param>
		/// <param name="newTSi"> Reference to the iterator object for the new time series. </param>
		/// <param name="oldTimeScale"> The time scale of the old time series. This parameter is used only for verification. </param>
		/// <param name="newTimeScale"> The time scale of the new time series. This parameter is used only for verification. </param>
		/// <param name="missingValueFlag"> Indicates how to treat missing values in the input TS </param>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: private boolean changeInterval_fromIRREGULAR(TSIterator oldTSi, TSIterator newTSi, RTi.Util.Time.TimeScaleType oldTimeScale, RTi.Util.Time.TimeScaleType newTimeScale, TSUtil_ChangeInterval_HandleMissingInputHowType handleMissingInputHow) throws Exception
		private bool changeInterval_fromIRREGULAR(TSIterator oldTSi, TSIterator newTSi, TimeScaleType oldTimeScale, TimeScaleType newTimeScale, TSUtil_ChangeInterval_HandleMissingInputHowType handleMissingInputHow)
		{
			...
			while (oldDate.lessThanOrEqualTo(minumumOldDate, highestPrecision))
			{
				...
				else
				{
					...
					throw new Exception("There is no data in the oldTS!");
				}
			}
			...
			while (true)
			{
				...
				if (oldData != null && oldDate.Equals(newDate, highestPrecision))
				{
					...
					else
					{
						...
						if (oldTS.isDataMissing(oldTSValue))
						{
							...
							if ((handleMissingInputHow == TSUtil_ChangeInterval_HandleMissingInputHowType.KEEP_MISSING) || (handleMissingInputHow == TSUtil_ChangeInterval_HandleMissingInputHowType.REPEAT) || (handleMissingInputHow == TSUtil_ChangeInterval_HandleMissingInputHowType.SET_TO_ZERO))
							{
				...
				else if (oldData != null && oldDate.greaterThanOrEqualTo(newDate, highestPrecision))
				{
					...
					else
					{
						...
						{
							switch (handleMissingInputHow.innerEnumValue)
							{
							case RTi.TS.TSUtil_ChangeInterval_HandleMissingInputHowType.InnerEnum.SET_TO_ZERO: // ALERT_INCREMENT
							case RTi.TS.TSUtil_ChangeInterval_HandleMissingInputHowType.InnerEnum.REPEAT: // ALERT_REGULAR
								...
								break;
							case RTi.TS.TSUtil_ChangeInterval_HandleMissingInputHowType.InnerEnum.KEEP_MISSING: // REGULAR
								// Regular data
								switch (newTimeScale.innerEnumValue)
								{
								case TimeScaleType.InnerEnum.ACCM:
									// Accumulated
									newTSValue = oldTSValue / noNewDataPoints;
									break;
								case TimeScaleType.InnerEnum.MEAN:
									// Mean
									newTSValue = oldTSValue;
									break;
								default:
							...
							{
								if ((handleMissingInputHow == TSUtil_ChangeInterval_HandleMissingInputHowType.SET_TO_ZERO) || (handleMissingInputHow == TSUtil_ChangeInterval_HandleMissingInputHowType.REPEAT))
								{
									// ALERT_INCREMENT and
									// ALERT REGULAR
									incrementDate = false;
		...
		/// <param name="newTSi"> Reference to the iterator object for the new time series. </param>
		/// <param name="intervalRelation"> ratio of intervals, negative if newTS interval is longer than the oldTS. </param>
		/// <param name="oldTimeScale"> the time scale of the old time series. </param>
		/// <param name="newTimeScale"> the time scale of the new time series. </param>
		/// <param name="handleMissingInputHow"> Indicates how to treat missing values in the input time series. </param>
		/// <param name="tolerance"> used to detect convergence </param>
		/// <returns> true if successful or false if an error. </returns>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: private boolean changeInterval_toINST(TSIterator oldTSi, TSIterator newTSi, int intervalRelation, RTi.Util.Time.TimeScaleType oldTimeScale, RTi.Util.Time.TimeScaleType newTimeScale, TSUtil_ChangeInterval_HandleMissingInputHowType handleMissingInputHow, double tolerance) throws Exception
		private bool changeInterval_toINST(TSIterator oldTSi, TSIterator newTSi, int intervalRelation, TimeScaleType oldTimeScale, TimeScaleType newTimeScale, TSUtil_ChangeInterval_HandleMissingInputHowType handleMissingInputHow, double tolerance)
		{
			...
			if (intervalRelation > 0)
			{
				...
				double newMissing = newTS.getMissing();
				double oldMissing = oldTS.getMissing();
				bool oldPreviousValueMissing = true;
				bool oldCurrentValueMissing = true;
				bool oldNextValueMissing = true;
				...
				while (!oldTSi.isLastDateProcessed())
				{
					...
					else if (Math.Abs(oldNextValue - oldCurrentValue) < tolerance && Math.Abs(oldPreviousValue - oldCurrentValue) < tolerance)
					{
						while (newTSi.getDate().lessThanOrEqualTo(oldTSNextDate))
						{
							newTS.setDataValue(newTSi.getDate(), oldCurrentValue);
					...
					else if ((oldPreviousValue <= oldCurrentValue && oldCurrentValue <= oldNextValue) || (oldPreviousValue >= oldCurrentValue && oldCurrentValue >= oldNextValue))
					{
						//
						...
						changeIntervalToInstByNWSRFSInterpolation(newTSi, startEndPointDate, startValue, endEndPointDate, endValue, oldPreviousValue, oldCurrentValue, oldNextValue);
					...
					{ // trough
						...
						changeIntervalToInstByInterpolation(newTSi, startEndPointDate, startValue, midpointDate, midpointValue);
						changeIntervalToInstByInterpolation(newTSi, midpointDate, midpointValue, endEndPointDate, endValue);
					}
			...
			{
				warning = "Conversion from \"" + oldTimeScale + "\" to \"" + newTimeScale + "\" is not supported!  To convert to instantaneous, the new interval must be shorter than the old interval.";
				throw new System.ArgumentException(warning);
			}
		}
	...
	/// accumulated) and MEAN (in which case the input values are averaged). </summary>
	/// <param name="allowMissingConsecutive"> only compute the new data value if the number of consecutive missing in the
	/// input interval is <= the specified count. </param>
	private void changeIntervalToDifferentYearType(TS oldTS, TS newTS, TimeScaleType newTimeScale, YearType outputYearType, int allowMissingCount, int allowMissingConsecutive)
	{
//JAVA TO C# CONVERTER WARNING: The .NET Type.FullName property will not always yield results identical to the Java Class.getName method:
		string routine = this.GetType().FullName + ".changeIntervalToDifferentYearType";
		...
		while (yearStart.lessThanOrEqualTo(newTS.getDate2()))
		{
			...
			Message.printStatus(2, routine, "Processing old time series for " + yearStart + " to " + yearEnd + " to create output year " + newYear);
			for (DateTime idate = new DateTime(yearStart); idate.lessThanOrEqualTo(yearEnd); idate.addInterval(intervalBase,intervalMult))
			{
			...
			Message.printStatus(2, routine, "For output year " + newYear + ", sum=" + sum + ", nMissing=" + nMissing + ", nNotMissing=" + nNotMissing + ", nMissingConsecutiveMax=" + nMissingConsecutiveMax);
			bool okToCompute = true;
			if (nNotMissing == 0)
			{
				// No data in input...
				okToCompute = false;
		...
		private bool changeIntervalToInstByInterpolation(TSIterator newTSi, DateTime previousDate, double previousValue, DateTime nextDate, double nextValue)
		{
		...
		private bool changeIntervalToInstByNWSRFSInterpolation(TSIterator newTSi, DateTime previousDate, double previousValue, DateTime nextDate, double nextValue, double Qinpre, double Qtoday, double Qtom)
		{
		...
		/// <param name="oldTSi"> Reference to the iterator object for the old time series. </param>
		/// <param name="newTSi"> Reference to the iterator object for the new time series. </param>
		/// <param name="intervalRelation"> ratio of intervals, negative if newTS interval is longer oldTS. </param>
		/// <param name="oldTimeScale"> the time scale of the old time series: MeasTimeScale.INST if the oldTS is an
		/// instantaneous, MeasTimeScale.ACCM if the oldTS is an
		/// accumulation, MeasTimeScale.MEAN if the oldTS is mean. </param>
		/// <param name="newTimeScale"> the time scale of the new time series, MeasTimeScale.ACCM if the newTS is an
		/// accumulation, MeasTimeScale.MEAN if the newTS is mean. </param>
		/// <param name="handleMissingInputHow"> Indicates how to treat missing values in the input TS </param>
		/// <param name="maxMissingPerInterval"> the maximum number of missing value in the old time series allowed per
		/// new time series interval. New value will be considered missing if this max value is exceeded.
		/// This is only applicable when going from larger intervals to shorter ones. </param>
		/// <param name="allowMissingConsecutive"> if >= 0, only compute the new data value if the number of
		/// consecutive missing in the input interval is <= the specified count. </param>
		/// <param name="outputFillMethod"> this argument is only used when going from larger intervals to smaller.
		...
		/// backward (CARRYBACKWARD) from the last data point of the oldTS time interval. </param>
		/// <param name="handleEndpointsHow"> when changing from small interval to large interval (but only daily or smaller),
		/// indicate how the end-points should be handled. </param>
		/// <returns> true if successful or false if an error. </returns>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: private boolean changeInterval_toMEANorACCM(TSIterator oldTSi, TSIterator newTSi, int intervalRelation, RTi.Util.Time.TimeScaleType oldTimeScale, RTi.Util.Time.TimeScaleType newTimeScale, TSUtil_ChangeInterval_HandleMissingInputHowType handleMissingInputHow, int maxMissingPerInterval, int allowMissingConsecutive, TSUtil_ChangeInterval_OutputFillMethodType outputFillMethod, TSUtil_ChangeInterval_HandleEndpointsHowType handleEndpointsHow) throws Exception
	   private bool changeInterval_toMEANorACCM(TSIterator oldTSi, TSIterator newTSi, int intervalRelation, TimeScaleType oldTimeScale, TimeScaleType newTimeScale, TSUtil_ChangeInterval_HandleMissingInputHowType handleMissingInputHow, int maxMissingPerInterval, int allowMissingConsecutive, TSUtil_ChangeInterval_OutputFillMethodType outputFillMethod, TSUtil_ChangeInterval_HandleEndpointsHowType handleEndpointsHow)
	   {
			...
			if (((oldTimeScale == TimeScaleType.ACCM) && (newTimeScale == TimeScaleType.MEAN)) || ((oldTimeScale == TimeScaleType.MEAN) && (newTimeScale == TimeScaleType.MEAN)) || ((oldTimeScale == TimeScaleType.INST) && (newTimeScale == TimeScaleType.MEAN)) || ((oldTimeScale == TimeScaleType.ACCM) && (newTimeScale == TimeScaleType.ACCM)) || ((oldTimeScale == TimeScaleType.MEAN) && (newTimeScale == TimeScaleType.ACCM)))
			{
				// Recognized combinations
			}
			else
			...
			bool oldTsTimeStampedAtEnd = true;
			...
			// we should not time stamp the new data at the beginning.
			bool newTsTimeStampedAtEnd = true;
			...
			bool previousIsMissing = false; // Is previous value in input interval missing?
			bool okToCompute = true; // is it OK to compute the output interval value?
			bool missingFlag, first_time;
			...
			if (intervalRelation < 0)
			{
				...
				bool averageEndpoints = false;
				if ((handleEndpointsHow != null) && (handleEndpointsHow == TSUtil_ChangeInterval_HandleEndpointsHowType.AVERAGE_ENDPOINTS) && (newTS.getDataIntervalBase() == TimeInterval.DAY || newTS.getDataIntervalBase() == TimeInterval.HOUR || newTS.getDataIntervalBase() == TimeInterval.MINUTE || newTS.getDataIntervalBase() == TimeInterval.SECOND || newTS.getDataIntervalBase() == TimeInterval.HSECOND))
				{
				...
				for (; newTSi.next() != null;)
				{
					...
					if (averageEndpoints && !oldTS.isDataMissing(lastEndpointValue) && !oldTS.isDataMissing(firstEndpointValue))
					{
					...
					if (!okToCompute)
					{
						...
						if (Message.isDebugOn)
						{
							Message.printDebug(dl, routine, "Set new TS " + newDate + " -> " + newMissing + " missingCount=" + missingCount + " dataCount=" + dataCount + ", nMissingConsecutiveMax=" + nMissingConsecutiveMax);
						}
					...
					else
					{
						...
						if (newTimeScale == TimeScaleType.MEAN)
						{
							if (Message.isDebugOn)
							{
								Message.printDebug(dl, routine, "Set new TS " + newDate + " -> " + sum + "/" + dataCount + "=" + sum / dataCount);
							}
							sum /= dataCount;
						}
						else
						{
							...
							if (Message.isDebugOn)
							{
								Message.printDebug(dl, routine, "Set new TS " + newDate + " -> " + sum + " dataCount=" + dataCount);
							}
						}
			...
			else
			{
				...
				if ((oldTimeScale == TimeScaleType.INST) && (newTimeScale == TimeScaleType.MEAN) && (outputFillMethod == TSUtil_ChangeInterval_OutputFillMethodType.INTERPOLATE))
				{
					return changeIntervalFromInstByInterpolation(oldTSi, newTSi, intervalRelation, oldTimeScale, newTimeScale, handleMissingInputHow, newTsTimeStampedAtEnd);
				}
				...
				for (; oldTSi.next() != null;)
				{
					...
					if ((newTimeScale == TimeScaleType.ACCM) || ((oldTimeScale == TimeScaleType.ACCM) && (newTimeScale == TimeScaleType.MEAN)))
					{
		...
		/// <returns> New DayTS that has been converted from an irregular time series.  This is used by some
		/// legacy code that at some point could be converted to use the above methods. </returns>
		/// <param name="oldts"> Irregular time series to convert. </param>
		/// <param name="newmult"> New daily time series. </param>
		public virtual DayTS OLDchangeToDayTS(IrregularTS oldts, int newmult)
		{
			...
			IList<TSData> alldata = oldts.getData();
			if (alldata == null)
			{
				// No data...
				return newts;
			...
			TSData data = null;
			int count = 0, i = 0, iend = alldata.Count;
		...
		/// </summary>
		/// <param name="oldts"> Daily time series to convert. </param>
		/// <param name="newmult"> Interval multiplier for monthly time series. </param>
		/// <param name="bracket"> number of days to search the daily time series from the end of the month, to find </param>
		/// <param name="createData"> if true, do full processing; if false, only create time series metadata
		/// the monthly value (required). </param>
		public virtual MonthTS newEndOfMonthTSFromDayTS(DayTS oldts, int newmult, int? bracket, bool createData)
		{
			string routine = this.GetType().Name + ".newEndOfMonthTSFromDayTS";
			if (bracket == null)
			{
				throw new System.ArgumentException("The bracket must be specified.");
			}

			// Else get the interval count... This should be a generic procedure
			// that can be applied to other conversions also...

			int nearness = bracket.Value;
	...
	/// <summary>
	/// Return the allowMissingConsecutive value. </summary>
	/// <returns> the allowMissingConsecutive value. </returns>
	...
	/// <summary>
	/// Return the allowMissingCount value. </summary>
	/// <returns> the allowMissingCount value. </returns>
	...
	/// <summary>
	/// Return the allowMissingPercent value. </summary>
	/// <returns> the allowMissingPercent value. </returns>
	private double getAllowMissingPercent()
	{
		...
		/// </summary>
		/// <returns> Array of two DateTime containing bounding dates for the new time series. </returns>
		/// <param name="oldts"> Original time series that is being changed to a different interval. </param>
		/// <param name="newbase"> Base data interval for new time series. </param>
		/// <param name="newmult"> Multiplier for new time series. </param>
	...
	/// <summary>
	/// Return the handleEndpointsHow value. </summary>
	/// <returns> the handleEndpointsHow value. </returns>
	...
	/// <summary>
	/// Return the handleMissingInputHow value. </summary>
	/// <returns> the handleMissingInputHow value. </returns>
	...
	/// </summary>
	/// <param name="TS1"> Reference to 1st time series (older). </param>
	/// <param name="TS2"> Reference to 2nd time series (newer). </param>
	/// <returns> -1 * TS2IntSeconds/TS1IntSeconds if the 1st time series interval is shorter than the 2nd,
	...
	/// 1 if the 2nd or both time series interval is IRREGULAR (not supported). </returns>
	...
	/// <summary>
	/// Return the new data type. </summary>
	...
	/// <summary>
	/// Return the new interval. </summary>
	...
	/// <summary>
	/// Return the new time scale. </summary>
	/// <returns> the new time scale. </returns>
	...
	/// <summary>
	/// Return the old time scale. </summary>
	/// <returns> the old time scale. </returns>
	...
	/// <summary>
	/// Return the old time series. </summary>
	/// <returns> the old time series. </returns>
	...
	/// <summary>
	/// Return the output fill method. </summary>
	/// <returns> the output fill method. </returns>
	...
	/// <summary>
	/// Return the statistic being computed. </summary>
	/// <returns> the statistic being computed. </returns>
	...
	/// computed value can be the MAX or MIN or default (new INST). </summary>
	/// <returns> the list of statistic display names as TSStatisticType. </returns>
	...
	/// <summary>
	/// Return the tolerance used with convergence tests. </summary>
	/// <returns> the tolerance used with convergence tests. </returns>
	private double getTolerance()
	{
		...
		/// </summary>
		/// <param name="handleMissingInputHow"> indicate how to handle missing values in input </param>
		/// <param name="replacementValue"> used to replace a data value when handleMissingInputHow = REPEAT </param>
		/// <param name="defaultReplacementValue"> used to replace a data value when handleMissingInputHow is not REPEAT or
		/// SET_TO_ZERO (generally the missing value). </param>
		private static double replaceDataValue(TSUtil_ChangeInterval_HandleMissingInputHowType handleMissingInputHow, double replacementValue, double defaultReplacementValue)
		{
			switch (handleMissingInputHow.innerEnumValue)
			{
			case RTi.TS.TSUtil_ChangeInterval_HandleMissingInputHowType.InnerEnum.SET_TO_ZERO:
				return 0.0;
			case RTi.TS.TSUtil_ChangeInterval_HandleMissingInputHowType.InnerEnum.REPEAT:
	...
	/// <summary>
	/// Set the number of input values allowed to be missing and still compute the result. </summary>
	/// <param name="allowMissingCount"> number of input values allowed to be missing and still compute the result. </param>
	...
	/// <summary>
	/// Set the percentage of input values allowed to be missing and still compute the result. </summary>
	/// <param name="allowMissingPercent"> percent of input values allowed to be missing and still compute the result. </param>
	...
	/// <summary>
	/// Set the handleEndpointsHow property. </summary>
	/// <param name="handleEndpointsHow"> method. </param>
	...
	/// <summary>
	/// Set the handleMissingInputHow property. </summary>
	/// <param name="handleMissingInputHow"> method. </param>
	...
	/// <summary>
	/// Set the new time series data type. </summary>
	/// <param name="newDataType"> new time series data type. </param>
	...
	/// <summary>
	/// Set the new time scale. </summary>
	...
	/// <summary>
	/// Set the new time series data units. </summary>
	/// <param name="newUnits"> new time series units. </param>
	...
	/// <summary>
	/// Set the old time scale. </summary>
	/// <param name="oldTimeScale"> old time scale (from the original time series). </param>
	...
	/// <summary>
	/// Set the old time series. </summary>
	/// <param name="oldTimeSeries"> old time series. </param>
	...
	/// <summary>
	/// Set the output fill method. </summary>
	/// <param name="outputFillMethod"> output fill method. </param>
	...
	/// <param name="statistic"> statistic being computed (e.g., MAX for INST-small to INST-large). </param>
	...
	/// <summary>
	/// Set the tolerance used with convergence tests. </summary>
	/// <param name="tolerance"> tolerance used with convergence tests. </param>
