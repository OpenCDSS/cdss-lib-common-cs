...
namespace RTi.TS
{
	...
	/// </summary>
	private bool __includeMissingValues = true;
	...
	/// </summary>
	private bool __useNullForMissing = false;

	/// <summary>
	/// Constructor. </summary>
	/// <param name="table"> Data table being filled with time series.  Column names need to have been defined but the
	/// table is expected to be empty (no rows). </param>
	/// <param name="tslist"> list of time series being transferred to table. </param>
	/// <param name="dateTimeColumn"> date/time column (0+). </param>
	/// <param name="tableTSIDColumn"> name of column to contain TSID, for one-column output </param>
	/// <param name="tableTSIDFormat"> format of TSID corresponding to tableTSIDColumn </param>
	/// <param name="includeMissingValues"> indicates whether missing values should be output in single-column output (setting to
	/// false can be advantageous when processing sparse time series) </param>
	/// <param name="valueColumns"> column numbers (0+) corresponding to the correct column names for the time series data values;
	/// if a single column is output, then the first array position will be used for each time series </param>
	/// <param name="flagColumns"> column numbers (0+) corresponding to the correct column names for the time series data flags;
	/// if a single column is output, then the first array position will be used for each time series </param>
	/// <param name="outputStart"> first date/time to be transferred (if null, output all) </param>
	/// <param name="outputEnd"> last date/time to be transferred (if null, output all) </param>
	/// <param name="outputWindow"> the window within a year to output (if null, output all) </param>
	/// <param name="useNullForMissing"> if true, use null in the table for missing values.  If false, transfer the
	/// time series missing value indicator (e.g., -999 or NaN).  These values are not universally handled. </param>
	public TSUtil_TimeSeriesToTable(DataTable table, IList<TS> tslist, int dateTimeColumn, int tableTSIDColumn, string tableTSIDFormat, bool includeMissingValues, int[] valueColumns, int[] flagColumns, DateTime outputStart, DateTime outputEnd, DateTimeWindow outputWindow, bool useNullForMissing)
	{
		__table = table;
		__tsList = tslist;
		...
		if ((tslist.Count > 0) && (tableTSIDColumn < 0))
		{
			if (!TSUtil.intervalsMatch(tslist))
			{
				throw new UnequalTimeIntervalException("Time series don't have the same interval - cannot convert to multi-column table.");
			}
			if (TSUtil.areAnyTimeSeriesIrregular(tslist))
			{
				throw new IrregularTimeSeriesNotSupportedException("Irregular time series cannot be converted to a multi-column table.");
			}
		}
	}
	...
	/// </summary>
	public virtual void timeSeriesToTable()
	{
//JAVA TO C# CONVERTER WARNING: The .NET Type.FullName property will not always yield results identical to the Java Class.getName method:
		string routine = this.GetType().FullName + ".timeSeriesToTable";
		...
		if (__tableTSIDColumn >= 0)
		{
			...
			foreach (TS ts in __tsList)
			{
				...
				{
					Message.printWarning(3,routine,"Error initializing iterator for " + ts.getIdentifier().toStringAliasAndTSID() + " (" + e + ")");
					continue;
				}
				TSData tsdata;
				DateTime date;
				...
				while ((tsdata = tsi.next()) != null)
				{
					...
					{
						__problems.Add("Error setting date " + date + " for row [" + setRow + "] uniquetempvar.");
					}
					// Set the TSID
					if (string.ReferenceEquals(__tableTSIDFormat, null))
					{
					...
					{
						__problems.Add("Error setting TSID " + tsid + " for row [" + setRow + "] uniquetempvar.");
					}
					...
					{
						__problems.Add("Error setting data value " + value + " at " + date + " [" + setRow + "][" + valueColumn + "] uniquetempvar.");
					}
					if (flagColumn >= 0)
					{
						...
						{
							__problems.Add("Error setting data flag " + flag + " at " + date + " [" + setRow + "][" + flagColumn + "] uniquetempvar.");
						}
					}
				}
			}
		...
		else
		{
			...
			if ((__outputStart == null) || (__outputEnd == null))
			{
				...
				catch (Exception)
				{
					...
					outputStart = __tsList[0].getDate1();
					outputEnd = __tsList[0].getDate2();
				}
			}
			else
			{
			...
			int intervalBase = __tsList[0].getDataIntervalBase();
			int intervalMult = __tsList[0].getDataIntervalMult();
			...
			if (outOfOrderCount > 0)
			{
				...
				// Cannot continue
				__problems.Add("Existing date/times in table are out of order.  Cannot add time series to table.  " + "If sorting the table, do it after all time series have been added.");
				return;
			}
				...
				else
				{
					if (dtFirst.greaterThan(outputStart))
					{
						...
						for (dt = new DateTime(outputStart); dt.lessThan(dtFirst); dt.addInterval(intervalBase,intervalMult))
						{
							...
							{
								__problems.Add("Unexpected error initializing blank record for date " + dt + " uniquetempvar.");
			...
			Message.printStatus(2, routine, "Adding time series for " + outputStart + " to " + outputEnd + " starting at table row [" + setRowFirst + "]");
			for (date = new DateTime(outputStart); date.lessThanOrEqualTo(outputEnd); date.addInterval(intervalBase,intervalMult))
			{
				if ((__outputWindow != null) && !__outputWindow.isDateTimeInWindow(date))
				{
					// Don't add the row...
				...
				if (o == null)
				{
					...
					{
						__problems.Add("Error setting date " + date + " for col [" + __dateTimeColumn + "] row [" + setRow + "] uniquetempvar.");
						Message.printWarning(3,routine,e);
					}
				}
				else
				{
					...
					{
						__problems.Add("Existing date/time in table row [" + setRow + "] (" + o + ") does not match time series (" + date + ", output start=" + outputStart + ", output end=" + outputEnd + ") - table date/times are misaligned so cannot append time series value to table.");
						break;
					}
				}
				...
				for (its = 0; its < tsListSize; its++)
				{
					...
					else
					{
						...
						{
							__problems.Add("Error setting data value " + value + " at " + date + " [" + setRow + "][" + setColumn + "] uniquetempvar.");
						}
					}
					// Set the data flag...
					flagColumn = -1;
	...
	/// <summary>
	/// Return the output window. </summary>
	/// <returns> the output window. </returns>
	public virtual DateTimeWindow getOutputWindow()
	{
