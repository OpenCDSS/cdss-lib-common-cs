...
namespace RTi.TS
{
	...
	//private int	_query_field = QUERY_VALUE;
					...
					// value.
	protected internal IList<string> _data_coverage_report_Vector = null;
	protected internal DateTime _data_coverage_report_date1 = null;
	protected internal DateTime _data_coverage_report_date2 = null;
	protected internal PropList _data_coverage_report_props = null;
	...
	/// Initialize the dates to null and the limits to zeros. </summary>
	/// <param name="ts"> Time series to analyze. </param>
	/// <param name="query_string"> Query string specifying analysis. </param>
	...
	/// <summary>
	/// Append a record to the overall data coverage summary report. </summary>
	/// <param name="monthts"> Statistics month time series. </param>
	public virtual void appendToDataCoverageSummaryReport(MonthTS monthts)
	{
		string delim = "|";
		string string100 = "##";
		string string75 = "**";
		...
		_data_coverage_report_Vector.Add(StringUtil.formatString(monthts.getLocation(),"%-20.20s") + delim + StringUtil.formatString(monthts.getDescription(),"%-40.40s") + delim + data_string.ToString());
	}

	/// <summary>
	/// Create a monthly time series that contains statistics in each data value (e.g.,
	/// percent missing, percent not missing).  The full period is used. </summary>
	/// <returns> The statistics time series. </returns>
	/// <param name="ts"> Time series to analyze (can be any interval). </param>
	/// <param name="props"> Properties to consider when analyzing.  Currently not used (the
	/// percentage of data available is always computed). </param>
	/// <exception cref="TSException"> if there is an error analyzing the time series. </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static MonthTS createStatisticMonthTS(TS ts, RTi.Util.IO.PropList props) throws TSException
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static MonthTS createStatisticsMonthTS(TS ts, RTi.Util.IO.PropList props) throws TSException
	...
	/// data coverage report but may want to change. </summary>
	/// <returns> The statistics time series. </returns>
	/// <param name="ts"> Time series to analyze (can be any interval). </param>
	/// <param name="start_date"> Starting date for analysis. </param>
	/// <param name="end_date"> Ending date for analysis. </param>
	/// <param name="props"> Properties to consider when analyzing.  Currently not used (the
	/// percentage of data available is always computed). </param>
	/// <exception cref="TSException"> if there is an error analyzing the time series. </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public MonthTS createStatisticMonthTS(TS ts, RTi.Util.Time.DateTime start_date, RTi.Util.Time.DateTime end_date, RTi.Util.IO.PropList props) throws TSException
	public virtual MonthTS createStatisticMonthTS(TS ts, DateTime start_date, DateTime end_date, PropList props)
	{
	...
	*/
		...
		else
		{
			...
			for (; date.lessThanOrEqualTo(end); date.addInterval(interval_base, interval_mult))
			{
				...
				if (((month != 0) && (month != lastmonth)) || (year != lastyear))
				{
					...
					{
						month_value = (double)data_count * 100.0 / TSUtil.numIntervalsInMonth(lastmonth, lastyear, interval_base, interval_mult);
					}
					if (Message.isDebugOn)
					{
	...
	/// <exception cref="TSException"> if there is an error analyzing the time series. </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static MonthTS createStatisticsMonthTS(TS ts, RTi.Util.Time.DateTime start_date, RTi.Util.Time.DateTime end_date, RTi.Util.IO.PropList props) throws TSException
	...
	/// <param name="AnalysisStart_DateTime"> Starting date/time for analysis, in precision of the
	/// original data. </param>
	...
	/// <param name="OutputEnd_DateTime"> Output end date/time.
	/// If null, the entire period will be analyzed. </param>
	/// <param name="props"> Properties to consider when analyzing.  The following properties are recognized:
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public TS createStatisticTimeSeries(Object tsobject, RTi.Util.Time.DateTime AnalysisStart_DateTime, RTi.Util.Time.DateTime AnalysisEnd_DateTime, RTi.Util.Time.DateTime OutputStart_DateTime, RTi.Util.Time.DateTime OutputEnd_DateTime, RTi.Util.IO.PropList props) throws Exception
	public virtual TS createStatisticTimeSeries(object tsobject, DateTime AnalysisStart_DateTime, DateTime AnalysisEnd_DateTime, DateTime OutputStart_DateTime, DateTime OutputEnd_DateTime, PropList props)
	{
		...
		else if (tsobject is TSEnsemble)
		{
			...
			if (Message.isDebugOn)
			{
				Message.printDebug(dl, routine, "Trying to create statistic time series using ensemble \"" + tsensemble.getEnsembleID() + "\" for input.");
			}
		}
		...
		else
		{
			...
			createStatisticTimeSeries_ComputeStatisticFromEnsemble(tsensemble, output_ts, analysis_start, analysis_end, statisticType);
		}
	...
	/// February 29 data to be computed as a statistic, although it may not be used in the final output. </summary>
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: private TS createStatisticTimeSeries_ComputeStatistic(TS ts, RTi.Util.Time.DateTime analysis_start, RTi.Util.Time.DateTime analysis_end, TSStatisticType statisticType) throws Exception
	private TS createStatisticTimeSeries_ComputeStatistic(TS ts, DateTime analysis_start, DateTime analysis_end, TSStatisticType statisticType)
	{
	...
	/// series having the interval of the input data. </summary>
	/// <param name="tsensemble"> Time series ensemble to be analyzed. </param>
	/// <param name="stat_ts"> Statistic (output) time series to be created. </param>
	/// <param name="analysis_start"> Start of period to analyze. </param>
	/// <param name="analysis_end"> End of period to analyze. </param>
	/// <param name="statisticType"> Statistic to compute. </param>
	/// <returns> The statistics in a single-year time series. </returns>
	/// @deprecated use the TSUtil_NewTimeSeriesFromEnsemble class. 
	private TS createStatisticTimeSeries_ComputeStatisticFromEnsemble(TSEnsemble tsensemble, TS stat_ts, DateTime analysis_start, DateTime analysis_end, TSStatisticType statisticType)
	{ // Initialize statistic integer to speed processing

		if ((statisticType != TSStatisticType.MEAN) && (statisticType != TSStatisticType.MEDIAN))
		{
		...
		{
			throw new Exception("Error creating irregular time series iterator (" + e + ").");
		}
	...
	/// @deprecated use the TSUtil_NewStatisticTimeSeries class. 
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: private void createStatisticTimeSeries_FillOuput(TS stat_ts, TS output_ts, RTi.Util.Time.DateTime output_start, RTi.Util.Time.DateTime output_end) throws Exception
	private void createStatisticTimeSeries_FillOuput(TS stat_ts, TS output_ts, DateTime output_start, DateTime output_end)
	{
	...
	/// <returns> The statistics time series. </returns>
	/// <param name="ts"> Time series to analyze (can be any interval). </param>
	/// <param name="AnalysisStart_DateTime"> Starting date for analysis, in precision of the
	/// original data.
	/// The year time series will have a start date with the year matching this date.
	/// If null, the entire period will be analyzed. </param>
	/// <param name="AnalysisEnd_DateTime"> Ending date for analysis, in precision of the
	/// original data.
	/// The year time series will have an end date with the year matching this date.
	/// If null, the entire period will be analyzed. </param>
	/// <param name="props"> Properties to consider when analyzing (see overloaded version for details). </param>
	/// @deprecated use the TSUtil_NewStatisticYearTS class. 
	/// <exception cref="TSException"> if there is an error analyzing the time series. </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public YearTS createStatisticYearTS(TS ts, RTi.Util.Time.DateTime AnalysisStart_DateTime, RTi.Util.Time.DateTime AnalysisEnd_DateTime, RTi.Util.IO.PropList props) throws TSException
	public virtual YearTS createStatisticYearTS(TS ts, DateTime AnalysisStart_DateTime, DateTime AnalysisEnd_DateTime, PropList props)
	{
	...
	/// The year time series will have an end date with the year matching this date.
	/// If null, the entire period will be analyzed. </param>
	/// <param name="AnalysisWindowStart_DateTime"> Starting date/time for analysis within the year, in precision of the
	/// original data.  If null, the entire year of data will be analyzed. </param>
	/// <param name="AnalysisWindowEnd_DateTime"> Ending date for analysis within the year, in precision of the
	/// original data.  If null, the entire year of data will be analyzed. </param>
	/// <param name="props"> Properties to consider when analyzing.  The following properties
	/// are recognized:
	/// <table width=100% cellpadding=10 cellspacing=0 border=2>
	/// <tr>
	/// <td><b>Property</b></td>	<td><b>Description</b></td>	<td><b>Default</b></td>
	...
	/// <exception cref="TSException"> if there is an error analyzing the time series. </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public YearTS createStatisticYearTS(TS ts, RTi.Util.Time.DateTime AnalysisStart_DateTime, RTi.Util.Time.DateTime AnalysisEnd_DateTime, RTi.Util.Time.DateTime AnalysisWindowStart_DateTime, RTi.Util.Time.DateTime AnalysisWindowEnd_DateTime, RTi.Util.IO.PropList props) throws TSException
	public virtual YearTS createStatisticYearTS(TS ts, DateTime AnalysisStart_DateTime, DateTime AnalysisEnd_DateTime, DateTime AnalysisWindowStart_DateTime, DateTime AnalysisWindowEnd_DateTime, PropList props)
	{
		...
		if (Message.isDebugOn)
		{
			Message.printDebug(dl, routine, "Trying to create statistic year TS for \"" + ts.getIdentifierString() + "\"");
		}
		if (ts == null)
		{
			// Nothing to do...
		...
		if ((statisticType == TSStatisticType.GE_COUNT) || (statisticType == TSStatisticType.GT_COUNT) || (statisticType == TSStatisticType.LE_COUNT) || (statisticType == TSStatisticType.LT_COUNT) || (statisticType == TSStatisticType.DAY_OF_FIRST_GE) || (statisticType == TSStatisticType.DAY_OF_FIRST_GT) || (statisticType == TSStatisticType.DAY_OF_FIRST_LE) || (statisticType == TSStatisticType.DAY_OF_FIRST_LT) || (statisticType == TSStatisticType.DAY_OF_LAST_GE) || (statisticType == TSStatisticType.DAY_OF_LAST_GT) || (statisticType == TSStatisticType.DAY_OF_LAST_LE) || (statisticType == TSStatisticType.DAY_OF_LAST_LT) || (statisticType == TSStatisticType.DAY_OF_MAX) || (statisticType == TSStatisticType.DAY_OF_MIN) || (statisticType == TSStatisticType.MAX) || (statisticType == TSStatisticType.MEAN) || (statisticType == TSStatisticType.MEDIAN) || (statisticType == TSStatisticType.MIN) || (statisticType == TSStatisticType.TOTAL))
		{
			processStatistic(ts, yearts, props, start, end, AnalysisWindowStart_DateTime, AnalysisWindowEnd_DateTime);
		}
		else
		{
...
//		base.finalize();
	...
	/// <summary>
	/// Return the contents of the data coverage report as a Vector of String. </summary>
	/// <returns> the results of the data coverage report.  This returns the report contents (not a copy). </returns>
	public virtual IList<string> getDataCoverageReport()
	{
	...
	{ //_query_string = query_string;
		_ts = ts;
		if ((_ts != null) && (!string.ReferenceEquals(query_string, null)))
		{
			parseQuery();
		}
	}
	...
	/// <summary>
	/// Parse the query string.  For now, always print results using messages. </summary>
	/// <param name="out"> PrintWriter to write results. </param>
	/// <param name="query_string"> Query string specifying analysis. </param>
	public virtual void parseQuery(PrintWriter @out, string query_string)
	{
		if (!string.ReferenceEquals(query_string, null))
		{
			//_query_string = query_string;
			parseQuery();
		}
	...
	/// </summary>
	public virtual void parseQuery()
	{
	/*
	...
	*/
	...
	/// member. </summary>
	/// <param name="tokens"> The Vector of token strings. </param>
	/// <param name="itoken"> The position in the Vector to start checking for the query field. </param>
	/// <param name="isfunction"> true if the tokens are part of a function (in which case the
	/// first token should be "(") or false if the tokens are the main part of the
	/// select (in which case the first token should be "value"). </param>
	/// <returns> The number of tokens to skip in the calling routine (-1 if there is
	/// a problem). </returns>
	private int parseQueryField(List<object> tokens, int itoken, bool isfunction)
	{
		...
		for (; itoken < size; itoken++)
		{
			...
			if (itoken == itoken0)
			{
				if (isfunction)
				{
					...
					if (!token.Equals("("))
					{
						Message.printWarning(2, routine, "\"(\" expected to start function " + "query field.");
						return -1;
					}
				}
				else
				{ // First token should be "value"...
					if (!token.Equals("value", StringComparison.OrdinalIgnoreCase))
					{
						Message.printWarning(2, routine, "\"value\" expected to start " + "query field.");
						return -1;
					}
				}
			}
			...
			else if (token.Equals("month", StringComparison.OrdinalIgnoreCase))
			{
				month_found = true;
			}
			if (isfunction)
			...
			{ // Break when we encounter something that is not
				// expected...
				if (!token.Equals("value", StringComparison.OrdinalIgnoreCase) && !token.Equals("by", StringComparison.OrdinalIgnoreCase) && !token.Equals("month", StringComparison.OrdinalIgnoreCase))
				{
					break;
				}
			}
	...
	*/ private void processStatistic(TS ts, YearTS yearts, PropList props, DateTime start, DateTime end, DateTime AnalysisWindowStart_DateTime, DateTime AnalysisWindowEnd_DateTime)
	{
		string routine = "TSAnalyst.processStatistic", message;
		DateTime year_date = new DateTime(end, DateTime.PRECISION_YEAR);
		double year_value = 0.0, value = 0.0, extreme_value = 0.0; // Extreme value in a year.
		bool iterate_forward = true; // Direction for iteration
		...
		bool is_regular = TimeInterval.isRegularInterval(interval_base);
		...
		if ((!string.ReferenceEquals(TestValue, null)) && (TestValue.Length > 0))
		{
		...
		int Test_int = -1;
		bool Statistic_isCount = false; // Indicates if the statistic is
		...
		if ((!string.ReferenceEquals(AllowMissingCount, null)) && !AllowMissingCount.Equals(""))
		{
		...
		if ((!string.ReferenceEquals(SearchStart, null)) && (SearchStart.Length > 0))
		{
			try
			{
		...
		DateTime date = null, date_search = null; // DateTime corresponding to SearchStart for a particular year
		if (iterate_forward)
		{
		...
		bool need_to_analyze = true; // Need to analyze value for current year
		...
		bool end_of_data = false; // Use to indicate end of data because
		...
		while (true)
		{
			...
			if ((year != year_prev) || (end_of_data) || first_interval)
			{ // First interval so initialize (but do not save).
				// New year or last interval so save the results from the previous interval analysis...
				if (!first_interval)
				{
					year_date.setYear(year_prev);
					if (Statistic_isCount && yearts.isDataMissing(year_value) && (missing_count <= AllowMissingCount_int))
					{
					...
					if (!yearts.isDataMissing(year_value))
					{
						...
						{
							if ((missing_count <= AllowMissingCount_int) && (nonmissing_count > 0) && !yearts.isDataMissing(year_value))
							{
						...
						else
						{
							...
							if (Message.isDebugOn)
							{
								Message.printDebug(dl, routine, "Setting value for " + year_prev + " to " + year_value);
							}
						}
					}
				}
				...
				if ((SearchStart_DateTime != null) || (AnalysisWindowStart_DateTime != null))
				{
					...
					if (AnalysisWindowStart_DateTime != null)
					{
						...
						if (Message.isDebugOn)
						{
							Message.printDebug(dl, routine, "Will start processing in year on AnalysisWindowStart: " + date_search);
							Message.printDebug(dl, routine, "Will end processing in year on AnalysisWindowEnd: " + AnalysisWindowEndInYear_DateTime);
						}
					}
						...
						if (is_regular)
						{
							...
							if (iterate_forward)
							{
								...
								else
								{
									...
									gap = TimeUtil.getNumIntervals(TimeUtil.min(date_search,start), TimeUtil.max(date_search, start), interval_base, interval_mult);
								}
							}
							else
							{
								...
								{
									// Not enough data before search start...
									gap = TimeUtil.getNumIntervals(TimeUtil.min(end, date_search), TimeUtil.max(end, date_search), interval_base, interval_mult);
								}
							}
							if (gap >= 0)
							{
								if (Message.isDebugOn)
								{
									Message.printDebug(dl, routine, "Found " + gap + " missing values between search start and data.");
								}
								missing_count += gap;
							}
							else
			...
			if (need_to_analyze && !end_of_data)
			{
				...
				if (Message.isDebugOn)
				{
					Message.printDebug(dl, routine, "Processing " + date + " value=" + value + " year value (before this value)=" + year_value);
				}
				...
				if (missing_count > AllowMissingCount_int)
				{
					...
					need_to_analyze = false;
					Message.printDebug(dl, "", "Will not analyze year because more than " + AllowMissingCount_int + " missing values were found (" + missing_count + ").");
					continue;
				}
				...
				if (ts.isDataMissing(value))
				{
					...
					if (missing_count > AllowMissingCount_int)
					{
						// Have too much missing data to generate the statistic...
						need_to_analyze = false;
						Message.printDebug(dl, "", "Will not analyze year because more than " + AllowMissingCount_int + " missing values were found.");
					}
				}
				else
				{
		...
		{
			yearts.addToGenesis("Created " + statisticType + " statistic time series (TestValue=" + TestValue + ",SearchStart=" + SearchStart + ",AllowMissingCount=" + AllowMissingCount + ") from input: " + ts.getAlias());
		}
		else
		{
			yearts.addToGenesis("Created " + statisticType + " statistic time series (TestValue=" + TestValue + ",SearchStart=" + SearchStart + ",AllowMissingCount=" + AllowMissingCount + ") from input: " + ts.getIdentifier());
		}
	}
	...
	/// </summary>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: private void processStatisticDataCoveragePercent(TS ts, YearTS yearts, RTi.Util.IO.PropList props, RTi.Util.Time.DateTime start, RTi.Util.Time.DateTime end) throws Exception
	private void processStatisticDataCoveragePercent(TS ts, YearTS yearts, PropList props, DateTime start, DateTime end)
	{
		...
		bool set_missing_if_missing = false;
		string SetMissingIfMissing = props.getValue("SetMissingIfMissing");
		if ((!string.ReferenceEquals(SetMissingIfMissing, null)) && SetMissingIfMissing.Equals("true", StringComparison.OrdinalIgnoreCase))
		{
	...
	public static void runShell(TS ts)
	{
		...
		while (true)
		{
			...
			{ // Clear the bytes first...
				...
				if (console_in.Read(bytes, 0, bytes.Length) == - 1)
				{
					// End of file;
					continue;
				}
				@string = StringHelper.NewString(bytes);
			}
			catch (IOException)
			{
				continue;
			...
			{
				Message.printStatus(1, routine, "Close \"" + output_file + "\".  Output now printed to console.");
				@out.flush();
				@out.close();
			...
			else if (@string.regionMatches(true,0,"open",0,4))
			{
				...
				try
				{
					...
					@out = new PrintWriter(new StreamWriter(output_file));
			...
			else if (@string.regionMatches(true,0,"summary",0, 7))
			{
				...
				try
				{
					IList<string> v = TSUtil.formatOutput(ts, props);
					if (v == null)
					{
						continue;
					}
					...
					for (i = 0; i < size; i++)
					{
						Message.printStatus(2, routine, (string)v[i]);
						@out.println(v[i]);
					}
				}
				catch (Exception)
	...
	/// generalize to other data statistics. </summary>
	/// <param name="start"> Start date for output. </param>
	/// <param name="end"> End date for output. </param>
	/// <param name="yearType"> the output year type for the report. </param>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public void startDataCoverageReport(RTi.Util.Time.DateTime start, RTi.Util.Time.DateTime end, RTi.Util.Time.YearType yearType) throws TSException
	public virtual void startDataCoverageReport(DateTime start, DateTime end, YearType yearType)
	{
		string delim = "|";
		if (start == null)
		{
		...
		_data_coverage_report_Vector = new List<string>(100);
		...
		_data_coverage_report_Vector.Add(StringUtil.formatString("Station","%-20.20s") + delim + StringUtil.formatString("Name","%-40.40s") + delim + data_string.ToString());
	}

	/// <summary>
	/// Return a string representation of the analyst.  Currently returns an empty string. </summary>
	/// <returns> A verbose string representation of the results. </returns>
