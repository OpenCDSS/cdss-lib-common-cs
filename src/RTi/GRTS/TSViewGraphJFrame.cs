...
//ORIGINAL LINE: @SuppressWarnings("serial") public class TSViewGraphJFrame extends javax.swing.JFrame implements java.awt.event.ActionListener, java.awt.event.ItemListener, TSViewListener, java.awt.event.WindowListener
	public class TSViewGraphJFrame : JFrame, ActionListener, ItemListener, TSViewListener, WindowListener
	{
	...
	protected internal TSGraphJComponent _ts_graph = null;
	protected internal TSGraphJComponent _ref_graph = null;
	...
	/// <summary>
	/// Controls whether in Edit mode </summary>
	private SimpleJToggleButton _edit_JToogleButton;
	/// <summary>
	/// Controls whether in Zoom mode (default) </summary>
	private SimpleJToggleButton _zoom_JToogleButton;
	/// <summary>
	/// Use interpolation to adjust data points </summary>
	private SimpleJButton _fillInterpolation_JButton = null;
	/// <summary>
	/// Encapsulates (most) editing functionality </summary>
	private TSGraphEditor _tsGraphEditor;
	/// <summary>
	/// Controls whether auto-connect is used during editing </summary>
	private JCheckBox _autoConnect_JCheckBox;

	/// <summary>
	/// Construct a TSViewGraphJFrame. </summary>
	/// <param name="tsview_gui"> Parent TSViewJFrame. </param>
	/// <param name="tslist"> List of time series to view. </param>
	/// <param name="props"> Properties to customize the data display.  The properties are
	/// defined in the TSViewJFrame constructor documentation. </param>
	/// <exception cref="Exception"> if there is an error displaying the view. </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public TSViewGraphJFrame(TSViewJFrame tsview_gui, java.util.List<RTi.TS.TS> tslist, RTi.Util.IO.PropList props) throws Exception
	public TSViewGraphJFrame(TSViewJFrame tsview_gui, IList<TS> tslist, PropList props) : base("Time Series - Graph View")
	{
	...
	/// Construct a TSViewGraphJFrame. </summary>
	/// <param name="tsview_gui"> Parent TSViewJFrame. </param>
	/// <param name="tsproduct"> TSProduct defining the graph(s). </param>
	/// <exception cref="Exception"> if there is an error displaying the view. </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public TSViewGraphJFrame(TSViewJFrame tsview_gui, TSProduct tsproduct) throws Exception
	public TSViewGraphJFrame(TSViewJFrame tsview_gui, TSProduct tsproduct) : base("Time Series - Graph View")
	{
		...
		{
			initialize(tsview_gui, new List<TS>(), props);
		}
		else
		{
	...
	/// <param name="event"> ActionEvent. </param>
	...
	*/
		...
		else if (o == __zoom_out_JButton)
		{
			...
			IList<TSGraph> tsgraphs = _ref_graph.getTSGraphs();
			// Reference graph should be first and only in list
			if (tsgraphs.Count >= 1)
			{
				TSGraph graph = tsgraphs[0];
	...
	/// transparent glass pane for the mouse tracker. </summary>
	...
	private void addLayeredPaneComponent(JLayeredPane container, Component component, int? z)
	{
	...
	/// <summary>
	/// Return the main TSGraphJComponent. </summary>
	/// <returns> the main TSGraphJComponent. </returns>
	public virtual TSGraphJComponent getMainJComponent()
	{
		return _ts_graph;
	}
	...
	/// This method handles whether properties come from the TSProduct or other property list. </summary>
	/// <param name="propname"> property name (e.g., "EnableReferenceGraph"). </param>
	...
	/// <summary>
	/// Returns the reference graph. </summary>
	/// <returns> the reference graph. </returns>
	protected internal virtual TSGraphJComponent getReferenceGraph()
	{
		return _ref_graph;
	}
	...
	/// </summary>
	public TSViewJFrame getTSViewJFrame()
	{
		return __tsview_JFrame;
	}

	/// <summary>
	/// Initialize data and open the GUI. </summary>
	/// <param name="tsview_gui"> Parent TSViewJFrame. </param>
	/// <param name="tslist"> List of time series to view. </param>
	/// <param name="props"> Properties to customize the data display.  The properties are
	/// defined in the TSViewJFrame constructor documentation. </param>
	...
	public virtual void itemStateChanged(ItemEvent evt)
	{
		...
		if ((o == __trackerModeJComboBox) && (evt.getStateChange() == ItemEvent.SELECTED))
		{
			...
			this.tsgraphGlassPane.setMouseTrackerType(TSGraphMouseTrackerType.valueOfIgnoreCase(__trackerModeJComboBox.getSelected()));
		}
	}
	...
	/// <param name="mode"> Indicates whether the GUI should be visible at creation, typically true
	/// unless processing a graph in batch mode, in which case an image file is usually the output. </param>
	private void openGUI(bool mode)
	{
		...
		for (int i = 0; i < size; i++)
		{
			...
			{
				nmonths = limits.getNonMissingDataDate2().getAbsoluteMonth() - limits.getNonMissingDataDate1().getAbsoluteMonth();
			}
			else
			{
		...
		bool doFrameGlassPane = true; // Default is to use glass pane on JFrame, but layout 2 uses JLayeredPane instead
		...
		else if (layoutType == 1)
		{
			...
			{
				Message.printStatus(2,routine,"TSGraphJComponent size before laying out in window, width=" + _ts_graph.getWidth() + ", height=" + _ts_graph.getHeight());
			}
			...
			JGUIUtil.addComponent(panelForTSGraphJComponent, _ts_graph, 0, 0, 1, 1, 1.0, 1.0, insetsNone, GridBagConstraints.BOTH, GridBagConstraints.CENTER);
			if (Message.isDebugOn)
			{
				Message.printStatus(2,routine,"TSGraphJComponent size after adding to panel, width=" + _ts_graph.getWidth() + ", height=" + _ts_graph.getHeight());
			}
			...
			JGUIUtil.addComponent(panelForJLayeredPane, layeredPane, 0, 0, 1, 1, 1.0, 1.0, insetsNone, GridBagConstraints.BOTH, GridBagConstraints.CENTER);
			if (Message.isDebugOn)
			{
				Message.printStatus(2,routine,"TSGraphJComponent size after JLayeredPane added to panel, width=" + _ts_graph.getWidth() + ", height=" + _ts_graph.getHeight());
			}
			// This should automatically resize to fill
			contentPane.add(panelForJLayeredPane, BorderLayout.CENTER);
			if (Message.isDebugOn)
			{
				Message.printStatus(2,routine,"TSGraphJComponent size after JLayeredPane added to contentPane, width=" + _ts_graph.getWidth() + ", height=" + _ts_graph.getHeight());
			}
		...
		else if (layoutType == 2)
		{
			...
			panelForTSGraphJComponent.setLayout(gbl);
			// Fill the whole panel and allow resize in both directions
			JGUIUtil.addComponent(panelForTSGraphJComponent, _ts_graph, 0, 0, 1, 1, 1.0, 1.0, insetsTLBR, GridBagConstraints.BOTH, GridBagConstraints.SOUTH);
			layeredPane = new JLayeredPane();
			layeredPane.setPreferredSize(new Dimension(_ts_graph.getWidth(),_ts_graph.getHeight()));
			layeredPane.add(panelForTSGraphJComponent,new int?(JLayeredPane.DEFAULT_LAYER));
			//_tsGraphGlassPane = new TSGraphJComponentGlassPane(_ts_graph, _ts_graph);
			//layeredPane.add(_tsGraphGlassPane,2);
			// This should automatically resize to fill
		...
		else if (layoutType == 3)
		{
			...
			addLayeredPaneComponent(layeredPane, _ts_graph, new int?(JLayeredPane.DEFAULT_LAYER));
			...
			addLayeredPaneComponent(layeredPane, this.tsgraphGlassPane, new int?(JLayeredPane.DEFAULT_LAYER));
		...
		if (!needToClose() && ((string.ReferenceEquals(prop_value, null)) || !prop_value.Equals("false", StringComparison.OrdinalIgnoreCase)))
		{
			...
			y++;
			JGUIUtil.addComponent(reference_JPanel, new JLabel("Visible Period (white):"), 0, 0, 1, 1, 0.0, 0.0, insetsTLBR, GridBagConstraints.NONE, GridBagConstraints.NORTH);
			JGUIUtil.addComponent(reference_JPanel, _ref_graph, 1, 0, 1, 1, 1.0, 0.0, insetsTLBR, GridBagConstraints.HORIZONTAL, GridBagConstraints.NORTH);
			JGUIUtil.addComponent(bottom_JPanel, reference_JPanel, 0, 1, 1, 1, 1.0, 0.0, insetsTLBR, GridBagConstraints.HORIZONTAL, GridBagConstraints.CENTER);
		...
		if ((string.ReferenceEquals(prop_value, null)) || ((!string.ReferenceEquals(prop_value, null)) && !prop_value.Equals("false", StringComparison.OrdinalIgnoreCase)))
		{
			// Add the button...
	...
	*/
		...
		if (TSUtil.areAnyTimeSeriesEditable(__tslist))
		{
			...
			if (!string.ReferenceEquals(DefaultSaveFile, null))
			{
		...
		++y;
		JGUIUtil.addComponent(bottom_JPanel, button_top_JPanel, 0, 2, 1, 1, 0.0, 0.0, insetsTLBR, GridBagConstraints.NONE, GridBagConstraints.CENTER);

		++y;
		JGUIUtil.addComponent(bottom_JPanel, button_bottom_JPanel, 0, 3, 1, 1, 1.0, 0.0, insetsTLBR, GridBagConstraints.NONE, GridBagConstraints.SOUTH);
		...
		JGUIUtil.addComponent(status_JPanel, __message_JTextField, 0, 0, 1, 1, .3, 1.0, insetsTLBR, GridBagConstraints.HORIZONTAL, GridBagConstraints.WEST);
		__tracker_JTextField = new JTextField(30);
		__tracker_JTextField.setEditable(false);
		JGUIUtil.addComponent(status_JPanel, __tracker_JTextField, 1, 0, 1, 1, .7, 1.0, insetsTLBR, GridBagConstraints.HORIZONTAL, GridBagConstraints.WEST);
		++y;
		JGUIUtil.addComponent(bottom_JPanel, status_JPanel, 0, 4, 1, 1, 1.0, 0.0, insetsTLBR, GridBagConstraints.HORIZONTAL, GridBagConstraints.SOUTH);
		...
		if (Message.isDebugOn)
		{
			Message.printStatus(2,routine,"TSGraphJComponent size before calling pack(), width=" + _ts_graph.getWidth() + ", height=" + _ts_graph.getHeight());
		}
		pack();
		if (Message.isDebugOn)
		{
			Message.printStatus(2,routine,"TSGraphJComponent size after calling pack(), width=" + _ts_graph.getWidth() + ", height=" + _ts_graph.getHeight());
		}
		//if ( includeFullCode ) {
	...
	private void saveEdits()
	{
		...
		IList<TS> editable_tslist = new List<TS>();
		...
		try
		{
			...
			Message.printStatus(2, routine, "Saving " + editable_tslist.Count + " editable time series to DateValue file \"" + DefaultSaveFile_full);
			DateValueTS.writeTimeSeriesList(editable_tslist, DefaultSaveFile_full);
		}
		catch (Exception e)
		{
			Message.printWarning(1, routine, "Error saving " + editable_tslist.Count + " editable time series to \"" + DefaultSaveFile_full + "\".");
			Message.printWarning(2, routine, e);
		}
	}
	...
	private void saveGraph()
	{
		...
		IList<TSProductDMI> tsProductDMIs = __tsview_JFrame.getTSProductDMIs();
		int size = tsProductDMIs.Count;
		...
		else
		{
			...
			if (dmiff != null)
			{
				for (int i = 0; i < size; i++)
				{
					...
					{
						TSProductDMI dmi = tsProductDMIs[i];
	...
	/// just calls the TSGraphJComponent version. </summary>
	/// <param name="mode"> Interaction mode (see TSGraphJComponent). </param>
	...
	/// the user wants to quit without saving. </summary>
	/// <returns> whether the GUI is allowed to close (true) or not. </returns>
	...
	/// <summary>
	/// Handle mouse motion events and display the mouse coordinates in the status TextField. </summary>
	/// <param name="devpt"> Mouse point in GR device coordinates. </param>
	/// <param name="datapt"> Mouse point in data coordinates. </param>
	...
	public virtual void tsViewSelect(TSGraph g, GRShape devlim, GRShape datalim, IList<object> selected)
	{
		checkGUIState();
	}

	public virtual void tsViewZoom(TSGraph g, GRShape devlim, GRShape datalim)
	{
		Message.printStatus(2, "tsViewZoom", "zooming to provided limits" + datalim);
		if (datalim is GRLimits)
		{
	...
	/// <summary>
	/// Responds to the window Closing window event. </summary>
