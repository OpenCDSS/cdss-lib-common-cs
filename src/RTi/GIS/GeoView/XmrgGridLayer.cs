...
namespace RTi.GIS.GeoView
{
	...
	/// </summary>
	public class XmrgGridLayer : GeoGridLayer
	{
	...
	private bool __big_endian = true;
	...
	/// the AWIPS build number is set to ???. </summary>
	/// <param name="filename">	Name of the xmrg file - this can be null. </param>
	/// <param name="user_id"> String user identifier for creator of the file (e.g., login
	/// name), up to 10 characters. </param>
	/// <param name="saved_date"> Date/time (to seconds) that the Xmrg file is created, Zulu
	/// time. </param>
	/// <param name="proc_flag"> AWIPS build 5.0 process flag, up to 8 characters.  For example,
	/// Use "RTM24" (RTi, Manual, 24 hour) if creating a 1day summed XMRG grid from the
	/// NWSRFS GUI. </param>
	/// <param name="valid_date"> Valid date (to seconds) for the Xmrg file, Zulu time -
	/// typically the same as the save date. </param>
	/// <param name="xor"> The HRAP x-coordinate of the southwest corner of the grid. </param>
	/// <param name="yor"> The HRAP y-coordinate of the southwest corner of the grid. </param>
	/// <param name="maxx"> The number of HRAP grid columns. </param>
	/// <param name="maxy"> The number of HRAP grid rows. </param>
	public XmrgGridLayer(string filename, string user_id, DateTime saved_date, string proc_flag, DateTime valid_date, int xor, int yor, int maxx, int maxy) : base(filename)
	{
		setDataFormat("XMRG");
		setShapeType(GRID);
	...
	/// <summary>
	/// Constructor for class Xmrg. </summary>
	/// <param name="filename">	 Name of xmrg file to read. </param>
	/// <param name="read_data">  A value of false indicates that only the header should be
	/// read in (the data can be read on-the-fly later).  A value of true
	/// indicates the header and all data will be read. </param>
	/// <param name="remain_open"> Indicates whether file should remain open (for additional reads). </param>
	/// <exception cref="IOException"> if there is a read error. </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public XmrgGridLayer(String filename, boolean read_data, boolean remain_open) throws java.io.IOException
	public XmrgGridLayer(string filename, bool read_data, bool remain_open) : base(filename)
	{
		setDataFormat("XMRG");
		setShapeType(GRID);
	...
	/// although any cell in the grid can be read if necessary. </summary>
	/// <param name="filename">	 Name of xmrg file to read. </param>
	/// <param name="read_data">  A value of false indicates that only the header should be
	/// read in.  A value of true indicates the header and all data will be read. </param>
	/// <param name="mincol"> Minimum HRAP column to consider. </param>
	/// <param name="minrow"> Minimum HRAP row to consider. </param>
	/// <param name="maxcol"> Maximum HRAP column to consider. </param>
	/// <param name="maxrow"> Maximum HRAP row to consider. </param>
	/// <param name="remain_open"> Indicates whether file should remain open (for additional reads). </param>
	/// <exception cref="IOException"> if there is a read error. </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public XmrgGridLayer(String filename, boolean read_data, boolean remain_open, int mincol, int minrow, int maxcol, int maxrow) throws java.io.IOException
	public XmrgGridLayer(string filename, bool read_data, bool remain_open, int mincol, int minrow, int maxcol, int maxrow) : base(filename)
	{
		setDataFormat("XMRG");
		setShapeType(GRID);
		// The entire grid will be read.
	...
	/// for missing information (-999.0). </summary>
	/// <param name="layer"> the layer from which to add values into the current layer. </param>
	public virtual void accumulate(XmrgGridLayer layer)
	{
		...
		{
			Message.printWarning(1, routine, "Could not accumulate on " + "layer due to errors.");
			Message.printWarning(2, routine, e);
		}
	}

	/// <summary>
	/// Convert an XMRG file from one endian-ness to another. </summary>
	/// <param name="input_file"> input xmrg file path. </param>
	/// <param name="output_file"> output xmrg file path. </param>
	/// <param name="big_endian">  True if the file should be written out as big-endian, 
	/// false if it should be written as little-endian. </param>
	/// <exception cref="Exception"> if an error occurs (input file does not exist, file cannot
	/// be processed, or output file cannot be written). </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static void convertEndianXmrgFile(String input_file, String output_file, boolean big_endian) throws Exception
	public static void convertEndianXmrgFile(string input_file, string output_file, bool big_endian)
	{ // Make a XmrgGridLayer (read all data and then close)...
		XmrgGridLayer input_grid = new XmrgGridLayer(input_file, true, false);
	...
	/// <exception cref="Exception"> if an error occurs (input file does not exist, file cannot
	/// be processed, or output file cannot be written). </exception>
	/// <exception cref="Exception"> if the number of columns or rows is less than 1, or if
	/// there is any other error resizing the grid. </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static void convertAndResizeEndianXmrgFile(String input_file, String output_file, boolean big_endian, int leftX, int bottomY, int numColumns, int numRows) throws Exception
	public static void convertAndResizeEndianXmrgFile(string input_file, string output_file, bool big_endian, int leftX, int bottomY, int numColumns, int numRows)
	{ // Make a XmrgGridLayer (read all data and then close)...
		XmrgGridLayer input_grid = new XmrgGridLayer(input_file, true, false);
	...
	/// </summary>
	public virtual void determineMaxValueHeader()
	{
	...
	/// <summary>
	/// Cleans up and calls garbage collector. </summary>
...
//		base.finalize();
	...
	/// <summary>
	/// Returns the data value for the column and row. </summary>
	/// <param name="col"> The value for the column. </param>
	/// <param name="row"> The value for the row. </param>
	/// <returns> Data value for the specified grid cell.  If the x-y value is out of 
	/// the range of the grid, the value used for missing data will be returned 
	/// (default -999). </returns>
	/// <exception cref="IOException"> if problem when reading. </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public double getDataValue(int col, int row) throws java.io.IOException
	public override double getDataValue(int col, int row)
	{
		GeoGrid grid = getGrid();
		if ((row < grid.getMinRowFull()) || (row > grid.getMaxRowFull()) || (col < grid.getMinColumnFull()) || (col > grid.getMaxColumnFull()))
		{
			Message.printWarning(2, "getDataValue", "column: " + col + " row: " + row + " is out of range. " + "Grid column range is: " + grid.getMinColumnFull() + "-" + grid.getMaxColumnFull() + " and row range is: " + grid.getMinRowFull() + "-" + grid.getMaxRowFull());
	...
	/// number of hundredths of MM. </summary>
	/// <returns> the maximum value in header. </returns>
	public virtual int getMaxValueHeader()
	{
		return __max_value_header;
	}

	/// <summary>
	/// Returns the operating system assigned in header. </summary>
	/// <returns> the operating system from the header ("HP" or "LX" or "" if unknown). </returns>
	public virtual string getOperSys()
	{
		return __oper_sys;
	}

	/// <summary>
	/// Returns the AWIPS process flag. </summary>
	/// <returns> String representing the process flag used in the file.
	/// For AWIPS Bld 5.0, the process flag is defined as:
	/// XXyHH where XX=process code, y=A (automatic) or M (manual),
	/// and HH = duration in hours. </returns>
	public virtual string getProcessFlag()
	{
		return __procFlag;
	}

	/// <summary>
	/// Returns the save date. </summary>
	/// <returns> File save date. </returns>
	public virtual DateTime getSavedDate()
	{
		return __savedDate;
	}

	/// <summary>
	/// Returns the user identifier assigned in header. </summary>
	/// <returns> String for the User ID from the header. </returns>
	...
	/// <summary>
	/// Returns the valid date as assigned in header. </summary>
	/// <returns> Valid date retrieved from header. </returns>
	public virtual DateTime getValidDate()
	{
		return __validDate;
	}

	/// <summary>
	/// Returns the version number assigned in header. </summary>
	/// <returns> Float for version number from header. </returns>
	...
	/// <summary>
	/// Returns whether the file is big endian or not. </summary>
	/// <returns> whether the file is big endian or not. </returns>
	...
	/// checking for "xmrg" in the file name with no "txt". </summary>
	/// <param name="filename"> name of file to evaluate. </param>
	/// <returns> true if the file is an Xmrg file, false if not. </returns>
	public static bool isXmrg(string filename)
	{
	...
	/// checking for "xmrg" in the file name with no "txt". </summary>
	/// <param name="filename"> name of file to evaluate. </param>
	/// <param name="strict"> If true, strict checks will be done (currently not enabled). </param>
	/// <returns> true if the file is an Xmrg file, false if not. </returns>
	public static bool isXmrg(string filename, bool strict)
	{
		...
		// results...
		if ((StringUtil.indexOfIgnoreCase(file.getName(),"XMRG",0) >= 0) && (StringUtil.indexOfIgnoreCase(file.getName(),"txt",0) < 0))
		{
			return true;
		}
		file = null;
	...
	/// <returns> a Vector of String with full paths that have non-zero data, or null if
	/// the directory is not found. </returns>
	/// <param name="directory"> Name of a directory with XMRG files. </param>
	public static IList<string> listNonZeroFiles(string directory)
	{
		...
		IList<string> nonzero_files = new List<string>();
		string file = null;
		for (int i = 0; i < list.Length; i++)
		{
			...
			try
			{
				xmrg = new XmrgGridLayer(file, false, false);
	...
	/// the string, the date is set to zeros. </summary>
	/// <param name="strDate"> String that represents the date. </param>
	/// <returns> DateTime from header (for either valid date or saved date). </returns>
	private DateTime makeDate(string strDate)
	{
		...
		catch (Exception)
		{
			...
			Message.printWarning(2, "XmrgGridLayer.makeDate", "String: \"" + strDate + "\" could not be formatted as a " + "DateTime.  Date set to 0.");
			try
			{
	...
	/// whether it is little or big endian. </summary>
	/// <param name="read_data"> Indicates whether the data should be read.  The header is
	/// always read. </param>
	/// <param name="remain_open"> Indicates whether the file should remain open.  If true, then
	...
	/// header read, and data will be accessed later. </param>
	/// <exception cref="IOException"> if there is an error reading the file. </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: private void read(boolean read_data, boolean remain_open) throws java.io.IOException
	private void read(bool read_data, bool remain_open)
	{
		int dl = 3;
		...
		//if ( Message.isDebugOn ) {
			...
			Message.printStatus(5, "XmrgGridLayer.read", "HRAP SW corner = " + minX + "," + minY);
			//Message.printDebug ( dl, "XmrgGridLayer.read",
			Message.printStatus(5, "XmrgGridLayer.read", "Number of columns = " + colCount + " number of rows = " + rowCount);
		//}
		...
		if ((grid.getMinColumn() == 0) && (grid.getMinRow() == 0) && (grid.getMaxColumn() == 0) && (grid.getMaxRow() == 0))
		{
			grid.setSize(minX, minY, (minX + colCount - 1), (minY + rowCount - 1));
		...
		sbyte[] bytebuffer = new sbyte[10];
		__raf.readFully(bytebuffer);
		__userID = StringHelper.NewString(bytebuffer);
		...
		if (__userID.regionMatches(false,0,"LX",0,2) || __userID.regionMatches(false,0,"HP",0,2))
		{
			...
			__userID = __userID.Substring(2, __userID.Length - 2);
		}
		else
		{ // Assume an old file.  In order to force migration to the new
		...
		if (Message.isDebugOn)
		{
			Message.printDebug(dl, "XmrgGridLayer.read", "Oper Sys = \"" + __oper_sys + "\"");
			Message.printDebug(dl, "XmrgGridLayer.read", "User ID = \"" + __userID + "\"");
		}

		// Saved date (handle the same whether big or little endian)

		bytebuffer = new sbyte[20];
		__raf.readFully(bytebuffer);
		string strSavedDate = StringHelper.NewString(bytebuffer);
		if (Message.isDebugOn)
		{
			Message.printDebug(dl, "XmrgGridLayer.read", "Save date = \"" + strSavedDate + "\"");
		}
		...
		bytebuffer = new sbyte[8];
		__raf.readFully(bytebuffer);
		__procFlag = StringHelper.NewString(bytebuffer);
		if (Message.isDebugOn)
		{
			Message.printDebug(dl, "XmrgGridLayer.read", "AWIPS process flag = \"" + __procFlag + "\"");
		}

		// Valid date (handle the same whether big or little endian).
		bytebuffer = new sbyte[20];
		__raf.readFully(bytebuffer);
		string strValidDate = StringHelper.NewString(bytebuffer);
		if (Message.isDebugOn)
		{
			Message.printDebug(dl, "XmrgGridLayer.read", "Valid date = \"" + strValidDate + "\"");
		}
		// Turn date string into a DateTime.  This will return a date value of
		// all 0's if string can't be converted to date
		...
		{
			__max_value_header = __raf.readLittleEndianInt();
		}

		if (Message.isDebugOn)
		{
			Message.printDebug(dl, "XmrgGridLayer.read", "Maximum value in full grid = " + __max_value_header);
		}
		...
		{
			__versionNum = __raf.readLittleEndianFloat();
		}
		if (Message.isDebugOn)
		{
			Message.printDebug(dl, "XmrgGridLayer.read", "AWIPS version number = " + __versionNum);
		}
	...
	/// few cells are needed.  This method does not set any values into the GeoGrid data space. </summary>
	/// <param name="c"> Column to read. </param>
	/// <param name="r"> Row to read. </param>
	/// <exception cref="IOException"> if there is an error reading the value. </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public double readDataValue(int c, int r) throws java.io.IOException
	public virtual double readDataValue(int c, int r)
	{ // Compute the position to read using the full grid extents.  The first
		...
		int dl = 50;
		GeoGrid grid = getGrid();
		long pos = 98 + (r - grid.getMinRowFull()) * (grid.getNumberOfColumnsFull() * 2 + 8) + 4 + (c - grid.getMinColumnFull()) * 2;
		...
		{
			value = (double)__raf.readLittleEndianShort();
		}
		double mm = 0.0;
		if (Message.isDebugOn)
		{
			Message.printDebug(dl, "XmrgGridLayer.readDataValue", "[C" + c + "][R" + r + "] = " + value);
		}
		if (value < 0.0)
		{
		...
		{ // Divide by 100 to get mm (values in the file are mm*100).
			mm = value / 100.0;
		}
		return mm;
	}
	...
	/// FOR A GRID SUBSET. </summary>
	/// <exception cref="IOException"> if error occurs while reading. </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: private void readGridData() throws java.io.IOException
	private void readGridData()
	{
		...
		if (Message.isDebugOn)
		{
			Message.printDebug(dl, "XmrgGridLayer.readGridData", "Cursor pointer is at: " + __raf.getFilePointer() + " when beginning to read data.");
		}
		...
		for (r = grid.getMinRowFull(); r <= rmax; r++)
		{
			...
			{
				startint = __raf.readLittleEndianInt();
			}
			...
			for (c = cmin; c <= cmax; c++)
			{
				...
				{
					short_value = __raf.readLittleEndianShort();
				}
				if (Message.isDebugOn)
				{
					Message.printDebug(dl + 2, "XmrgGridLayer.readGridData", "[C" + c + "][R" + r + "] = " + short_value);
				}
				if (short_value < 0)
				{
				...
				{ // Divide by 100 to get mm (values in the file
					// are mm*100).
			...
			{
				endint = __raf.readLittleEndianInt();
			}
			...
			if (startint != endint)
			{
				throw new IOException("[C" + c + "][R" + r + "] Starting and " + "ending integers for each record should be " + "equal.  Begin = " + startint + ", end = " + endint);
			}
		}
		...
		if (Message.isDebugOn)
		{
			Message.printDebug(dl, "XmrgGridLayer.readGridData", "Total number of values >= 0.0: " + num_positive_values);
		}
	}
	...
	/// original grid will be filled with missing values. </summary>
	...
	/// <exception cref="Exception"> if there is an error closing the random access file. </exception>
	/// <exception cref="Exception"> if the number of columns or rows is less than 1. </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public void resize(int leftX, int bottomY, int numColumns, int numRows) throws Exception
	...
	/// <summary>
	/// Set the maximum value in the header. </summary>
	/// <param name="max_value_header"> The maximum value for the grid, specified in the header. </param>
	public virtual void setMaxValueHeader(int max_value_header)
	{
		__max_value_header = max_value_header;
	}

	/// <summary>
	/// Sets the process flag in the header. </summary>
	/// <param name="procFlag"> the process flag to set in the header. </param>
	public virtual void setProcessFlag(string procFlag)
	{
		__procFlag = procFlag;
	}

	/// <summary>
	/// Sets the user ID in the header. </summary>
	/// <param name="userID"> the user ID to set in the header. </param>
	...
	/// <summary>
	/// Write the Xmrg grid as an ESRI shapefile.  All data values are written. </summary>
	/// <param name="filename"> name of shapefile to write. </param>
	/// <param name="projection"> Projection for the output. </param>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public void writeShapefile(String filename, GeoProjection projection) throws java.io.IOException
	public override void writeShapefile(string filename, GeoProjection projection)
	{ // Call overloaded method
		writeShapefile(filename, projection, false, 0.0, 0.0);
	}

	/// <summary>
	/// Write the Xmrg grid as an ESRI shapefile. </summary>
	/// <param name="filename"> name of shapefile to write. </param>
	/// <param name="projection"> Projection that the data are to be written. </param>
	/// <param name="use_data_limits"> If true, the following arguments are used to limit
	/// output to cells within the specified range. </param>
	/// <param name="min_data_value"> Cells with data values >= the value are written. </param>
	/// <param name="max_data_value"> Cells with data values <= the value are written.
	/// WHY IS THIS METHOD NEEDED? </param>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public void writeShapefile(String filename, GeoProjection projection, boolean use_data_limits, double min_data_value, double max_data_value) throws java.io.IOException
	public override void writeShapefile(string filename, GeoProjection projection, bool use_data_limits, double min_data_value, double max_data_value)
	{ // Call overloaded method
		base.writeShapefile(filename, projection, use_data_limits, min_data_value, max_data_value);
	}
	...
	/// </TABLE>
	/// </PRE> </summary>
	/// <param name="filename"> Name of the file to create, including the extension. </param>
	/// <exception cref="IOException"> if error occurs while reading. </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public void writeTextFile(String filename) throws java.io.IOException
	public virtual void writeTextFile(string filename)
	{ // Open up a PrintWriter with automatic flushing for outputfile
		PrintWriter pw_out = new PrintWriter(new StreamWriter(filename), true);
		...
		pw_out.println("Number of values > 0.0: " + grid.getNumberOfPositiveValues());
		pw_out.println();
		...
		StringBuilder buffer = new StringBuilder("row/col|");
	...
	/// file was read (if data were created in memory), use the name "xmrg.txt". </summary>
	/// <exception cref="IOException"> if error occurs while reading. </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public void writeTextFile() throws java.io.IOException
	...
	/// <summary>
	/// Write the entire layer to an Xmrg file as a big-endian file. </summary>
	/// <param name="filename"> Full name of the file to write. </param>
	/// <exception cref="IOException"> if there is an error writing the file. </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public void writeXmrgFile(String filename) throws java.io.IOException
	public virtual void writeXmrgFile(string filename)
	{
		GeoGrid grid = getGrid();
		writeXmrgFile(filename, grid.getMinColumnFull(), grid.getMinRowFull(), (grid.getMaxColumnFull() - grid.getMinColumnFull() + 1), (grid.getMaxRowFull() - grid.getMinRowFull() + 1), true);
	}

	/// <summary>
	/// Write the entire layer to an Xmrg file for the requested endian-ness. </summary>
	/// <param name="filename"> Full name of the file to write. </param>
	/// <param name="big_endian"> If true, the file will be written for use on a big-endian
	/// system (e.g., HP UNIX).  If false, the file will be written for use on a
	/// little-endian system (e.g., Linux). </param>
	/// <exception cref="IOException"> if there is an error writing the file. </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public void writeXmrgFile(String filename, boolean big_endian) throws java.io.IOException
	public virtual void writeXmrgFile(string filename, bool big_endian)
	{
		GeoGrid grid = getGrid();
		writeXmrgFile(filename, grid.getMinColumnFull(), grid.getMinRowFull(), (grid.getMaxColumnFull() - grid.getMinColumnFull() + 1), (grid.getMaxRowFull() - grid.getMinRowFull() + 1), big_endian);
	}
	...
	/// data flag. </summary>
	/// <param name="filename"> Full name of the file to write.  This can be used to specify a
	/// file other than the original file. </param>
	/// <param name="xor"> The HRAP x-coordinate of the southwest corner of the grid. </param>
	/// <param name="yor"> The HRAP y-coordinate of the southwest corner of the grid. </param>
	/// <param name="maxx"> The number of HRAP grid columns. </param>
	/// <param name="maxy"> The number of HRAP grid rows. </param>
	/// <exception cref="IOException"> if there is an error writing the file. </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public void writeXmrgFile(String filename, int xor, int yor, int maxx, int maxy) throws java.io.IOException
	public virtual void writeXmrgFile(string filename, int xor, int yor, int maxx, int maxy)
	{
	...
	/// <param name="yor"> The HRAP y-coordinate of the southwest corner of the grid. </param>
	/// <param name="maxx"> The number of HRAP grid columns. </param>
	/// <param name="maxy"> The number of HRAP grid rows.
	/// system (e.g., HP UNIX).  If false, the file will be written for use on a
	/// little-endian system (e.g., Linux). </param>
	/// <exception cref="IOException"> if there is an error writing the file. </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public void writeXmrgFile(String filename, int xor, int yor, int maxx, int maxy, boolean big_endian) throws java.io.IOException
	public virtual void writeXmrgFile(string filename, int xor, int yor, int maxx, int maxy, bool big_endian)
	{
		int dl = 3;
		...
		buffer = new StringBuilder(__userID);
		...
		if (__savedDate == null)
		{
			buffer = new StringBuilder("0000-00-00 00:00:00");
		}
		else
		{
			buffer = new StringBuilder(__savedDate.ToString(DateTime.FORMAT_YYYY_MM_DD_HH_mm_SS));
		}
		...
		buffer = new StringBuilder(__procFlag);
		for (int i = buffer.Length; i < 8; i++)
		{
			buffer.Append('\0');
		}
		...
		{
			buffer = new StringBuilder(__validDate.ToString(DateTime.FORMAT_YYYY_MM_DD_HH_mm_SS));
		}
		for (int i = buffer.Length; i < 20; i++)
		{
			buffer.Append('\0');
		...
		{
			raf.writeLittleEndianInt(__max_value_header);
		}
		...
		{
			raf.writeLittleEndianFloat(__versionNum);
		}
		...
		for (r = yor; r <= rmax_req; r++)
		{
			...
			{
				raf.writeLittleEndianInt(recsize);
			}
	...
	//	+ ") (" + cmax + "," + rmax + ") (" + maxx + "," + maxy + ")");
				...
				{ // Multiply by 100 to get mm (values in the file
					// are mm*100).
					value *= 100.0;
				}
				if (big_endian)
				...
				{
					raf.writeLittleEndianShort((short)value);
				}
				if (Message.isDebugOn)
				{
					Message.printDebug(dl, "XmrgGridLayer.readGridData", "[C" + c + "][R" + r + "] = " + value);
				}
			}
