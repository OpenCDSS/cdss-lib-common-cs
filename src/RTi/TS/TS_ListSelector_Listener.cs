...
namespace RTi.TS
{
	...
	/// <summary>
	/// Called when time series are selected from a TS_ListSelector_JFrame. </summary>
	/// <param name="sender"> the instance of TS_ListSelector_JFrame that is sending the 
	/// notification. </param>
	/// <param name="tsList"> the list of time series that were selected.  The list
	/// will never be null, though it could be empty. </param>
	/// <param name="action"> the action performed by the button the user pushed which caused
	/// this listener to be notified.  Possible values are:<para>
	/// <ul>
	...
	/// </ul> </param>
	void timeSeriesSelected(TS_ListSelector_JFrame sender, IList<TS> tsList, string action);
