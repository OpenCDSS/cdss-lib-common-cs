...
namespace RTi.TS
{
	...
	/// retrieve the data array and process as a list. </summary>
	/// <seealso cref= #getData </seealso>
//JAVA TO C# CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
//ORIGINAL LINE: @SuppressWarnings("serial") public class IrregularTS extends TS implements Cloneable, java.io.Serializable, java.awt.datatransfer.Transferable
	[Serializable]
	public class IrregularTS : TS, ICloneable, Transferable
	{

	/// <summary>
	/// The DataFlavor for transferring this specific class.
	/// </summary>
	public static DataFlavor irregularTSFlavor = new DataFlavor(typeof(RTi.TS.IrregularTS), "RTi.TS.IrregularTS");

	/// <summary>
	/// List of data points, initially null and will be initialized on first data point set.
	/// </summary>
	private IList<TSData> __tsDataList = null;
	...
	public IrregularTS(IrregularTS ts)
	{
		...
		IList<TSData> all_tsdata = ts.getData();
		if (all_tsdata == null)
		{
		...
		int nalltsdata = all_tsdata.Count;
	...
	/// space is adjusted as necessary as data are added with setDataValue(). </summary>
	/// <param name="data_flag_length"> Maximum length of data flags.  If the data flags array
	/// is already allocated, then the flag size will be increased by the specified
	/// length.  This allows multiple flags to be concatenated. </param>
	/// <param name="initial_value"> Initial value (null is allowed and will result in the flags being initialized to spaces). </param>
...
//ORIGINAL LINE: public void allocateDataFlagSpace(String initial_value, boolean retain_previous_values) throws Exception
	public override void allocateDataFlagSpace(string initial_value, bool retain_previous_values)
	{
		return;
	}
	...
	/// <b>This method is not valid for IrregularTS but is maintained for compatibility.  Use the non-static calculateDataSize() method.</b> </summary>
	/// <returns> A count of the data recordings between the given dates (always zero here). </returns>
	/// <param name="start_date"> First date of interest. </param>
	/// <param name="end_date"> Last date of interest. </param>
	/// <param name="interval_mult"> Data interval multiplier (not used). </param>
	public static int calculateDataSize(DateTime start_date, DateTime end_date, int interval_mult)
	{
		Message.printWarning(1, "IrregularTS.calculateDataSize", "Must have an instance of IrregularTS to compute the data size.  Use " + "the non-static calculateDataSize() method");
		return 0;
	}

	/// <summary>
	/// Calculate the number of data points between two dates. </summary>
	/// <returns> A count of the data recordings between the given dates. </returns>
	/// <param name="start_date"> First date of interest. </param>
	/// <param name="end_date"> Last date of interest. </param>
	public virtual int calculateDataSize(DateTime start_date, DateTime end_date)
	{
		int datasize = 0;
...
//ORIGINAL LINE: public void changePeriodOfRecord(RTi.Util.Time.DateTime date1, RTi.Util.Time.DateTime date2) throws TSException
	...
	{
		IrregularTS ts = (IrregularTS)base.clone(); // Clone base class data

		ts.copyHeader(this);
		// Get the data and loop through the list...
		IList<TSData> all_tsdata = getData();
		if (all_tsdata == null)
		{
			// No data for the time series...
	...
	/// GRTS TSView* package correctly displays irregular data.</b> </summary>
	/// <returns> Zero if successful, one if not. </returns>
	/// <param name="interval_base"> Base interval of the maximum allowable gap. </param>
	/// <param name="interval_mult"> Interval multiplier of the maximum allowable gap. </param>
	/// <param name="start_date"> First date of interest. </param>
	/// <param name="end_date"> Last date of interest. </param>
...
//		base.finalize();
	...
	/// of DateTime in the time series. </summary>
	/// <param name="dt"> DateTime that is is of interest to match </param>
	/// <param name="searchStart"> a DateTime known to exist in the time series, to start the search </param>
	/// <param name="searchEnd"> a DateTime known to exist in the time series, to end the search </param>
	/// <param name="returnMatch"> if true and the requested DateTime matches an existing value, return the data for that point
	/// (false will return the next point). </param>
	/// <returns> a TSData objects containing the nearest future data point to the requested,
	/// or null if not found (goes beyond end of data) </returns>
	public virtual TSData findNearestNext(DateTime dt, DateTime searchStart, DateTime searchEnd, bool returnMatch)
	{
		...
		int iLeft = 0, iRight = this.__tsDataList.Count - 1, iMiddle;
		TSData tsdataMiddle;
		DateTime dtMiddle;
		while (true)
		{
			...
			tsdataMiddle = this.__tsDataList[iMiddle];
			...
			if ((iRight - iLeft) == 1)
			{
				...
				tsdataNext = new TSData(__tsDataList[iRight]);
				break;
			}
		}
		return tsdataNext;
	...
	/// irregular time series are always output in a sequential summary format. </summary>
	...
	/// </para>
	/// </param>
	/// <exception cref="RTi.TS.TSException"> Throws if there is a problem formatting the
	/// output. </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public java.util.List<String> formatOutput(RTi.Util.IO.PropList proplist) throws TSException
	public override IList<string> formatOutput(PropList proplist)
	{
		...
		else if (format.Equals("Summary", StringComparison.OrdinalIgnoreCase))
		{
			...
			double data_value;
			int nalltsdata = __tsDataList.Count;
			TSData tsdata = null;
			DateTime date = null;
			for (int i = 0; i < nalltsdata; i++)
			{
				tsdata = __tsDataList[i];
				date = tsdata.getDate();
				...
				if (date.greaterThanOrEqualTo(start_date))
				{
					...
					strings.Add(StringUtil.formatString(date.ToString(), "%-26.26s") + "  " + StringUtil.formatString(data_value,data_format));
				}
			}
			strings.Add("--------------------------------------------");
...
//ORIGINAL LINE: public java.util.List<String> formatOutput(String fname, RTi.Util.IO.PropList props) throws TSException
	public override IList<string> formatOutput(string fname, PropList props)
	{
		...
		try
		{
			stream = new PrintWriter(new StreamWriter(fname));
		}
		catch (Exception)
		{
	...
	/// <summary>
	/// Return the data array list. </summary>
	/// <returns> The reference to the data array.  Use caution when manipulating. </returns>
	public virtual IList<TSData> getData()
	{
		return __tsDataList;
	}

	/// <summary>
	/// Return a data point for a date. </summary>
	/// <returns> A TSData for the date of interest. </returns>
	/// <param name="date"> Date of interest. </param>
	public override TSData getDataPoint(DateTime date, TSData data_point)
	{
		int i = 0;
		TSData ptr = null;
		...
		if (date.lessThan(_date1) || date.greaterThan(_date2))
		{
			if (Message.isDebugOn)
			{
				Message.printDebug(30, "IrregularTS.getDataPoint", date + " not within POR (" + _date1 + " - " + _date2 + ")");
				// Leave __data_index as is.
				data_point.setDataValue(_missing);
				return data_point;
			}
		...
		if (Message.isDebugOn)
		{
			Message.printDebug(30, "IrregularTS.getDataPoint", ptr.getDataValue() + " for " + date + " from _data[" + i + "].");
		}
	...
	/// but might need to be in the future to increase performance. </summary>
	/// <returns> The data value in the data array given a date, or the missing data
	/// value if the date cannot be found in the data. </returns>
	/// <param name="date"> Date of interest. </param>
	...
	{ // Do not define routine here to increase performance.
		...
		if (Message.isDebugOn)
		{
			Message.printDebug(dl, "IrregularTS.getDataValue", ptr.getDataValue() + " for " + date + " from _data[" + i + "].");
		}
	...
	/// </summary>
	public override DateTime getDate1()
	{
		if ((__tsDataList == null) || (__tsDataList.Count == 0))
		{
			return base.getDate1();
		}
		else
		{
			return __tsDataList[0].getDate();
		}
	}
	...
	/// (e.g., in cases when the time series header is defined without data).
	/// </summary>
	public override DateTime getDate2()
	{
		if ((__tsDataList == null) || (__tsDataList.Count == 0))
		{
			return base.getDate2();
		}
		else
		{
			return __tsDataList[__tsDataList.Count - 1].getDate();
		}
	}
	...
	/// to get the next data value.  The routine returns null if at the end of the data array. </summary>
	/// <returns> The next data point after the previous access. </returns>
	public virtual TSData getNextElement()
	{ // Do not define routine here to increase performance.
		TSData tsdata = null;
		...
		if (((__data_index + 1) >= size) || ((__data_index + 1) < 0))
		{
			if (Message.isDebugOn)
			{
				Message.printDebug(20, "IrregularTS.getNextElement", "Data index " + __data_index + " out of range, returning NULL.");
			}
			return null;
		}
		...
		tsdata = __tsDataList[__data_index + 1];
	...
	/// strings are memory-efficient. </summary>
	/// <returns> true if data flags are used, false if not. </returns>
	public override bool hasDataFlags()
	{
		if (!hasData())
		{
			return false;
	...
	/// <summary>
	/// Return an iterator for the time series using the full period for the time series. </summary>
	/// <returns> an iterator for the time series. </returns>
	/// <exception cref="Exception"> if the time series dates are null. </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public TSIterator iterator() throws Exception
	public override TSIterator iterator()
	{
	...
	/// <param name="date1"> Start of data iteration. </param>
	/// <param name="date2"> End of data iteration. </param>
	/// <returns> an iterator for the time series. </returns>
	/// <exception cref="Exception"> if the time series dates are null. </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public TSIterator iterator(RTi.Util.Time.DateTime date1, RTi.Util.Time.DateTime date2) throws Exception
	public override TSIterator iterator(DateTime date1, DateTime date2)
	{
	...
	/// <summary>
	/// Remove a data point corresponding to the date. </summary>
	/// <param name="date"> date/time for which to remove the data point. </param>
	/// <returns> true if the point was removed, false if not (date was not found). </returns>
	public virtual bool removeDataPoint(DateTime date)
	{
		if (date == null)
		{
			return false;
		...
		if (!pointFound)
		{
			...
			foreach (TSData ptr2 in __tsDataList)
			{
	...
	/// method with a "" flag and duration of 0. </summary>
	/// <param name="date"> Date of interest. </param>
	/// <param name="value"> Data value corresponding to the date. </param>
	...
	/// If not, utilize a bisection approach to find the point to set. </summary>
	/// <param name="date"> Date of interest. </param>
	/// <param name="value"> Data value corresponding to date. </param>
	/// <param name="data_flag"> Data flag for value. </param>
	/// <param name="duration"> Duration for value </param>
	...
	{ // Do not define routine here to increase performance.
		...
		if ((__tsDataList != null) && (__tsDataList.Count > 0))
		{
			date1_double = __tsDataList[0].getDate().toDouble();
			date2_double = __tsDataList[__tsDataList.Count - 1].getDate().toDouble();
		}
		...
		if (insert_position >= 0)
		{
			...
			__tsDataList.Insert(insert_position, tsdata);
			...
			else if (insert_position == (__tsDataList.Count - 1))
			{
				// Added at end
				ptr = __tsDataList[insert_position - 1];
