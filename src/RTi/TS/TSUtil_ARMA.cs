...
namespace RTi.TS
{
	...
	public class TSUtil_ARMA
	{
		...
		/// Startup values that are missing in O are set to I. </summary>
		/// <param name="inputts"> input time series to shift (if outputts is null then this will be updated). </param>
		/// <param name="outputts"> output time series (if null update the input time series). </param>
		/// <param name="a"> Array of a coefficients. </param>
		/// <param name="b"> Array of b coefficients. </param>
		/// <param name="inputPreviousValues"> array of values to be prepended to input time series </param>
		/// <param name="outputPreviousValues"> array of values to be prepended before output time series, prior to output start </param>
		/// <param name="ARMA_interval"> The interval used to develop ARMA coefficients.  The
		/// interval must be <= that of the time series.  Use an interval like "1Day", "6Hour", etc. </param>
		/// <param name="outputStart"> output start date/time or null to be same as input time series </param>
		/// <param name="outputEnd"> output end date/time or null to be the same as input time series </param>
		/// <returns> new time series. </returns>
		/// <exception cref="Exception"> if there is a problem with input </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public TS ARMA(TS inputts, TS outputts, String ARMA_interval, double a[], double b[], double [] inputPreviousValues, double [] outputPreviousValues, double outputMinimum, double outputMaximum, RTi.Util.Time.DateTime outputStart, RTi.Util.Time.DateTime outputEnd) throws Exception
		public virtual TS ARMA(TS inputts, TS outputts, string ARMA_interval, double[] a, double[] b, double[] inputPreviousValues, double[] outputPreviousValues, double outputMinimum, double outputMaximum, DateTime outputStart, DateTime outputEnd)
		{
			string message, routine = this.GetType().Name + ".ARMA";
			...
			if (inputPreviousValues.Length > 0)
			{
				...
				Array.Copy(inputPreviousValues, 0, oldts_data_tmp, 0, inputPreviousValues.Length);
				Array.Copy(oldts_data0, 0, oldts_data_tmp, inputPreviousValues.Length, oldts_data0.Length);
			...
			double data_value;
			int ia = 0, ib = 0;
			bool value_set = false; // Indicates if newts_data[t] value has been set (e.g., to missing) and should not be reset by calculations
			...
			for (int t = 0; t < oldts_data.Length; t++)
			{
				...
				for (ia = 0; ia < a.Length; ia++)
				{
					...
					if (t < oldtsDataIndexForT0)
					{
						...
						if (Message.isDebugOn)
						{
							Message.printStatus(2,routine,"processing before original input time series start so always missing, t (" + t + ") < oldtsDataIndexForT0 (" + oldtsDataIndexForT0 + ") - setting newts_data[" + t + "].");
						}
					...
					else if ((t >= oldtsDataIndexForT0) && (t <= lastIndexForOutputPreviousValues))
					{
						...
						if (Message.isDebugOn)
						{
							Message.printStatus(2,routine,"outputPreviousValues can be used since t (" + t + ") >= oldtsDataIndexForT0 (" + oldtsDataIndexForT0 + ") and <= lastIndexForOutputPreviousValues (" + lastIndexForOutputPreviousValues + ").");
						}
						if (outputPreviousValues.Length == 0)
						{
						...
						else
						{
							...
							if (tpos < oldtsDataIndexForT0)
							{
								...
								if (Message.isDebugOn)
								{
									Message.printStatus(2, routine, "Using output (previous value), tpos=" + tpos + " oldtsDataIndexForT0=" + oldtsDataIndexForT0 + " data_value = " + outputPreviousValues[outputPreviousValues.Length + toffset]);
								}
							}
							else
							{
								...
								if (Message.isDebugOn)
								{
									Message.printStatus(2, routine, "Using output (not previous value), tpos=" + tpos + " oldtsDataIndexForT0=" + oldtsDataIndexForT0 + " data_value = " + outputPreviousValues[outputPreviousValues.Length + toffset]);
								}
							}
						}
					}
			...
			for (DateTime date = new DateTime(date1); date.lessThanOrEqualTo(date2); date.addInterval(interval_base,interval_mult), i++)
			{
			...
			IList<string> genesis = outputts.getGenesis();
			outputts.setDescription(outputts.getDescription() + ",ARMA");
			int genesis_length = outputts.getGenesis().Count;
			genesis.Insert(genesis_length, "Applied ARMA(p=" + a.Length + ",q=" + (b.Length - 1) + ") using ARMA interval " + ARMA_interval + " and coefficients:");
			for (i = 0; i < a.Length; i++)
			{
				genesis.Insert((genesis_length + 1 + i), "    a" + (i + 1) + " = " + StringUtil.formatString(a[i], "%.6f"));
			}
			for (i = 0; i < b.Length; i++)
			{
				genesis.Insert((genesis_length + a.Length + 1 + i), "    b" + i + " = " + StringUtil.formatString(b[i], "%.6f"));
			}
			genesis.Insert((genesis_length + a.Length + b.Length + 1), "ARMA: The original number of data points were expanded by a factor ");
			genesis.Insert((genesis_length + a.Length + b.Length + 2), "ARMA: of " + interval_ratio + " before applying ARMA.");
			if (ARMA_ratio == 1)
			{
				genesis.Insert((genesis_length + a.Length + b.Length + 3), "ARMA: All points were then used as the final result.");
			}
			else
			{
				genesis.Insert((genesis_length + a.Length + b.Length + 3), "ARMA: 1/" + ARMA_ratio + " points were then used to compute the averaged final result.");
			}
