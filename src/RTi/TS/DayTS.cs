...
//ORIGINAL LINE: @SuppressWarnings("serial") public class DayTS extends TS implements Cloneable, java.io.Serializable, java.awt.datatransfer.Transferable
	...
	/// by calling copyHeader() and then copying the data values.<para>
	...
	public override object clone()
	{
		...
		else
		{
		   ...
		   for (int imon = 0; imon < _data.Length; imon++)
		   {
			   ...
			   Array.Copy(_data[imon], 0, ts._data[imon], 0,_data[imon].Length);
		   }
		}
		int iday = 0;
		if (_has_data_flags)
...
//		base.finalize();
	...
	/// daily time series are always output in a matrix summary format. </summary>
...
//ORIGINAL LINE: double[][] data = new double[31][12];
			...
			for (; date.lessThanOrEqualTo(end_date); date.addInterval(_data_interval_base,_data_interval_mult))
			{
				...
				if ((month == month_to_end) && (day == ndays_in_month))
				{
					...
					{
						strings.Add("                                                 " + calendar + " " + date.getYear() + " (Nov " + (date.getYear() - 1) + " to Oct " + date.getYear() + ")");
					}
					else
					{
...
//ORIGINAL LINE: public java.util.List<String> formatOutput(String fname, RTi.Util.IO.PropList props) throws TSException
	...
	/// <param name="data"> Data array to generate statistics from. </param>
	/// <param name="label"> Label for statistics row (e.g., "Mean"). </param>
	/// <param name="data_format"> Format for individual floating point values. </param>
	private IList<string> formatOutputStats(double[][] data, string label, string data_format)
	{
		IList<string> strings = new List<string>();
		double stat = 0.0;
		StringBuilder buffer = null;
	...
	/// the data flag space has been allocated and data flags are enabled for the time series. </summary>
	/// <returns> true if data flags are available (the data flag space has been allocated).
	/// Note that true will be returned even if all the data flag values are set to the missing data flag value. </returns>
	public override bool hasDataFlags()
	{
		if (_dataFlags != null)
		{
			return base.hasDataFlags();
		}
		else
		{
			return false;
	...
	public override void setDataValue(DateTime date, double value, string data_flag, int duration)
	{
		...
		if (Message.isDebugOn)
		{
			Message.printDebug(30, "DayTS.setDataValue", "Setting " + value + " flag=" + data_flag + " for " + date + " at " + _row + "," + _column);
		}
		...
		if ((!string.ReferenceEquals(data_flag, null)) && (data_flag.Length > 0))
		{
			if (!_has_data_flags)
			{
				...
				catch (Exception e)
				{
					...
					if (Message.isDebugOn)
					{
						Message.printDebug(30, "DayTS.setDataValue", "Error allocating data flag space (" + e + ") - will not use flags.");
					}
