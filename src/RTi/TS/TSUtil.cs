...
namespace RTi.TS
{
	...
	using InvalidTimeIntervalException = RTi.Util.Time.InvalidTimeIntervalException;
	...
	*/
	...
	/// The IGNORE_MISSING flag is used for missing data. </summary>
	/// <returns> The sum of the time series. </returns>
...
//ORIGINAL LINE: public static TS add(TS ts, TS ts_to_add) throws TSException, Exception
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static TS add(TS ts, java.util.List<TS> ts_to_add) throws Exception
	public static TS add(TS ts, IList<TS> ts_to_add)
	{
	...
	/// <returns> The sum of the time series. </returns>
	/// <param name="ts"> Time series to be added to. </param>
	/// <param name="tsToAddList"> List of time series to add to "ts". </param>
	/// <param name="missing_flag"> See overloaded version for description. </param>
	/// <exception cref="TSException"> if an error occurs adding the time series. </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static TS add(TS ts, java.util.List<? extends TS> tsToAddList, int missing_flag) throws Exception
	public static TS add<T1>(TS ts, IList<T1> tsToAddList, int missing_flag) where T1 : TS
	{
	...
	/// <param name="ts"> Time series to be added to. </param>
	/// <param name="tsToAddList"> List of time series to add to "ts". </param>
	/// <param name="missingFlag"> See overloaded version for description. </param>
	/// <param name="addStart"> starting date/time to add, or null to process full period </param>
	/// <param name="addEnd"> startin date/time to add, or null to process full period </param>
	/// <exception cref="TSException"> if an error occurs adding the time series. </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static TS add(TS ts, java.util.List<? extends TS> tsToAddList, int missingFlag, RTi.Util.Time.DateTime addStart, RTi.Util.Time.DateTime addEnd) throws Exception
	public static TS add<T1>(TS ts, IList<T1> tsToAddList, int missingFlag, DateTime addStart, DateTime addEnd) where T1 : TS
	{
		string message, routine = "TSUtil.add";
	...
	/// each time series by before adding.  The factors are applied after units conversion. </param>
	/// <param name="missingFlag"> Handle missing data as follows:
	/// <pre>
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static TS add(TS ts, java.util.List<? extends TS> tsToAddList, double factor[], int missingFlag) throws Exception
	public static TS add<T1>(TS ts, IList<T1> tsToAddList, double[] factor, int missingFlag) where T1 : TS
	{
	...
	/// <exception cref="RTi.TS.TSException"> if there is an error adding the time series. </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static TS add(TS ts, java.util.List<? extends TS> tsToAddList, double factor[], int missingFlag, RTi.Util.Time.DateTime addStart, RTi.Util.Time.DateTime addEnd) throws TSException, Exception
	public static TS add<T1>(TS ts, IList<T1> tsToAddList, double[] factor, int missingFlag, DateTime addStart, DateTime addEnd) where T1 : TS
	{
		string message, routine = "TSUtil.add";
		...
		{
			message = "All time series in the list are not of interval " + TimeInterval.getName(ts.getDataIntervalBase(),1) + "," + ts.getDataIntervalMult();
			Message.printWarning(2, routine, message);
			throw new TSException(message);
		}
	...
	*/
		...
		if ((missingFlag == SET_MISSING_IF_OTHER_MISSING) || (missingFlag == SET_MISSING_IF_ANY_MISSING))
		{
			...
			missingIndicators = new bool[ndata];
		...
		for (int i = 0; i < ntslist; i++)
		{
			...
			catch (Exception)
			{
				...
				message = "Cannot get conversion from \"" + tspt.getIdentifier().ToString() + "\" data units \"" + tspt.getDataUnits() + "\" to \"" + ts.getIdentifier().ToString() + "\" data units \"" + req_units + "\"";
				Message.printWarning(3, routine, message);
				throw new TSException(message);
			}
	...
	*/
			...
			else
			{
				...
				for (timestepIndex = 0; date.lessThanOrEqualTo(endDate); date.addInterval(intervalBase, intervalMult), ++timestepIndex)
				{
					...
					if (ts.isDataMissing(dataValue))
					{
						...
						else
						{
							...
							{
								Message.printDebug(dl, routine, "At " + date + ", setting " + (dataValueToAdd * mult + add) * factor[i] + " to " + dataValue);
							}
					...
					else
					{
						...
						if (Message.isDebugOn)
						{
							Message.printDebug(dl, routine, "At " + date + ", adding " + (dataValueToAdd * mult + add) * factor[i] + " to " + dataValue);
						}
				...
				if (factor[i] >= 0.0)
				{
					if (factor[i] == 1.0)
					{
						...
						ts.addToGenesis("Added \"" + tspt.getIdentifierString() + "\" to this time series (#missing=" + nmissing + ").");
					}
					else
					{
						ts.setDescription(ts.getDescription() + " + " + StringUtil.formatString(factor[i],"%.3f") + "*" + tspt.getDescription());
						ts.addToGenesis("Added \"" + StringUtil.formatString(factor[i],"%.3f") + "*" + tspt.getIdentifierString() + "\" to this time series (#missing=" + nmissing + ").");
					}
				}
				else
				{
					if (factor[i] != -1.0)
					{
							ts.setDescription(ts.getDescription() + " minus " + StringUtil.formatString(-factor[i],"%.3f") + "*" + tspt.getDescription());
						ts.addToGenesis("Subtracted \"" + StringUtil.formatString(factor[i], "%.3f") + "*" + tspt.getIdentifierString() + "\" from this time series (#missing=" + nmissing + ").");
					}
					else
					{
						ts.setDescription(ts.getDescription() + " minus " + tspt.getDescription());
						ts.addToGenesis("Subtracted \"" + tspt.getIdentifierString() + "\" from this time series (#missing=" + nmissing + ").");
					}
				}
			}
		}
	...
	/// <summary>
	/// Add a constant to the time series.  The overloaded version is called with missing_flag=SET_MISSING_IF_ANY_MISSING. </summary>
	/// <param name="ts"> Time series to add to. </param>
	/// <param name="start_date"> Date to start adding. </param>
	/// <param name="end_date"> Date to stop adding. </param>
	/// <param name="add_value"> Data value to add to the time series. </param>
	...
	/// missing_flag=SET_MISSING_IF_ANY_MISSING. </summary>
	/// <param name="ts"> Time series to add to. </param>
	/// <param name="start_date"> Date to start adding. </param>
	/// <param name="end_date"> Date to stop adding. </param>
	/// <param name="month"> Month to add.  If negative, add for every month. </param>
	/// <param name="add_value"> Data value to add to time series. </param>
	...
	/// <summary>
	/// Add a constant value to the time series. </summary>
	/// <param name="ts"> Time series to add to. </param>
	/// <param name="start_date"> Date to start adding. </param>
	/// <param name="end_date"> Date to stop adding. </param>
	/// <param name="month"> Month to add.  If negative, add for every month (no check for month). </param>
	/// <param name="add_value"> Data value to add to time series. </param>
	/// <param name="missing_flag"> Handle missing data as follows:
	/// <pre>
	/// IGNORE_MISSING               Missing data are ignored and have no effect on
	...
	///	                             case).
	/// </pre> </param>
	public static void addConstant(TS ts, DateTime start_date, DateTime end_date, int month, double add_value, int missing_flag)
	{
		double oldvalue;
		...
		if (month >= 0)
		{
			ts.addToGenesis("Added " + StringUtil.formatString(add_value,"%.6f") + " " + start + " to " + end + " uniquetempvar.");
		}
		else
		{
			ts.addToGenesis("Added " + StringUtil.formatString(add_value,"%.6f") + " " + start + " to " + end + ".");
		}
	...
	/// <summary>
	/// Adjust extreme values in a time series by maintaining "mass" and adjusting nearby values. </summary>
	/// <param name="ts"> time series to adjust. </param>
	/// <param name="adjustMethod"> If "Average", then values on each side of the extreme are
	...
	/// orginal data values, with values that exceed the extreme being set to the extreme. </param>
	/// <param name="extremeToAdjust"> AdjustMinimum or AdjustMaximum, indicating whether low or
	/// high extremes are being adjusted. </param>
	/// <param name="extremeValue"> The extreme value that when exceeded will be adjusted.  For
	/// example, specify zero and extremeToAdjust="AdjustMinimum" to adjust negative values. </param>
	/// <param name="maxIntervals"> The maximum number of intervals on each side of the extreme
	/// that can be used to computed an adjusted value.  If the value is exceeded
	/// without reaching a satisfactory adjustment, do not adjust.  Specify zero to allow any number of intervals. </param>
	/// <param name="analysisStart"> Start of the analysis period or null for the full period. </param>
	/// <param name="analysisEnd"> End of the analysis period or null for the full period. </param>
	/// <param name="setFlag"> flag to set for modified values (if null or empty don't set). </param>
	/// <param name="setFlagDescription"> description for setFlag. </param>
	/// <exception cref="Exception"> if there is an input error. </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static void adjustExtremes(TS ts, String adjustMethod, String extremeToAdjust, double extremeValue, int maxIntervals, RTi.Util.Time.DateTime analysisStart, RTi.Util.Time.DateTime analysisEnd, String setFlag, String setFlagDescription) throws Exception
	public static void adjustExtremes(TS ts, string adjustMethod, string extremeToAdjust, double extremeValue, int maxIntervals, DateTime analysisStart, DateTime analysisEnd, string setFlag, string setFlagDescription)
	{
		...
		bool adjust_low = true; // Whether adjusting the low values
		if (extremeToAdjust.Equals("AdjustMaximum", StringComparison.OrdinalIgnoreCase))
		{
			adjust_low = false; // Now adjusting the high values
		}
		bool adjust_average = true;
		if (adjustMethod.Equals("WeightedAverage", StringComparison.OrdinalIgnoreCase))
		{
			adjust_average = false;
		}

		bool doSetFlag = false;
		if ((!string.ReferenceEquals(setFlag, null)) && setFlag.Length > 0)
		{
			doSetFlag = true;
		}
		...
		double right_value;
		bool do_adjust; // Indicates whether adjustment should be made.
			...
			if ((adjust_low && (oldvalue < extremeValue)) || (!adjust_low && (oldvalue > extremeValue)))
			{
				...
				while (true)
				{
					...
					if ((maxIntervals != 0) && (nintervals > maxIntervals))
					{
						if (Message.isDebugOn)
						{
							Message.printDebug(1,routine, "Maximum intervals (" + maxIntervals + ") exceeded trying to adjust value at " + date + " - not adjusting.");
						}
						do_adjust = false;
						break;
					}
				...
				for (; left_date.lessThanOrEqualTo(right_date); left_date.addInterval(interval_base, interval_mult))
				{
					...
					if (adjust_average)
					{
						...
						ts.addToGenesis("AdjustExtremes:  adjusted " + StringUtil.formatString(oldvalue, "%.6f") + " to " + StringUtil.formatString(average, "%.6f") + " at " + left_date);
						if (doSetFlag)
						{
					...
					else
					{
						...
						ts.addToGenesis("AdjustExtremes:  adjusted " + StringUtil.formatString(oldvalue, "%.6f") + " to " + StringUtil.formatString(left_value, "%.6f") + " at " + left_date);
						if (doSetFlag)
						{
		...
		ts.addToGenesis("AdjustExtremes:  Adjusted extremes " + start + " to " + end + " AdjustMethod=" + adjustMethod + " ExtremeToAdjust=" + extremeToAdjust + " ExtremeValue=" + StringUtil.formatString(extremeValue,"%.6f") + " MaxIntervals=" + maxIntervals + ".");
		ts.setDescription(ts.getDescription() + ", AdjustExtremes");
		if (adjustCount > 0)
		{
			// Also set the flag description
			if ((!string.ReferenceEquals(setFlagDescription, null)) && setFlagDescription.Length > 0)
			{
	...
	/// <returns> True if a time series is editable </returns>
	public static bool areAnyTimeSeriesEditable(IList<TS> tslist)
	{
	  int size = 0;
	  if (tslist != null)
	  {
	...
	/// <summary>
	/// Determine whether any time series are irregular. </summary>
	/// <returns> true if any time series are irregular, false if not. </returns>
	/// <param name="tslist"> list of time series to evaluate. </param>
	public static bool areAnyTimeSeriesIrregular(IList<TS> tslist)
	{
		int size = 0;
		if (tslist != null)
		{
	...
	/// multiplier for the interval must agree). </summary>
	/// <returns> true if the intervals are the same. </returns>
	/// <param name="tslist"> list of time series. </param>
	public static bool areIntervalsSame(IList<TS> tslist)
	{
		if (tslist == null)
		{
	...
	/// <returns> true if the units in the time series are compatible. </returns>
	/// <param name="tslist"> list of time series. </param>
	public static bool areUnitsCompatible(IList<TS> tslist)
	{
	...
	/// The units are allowed to be different as long as they are within the same
	/// dimension (e.g., each is a length). </summary>
	/// <returns> true if the units in the time series are compatible, according to the "require_same" flag. </returns>
	/// <param name="tslist"> list of time series. </param>
	...
	public static bool areUnitsCompatible(IList<TS> tslist, bool require_same)
	{
		if (tslist == null)
		{
			// No units.  Decide later whether to throw an exception.
		...
		for (int i = 0; i < size; i++)
		{
			...
			if (string.ReferenceEquals(units_string, null))
			{
				continue;
			}
			units.Add(units_string);
		}
		bool result = DataUnits.areUnitsStringsCompatible(units, require_same);
		return result;
	}
	...
	/// series align in date/time (i.e., this method cannot be used to shift time series in time before averaging). </summary>
	/// <param name="tslist"> Time series to get data from, in order to calculate the average. </param>
	/// <param name="start_date"> Date to start data analysis (relative to the new average
	/// time series).  If null, the earliest date/time is taken from the time series
	/// list. </param>
	/// <param name="end_date"> Date to stop the data transfer (relative to the new average
	/// time series).  If null, the latest date/time is taken from the time series list. </param>
	/// <param name="props"> Properties to control the average.  Recognized properties are:
	...
	/// </tr>
	/// </table> </param>
	/// <exception cref="Exception"> if an error occurs (usually null input). </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static TS average(java.util.List<TS> tslist, RTi.Util.Time.DateTime start_date, RTi.Util.Time.DateTime end_date, RTi.Util.IO.PropList props) throws Exception
	public static TS average(IList<TS> tslist, DateTime start_date, DateTime end_date, PropList props)
	{
		...
		bool transfer_bydate = true; // Default - make dates match in both time series.
		if (props != null)
		{
			string prop_val = props.getValue("TransferData");
			if ((!string.ReferenceEquals(prop_val, null)) && prop_val.Equals(TRANSFER_SEQUENTIALLY, StringComparison.OrdinalIgnoreCase))
			{
	...
	/// time series.  Only data after the last date in the original time series will be blended. </summary>
	/// <param name="ts"> Time series to be modified. </param>
	/// <param name="tsb"> Time series to blend. </param>
	/// <exception cref="Exception"> if there is an error processing (e.g., incompatible units). </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static void blend(TS ts, TS tsb) throws Exception
	public static void blend(TS ts, TS tsb)
	{
		...
		{
			throw new Exception("Cannot blend data with different intervals (" + ts.getIdentifierString() + " and " + tsb.getIdentifierString() + ").");
		}
		if (interval_base == TimeInterval.IRREGULAR)
		{
	...
	/// data size is needed, call the non-static IrregularTS.calculateDataSize() method. </summary>
	/// <returns> The number of data points for a time series of the given data
	...
	/// spacing of data is unknown unless a time series is supplied). </returns>
	/// <param name="start_date"> The first date of the period. </param>
	/// <param name="end_date"> The last date of the period. </param>
	/// <param name="interval_base"> The time series data interval base. </param>
	...
	/// end dates are not specifiec, the time series limits will be used. </returns>
	...
	/// units abbreviations stored in the DataUnits class data. </summary>
	/// <param name="ts"> Time series to convert units. </param>
	/// <param name="req_units"> Requested units. </param>
	/// <seealso cref= DataUnits </seealso>
	/// <exception cref="RTi.TS.TSException"> If the time series is null or if the conversion
	/// cannot be found.  Requested units of blank or null result in no change. </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static void convertUnits(TS ts, String req_units) throws TSException
	public static void convertUnits(TS ts, string req_units)
	{
		double mult, add;
		...
		if ((string.ReferenceEquals(req_units, null)) || (req_units.Length == 0))
		{
		...
		if (req_units.Equals(units, StringComparison.OrdinalIgnoreCase))
		{
			if (Message.isDebugOn)
			{
				Message.printDebug(dl, rtn, "No coversion necessary since units \"" + units + "\" are the same as the requested units \"" + req_units + "\".");
			}
			return;
		}
		...
		{
			throw new TSException("Unable to get conversion from \"" + units + "\" to \"" + req_units + "\"");
		}
		if (conversion == null)
		{
			throw new TSException("Unable to get conversion from \"" + units + "\" to \"" + req_units + "\"");
		...
		if (add == 0.0)
		{
			ts.addToGenesis("Converted data units from \"" + units + "\" to \"" + req_units + "\" using *" + StringUtil.formatString(mult,"%.6f"));
		}
		else
		{
			ts.addToGenesis("Converted data units from \"" + units + "\" to \"" + req_units + "\" using *" + StringUtil.formatString(mult,"%.6f") + "+" + StringUtil.formatString(add,"%.6f"));
		}
	...
	/// method. </summary>
	/// <param name="ts1"> Source copy. </param>
	/// <param name="ts2"> New copy. </param>
	/// <seealso cref= TS#copyHeader </seealso>
	...
	/// is useful, for example, for determining whether graphs should be drawn with symbols to show the data. </summary>
	/// <param name="ts"> time series to process </param>
	/// <param name="start"> the starting date/time for the count, or null to process the entire time series </param>
	/// <param name="end"> the ending date/time for the count, or null to process the entire time series </param>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static int countIsolatedPoints(TS ts, RTi.Util.Time.DateTime start, RTi.Util.Time.DateTime end) throws Exception
	...
	/// should not be confused with the MonthTS.formatOutput() method, which just displays raw data values. </summary>
	/// <param name="ts"> Time series to process. </param>
	/// <param name="date1"> First date/time to process. </param>
	/// <param name="date2"> Last date/time to process. </param>
	/// <param name="props"> Properties to control formatting:
	/// <table width=100% cellpadding=10 cellspacing=0 border=2>
	/// <tr>
	/// <td><b>Property</b></td>	<td><b>Description</b></td>	<td><b>Default</b></td>
	/// </tr>
	...
	/// 
	/// </table> </param>
	/// <returns> a list of String containing the report. </returns>
	/// <exception cref="Exception"> if there is an error generating the report. </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static java.util.List<String> createMonthSummary(TS ts, RTi.Util.Time.DateTime date1, RTi.Util.Time.DateTime date2, RTi.Util.IO.PropList props) throws Exception
	public static IList<string> createMonthSummary(TS ts, DateTime date1, DateTime date2, PropList props)
	{ // Pull much of the code from the MonthTS.formatOutput() method.  This
		...
		if ((data_interval_base != TimeInterval.DAY) && (data_interval_base != TimeInterval.HOUR) && (data_interval_base != TimeInterval.MINUTE))
		{
		...
		bool day_is_average = true;
		if ((!string.ReferenceEquals(prop_value, null)) && prop_value.Equals("Total", StringComparison.OrdinalIgnoreCase))
		{
...
//ORIGINAL LINE: double[][] data = new double[num_years][13]; // Monthly values
		double[][] data = RectangularArrays.RectangularDoubleArray(num_years, 13); // Monthly values
//JAVA TO C# CONVERTER NOTE: The following call to the 'RectangularArrays' helper class reproduces the rectangular array initialization that is automatic in Java:
//ORIGINAL LINE: double[][] data_max = new double[num_years][13]; // Max daily value in a month.
		double[][] data_max = RectangularArrays.RectangularDoubleArray(num_years, 13); // Max daily value in a month.
//JAVA TO C# CONVERTER NOTE: The following call to the 'RectangularArrays' helper class reproduces the rectangular array initialization that is automatic in Java:
//ORIGINAL LINE: double[][] data_min = new double[num_years][13]; // Min daily value in a month.
		double[][] data_min = RectangularArrays.RectangularDoubleArray(num_years, 13); // Min daily value in a month.
		...
		bool need_to_process_day = false;
		bool need_to_process_month = false;
	...
	*/
			...
			{
				buffer.Append(StringUtil.formatString(month_value, data_format) + " ");
		...
		if (!day_is_average)
		{
			...
			strings = StringUtil.addListToStringList(strings, createMonthSummaryStats(ts,data,num_years,"Tot ", data_format));
		}
		strings = StringUtil.addListToStringList(strings, createMonthSummaryStats(ts,data,num_years,"Min ", data_format));
		strings = StringUtil.addListToStringList(strings, createMonthSummaryStats(ts,data,num_years,"Max ", data_format));
		strings = StringUtil.addListToStringList(strings, createMonthSummaryStats(ts,data,num_years,"Mean", data_format));
		strings = StringUtil.addListToStringList(strings, createMonthSummaryStats(ts,data,num_years,"SDev", data_format));
		strings = StringUtil.addListToStringList(strings, createMonthSummaryStats(ts,data_max,num_years,"MMxD", data_format));
		strings = StringUtil.addListToStringList(strings, createMonthSummaryStats(ts,data_min,num_years,"MMnD", data_format));
	...
	/// <summary>
	/// Format the output statistics row given for the month summary report for the data array. </summary>
	/// <param name="ts"> Time series that is being processed. </param>
	/// <param name="data"> Data array to process. </param>
	/// <param name="num_years"> Number of years of data. </param>
	/// <param name="label"> Label for the statistics row. </param>
	/// <param name="data_format"> Format like "%9.1f" to format values. </param>
	private static IList<string> createMonthSummaryStats(TS ts, double[][] data, int num_years, string label, string data_format)
	{
		...
		for (column = 0; column < 13; column++)
		{
			...
			if (num_not_missing > 0)
			{
				...
				try
				{
					...
					else if (label.StartsWith("Mean", StringComparison.Ordinal) || label.StartsWith("MM", StringComparison.Ordinal))
					{
						stat = MathUtil.mean(array);
					}
					else if (label.StartsWith("SDev", StringComparison.Ordinal))
					{
						stat = MathUtil.standardDeviation(array);
					}
					else if (label.StartsWith("Tot", StringComparison.Ordinal))
					{
	...
	/// old time series is copied.  The resulting time series can be used for graphing. </summary>
	/// <param name="ts"> Time series for which to create the period of record time series. </param>
	/// <param name="value"> Value to fill the period of record time series. </param>
	/// <exception cref="RTi.TS.TSException"> if there is a problem creating and filling the new time series. </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static TS createPeriodOfRecordTS(TS ts, double value) throws TSException
	...
	/// starting for interval-ending 06, 12, 18, and 00 hours. </summary>
	/// <returns> a new time series that is the disaggregated result. </returns>
	/// <param name="ts"> Time series to disaggregate. </param>
	/// <param name="method"> Method to disaggregate.  If "Ormsbee", use the procedure given
	/// by TVA.  If "Transfer", do a simple transfer of values (currently not implemented). </param>
	/// <param name="new_datatype"> If "" or "*", transfer the datatype from the original
	/// time series.  Otherwise, use the new data type in the disaggregated time series (including the identifier). </param>
	/// <param name="new_units"> If "" or "*", transfer the units from the original
	/// time series.  Otherwise, use the new units in the disaggregated time series. </param>
	/// <param name="req_interval_base"> Interval base to disaggregate to (as per TimeInterval class). </param>
	/// <param name="req_interval_mult"> Interval multiplier to disaggregate to (as per TimeInterval class). </param>
	/// <param name="createData"> if true, then the data space will be allocated and computed; if false, only the main time
	/// series metadata will be created (useful for building workflows without full run) </param>
	/// <exception cref="Exception"> if an error occurs (e.g., improper disaggregation parameters). </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static TS disaggregate(TS ts, String method, String new_datatype, String new_units, int req_interval_base, int req_interval_mult, boolean createData) throws Exception
	public static TS disaggregate(TS ts, string method, string new_datatype, string new_units, int req_interval_base, int req_interval_mult, bool createData)
	{
		...
		if (method.Equals("Ormsbee", StringComparison.OrdinalIgnoreCase))
		{
			...
			if ((!string.ReferenceEquals(new_units, null)) && !new_units.Equals("") && !new_units.Equals("*"))
			{
				newts.setDataUnits(new_units);
			}
			if ((!string.ReferenceEquals(new_datatype, null)) && !new_datatype.Equals("") && !new_datatype.Equals("*"))
			{
			...
			for (; date.lessThanOrEqualTo(end); ++nprocessed, date.addInterval(interval_base, interval_mult))
			{
				...
				if (Message.isDebugOn)
				{
					Message.printStatus(1, routine, "Day data:  " + date + " f[0]=" + f[0] + " f[1]=" + f[1] + " f[2]=" + f[2]);
				}
			...
			newts.addToGenesis("disaggregate: Ormsbee method used to create 6Hour time series" + " from 1Day time series " + ts.getIdentifierString() + ", ");
			newts.addToGenesis("disaggregate:   " + ts.getDescription());
			newts.addToGenesis("disaggregate: First day " + ts.getDate1() + " applied as interval-ending hours 06, 12, 18, and 00");
			return newts;
		}
		else if (method.Equals("SameValue", StringComparison.OrdinalIgnoreCase))
		{
			TS newts = null;
			TSIdent tsident = null;
			if ((interval_base == TimeInterval.YEAR) && (interval_mult == 1) && (req_interval_base == TimeInterval.MONTH) && (req_interval_mult == 1))
			{
				// Year -> Month
			...
			else if ((interval_base == TimeInterval.MONTH) && (interval_mult == 1) && (req_interval_base == TimeInterval.DAY) && (req_interval_mult == 1))
			{
				// Month -> Day
			...
			else if ((interval_base == TimeInterval.DAY) && (interval_mult == 1) && (req_interval_base == TimeInterval.HOUR))
			{
				// Day -> NHour
			...
			else if ((interval_base == TimeInterval.HOUR) && (interval_mult == 1) && (req_interval_base == TimeInterval.MINUTE))
			{
				// Hour -> Nminute
			...
			if (newts != null)
			{
				...
				newts.addToGenesis("disaggregate: SameValue method used to create " + tsident.getInterval() + " time series from " + ts.getIdentifier().getInterval() + " time series " + ts.getIdentifierString() + ",");
				newts.addToGenesis("disaggregate:   " + ts.getDescription());
			}
			return newts;
		}
	...
	/// missing.  If the denominator is zero, the result is set to missing. </summary>
	/// <param name="ts"> Time series to multiply. </param>
	/// <param name="tsd"> Time series to divide by. </param>
	/// <exception cref="Exception"> if there is an error processing. </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static void divide(TS ts, TS tsd) throws Exception
	...
	/// missing.  The units are set to UNITS/UNITS. </summary>
	/// <param name="ts"> Time series to divide. </param>
	/// <param name="tsd"> Time series to divide by. </param>
	/// <param name="start_date"> Date to start divide. </param>
	/// <param name="end_date"> Date to stop divide. </param>
	/// <exception cref="Exception"> if there is an error processing. </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static void divide(TS ts, TS tsd, RTi.Util.Time.DateTime start_date, RTi.Util.Time.DateTime end_date) throws Exception
	public static void divide(TS ts, TS tsd, DateTime start_date, DateTime end_date)
	{
		...
		{
			throw new Exception("Cannot divide data with different intervals (" + ts.getIdentifierString() + " and " + tsd.getIdentifierString() + ").");
		}
		if (interval_base == TimeInterval.IRREGULAR)
		{
		...
		ts.addToGenesis("Divided " + start_date.ToString() + " to " + end_date.ToString() + " by " + tsd.getDescription() + ".");
	...
	/// the maximum allowed values will be set to the maximum. </summary>
	/// <returns> The total number of time steps changed. </returns>
	/// <param name="ts"> Time series to check. </param>
	/// <param name="dates"> list of dates corresponding to the limiting values.  The
	/// date/limit pairs are in effect from the date until the next date encountered. </param>
	/// <param name="max_values"> Data value to check. </param>
	/// <param name="props"> See the overloaded version for a description. </param>
	/// <exception cref="RTi.TS.TSException"> if there is a problem with input. </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static int enforceLimits(TS ts, java.util.List<RTi.Util.Time.DateTime> dates, double [] max_values, RTi.Util.IO.PropList props) throws TSException, Exception
	public static int enforceLimits(TS ts, IList<DateTime> dates, double[] max_values, PropList props)
	{
	...
	/// the maximum allowed values will be set to the maximum. </summary>
	/// <returns> The total number of time steps changed. </returns>
	/// <param name="start_date"> Date to start the enforcement. </param>
	/// <param name="end_date"> Date to end the enforcement. </param>
	/// <param name="ts"> Time series to check. </param>
	/// <param name="limit_dates_given"> list of dates corresponding to the limiting values.  
	/// The
	...
	/// value in ACFT and adjust according to the number of days in each month). </param>
	/// <param name="max_values_given"> Data value to check. </param>
	/// <param name="props"> Property list to control function, as follows:
	/// <table width=100% cellpadding=10 cellspacing=0 border=2>
	/// <tr>
	/// <td><b>Property</b></td>	<td><b>Description</b></td>	<td><b>Default</b></td>
	/// </tr>
	...
	/// <exception cref="RTi.TS.TSException"> if there is a problem with input. </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static int enforceLimits(TS ts, RTi.Util.Time.DateTime start_date, RTi.Util.Time.DateTime end_date, java.util.List<RTi.Util.Time.DateTime> limit_dates_given, double [] max_values_given, RTi.Util.IO.PropList props) throws TSException, Exception
	public static int enforceLimits(TS ts, DateTime start_date, DateTime end_date, IList<DateTime> limit_dates_given, double[] max_values_given, PropList props)
	{
		string routine = "TSUtil.enforceLimits";
		string message;
		double value = 0.0;
		...
		bool scale_by_days = false;
		string SetFlag = null;
		bool SetFlag_boolean = false;
		if (props != null)
		{
			string prop_value = props.getValue("LimitsScaleFactor");
			if ((!string.ReferenceEquals(prop_value, null)) && prop_value.Equals("DaysInMonth", StringComparison.OrdinalIgnoreCase))
			{
				scale_by_days = true;
			}
			...
			if ((!string.ReferenceEquals(SetFlag, null)) && (SetFlag.Length > 0))
			{
				...
				ts.allocateDataFlagSpace(null, true); // Keep old flags if already allocated
			}
		}
		int num_limit_dates = limit_dates_given.Count;
		...
		{
			limit_date = limit_dates_given[i];
		...
		IList<DateTime> limit_dates = new List<DateTime>(num_limit_dates);
		...
		for (int i = 0; i < num_limit_dates; i++)
		{
			limit_dates.Add(limit_dates_given[sort_order[i]]);
		...
		if (Message.isDebugOn)
		{
			for (int i = 0; i < num_limit_dates; i++)
			{
				limit_date = (DateTime)limit_dates[i];
		...
		int limit_pos = 0;
		limit_date = limit_dates[limit_pos];
		...
		{
			next_limit_date = limit_dates[limit_pos + 1];
		}
		else
		{
					...
					if (next_limit_date != null)
					{
						while (true)
						{
							...
							if (next_limit_date.lessThanOrEqualTo(date))
							{
								...
								if ((limit_pos + 1) != num_limit_dates)
								{
									next_limit_date = (DateTime)limit_dates[limit_pos + 1];
								}
								else
								{
									next_limit_date = null;
	...
	/// specified as one of the fill methods to guarantee that missing data are filled. </summary>
	/// <returns> Filled time series. </returns>
	/// <param name="ts"> Time series to fill.  The time series is modified and returned. </param>
	/// <param name="needed_ts"> Time series that are needed for filling (e.g., when regression
	/// methods are used).  If not needed, specify null. </param>
	/// <param name="props"> Properties that specify fill methodology.  The following properties
	...
	/// </para>
	/// </param>
	/// <exception cref="RTi.TS.TSException"> If there is a problem filling the time series
	/// (generally an input problem). </exception>
	...
	*/
	...
	/// <summary>
	/// Version of fill that does not require additional time series. </summary>
	/// <returns> Filled time series. </returns>
	/// <param name="ts"> Time series to fill. </param>
	/// <param name="props"> Property list to control filling (see main version of this method). </param>
	/// <exception cref="RTi.TS.TSException"> If there is a problem filling the time series (generally an input problem). </exception>
	/// @deprecated use specific fill methods because this one is getting too complicated to handle errors. 
	/*
	...
	*/
	...
	/// <summary>
	/// Fill missing data by carrying forward the last known value. </summary>
	/// <param name="ts"> Time series to fill. </param>
	/// <exception cref="RTi.TS.TSException"> if an error occurs (usually null input). </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static void fillCarryForward(TS ts) throws TSException
	...
	/// Fill missing data by carrying forward the last known value. </summary>
	/// <param name="ts"> Time series to fill. </param>
	/// <param name="start_date"> Date to start assignment. </param>
	/// <param name="end_date"> Date to stop assignment. </param>
	/// <exception cref="RTi.TS.TSException"> if there is a problem filling data. </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static void fillCarryForward(TS ts, RTi.Util.Time.DateTime start_date, RTi.Util.Time.DateTime end_date) throws TSException
	public static void fillCarryForward(TS ts, DateTime start_date, DateTime end_date)
	{
		...
		ts.addToGenesis("Filled missing data " + start.ToString() + " to " + end.ToString() + " by carrying forward known values.");
	}

	/// <summary>
	/// Fill missing data in the time series with a constant value. </summary>
	/// <param name="value"> Data value to fill time series. </param>
	/// <param name="ts"> Time series to fill. </param>
	/// @deprecated Use the version that takes a PropList. 
	/// <exception cref="if"> there is an error performing the fill. </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static void fillConstant(TS ts, double value) throws Exception
	...
	/// <exception cref="if"> there is an error performing the fill. </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static void fillConstant(TS ts, RTi.Util.Time.DateTime start_date, RTi.Util.Time.DateTime end_date, double value) throws Exception
	...
	/// <param name="end_date"> Date to stop assignment. </param>
	/// <param name="value"> Data value to fill time series. </param>
	/// <param name="props"> Properties to control the method, as follows:
	/// <table width=100% cellpadding=10 cellspacing=0 border=2>
	/// <tr>
	/// <td><b>Property</b></td>	<td><b>Description</b></td>	<td><b>Default</b></td>
	/// </tr>
	...
	/// <exception cref="if"> there is an error performing the fill. </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static void fillConstant(TS ts, RTi.Util.Time.DateTime start_date, RTi.Util.Time.DateTime end_date, double value, RTi.Util.IO.PropList props) throws Exception
	...
	{ // Get valid dates because the ones passed in may have been null...
		...
		if (nfilled > 0)
		{
			...
			if (FillFlag_boolean)
			{
				ts.addToGenesis("Filled missing data " + start + " to " + end + " using constant " + value + ", flag \"" + FillFlag + "\" uniquetempvar.");
				if ((!string.ReferenceEquals(fillFlagDescription, null)) && fillFlagDescription.Length > 0)
				{
			...
			{
				ts.addToGenesis("Filled missing data " + start + " to " + end + " using constant " + value + " uniquetempvar.");
			}
		}
	}
	...
	/// period with a repetitive monthly pattern. </summary>
	/// <returns> the number of values that were filled. </returns>
	/// <param name="ts"> Time series to fill. </param>
	/// <param name="start_date"> Date to start assignment. </param>
	/// <param name="end_date"> Date to stop assignment. </param>
	/// <param name="values"> Data values to fill time series (the first value is for January, the last is for December). </param>
	/// <param name="descriptionSuffix"> a string to append to the description (specify as a
	/// null string to append default ", fill w/ mon avg"). </param>
	/// <param name="fillFlag"> a string to use as the data flag when a value is filled.  Null will result in no flag being used.
	/// Specify as "auto" to use the month abbreviation and "Avg". </param>
	/// <param name="fillFlagDesc"> the description of the fill flag.  If specified, append to the month abbreviation. </param>
	/// <exception cref="if"> there is an error performing the fill. </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static int fillConstantByMonth(TS ts, RTi.Util.Time.DateTime start_date, RTi.Util.Time.DateTime end_date, double values[], String descriptionSuffix, String fillFlag, String fillFlagDescription) throws Exception
	public static int fillConstantByMonth(TS ts, DateTime start_date, DateTime end_date, double[] values, string descriptionSuffix, string fillFlag, string fillFlagDescription)
	{ // Get valid dates because the ones passed in may have been null...
		...
		if ((!string.ReferenceEquals(fillFlag, null)) && (fillFlag.Length > 0))
		{
			...
			for (int i = 0; i < 12; i++)
			{
				if (fillFlag.Equals("auto", StringComparison.OrdinalIgnoreCase))
				{
			...
			for (; date.lessThanOrEqualTo(end); date.addInterval(interval_base, interval_mult))
			{
				...
				if (ts.isDataMissing(oldvalue))
				{
					if (FillFlag_boolean)
					{
						// Set the data flag, appending to the old value...
						tsdata = ts.getDataPoint(date, tsdata);
						ts.setDataValue(date, values[date.getMonth() - 1], (tsdata.getDataFlag().Trim() + fillFlagByMonth[date.getMonth() - 1]), 1);
					}
					else
					{
						// No data flag...
		...
		ts.addToGenesis("Filled " + nfilledTotal + " total missing data value" + StringUtil.pluralS(nfilledTotal) + " " + start.ToString() + " to " + end.ToString() + " with historical monthly average values:");
		for (int i = 0; i < 12; i++)
		{
			if (!ts.isDataMissing(values[i]))
			{
				ts.addToGenesis("    " + TimeUtil.monthAbbreviation(i + 1) + ": " + StringUtil.formatString(values[i],"%12.4f") + " (filled " + nfilled[i] + " value" + StringUtil.pluralS(nfilled[i]) + ")");
			}
			else
			{
			...
			if (fillFlagByMonth != null)
			{
				...
				if (nfilled[i] > 0)
				{
					...
					if ((!string.ReferenceEquals(fillFlag, null)) && fillFlag.Equals("auto", StringComparison.OrdinalIgnoreCase))
					{
						...
						{
							ts.addDataFlagMetadata(new TSDataFlagMetadata(fillFlagByMonth[i], "Filled " + nfilled[i] + " missing value" + StringUtil.pluralS(nfilled[i]) + " using historical monthly average " + StringUtil.formatString(values[i],"%.4f")));
						}
					}
				}
			}
		}
		if ((fillFlagByMonth != null) && !fillFlag.Equals("auto", StringComparison.OrdinalIgnoreCase))
		{
			...
			{
				ts.addDataFlagMetadata(new TSDataFlagMetadata(fillFlagByMonth[0], "Filled " + nfilledTotal + " missing value" + StringUtil.pluralS(nfilledTotal) + " using historical monthly average values."));
			}
		}
		if (string.ReferenceEquals(descriptionSuffix, null))
		{
			if (nfilledTotal > 0)
			{
	...
	/// <summary>
	/// Fill missing data in a daily time series using D1 = D2 * M1/M2. </summary>
	/// <param name="dayts1"> Daily time series to fill, D1. </param>
	/// <param name="monthts1"> Monthly time series, M1. </param>
	/// <param name="dayts2"> Daily time series, D2. </param>
	/// <param name="monthts2"> Monthly time series, M2. </param>
	/// <exception cref="RTi.TS.TSException"> if an error occurs (usually null input). </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static void fillDayTSFrom2MonthTSAnd1DayTS(DayTS dayts1, MonthTS monthts1, DayTS dayts2, MonthTS monthts2) throws TSException
	...
	/// <exception cref="RTi.TS.TSException"> if there is a problem filling data. </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static void fillDayTSFrom2MonthTSAnd1DayTS(TS dayts1, MonthTS monthts1, DayTS dayts2, MonthTS monthts2, RTi.Util.Time.DateTime start_date, RTi.Util.Time.DateTime end_date) throws TSException
	public static void fillDayTSFrom2MonthTSAnd1DayTS(TS dayts1, MonthTS monthts1, DayTS dayts2, MonthTS monthts2, DateTime start_date, DateTime end_date)
	{
		string routine = "TSUtil.fillDayTSFrom2MonthTSAnd1DayTS";
		...
		dayts1.addToGenesis("Filled " + fill_count + " missing data points " + start.ToString() + " to " + end.ToString() + " using D1 = D2 * M1/M2.");
	}

	/// <summary>
	/// Fill missing data by getting non-missing values from another time series. </summary>
	/// <param name="dependentTS"> Time series to fill. </param>
	/// <param name="independentTS"> Time series to fill from. </param>
	/// <exception cref="RTi.TS.TSException"> if an error occurs (usually null input). </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static void fillFromTS(TS dependentTS, TS independentTS) throws TSException
	...
	/// The data intervals do not need to be the same. </summary>
	/// <param name="dependentTS"> Time series to fill. </param>
	/// <param name="independentTS"> Time series to fill from. </param>
	/// <exception cref="RTi.TS.TSException"> if an error occurs (usually null input). </exception>
	/// <param name="start_date"> Date to start assignment. </param>
	/// <param name="end_date"> Date to stop assignment. </param>
	/// <param name="fillFlag"> a string to use as the data flag when a value is filled (null or empty string result in no flag). </param>
	/// <param name="fillFlagDesc"> the description of the fill flag.  If specified, append to the month abbreviation. </param>
	/// <exception cref="RTi.TS.TSException"> if there is a problem filling data. </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static void fillFromTS(TS dependentTS, TS independentTS, RTi.Util.Time.DateTime fillStart, RTi.Util.Time.DateTime fillEnd, String fillFlag, String fillFlagDescription) throws TSException
	public static void fillFromTS(TS dependentTS, TS independentTS, DateTime fillStart, DateTime fillEnd, string fillFlag, string fillFlagDescription)
	{
		string routine = "TSUtil.fillFromTS";
		string message;
		...
		bool fillFlagSpecified = false; // Indicate whether to use flag
		if ((!string.ReferenceEquals(fillFlag, null)) && (fillFlag.Length > 0))
		{
		...
		dependentTS.addToGenesis("Filled missing data " + start.ToString() + " to " + end.ToString() + " by using known values from " + independentTS.getIdentifierString());
		if (fillFlagSpecified && (!string.ReferenceEquals(fillFlagDescription, null)) && (fillFlagDescription.Length > 0))
		{
	...
	/// <summary>
	/// Fill missing data in the time series using linear interpolation between non-missing values. </summary>
	/// <param name="ts"> Time series to fill. </param>
	/// <param name="start_date"> Date to start assignment. </param>
	/// <param name="end_date"> Date to stop assignment. </param>
	/// <param name="intervals_can_fill"> Number of intervals (width of gap) that can be filled.
	/// If 0 then there is no limit on the number of intervals that can be filled. </param>
	/// <param name="interpolate_method"> Reserved for future use.  Currently always linear. </param>
	/// <exception cref="TSException"> if there is an error filling the time series. </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static void fillInterpolate(TS ts, RTi.Util.Time.DateTime start_date, RTi.Util.Time.DateTime end_date, int intervals_can_fill, int interpolate_method) throws TSException, Exception
	public static void fillInterpolate(TS ts, DateTime start_date, DateTime end_date, int intervals_can_fill, int interpolate_method)
	{
	...
	/// non-missing values.  There is no limit on how many points can be filled between known points. </summary>
	/// <param name="ts"> Time series to fill. </param>
	/// <param name="start_date"> Date to start assignment. </param>
	/// <param name="end_date"> Date to stop assignment. </param>
	/// <exception cref="TSException"> if there is an error filling the time series. </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static void fillInterpolate(TS ts, RTi.Util.Time.DateTime start_date, RTi.Util.Time.DateTime end_date) throws TSException, Exception
	...
	/// <exception cref="TSException"> if there is an error filling the time series. </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static void fillInterpolate(TS ts, RTi.Util.Time.DateTime start_date, RTi.Util.Time.DateTime end_date, RTi.Util.IO.PropList props) throws Exception
	public static void fillInterpolate(TS ts, DateTime start_date, DateTime end_date, PropList props)
	{
		...
		if (!string.ReferenceEquals(MaxIntervals, null))
		{
		...
		else
		{
			...
			bool after_found = false, before_found = false;
			int num_after_intervals = 0, num_before_intervals = 0;
			bool previous_was_filled = false;
			...
			for (; date.lessThanOrEqualTo(end); date.addInterval(interval_base, interval_mult))
			{
				...
				if (ts.isDataMissing(oldvalue))
				{
					...
					if (before_found && after_found && ((intervals_can_fill == 0) || ((num_before_intervals + num_after_intervals - 1) <= intervals_can_fill)))
					{
		...
		ts.addToGenesis("Filled " + fillCount + " missing values " + start + " to " + end + " using linear interpolation from known values, " + maxString);
		if (fillCount > 0)
		{
	...
	/// For example, values[0] is used for any date in January. </summary>
	/// <param name="ts"> Time series to fill. </param>
	/// <param name="values"> Data values to fill time series (the first value is for January, the last is for December). </param>
	...
	/// "wet" year according to the pattern, fill Dec, 1995 with the average of all wet year Decembers. </summary>
	/// <param name="ts"> Time series to fill. </param>
	/// <param name="pattern_ts"> Pattern time series. </param>
	/// <param name="props"> See the full-argument version for information. </param>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static void fillPattern(TS ts, StringMonthTS pattern_ts, RTi.Util.IO.PropList props) throws Exception
	...
	/// <param name="pattern_ts"> Pattern time series. </param>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static void fillPattern(TS ts, StringMonthTS pattern_ts) throws Exception
	...
	/// <param name="ts"> Time series to fill. </param>
	/// <param name="pattern_ts"> Pattern time series. </param>
	/// <param name="pattern_stats"> Time series pattern statistics returned from
	/// TSUtil.getPatternStats.  In general, this is only used by complicated analysis code. </param>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static void fillPattern(TS ts, StringMonthTS pattern_ts, TSPatternStats pattern_stats) throws Exception
	...
	/// <param name="ts"> Time series to fill. </param>
	/// <param name="pattern_ts"> Pattern time series. </param>
	/// <param name="start_date"> Starting date for fill. </param>
	/// <param name="end_date"> Ending date for fill. </param>
	/// <param name="pattern_stats"> Time series pattern statistics returned from
	/// TSUtil.getPatternStats.  In general, this is only used by complicated analysis code. </param>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static void fillPattern(TS ts, StringMonthTS pattern_ts, RTi.Util.Time.DateTime start_date, RTi.Util.Time.DateTime end_date, TSPatternStats pattern_stats) throws Exception
	public static void fillPattern(TS ts, StringMonthTS pattern_ts, DateTime start_date, DateTime end_date, TSPatternStats pattern_stats)
	{
	...
	/// <param name="pattern_stats"> Time series pattern statistics returned from
	/// TSUtil.getPatternStats.  In general, this is only used by complicated analysis code. </param>
	/// <param name="props"> PropList containing modifiers.
	/// <table width=100% cellpadding=10 cellspacing=0 border=2>
	/// <tr>
	/// <td><b>Property</b></td>	<td><b>Description</b></td>	<td><b>Default</b></td>
	/// </tr>
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static void fillPattern(TS ts, StringMonthTS pattern_ts, RTi.Util.Time.DateTime start_date, RTi.Util.Time.DateTime end_date, TSPatternStats pattern_stats, RTi.Util.IO.PropList props) throws Exception
	public static void fillPattern(TS ts, StringMonthTS pattern_ts, DateTime start_date, DateTime end_date, TSPatternStats pattern_stats, PropList props)
	{
		string routine = "TSUtil.fillPattern";
		double fill_value;
		string indicator;
		...
		string FillFlag = props.getValue("FillFlag");
		bool FillFlag_boolean = false; // Indicate whether to use flag
		bool FillFlagAuto_boolean = false;
		if (!string.ReferenceEquals(FillFlag, null))
		{
			FillFlag_boolean = true;
			// Make sure that the data flag is allocated.
			if (FillFlag.Equals("Auto", StringComparison.OrdinalIgnoreCase))
					...
					if (ts.isDataMissing(ts.getDataValue(date)))
					{
						...
						try
						{
							...
							if (!ts.isDataMissing(fill_value))
							{
								...
								if (Message.isDebugOn)
								{
									Message.printDebug(dl, routine, "Filled using pattern \"" + indicator + "\" average: " + fill_value + " at " + date);
								}
								++nfilled;
							}
						}
						...
						{
							Message.printWarning(2, routine, "Unable to get fill value for pattern \"" + indicator + "\" on date " + date);
						}
					}
				}
			}
		...
		ts.addToGenesis("Filled using pattern information for " + start + " to " + end + " using pattern " + pattern_ts.getLocation() + " uniquetempvar.");
	}
	...
	/// when computing the ratio (fillConstant() can be used later to fill with zero if appropriate. </summary>
	/// <param name="ts"> Time series to fill. </param>
	/// <param name="independent_ts"> Independent time series to find non-missing values to compute the ratio. </param>
	/// <param name="start_date"> Date to start assignment. </param>
	/// <param name="end_date"> Date to stop assignment. </param>
	/// <param name="direction"> -1 if filling backward, >= 0 if filling forward. </param>
	/// <param name="initial_value"> Initial value of the fill time series, to compute the ratio
	/// on the end of the period.  See the overloaded method for possible values. </param>
	/// <exception cref="RTi.TS.Exception"> if there is a problem filling data. </exception>
	/// @deprecated Use the PropList version of this method. 
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static void fillProrate(TS ts, TS independent_ts, RTi.Util.Time.DateTime start_date, RTi.Util.Time.DateTime end_date, int direction, double initial_value) throws Exception
	public static void fillProrate(TS ts, TS independent_ts, DateTime start_date, DateTime end_date, int direction, double initial_value)
	{
	...
	/// value is estimated as ts_value = independent_value*(ratio). </summary>
	/// <param name="ts"> Time series to fill. </param>
	/// <param name="independent_ts"> Independent time series to find non-missing values to
	/// compute the ratio. </param>
	/// <param name="start_date"> Date to start assignment. </param>
	/// <param name="end_date"> Date to stop assignment. </param>
	/// <param name="props"> Properties to control the fill, as follows:
	/// <table width=100% cellpadding=10 cellspacing=0 border=2>
	/// <tr>
	/// <td><b>Property</b></td>	<td><b>Description</b></td>	<td><b>Default</b></td>
	/// </tr>
	...
	/// <exception cref="RTi.TS.Exception"> if there is a problem filling data. </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static void fillProrate(TS ts, TS independent_ts, RTi.Util.Time.DateTime start_date, RTi.Util.Time.DateTime end_date, RTi.Util.IO.PropList props) throws Exception
	public static void fillProrate(TS ts, TS independent_ts, DateTime start_date, DateTime end_date, PropList props)
	{
		...
		bool AnalyzeAverage_boolean = false;
		...
		if ((!string.ReferenceEquals(prop, null)) && prop.Equals("AnalyzeAverage", StringComparison.OrdinalIgnoreCase))
		{
			// TODO SAM 2007-03-01 Evaluate use
		...
		bool DivisorIndependentTS_boolean = true; // Divisior is IndependentTS
		prop = props.getValue("Divisor");
		if ((!string.ReferenceEquals(prop, null)) && prop.Equals("TS", StringComparison.OrdinalIgnoreCase))
		{
			DivisorIndependentTS_boolean = false; // Divisior is TS
		}

		bool Forward_boolean = true; // default
		...
		if ((!string.ReferenceEquals(prop, null)) && prop.Equals("Backward", StringComparison.OrdinalIgnoreCase))
		{
			Forward_boolean = false;
		}

		double InitialValue_double = ts.getMissing();
		bool InitialValueDouble_boolean = false;
		string InitialValue = props.getValue("InitialValue");
		if ((!string.ReferenceEquals(InitialValue, null)) && StringUtil.isDouble(prop))
		{
		...
		string FillFlag = props.getValue("FillFlag");
		if ((!string.ReferenceEquals(FillFlag, null)) && (FillFlag.Length > 1))
		{
		...
		if (AnalyzeAverage_boolean)
		{
			...
			int ratio_count = 0;
			for (date = new DateTime(AnalysisStart_DateTime); date.lessThanOrEqualTo(AnalysisEnd_DateTime); date.addInterval(interval_base, interval_mult))
			{
				data_value = ts.getDataValue(date);
			...
			if (ratio_count > 0)
			{
				...
				for (date = new DateTime(FillStart_DateTime); date.lessThanOrEqualTo(FillEnd_DateTime); date.addInterval(interval_base, interval_mult))
				{
					data_value = ts.getDataValue(date);
					independent_data_value = independent_ts.getDataValue(date);
				...
				if (fill_count > 0)
				{
					...
					ts.addToGenesis("Filled " + fill_count + " missing data points " + FillStart_DateTime + " to " + FillEnd_DateTime + " by prorating known values from \"" + independent_ts.getIdentifierString() + "\" " + note + " average factor " + ratio);
				}
			}
		}
		else
		{
			...
			if (Forward_boolean)
			{
				...
				if ((!string.ReferenceEquals(InitialValue, null)) && InitialValue.Equals("NearestForward", StringComparison.OrdinalIgnoreCase))
				{
					for (date = new DateTime(FillStart_DateTime); date.lessThanOrEqualTo(FillEnd_DateTime); date.addInterval(interval_base, interval_mult))
					{
						independent_data_value = independent_ts.getDataValue(date);
						if (!independent_ts.isDataMissing(independent_data_value) && (independent_data_value != 0.0))
						{
							...
							if (!ts.isDataMissing(data_value))
							{
								ratio = data_value / independent_data_value;
								ratio_found = true;
								Message.printStatus(2, routine, loc + " " + dt + " Ratio ts/indepts computed on " + date + " " + StringUtil.formatString(data_value,"%.6f") + "/" + StringUtil.formatString(independent_data_value,"%.6f") + "=" + StringUtil.formatString(ratio,"%.6f"));
								break;
							}
							...
							if (InitialValueDouble_boolean)
							{
								...
								ratio_found = true;
								Message.printStatus(2,routine, loc + " " + dt + " Ratio ts/indepts computed on " + date + " " + StringUtil.formatString(InitialValue_double,"%.6f") + "/" + StringUtil.formatString(independent_data_value,"%.6f") + "=" + StringUtil.formatString(ratio,"%.6f"));
								break;
							}
							// Else, keep searching...
						}
				...
				else if ((!string.ReferenceEquals(InitialValue, null)) && InitialValue.Equals("NearestBackward", StringComparison.OrdinalIgnoreCase))
				{
					// Search backward from the fill start to the start of the time series...
					DateTime date1 = ts.getDate1();
					for (date = new DateTime(FillStart_DateTime); date.greaterThanOrEqualTo(date1); date.addInterval(interval_base,-interval_mult))
					{
						independent_data_value = independent_ts.getDataValue(date);
						if (!independent_ts.isDataMissing(independent_data_value) && (independent_data_value != 0.0))
						{
				...
				for (date = new DateTime(FillStart_DateTime); date.lessThanOrEqualTo(FillEnd_DateTime); date.addInterval(interval_base, interval_mult))
				{
					...
					if (ts.isDataMissing(data_value))
					{
						...
						if (ratio_found && !independent_ts.isDataMissing(independent_data_value))
						{
							...
							// For development...
							Message.printStatus(2, routine, "Filling " + date + " with val*ratio " + StringUtil.formatString(independent_data_value,"%.6f") + "*" + StringUtil.formatString(ratio,"%.6f") + "=" + StringUtil.formatString(independent_data_value * ratio,"%.6f"));
							++fill_count;
						}
					}
					else
			...
			else
			{
				...
				if ((!string.ReferenceEquals(InitialValue, null)) && InitialValue.Equals("NearestBackward", StringComparison.OrdinalIgnoreCase))
				{
					for (date = new DateTime(FillEnd_DateTime); date.greaterThanOrEqualTo(FillStart_DateTime); date.addInterval(interval_base,-interval_mult))
					{
						independent_data_value = independent_ts.getDataValue(date);
						if (!independent_ts.isDataMissing(independent_data_value) && (independent_data_value != 0.0))
						{
							...
							if (!ts.isDataMissing(data_value))
							{
								ratio = data_value / independent_data_value;
								// For development...
								Message.printStatus(2,routine, "Ratio ts/indepts computed on " + date + " " + StringUtil.formatString(data_value,"%.6f") + "/" + StringUtil.formatString(independent_data_value,"%.6f") + "=" + StringUtil.formatString(ratio,"%.6f"));
								ratio_found = true;
								break;
							}
							// Next, if an initial value is given for ts, use it...
							if (InitialValueDouble_boolean)
							{
								ratio = InitialValue_double / independent_data_value;
								// For development...
								Message.printStatus(2,routine, "Ratio ts/indepts computed on " + date + " " + StringUtil.formatString(InitialValue_double,"%.6f") + "/" + StringUtil.formatString(independent_data_value,"%.6f") + "=" + StringUtil.formatString(ratio,"%.6f"));
								ratio_found = true;
								break;
							}
							// Else, keep searching...
				...
				else if ((!string.ReferenceEquals(InitialValue, null)) && InitialValue.Equals("NearestForward", StringComparison.OrdinalIgnoreCase))
				{
					// Search foreward from the fill end to the end of the time series...
					DateTime date2 = ts.getDate2();
					for (date = new DateTime(FillEnd_DateTime); date.lessThanOrEqualTo(date2); date.addInterval(interval_base, interval_mult))
					{
						independent_data_value = independent_ts.getDataValue(date);
						if (!independent_ts.isDataMissing(independent_data_value) && (independent_data_value != 0.0))
						{
			...
			if (fill_count > 0)
			{
				if (Forward_boolean)
				{
					...
					ts.addToGenesis("Filled " + fill_count + " missing data points " + FillStart_DateTime + " to " + FillEnd_DateTime + " by prorating known values (forward) from \"" + independent_ts.getIdentifierString() + "\"");
				}
				else
				{
					ts.setDescription(ts.getDescription() + ", fill prorate backward");
					ts.addToGenesis("Filled " + fill_count + " missing data points " + FillStart_DateTime + " to " + FillEnd_DateTime + " by prorating known values (backward) from \"" + independent_ts.getIdentifierString() + "\"");
				}
			}
		} // NearestPoint
	}
	...
	/// <summary>
	/// Fill the missing data within a time series with values based on ordinary least squares, MOVE1, or MOVE2. </summary>
	/// <returns> An instance of TSRegression containing the regression information
	/// (references to inputs and statistical results). </returns>
	/// <param name="tsToFill"> Time series to fill. </param>
	/// <param name="tsIndependent"> Independent time series. </param>
	/// <param name="tsRegression"> A previously computed TSRegression object - if specified as non-null it will be
	/// used rather than computing the regression information from other parameters.  This may be useful for performance
	/// reasons and memory management. </param>
	/// <param name="analysisMethod"> analysis method to use for the analysis </param>
	/// <param name="numberOfEquatiosn"> the number of equations (1 or 12 for monthly analysis) </param>
	/// <param name="intercept"> can be set to zero for OLS regression, ignored otherwise </param>
	/// <param name="analysisMonths"> an array of months (values 1 to 12) that indicate the months to analyze.  If one equation
	/// is used, then only data for the specified months will be used. </param>
	/// <param name="transformation"> the transformation to apply to data. </param>
	/// <param name="leZeroLogValue"> if the log tranform is used this is the value used to replace values <= zero (if null
	/// use the default given by TSRegression.getDefaultLEZeroLogValue() and NaN will cause the values to be discarded. </param>
	/// <param name="minimumDataCount"> the minimum number of overlapping values in a sample to allow filling.  If null,
	/// don't have a minimum (other than constraints calculating the statistics). </param>
	/// <param name="minimumR"> the minimum correlation coefficient to have a relationship and allow filling.  If null,
	/// don't have a minimum. </param>
	...
	/// <param name="dependentAnalysisStart"> date/time to start the analysis of the dependent time series. </param>
	/// <param name="dependentAnalysisEnd"> date/time to end the analysis of the dependent time series. </param>
	/// <param name="independentAnalysisStart"> date/time to start the analysis of the independent time series. </param>
	/// <param name="independentAnalysisEnd"> date/time to end the analysis of the independent time series. </param>
	/// <param name="fillStart"> Date/time to start filling (this is used for the
	/// analysis period for MOVE1 and OLS - MOVE2 uses TSRegression properties for analysis periods). </param>
	/// <param name="fillEnd"> Date/time to end filling (this is used for the
	/// analysis period for MOVE1 and OLS - MOVE2 uses TSRegression properties for analysis periods). </param>
	/// <param name="fillFlag"> a string to mark filled values with </param>
	/// <param name="descriptionString"> a string to append to the existing time series description. </param>
	/// <param name="doFill"> if true, then fill the depending time series; if false, compute the relationships and
	/// statistics, but do not actually fill the time series </param>
	/// <exception cref="RTi.TS.TSException"> if there is a problem performing regression. </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static TSRegression fillRegress(TS tsToFill, TS tsIndependent, TSRegression tsRegression, RTi.Util.Math.RegressionType analysisMethod, RTi.Util.Math.NumberOfEquationsType numberOfEquations, System.Nullable<double> intercept, int [] analysisMonths, RTi.Util.Math.DataTransformationType transformation, System.Nullable<double> leZeroLogValue, System.Nullable<int> minimumDataCount, System.Nullable<double> minimumR, System.Nullable<double> confidenceInterval, RTi.Util.Time.DateTime dependentAnalysisStart, RTi.Util.Time.DateTime dependentAnalysisEnd, RTi.Util.Time.DateTime independentAnalysisStart, RTi.Util.Time.DateTime independentAnalysisEnd, RTi.Util.Time.DateTime fillStart, RTi.Util.Time.DateTime fillEnd, String fillFlag, String descriptionString, boolean doFill) throws TSException, Exception
	public static TSRegression fillRegress(TS tsToFill, TS tsIndependent, TSRegression tsRegression, RegressionType analysisMethod, NumberOfEquationsType numberOfEquations, double? intercept, int[] analysisMonths, DataTransformationType transformation, double? leZeroLogValue, int? minimumDataCount, double? minimumR, double? confidenceInterval, DateTime dependentAnalysisStart, DateTime dependentAnalysisEnd, DateTime independentAnalysisStart, DateTime independentAnalysisEnd, DateTime fillStart, DateTime fillEnd, string fillFlag, string descriptionString, bool doFill)
	{
		...
		if ((interval_base != tsIndependent.getDataIntervalBase()) || (interval_mult != tsIndependent.getDataIntervalMult()))
		{
		...
		{
			return fillRegressMonthly(tsToFill, tsIndependent, tsRegression, analysisMethod, intercept, analysisMonths, transformation, leZeroLogValue, minimumDataCount, minimumR, confidenceInterval, dependentAnalysisStart, dependentAnalysisEnd, independentAnalysisStart, independentAnalysisEnd, fillStart, fillEnd, fillFlag, descriptionString, doFill);
		}
		else
		{
			return fillRegressTotal(tsToFill, tsIndependent, tsRegression, analysisMethod, intercept, analysisMonths, transformation, leZeroLogValue, minimumDataCount, minimumR, confidenceInterval, dependentAnalysisStart, dependentAnalysisEnd, independentAnalysisStart, independentAnalysisEnd, fillStart, fillEnd, fillFlag, descriptionString, doFill);
		}
	}

	/// <summary>
	/// Fill the missing data within the time series with values based on monthly
	/// equations.  Twelve relationships are determined. </summary>
	/// <param name="tsToFill"> Time series to fill. </param>
	/// <param name="tsIndependent"> Independent time series. </param>
	/// <param name="tsRegression"> if null, then regression relationships will be determined using the specified parameters.
	/// If not null, an existing regression relationship will be used.  This allows a previously calculated A, B to be
	/// used rather than recalculating using data that may have been filled in some way. </param>
	/// <param name="analysisMethod"> the regression analysis method to use </param>
	/// <param name="intercept"> can be specified as zero for OLS regression to force the relationship through zero, not used with
	/// other methods </param>
	/// <param name="analysisMonths"> if using one relationship, this specifies that only the specified months (array of values 1-12)
	/// should be considered to develop the relationship, and only those months will be filled.  If monthly relationships, only
	/// the specific months will be analyzed and filled. </param>
	/// <param name="transformation"> the data transformation to apply before analyzing. </param>
	/// <param name="leZeroLogValue"> if the log tranform is used this is the value used to replace values <= zero (if null
	/// use the default given by TSRegression.getDefaultLEZeroLogValue() and NaN will cause the values to be discarded. </param>
	/// <param name="minimumDataCount"> the minimum number of overlapping values in a sample to allow filling.  If null,
	/// don't have a minimum (other than constraints calculating the statistics). </param>
	...
	/// corresponding level T-test (not a strong relationship) are not used.  If null, the confidence interval
	/// is not examined. </param>
	/// <param name="dependentAnalysisStart"> the analysis start for the dependent time series, used by all methods. </param>
	/// <param name="dependentAnalysisEnd"> the analysis end for the dependent time series, used by all methods. </param>
	/// <param name="independentAnalysisStart"> the analysis start for the independent time series, used by the MOVE2 method. </param>
	/// <param name="independentAnalysisEnd"> the analysis end for the independent time series, used by the MOVE2 method. </param>
	/// <param name="fillStart"> date/time to start filling </param>
	/// <param name="fillEnd"> date/time to end filling </param>
	/// <param name="fillFlag"> the flag to use for filled data values </param>
	/// <param name="descriptionString"> the string to append to the time series description, if any values are filled </param>
	/// <param name="doFill"> if true, then fill the depending time series; if false, compute the relationships and
	/// statistics, but do not actually fill the time series </param>
	/// <exception cref="Exception"> if there is an error performing the regression. </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: private static TSRegression fillRegressMonthly(TS tsToFill, TS tsIndependent, TSRegression tsRegression, RTi.Util.Math.RegressionType analysisMethod, System.Nullable<double> intercept, int [] analysisMonths, RTi.Util.Math.DataTransformationType transformation, System.Nullable<double> leZeroLogValue, System.Nullable<int> minimumDataCount, System.Nullable<double> minimumR, System.Nullable<double> confidenceInterval, RTi.Util.Time.DateTime dependentAnalysisStart, RTi.Util.Time.DateTime dependentAnalysisEnd, RTi.Util.Time.DateTime independentAnalysisStart, RTi.Util.Time.DateTime independentAnalysisEnd, RTi.Util.Time.DateTime fillStart, RTi.Util.Time.DateTime fillEnd, String fillFlag, String descriptionString, boolean doFill) throws TSException, Exception
	private static TSRegression fillRegressMonthly(TS tsToFill, TS tsIndependent, TSRegression tsRegression, RegressionType analysisMethod, double? intercept, int[] analysisMonths, DataTransformationType transformation, double? leZeroLogValue, int? minimumDataCount, double? minimumR, double? confidenceInterval, DateTime dependentAnalysisStart, DateTime dependentAnalysisEnd, DateTime independentAnalysisStart, DateTime independentAnalysisEnd, DateTime fillStart, DateTime fillEnd, string fillFlag, string descriptionString, bool doFill)
	{
		...
		if ((!string.ReferenceEquals(fillFlag, null)) && (fillFlag.Length > 0))
		{
			fillFlag_boolean = true;
			// Make sure that the data flag is allocated.
			tsToFill.allocateDataFlagSpace(null, true); // Keep old flags if already allocated
		}
		...
		else
		{
			...
			try
			{
				rd = new TSRegression(tsIndependent, tsToFill, true, analysisMethod, intercept, NumberOfEquationsType.MONTHLY_EQUATIONS, analysisMonths, transformation, leZeroLogValue, confidenceInterval, dependentAnalysisStart, dependentAnalysisEnd, independentAnalysisStart, independentAnalysisEnd, fillStart, fillEnd);
			}
			catch (Exception e)
			{
		...
		bool foundMonth = false; // Used for analysisMonths check
		for (DateTime date = new DateTime(start); date.lessThanOrEqualTo(end); date.addInterval(interval_base, interval_mult))
		{
			try
			{
		...
		if (fillCount > 0)
		{
			...
			IList<string> strings = StringUtil.breakStringList(rd.ToString(), System.getProperty("line.separator"), StringUtil.DELIM_SKIP_BLANKS);
			if (strings != null)
			{
				int size = strings.Count;
				for (int j = 0; j < size; j++)
				{
					tsToFill.addToGenesis((string)strings[j]);
				}
			}
			...
			else
			{
				...
				else
				{
					...
					if (regressLog)
					{
						tsToFill.setDescription(tsToFill.getDescription() + ", fill log " + analysisMethod + " " + monthString + " using " + tsIndependent.getIdentifierString());
					}
					else
					{
						tsToFill.setDescription(tsToFill.getDescription() + ", fill " + analysisMethod + " " + monthString + " using " + tsIndependent.getIdentifierString());
					}
				}
			}
		}
	...
	/// MOVE1, or MOVE2 analysis. </summary>
	/// <param name="tsToFill"> Time series to fill. </param>
	/// <param name="tsIndependent"> Independent time series. </param>
	/// <param name="leZeroLogValue"> if the log tranform is used this is the value used to replace values <= zero (if null
	/// use the default given by TSRegression.getDefaultLEZeroLogValue() and NaN will cause the values to be discarded. </param>
	...
	/// corresponding level T-test (not a strong relationship) are not used.  If null, the confidence interval
	/// is not examined. </param>
	/// <param name="fill_period_start"> Date/time to start filling (this is used for the
	/// analysis period for MOVE1 and OLS - MOVE2 uses TSRegression properties for analysis periods). </param>
	/// <param name="fill_period_end"> Date/time to end filling (this is used for the
	/// analysis period for MOVE1 and OLS - MOVE2 uses TSRegression properties for analysis periods). </param>
	/// <param name="prop_list"> Properties to control filling.  See the TSRegression properties. </param>
	/// <param name="descriptionString"> will be appended to the description (if not set, "fill regress using TSID" or
	/// "fill log regress using TSID" will be used). </param>
	/// <param name="doFill"> if true, then fill the depending time series; if false, compute the relationships and
	/// statistics, but do not actually fill the time series </param>
	/// <exception cref="Exception"> if there is a problem doing regression. </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: private static TSRegression fillRegressTotal(TS tsToFill, TS tsIndependent, TSRegression tsRegression, RTi.Util.Math.RegressionType analysisMethod, System.Nullable<double> intercept, int [] analysisMonths, RTi.Util.Math.DataTransformationType transformation, System.Nullable<double> leZeroLogValue, System.Nullable<int> minimumDataCount, System.Nullable<double> minimumR, System.Nullable<double> confidenceInterval, RTi.Util.Time.DateTime dependentAnalysisStart, RTi.Util.Time.DateTime dependentAnalysisEnd, RTi.Util.Time.DateTime independentAnalysisStart, RTi.Util.Time.DateTime independentAnalysisEnd, RTi.Util.Time.DateTime fillStart, RTi.Util.Time.DateTime fillEnd, String fillFlag, String descriptionString, boolean doFill) throws TSException, Exception
	private static TSRegression fillRegressTotal(TS tsToFill, TS tsIndependent, TSRegression tsRegression, RegressionType analysisMethod, double? intercept, int[] analysisMonths, DataTransformationType transformation, double? leZeroLogValue, int? minimumDataCount, double? minimumR, double? confidenceInterval, DateTime dependentAnalysisStart, DateTime dependentAnalysisEnd, DateTime independentAnalysisStart, DateTime independentAnalysisEnd, DateTime fillStart, DateTime fillEnd, string fillFlag, string descriptionString, bool doFill)
	{
		...
		else
		{
			...
			try
			{
				rd = new TSRegression(tsIndependent, tsToFill, true, analysisMethod, intercept, NumberOfEquationsType.ONE_EQUATION, analysisMonths, transformation, leZeroLogValue, confidenceInterval, dependentAnalysisStart, dependentAnalysisEnd, independentAnalysisStart, independentAnalysisEnd, fillStart, fillEnd);
			}
			catch (Exception e)
			{
		...
		if (fillCount > 0)
		{
			...
			if (strings != null)
			{
				...
				for (int j = 0; j < size; j++)
				{
					tsToFill.addToGenesis(strings[j]);
				}
			}
	...
	/// <summary>
	/// Fill missing data by repeating the last known value, processing either forward or backward. </summary>
	/// <param name="ts"> Time series to fill. </param>
	/// <param name="start_date"> Date to start assignment. </param>
	/// <param name="end_date"> Date to stop assignment. </param>
	/// <param name="direction"> -1 if filling backward, >= 0 if filling forward. </param>
	/// <param name="max_intervals"> Maximum number of intervals to fill in a gap.  Zero indicates to fill all (no maximum). </param>
	/// <param name="fillFlag"> flag to mark values that were filled, null or blank to not mark </param>
	/// <exception cref="RTi.TS.TSException"> if there is a problem filling data. </exception>
	public static void fillRepeat(TS ts, DateTime start_date, DateTime end_date, int direction, int max_intervals, string fillFlag)
	{
		string routine = "TSUtil.fillRepeat";
		string message;
		if ((!string.ReferenceEquals(fillFlag, null)) && fillFlag.Equals(""))
		{
		...
		{
			// Iterate forward...
			for (date = new DateTime(start); date.lessThanOrEqualTo(end); date.addInterval(interval_base, interval_mult))
			{
				data_value = ts.getDataValue(date);
				if (ts.isDataMissing(data_value))
				{
		...
		{
			// Iterate backward...
			for (date = new DateTime(end); date.greaterThanOrEqualTo(start); date.addInterval(interval_base, -interval_mult))
			{
				data_value = ts.getDataValue(date);
				if (ts.isDataMissing(data_value))
				{
	...
	/// <returns> The nearest non-missing data value.  The search will not go past the
	/// end of the time series.  Return the missing data value if no non-missing value
	/// can be found.  If an irregular time series, always return missing (not implemented for irregular time series). </returns>
	/// <param name="ts"> Time series to search. </param>
	/// <param name="date"> Starting date for search. </param>
	/// <param name="direction"> If zero or negative, then for each time step search backwards
	/// first.  If positive, search forward first. </param>
	/// <param name="nforward"> Indicates how many time steps to search forward.  If zero, there
	/// is no limitation on how far forward to search. </param>
	/// <param name="nback"> Indicates how many time steps to search backward.  If zero, there
	/// is no limitation on how far back to search. </param>
	public static double findNearestDataValue(TS ts, DateTime date, int direction, int nforward, int nback)
	{
		...
		if (ts.getDataIntervalBase() == TimeInterval.IRREGULAR)
		{
			throw new IrregularTimeSeriesNotSupportedException("Unable to find data nearest " + date + " for irregular time series " + ts.getIdentifierString());
		}
		...
		while (true)
		{
			...
			if (forward_date.greaterThan(date2) || ((nforward != 0) && (count > nforward)))
			{
				forward_done = true;
			}
			if (back_date.lessThan(date1) || ((nback != 0) && (count > nback)))
			{
				back_done = true;
			}
	...
	/// there are no non-missing data points found then null is returned. </summary>
	/// <param name="inputTS"> Time series object to iterate. </param>
	/// <param name="date1"> First DateTime of the iteration. If null, uses start date set for the given TS. </param>
	/// <param name="date2"> Last DateTime of the iteration.  If null, uses end date set for the given TS. </param>
	/// <param name="reverse"> If true then the iteration will go from date2 to date1.  If
	/// false then iteration starts at date1 and finishes at date2.
	/// @return </param>
	public static TSData findNearestDataPoint(TS inputTS, DateTime date1, DateTime date2, bool reverse)
	{
		if (inputTS == null)
		{
			return null;
	...
	/// </summary>
	public static int findTSIndex(IList<TS> tsVector, string key, PropList prop_list)
	{
		PropList props;
		if (prop_list == null)
		{
		...
		if (!string.ReferenceEquals(prop_value, null) && prop_value.Equals("true"))
		{
		...
		int size = tsVector.Count;
		for (int i = 0; i < size; i++)
		{
			TS ts = (TS)tsVector[i];
			if (matchFullID)
			{
				if (ts.getIdentifierString().Equals(token, StringComparison.OrdinalIgnoreCase))
				{
					return i;
				}
			}
			else if (ts.getLocation().Equals(token, StringComparison.OrdinalIgnoreCase))
			{
				return i;
			}
		}
	...
	/// series (some of which may not be implemented at this time).  Null time series are skipped. </summary>
	/// <returns> The lsit of strings for the complete output. </returns>
	/// <param name="tslist"> List of time series to format. </param>
	/// <param name="proplist"> List of properties to modify output.  This list will be passed
	...
	/// </para>
	/// </param>
	/// <seealso cref= MinuteTS#formatOutput </seealso>
	/// <seealso cref= MonthTS#formatOutput </seealso>
	/// <seealso cref= HourTS#formatOutput </seealso>
	/// <seealso cref= YearTS#formatOutput </seealso>
	/// <seealso cref= IrregularTS#formatOutput </seealso>
	/// <exception cref="RTi.TS.TSException"> Thrown if a lower-level routine throws an exception. </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static java.util.List<String> formatOutput(java.util.List<TS> tslist, RTi.Util.IO.PropList proplist) throws TSException
	public static IList<string> formatOutput(IList<TS> tslist, PropList proplist)
	{
	...
	/// <summary>
	/// Version to take a single time series. </summary>
	/// <param name="ts"> Time series to format. </param>
	/// <param name="proplist"> Property list to control formatting. </param>
	/// <exception cref="TSException"> if there is a problem formatting output. </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static java.util.List<String> formatOutput(TS ts, RTi.Util.IO.PropList proplist) throws TSException
	public static IList<string> formatOutput(TS ts, PropList proplist)
	{
		// Call the main utility version...
	...
	/// <summary>
	/// Version to write output to a file. </summary>
	/// <returns> List of formatted output. </returns>
	/// <param name="fname"> Name of file to receive output.  File is closed at output. </param>
	/// <param name="tslist"> list of time series to process. </param>
	/// <param name="proplist"> Properties to modify output. </param>
	/// <exception cref="RTi.TS.TSException"> Thrown if there is an exception in the low-level code. </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static java.util.List<String> formatOutput(String fname, java.util.List<TS> tslist, RTi.Util.IO.PropList proplist) throws TSException
	public static IList<string> formatOutput(string fname, IList<TS> tslist, PropList proplist)
	{
		// Call the main utility version...
	...
	/// <summary>
	/// Version to write output to a Writer. </summary>
	/// <returns> List of formatted output. </returns>
	/// <param name="fp"> PrintWrither to receive output. </param>
	/// <param name="tslist"> list of time series to process. </param>
	/// <param name="proplist"> Properties to modify output. </param>
	/// <exception cref="RTi.TS.TSException"> Thrown if there is an exception in the low-level code. </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static java.util.List<String> formatOutput(java.io.PrintWriter fp, java.util.List<TS> tslist, RTi.Util.IO.PropList proplist) throws TSException
	public static IList<string> formatOutput(PrintWriter fp, IList<TS> tslist, PropList proplist)
	{
		// Call the main utility version...
	...
	/// <summary>
	/// This is the main routine for formatOutput. </summary>
	/// <param name="fp"> PrintWriter to receive output.  If not null, then output will be written.  If "fp" is null but "fname" is
	/// not, then a writer will be opened.  If both are null, just generate the strings.  This should give a enough flexibility. </param>
	/// <param name="fname"> File name to write. </param>
	/// <param name="tslist"> List of time series to output. </param>
	/// <param name="proplist"> Properties to format the output. </param>
	/// <exception cref="RTi.TS.TSException"> Thrown if there is an exception in the low-level routines. </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: private static java.util.List<String> formatOutput(java.io.PrintWriter fp, String fname, java.util.List<TS> tslist, RTi.Util.IO.PropList proplist) throws TSException
	private static IList<string> formatOutput(PrintWriter fp, string fname, IList<TS> tslist, PropList proplist)
	{
		...
		bool opened_here = false; // Indicates whether Writer was opened in this routine.
		if (fp == null)
		{
			// Check to see if a filename has been specified...
			if (!string.ReferenceEquals(full_fname, null))
			{
				try
				{
					fp = new PrintWriter(new StreamWriter(full_fname));
					opened_here = true;
				}
				catch (Exception)
				{
	...
	/// <returns> The data limits for a time series between two dates. </returns>
	/// <param name="ts"> Time series of interest. </param>
	/// <param name="start"> Starting date for the check. </param>
	/// <param name="end"> Ending date for the check. </param>
	/// <seealso cref= TSLimits </seealso>
	...
	/// refresh() method, the flag should be false. </param>
	/// <seealso cref= TSLimits </seealso>
	protected internal static TSLimits getDataLimits(TS ts, DateTime start0, DateTime end0, bool refresh_flag)
	{
	...
	/// <summary>
	/// Determine the overall data limits for a list of time series. </summary>
	/// <returns> The overall data limits for a list of time series between two dates.
	/// The dates in the limits will be for the data, not the dates that are passed in. </returns>
	/// <param name="tslist"> List of time series of interest. </param>
	/// <param name="start"> Starting date for the check. </param>
	/// <param name="end"> Ending date for the check. </param>
	/// <param name="req_units"> Units to use for check.  If not specified, the units from the first time series will be used. </param>
	/// <param name="refresh_flag"> Indicates whether the time series should be refreshed first
	/// (in general this is used only within the TS package and the version of this
	/// routine without the flag should be called).  Specifying true will result in slower execution. </param>
	/// <exception cref="Exception"> If the data limits cannot be found. </exception>
	/// <seealso cref= TSLimits </seealso>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static TSLimits getDataLimits(java.util.List<TS> tslist, RTi.Util.Time.DateTime start, RTi.Util.Time.DateTime end, String req_units, boolean refresh_flag) throws Exception
	public static TSLimits getDataLimits(IList<TS> tslist, DateTime start, DateTime end, string req_units, bool refresh_flag)
	{
	...
	/// The dates in the limits will be for the data, not the dates that are passed in. </returns>
	/// <param name="tslist"> List of time series of interest. </param>
	/// <param name="start"> Starting date for the check, or null to evaluate the full period for each time series. </param>
	/// <param name="end"> Ending date for the check, or null to evaluate the full period for each time series. </param>
	/// <param name="req_units"> Units to use for check.  If not specified, the units from the
	/// first non-null time series will be used. </param>
	/// <param name="refresh_flag"> Indicates whether the time series should be refreshed first
	/// (in general this is used only within the TS package and the version of this
	/// routine without the flag should be called).  Specifying true will result in slower execution. </param>
	/// <param name="ignore_units"> Indicates whether units should be ignored.  This is suitable
	/// for getting the overall data limits for graphs where units can be ignored. </param>
	/// <exception cref="Exception"> If the data limits cannot be found. </exception>
	/// <seealso cref= TSLimits </seealso>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static TSLimits getDataLimits(java.util.List<TS> tslist, RTi.Util.Time.DateTime start, RTi.Util.Time.DateTime end, String req_units, boolean refresh_flag, boolean ignore_units) throws Exception
	public static TSLimits getDataLimits(IList<TS> tslist, DateTime start, DateTime end, string req_units, bool refresh_flag, bool ignore_units)
	{
		...
		if (!ignore_units)
		{
			if ((string.ReferenceEquals(req_units, null)) || req_units.Equals(""))
			{
				for (int i = 0; i < size; i++)
				{
					ts = tslist[i];
	...
	/// <param name="start"> Starting date for the check. </param>
	/// <param name="end"> Ending date for the check. </param>
	/// <param name="units"> Units to use for check. </param>
	/// <exception cref="Exception"> If null data prevent limits from being computed. </exception>
	/// <seealso cref= TSLimits </seealso>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static TSLimits getDataLimits(java.util.List<TS> tslist, RTi.Util.Time.DateTime start, RTi.Util.Time.DateTime end, String units) throws Exception
	public static TSLimits getDataLimits(IList<TS> tslist, DateTime start, DateTime end, string units)
	{
	...
	/// <summary>
	/// Determine the overall data limits for a list of time series. </summary>
	/// <returns> The overall data limits for a list of time series. </returns>
	/// <param name="tslist"> List of time series of interest. </param>
	/// <param name="refresh_flag"> Indicates whether the time series should be refreshed first
	/// (in general this is used only within the TS package and the version of this
	/// routine without the flag should be called).  Specifying true will result in slower execution. </param>
	...
	/// <seealso cref= TSLimits </seealso>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static TSLimits getDataLimits(java.util.List<TS> tslist, boolean refresh_flag) throws Exception
	public static TSLimits getDataLimits(IList<TS> tslist, bool refresh_flag)
	{
	...
	/// <seealso cref= TSLimits </seealso>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static TSLimits getDataLimits(java.util.List<TS> tslist) throws Exception
	public static TSLimits getDataLimits(IList<TS> tslist)
	{
	...
	*/
	...
	/// some point.  It is used in the State of Colorado StateDMI software. </summary>
	/// <returns> An instance of MonthTotals containing the monthly totals for the time series. </returns>
	/// <param name="ts"> Time series to evaluate. </param>
	/// <exception cref="RTi.TS.TSException"> if there is a problem determining the totals. </exception>
	/// <seealso cref= MonthTotals </seealso>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static MonthTotals getMonthTotals(TS ts) throws TSException
	...
	/// This routine is a port of the legacy TSMonthTotals and may be deprecated at
	/// some point.  It is used in the State of Colorado StateDMI software. </summary>
	/// <returns> An instance of MonthTotals containing the monthly totals for the
	/// time series.  Data values (sums and averages) are initialized with the missing data value for the time series. </returns>
	/// <param name="ts"> Time series to evaluate. </param>
	/// <param name="date1"> Starting date to evaluate for totals. </param>
	/// <param name="date2"> Ending date to evaluate for totals. </param>
	/// <exception cref="RTi.TS.TSException"> if there is a problem determining the totals. </exception>
	/// <seealso cref= MonthTotals </seealso>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static MonthTotals getMonthTotals(TS ts, RTi.Util.Time.DateTime date1, RTi.Util.Time.DateTime date2) throws TSException
	...
	/// <param name="date1"> Starting date to evaluate for totals. </param>
	/// <param name="date2"> Ending date to evaluate for totals. </param>
	/// <param name="reference_ts"> If specified and the CheckReferencTS property is true, then
	...
	/// for diversion and demand time series. </param>
	/// <param name="props"> Properties to control execution, as follows:
	/// <table width=100% cellpadding=10 cellspacing=0 border=2>
	/// <tr>
	/// <td><b>Property</b></td>	<td><b>Description</b></td>	<td><b>Default</b></td>
	/// </tr>
	...
	/// <seealso cref= MonthTotals </seealso>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static MonthTotals getMonthTotals(TS ts, RTi.Util.Time.DateTime date1, RTi.Util.Time.DateTime date2, TS reference_ts, RTi.Util.IO.PropList props) throws TSException
	public static MonthTotals getMonthTotals(TS ts, DateTime date1, DateTime date2, TS reference_ts, PropList props)
	{
		...
		bool check_ref_ts = false;
		bool ignore_le_zero = false;
	...
	/// the full time series identifier string will be returned. </summary>
	/// <param name="tsList"> list of time series to process. </param>
	/// <param name="includeInput"> if true, include the input part of the time series identifier.
	/// If false, only include the 5-part information. </param>
	/// <returns> the list of time series identifier strings, useful for choices in applications - a non-null list is guaranteed. </returns>
	public static IList<string> getTimeSeriesIdentifiers(IList<TS> tsList, bool includeInput)
	{
		IList<string> list = new List<string>();
		if (tsList != null)
		{
			int tssize = tsList.Count;
			TS ts;
	...
	/// <summary>
	/// Return the interpolated data value between two time steps for a time series. </summary>
	/// <param name="ts"> Time series to get data from.  Irregular time series are currently
	/// not supported and always return missing data. </param>
	/// <param name="date"> Date/time for the end of the interval.
	/// The precision of the date/time should be the same as the time series for accurate date comparisons. </param>
	/// <param name="double"> timeStepFraction Fraction of the time step to look back in time
	/// from date.  Must be betweem 0.0 (return value at date) and 1.0 (return value one time step prior to date). </param>
	...
	/// to the time series.  This is called by fillPattern. </summary>
	/// <returns> The pattern statistics as a TSPatternStats instance, or null if there is a problem. </returns>
	...
	/// <param name="pattern_ts"> Time series containing patterns. </param>
	/// <param name="props"> PropList to modify behavior.  Currently, only
	/// "IgnoreLessThanOrEqualZero=true" is recognized.  If true, averages will ignore values <= 0.  The default is false. </param>
	public static TSPatternStats getPatternStats(TS ts, StringMonthTS pattern_ts, PropList props)
	{
		...
		IList<string> indicators = pattern_ts.getUniqueData();
		...
		if (interval_base == TimeInterval.IRREGULAR)
		{
			...
			for (int i = 0; i < nalltsdata; i++)
			{
				...
				data_value = tsdata.getDataValue();
				date = tsdata.getDate();
				if ((!ignore_lezero && !ts.isDataMissing(data_value)) || (ignore_lezero && ((data_value > 0.0) && !ts.isDataMissing(data_value))))
				{
	...
	*/
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static TSLimits getPeriodFromDates(RTi.Util.Time.DateTime date1, RTi.Util.Time.DateTime date2, RTi.Util.Time.YearType calendar_type, int por_flag) throws TSException
	...
	/// </para>
	/// </summary>
	/// <returns> The TSLimits resulting from the combination of the given TSLimits. </returns>
	/// <param name="limits"> A list of TSLimits of interest. </param>
	/// <param name="por_flag"> Use a *_POR flag. </param>
	/// <exception cref="RTi.TS.TSException"> If the period cannot be determined from the
	/// limits (e.g., requesting minimum and there is no overlap). </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static TSLimits getPeriodFromLimits(java.util.List<TSLimits> limits, int por_flag) throws TSException
	public static TSLimits getPeriodFromLimits(IList<TSLimits> limits, int por_flag)
	{
		...
		TSLimits limits_pointer = limits[0];
	...
	/// overload the version that accepts a vector of time series. </summary>
	/// <returns> The TSLimits for the single time series (recomputed). </returns>
	/// <param name="ts"> The time series of interest. </param>
	/// <param name="por_flag"> Use a *_POR flag. </param>
	/// <exception cref="RTi.TS.TSException"> If the period cannot be determined from the time
	/// series (e.g., requesting minimum and there is no overlap). </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static TSLimits getPeriodFromTS(TS ts, int por_flag) throws TSException
	public static TSLimits getPeriodFromTS(TS ts, int por_flag)
	{
		if (ts == null)
		{
			// Let other routine handle...
			return getPeriodFromTS((IList<TS>)null, por_flag);
		}
		else
		{
	...
	/// overload the version that accepts a vector of time series. </summary>
	/// <returns> The TSLimits for all the time series (recomputed). </returns>
	/// <param name="ts"> The time series of interest. </param>
	/// <param name="tslist"> A list of time series of interest. </param>
	/// <param name="por_flag"> Use a *_POR flag. </param>
	/// <exception cref="RTi.TS.TSException"> If the period cannot be determined from the time
	/// series (e.g., requesting minimum and there is no overlap). </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static TSLimits getPeriodFromTS(TS ts, java.util.List<TS> tslist, int por_flag) throws TSException
	public static TSLimits getPeriodFromTS(TS ts, IList<TS> tslist, int por_flag)
	{
...
//ORIGINAL LINE: public static TSLimits getPeriodFromTS(java.util.List<? extends TS> ts, int por_flag) throws TSException
	...
	public static string[] getTSFormatSpecifiers(bool includeDescription)
	{
	...
	/// If the start date from the time series is later than the specified end date, then what? </summary>
	/// <returns> The limits given a suggested start and end date.  The date limits can
	...
	/// are created to protect against changing the original dates. </returns>
	/// <param name="ts"> Time series of interest. </param>
	/// <param name="suggested_start"> Suggested start date. </param>
	/// <param name="suggested_end"> Suggested end date. </param>
	...
	/// If the suggested start and end are null, determine the start and end from the maximum time series period. </summary>
	/// <param name="tslist"> list of time series of interest </param>
	/// <param name="suggestedStart"> Suggested start date/time </param>
	/// <param name="suggestedEnd"> Suggested end date/time </param>
	public static TSLimits getValidPeriod(IList<TS> tslist, DateTime suggestedStart, DateTime suggestedEnd)
	{
		TSLimits dates = new TSLimits();
	...
	/// case-insensitive query is made.  The trace ID is not used in the search. </summary>
	/// <param name="tslist"> List of time series to search. </param>
	/// <param name="id"> String identifier to match. </param>
	/// <param name="field"> Field to match (currently can only be "Alias" or "Location"). </param>
	/// <param name="direction"> If >= 0, search forward.  If < 0, search backward. </param>
	/// <returns> the Vectorposition of the match or -1 if no match or the field is not recognized. </returns>
	public static int indexOf<T1>(IList<T1> tslist, string id, string field, int direction) where T1 : TS
	{
		return indexOf(tslist, id, field, null, direction);
	}

	/// <summary>
	/// Find a time series in a list.  The indicated field is searched and a case-insensitive query is made. </summary>
	/// <param name="tslist"> List of time series to search. </param>
	/// <param name="id"> String identifier to match. </param>
	/// <param name="field"> Field to match (currently can only be "Alias" or "Location"). </param>
	/// <param name="traceID"> If non-null and non-blank, the trace identifier is also checked to make a match.  This is used for ensembles. </param>
	/// <param name="direction"> If >= 0, search forward.  If < 0, search backward. </param>
	/// <returns> the list position (0+) of the match or -1 if no match or the field is not recognized. </returns>
	public static int indexOf<T1>(IList<T1> tslist, string id, string field, string traceID, int direction) where T1 : TS
	{
		if (tslist == null)
		{
			return -1;
		...
		else if (field.Equals("Location", StringComparison.OrdinalIgnoreCase))
		{
			ifield = 2;
		}
		...
		if (direction >= 0)
		{
			...
			for (int i = 0; i < size; i++)
			{
				...
				if (ifield == 1)
				{
					if (id.Equals(ts.getAlias(), StringComparison.OrdinalIgnoreCase))
					{
						if ((!string.ReferenceEquals(traceID, null)) && !traceID.Equals(""))
						{
							if (ts.getSequenceID().Equals(traceID, StringComparison.OrdinalIgnoreCase))
							{
								return i;
							}
						}
	...
	/// <summary>
	/// True if the first TS interval is greater. </summary>
	/// <returns> true if the first TS interval is greater than the second TS interval </returns>
	/// <param name="ts"> Time Series. </param>
	/// <param name="comparets"> Time Series to compare to. </param>
	public static bool isGreaterInterval(TS ts, TS comparets)
	{
		bool result = false;
	...
	/// <summary>
	/// True if the first TS interval is smaller. </summary>
	/// <returns> true if the first TS interval is smaller than the second TS interval </returns>
	/// <param name="ts"> Time Series. </param>
	/// <param name="comparets"> Time Series to compare to. </param>
	public static bool isSmallerInterval(TS ts, TS comparets)
	{
		bool result = false;
		if (ts == null || comparets == null)
		{
	...
	/// are compared, which does not require full time series objects. </summary>
	/// <returns> true if the time series have the same data interval, false if not. </returns>
	/// <param name="tsident1"> first time series identifier to check </param>
	/// <param name="tsident2"> second time series identifier to check </param>
	public static bool intervalsMatch(TSIdent tsident1, TSIdent tsident2)
	{
		if ((tsident1 == null) || (tsident2 == null))
		{
			return false;
		}
		if ((tsident1.getIntervalBase() == tsident2.getIntervalBase()) && (tsident1.getIntervalMult() == tsident2.getIntervalMult()))
		{
			return true;
		}
		return false;
	...
	/// <summary>
	/// Determine whether two time series have matching data intervals.  The intervals must exactly match and not
	/// just be equivalent (i.e., 1Day != 24Hour). </summary>
	/// <returns> true if the time series have the same data interval, false if not. </returns>
	/// <param name="ts1"> first time series to check </param>
	/// <param name="ts2"> second time series to check </param>
	public static bool intervalsMatch(TS ts1, TS ts2)
	{
	...
	/// just be equivalent (i.e., 1Day != 24Hour). </summary>
	/// <returns> true if all the time series in the list have the same interval as
	/// the first time series in the list, false if not </returns>
	/// <param name="tsList"> List of time series to check </param>
	public static bool intervalsMatch(IList<TS> tsList)
	{
		if (tsList == null)
		{
			return false;
	...
	/// The intervals must exactly match and not just be equivalent (i.e., 1Day != 24Hour). </summary>
	/// <returns> true if all the time series in the list have the same interval as specified, false if not. </returns>
	/// <param name="tsList"> List of time series to check </param>
	/// <param name="intervalBase"> Data interval base (e.g., TimeInterval.HOUR) </param>
	/// <param name="intervalMult"> Data interval multiplier </param>
	public static bool intervalsMatch<T1>(IList<T1> tsList, int intervalBase, int intervalMult) where T1 : TS
	{
		if (tsList == null)
		{
			return false;
	...
	/// description and genesis information are updated to reflect the processing. </summary>
	/// <returns> The time series with maximum time series values in each interval. </returns>
	/// <param name="ts"> Time series to be added modified. </param>
	/// <param name="ts_to_check"> Time series to check against "ts". </param>
	/// <exception cref="TSException"> if there is an error processing the time series. </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static TS max(TS ts, TS ts_to_check) throws TSException
	...
	/// series description and genesis information are updated to reflect the addition. </summary>
	/// <returns> The time series with maximum time series values in each interval. </returns>
	/// <param name="ts"> Time series to be modified. </param>
	/// <param name="ts_to_check"> List of time series to check against "ts". </param>
	/// <exception cref="RTi.TS.TSException"> if there is an error processing the time series. </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static TS max(TS ts, java.util.List<TS> ts_to_check) throws TSException
	public static TS max(TS ts, IList<TS> ts_to_check)
	{
		...
		for (int i = 0; i < ntslist; i++)
		{
			...
			if (tspt_interval_base == TimeInterval.IRREGULAR)
			{
				...
				Message.printWarning(2, routine, "IrregularTS not supported.  Not processing max()." + tspt.getIdentifier().ToString());
				continue;
			}
			else
			{
				...
				for (; date.lessThanOrEqualTo(end_date); date.addInterval(interval_base, interval_mult))
				{
					...
					if (Message.isDebugOn)
					{
						Message.printDebug(dl, routine, "At " + date.ToString() + ", changed max from " + data_value + " to " + data_value_to_check);
					}
				}
	...
	/// description and genesis information are updated to reflect the processing. </summary>
	/// <returns> The time series with minimum time series values in each interval. </returns>
	/// <param name="ts"> Time series to be added modified. </param>
	/// <param name="ts_to_check"> Time series to check against "ts". </param>
	/// <exception cref="TSException"> if there is an error processing the time series. </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static TS min(TS ts, TS ts_to_check) throws TSException
	...
	/// series description and genesis information are updated to reflect the processing. </summary>
	/// <returns> The time series with minimum time series values in each interval. </returns>
	/// <param name="ts"> Time series to be modified. </param>
	/// <param name="ts_to_check"> List of time series to check against "ts". </param>
	/// <exception cref="RTi.TS.TSException"> if there is an error processing the time series. </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static TS min(TS ts, java.util.List<TS> ts_to_check) throws TSException
	public static TS min(TS ts, IList<TS> ts_to_check)
	{
		...
		for (int i = 0; i < ntslist; i++)
		{
			...
			else
			{
				...
				for (; date.lessThanOrEqualTo(end_date); date.addInterval(interval_base, interval_mult))
				{
					...
					if (Message.isDebugOn)
					{
						Message.printDebug(dl, routine, "At " + date.ToString() + ", changed min from " + data_value + " to " + data_value_to_check);
					}
				}
				ts.setDescription(ts.getDescription() + ",min(" + tspt.getDescription() + ")");
	...
	/// <summary>
	/// Get the count of missing data in a period, for all months in a year. </summary>
	/// <returns> the count of missing values in a period. </returns>
	/// <param name="start"> the starting date/time for the check. </param>
	/// <param name="end"> the ending date/time for the check. </param>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static int missingCount(TS ts, RTi.Util.Time.DateTime start, RTi.Util.Time.DateTime end) throws Exception
	...
	/// <summary>
	/// Get the count of missing data in a period, optionally analyzing only specified months. </summary>
	/// <returns> the count of missing values in a period. </returns>
	/// <param name="start"> the starting date/time for the check. </param>
	/// <param name="end"> the ending date/time for the check. </param>
	/// <param name="months"> an array specifying month numbers 1-12.  Any months that are indicated will be analyzed. </param>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static int missingCountForMonths(TS ts, RTi.Util.Time.DateTime start, RTi.Util.Time.DateTime end, int [] months) throws Exception
	...
	/// can be multiplied.  If either value is missing, then the result is set to missing. </summary>
	/// <param name="ts"> Time series to multiply. </param>
	/// <param name="tsm"> Time series to multiply by. </param>
	/// <exception cref="Exception"> if there is an error processing. </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static void multiply(TS ts, TS tsm) throws Exception
	...
	/// regular time series can be multiplied.  If either value is missing, then the result is set to missing. </summary>
	/// <param name="ts"> Time series to multiply. </param>
	/// <param name="tsm"> Time series to multiply by. </param>
	/// <param name="startDate"> Date to start multiply. </param>
	/// <param name="endDate"> Date to stop multiply. </param>
	/// <exception cref="Exception"> if there is an error processing. </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static void multiply(TS ts, TS tsm, RTi.Util.Time.DateTime startDate, RTi.Util.Time.DateTime endDate) throws Exception
	public static void multiply(TS ts, TS tsm, DateTime startDate, DateTime endDate)
	{
		...
		{
			throw new Exception("Cannot multiply data with different intervals (" + ts.getIdentifierString() + " and " + tsm.getIdentifierString() + ").");
		}
	...
	/// further manipulated to set its data values. </summary>
	/// <returns> new DateTime of requested precision or null if the precision cannot
	/// be determined from the time series. </returns>
	/// <param name="ts"> Time series to check for date precision (date1 is checked). </param>
	/// <param name="date"> If null, the start date/time from the time series is used to initialize the date/time.
	/// If not null, use the date information to initialize the returned date, to the appropriate precision. </param>
...
//ORIGINAL LINE: public static TS newTimeSeries(String id, boolean long_id) throws Exception
	...
	/// can be found (all missing data), the data values are left as is but the other changes are made. </summary>
	/// <param name="ts"> Time series to normalized (the time series will be modified). </param>
	/// <param name="minfromdata"> Indicate true if the minimum data value to use in the
	/// normalization interpolation should be the minimum time series data value.
	/// Indicate false if the minimum value should used for interpolation should be zero. </param>
	/// <param name="newmin"> New minimum value (generally specify 0.0). </param>
	/// <param name="newmax"> New maximum value (generally specify 1.0). </param>
	public static void normalize(TS ts, bool minfromdata, double newmin, double newmax)
	{ // Get the data limits...
		...
		// Set the time series properties...
		ts.setDataUnits("");
		ts.addToGenesis("Normalized time series using linear interpolation between min value " + min + " and max value " + max);
		ts.setDescription(ts.getDescription() + ", normalized");
	}

	/// <returns> the number of intervals in a month.  This is useful when doing averages,
	/// etc.  May need to later make this more generic. </returns>
	/// <exception cref="TSException"> if there is an error with input or an unsupported interval. </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static int numIntervalsInMonth(int month, int year, int interval_base, int interval_mult) throws TSException
	...
	/// is determined using consistent units (although units are not currently checked). </summary>
	/// <param name="ts1"> Time series to be subtracted from.  This time series is also modified and returned. </param>
	/// <param name="ts2"> Time series to subtract. </param>
	/// <param name="divisor"> The time series to divide by.  Although it can be any time
	/// series, it should typically be ts1 or ts2. </param>
	/// <exception cref="Exception"> if there is an error processing. </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static void relativeDiff(TS ts1, TS ts2, TS divisor) throws Exception
	...
	/// <param name="divisor"> The time series to divide by.  Although it can be any time
	/// series, it should typically be ts1 or ts2. </param>
	/// <param name="start_date"> First date to process. </param>
	/// <param name="end_date"> Last date to process. </param>
	/// <exception cref="Exception"> if there is an error processing. </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static void relativeDiff(TS ts1, TS ts2, TS divisor, RTi.Util.Time.DateTime start_date, RTi.Util.Time.DateTime end_date) throws Exception
	public static void relativeDiff(TS ts1, TS ts2, TS divisor, DateTime start_date, DateTime end_date)
	{
		...
		{
			throw new Exception("Cannot determine relativeDiff for TS1, TS2 data with different intervals (" + ts1.getIdentifierString() + " and " + ts2.getIdentifierString() + ").");
		}
		if ((interval_base != divisor.getDataIntervalBase()) || (interval_mult != divisor.getDataIntervalMult()))
		{
			throw new Exception("Cannot determine relativeDiff for TS1, divisor data with different intervals (" + ts1.getIdentifierString() + " and " + divisor.getIdentifierString() + ").");
		}
		if (interval_base == TimeInterval.IRREGULAR)
		{
		...
		ts1.setDataUnits("");
		ts1.addToGenesis("Converted to relativeDiff " + start_date.ToString() + " to " + end_date.ToString() + " by subtracting " + ts2.getDescription() + " and dividing by " + divisor.getDescription());
	...
	/// The data range is checked regardless of whether the missing data value is in the range. </summary>
	/// <param name="ts"> Time series to update. </param>
	/// <param name="start_date"> Date to start assignment. </param>
	/// <param name="end_date"> Date to stop assignment. </param>
	/// <param name="minValue"> Minimum data value to replace. </param>
	/// <param name="maxValue"> Maximum data value to replace. </param>
	/// <param name="matchFlag"> flag to match, in addition to matching the numerical value (ok if no numerical value) - currently
	/// a simple case-dependent string match is performed </param>
	/// <param name="newValue"> Replacement data value (optional - do not need if action is specified). </param>
	/// <param name="action"> Action to perform: "Remove" to remove the point (only for irregular interval, treated as SetMissing
	/// for regular interval data), "SetMissing" to set values to missing, and otherwise use "newvalue" to replace. </param>
	/// <param name="analysisWindowStart"> the starting date/time within a year to start the replacement,
	/// consistent with the time series interval </param>
	/// <param name="analysisWindowEnd"> the endin date/time within a year to start the replacement,
	/// consistent with the time series interval </param>
	/// <param name="setFlag"> flag value to set on data values when resetting a value </param>
	/// <param name="setFlagDesc"> descriptionion for set flag </param>
	public static void replaceValue(TS ts, DateTime start_date, DateTime end_date, double? minValue, double? maxValue, double? newValue, string matchFlag, string action, DateTime analysisWindowStart, DateTime analysisWindowEnd, string setFlag, string setFlagDescription)
	{
		if ((newValue == null) && ((string.ReferenceEquals(action, null)) || action.Equals("")))
		{
			throw new InvalidParameterException("Neither new value or action have been specified.  Cannot replace value in time series.");
		}
		if ((minValue == null) && ((string.ReferenceEquals(matchFlag, null)) || matchFlag.Equals("")))
		{
			throw new InvalidParameterException("Neither minimum value or match flag have been specified.  Cannot replace value in time series.");
		}
		double minvalue = Double.NaN;
		if (minValue != null)
		{
			minvalue = minValue.Value; // Does this increase peformance?
		}
		...
		DateTime end = valid_dates.getDate2();
		bool doRemove = false;
		bool doSetMissing = false;
		double missing = ts.getMissing();
		if (!string.ReferenceEquals(action, null))
		{
		...
		bool doMatchValue = false; // Checking numerical value for match
		if (minValue != null)
		{
			doMatchValue = true;
		}
		bool doMatchFlag = false; // Checking flg for match
		if ((!string.ReferenceEquals(matchFlag, null)) && !matchFlag.Equals(""))
		{
			doMatchFlag = true;
		}
		bool doSetFlag = false;
		if ((!string.ReferenceEquals(setFlag, null)) && !setFlag.Equals(""))
		{
			doSetFlag = true;
		}
		...
		if (interval_base == TimeInterval.IRREGULAR)
		{
			...
			TSData tsdata = null;
			DateTime date = null;
			bool pointRemoved; // To know if remove was success
			for (int i = 0; i < nalltsdata; i++)
			{
		...
		if (replaceCount > 0)
		{
			...
			else
			{
				...
				if (doRemove)
				{
					ts.addDataFlagMetadata(new TSDataFlagMetadata(setFlag, "Removed values that matched min=" + StringUtil.formatString(minValue,"%.6f") + " max=" + StringUtil.formatString(maxValue,"%.6f") + " flag=" + matchFlag));
				}
				else if (doSetMissing)
				{
					ts.addDataFlagMetadata(new TSDataFlagMetadata(setFlag, "Replaced values with missing for values that matched min=" + StringUtil.formatString(minValue,"%.6f") + " max=" + StringUtil.formatString(maxValue,"%.6f") + " flag=" + matchFlag));
				}
				else
				{
					ts.addDataFlagMetadata(new TSDataFlagMetadata(setFlag, "Replaced values with " + StringUtil.formatString(newValue,"%.6f") + "for values that matched min=" + StringUtil.formatString(minValue,"%.6f") + " max=" + StringUtil.formatString(maxValue,"%.6f") + " flag=" + matchFlag));
				}
			}
			if (doRemove || doSetMissing)
			{
				ts.setDescription(ts.getDescription() + ", replaceValue(" + StringUtil.formatString(minvalue, "%.3f") + "," + StringUtil.formatString(maxvalue, "%.3f") + "," + action + ")");
				ts.addToGenesis("Replace " + StringUtil.formatString(minvalue,"%.3f") + " - " + StringUtil.formatString(maxvalue,"%.3f") + " using action \"" + action + "\" " + start + " to " + end + " uniquetempvar.");
			}
			else
			{
				ts.setDescription(ts.getDescription() + ", replaceValue(" + StringUtil.formatString(minvalue, "%.3f") + "," + StringUtil.formatString(maxvalue, "%.3f") + "," + StringUtil.formatString(newvalue, "%.3f") + ")");
				ts.addToGenesis("Replace " + StringUtil.formatString(minvalue,"%.3f") + " - " + StringUtil.formatString(maxvalue,"%.3f") + " with " + StringUtil.formatString(newvalue,"%.3f") + " " + start + " to " + end + " uniquetempvar.");
			}
		}
	}

	/// <summary>
	/// Scale the entire time series by a constant value. </summary>
	/// <param name="scale"> Data value to scale time series. </param>
	/// <param name="ts"> Time series to scale. </param>
	/// <exception cref="Exception"> if the scale value is bad or another error occurs. </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static void scale(TS ts, double scale) throws Exception
	...
	/// <summary>
	/// Scale the time series by a constant value. </summary>
	/// <param name="ts"> Time series to scale. </param>
	/// <param name="start_date"> Date to start scaling. </param>
	/// <param name="end_date"> Date to stop scaling. </param>
	/// <param name="scale"> Data value to scale time series. </param>
	/// <exception cref="Exception"> if the scale value is bad or another error occurs. </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static void scale(TS ts, RTi.Util.Time.DateTime start_date, RTi.Util.Time.DateTime end_date, double scale) throws Exception
	...
	/// <param name="start_date"> Date to start scaling. </param>
	/// <param name="end_date"> Date to stop scaling. </param>
	/// <param name="month"> Month to scale (1-12).  If negative, scale every month. </param>
	/// <param name="scale"> Data value to scale time series. </param>
	/// <exception cref="Exception"> if the scale value is bad or another error occurs. </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static void scale(TS ts, RTi.Util.Time.DateTime start_date, RTi.Util.Time.DateTime end_date, int month, double scale) throws Exception
	...
	/// <param name="end_date"> Date to stop scaling. </param>
	/// <param name="month"> Month to scale (1-12).  If negative, scale every month. </param>
	/// <param name="ScaleValue"> Data value to scale time series, as a number, DaysInMonth, or
	...
	/// converting between a monthly interval and an interval shorter than monthly. </param>
	/// <exception cref="Exception"> if the scale value is bad or another error occurs. </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static void scale(TS ts, RTi.Util.Time.DateTime start_date, RTi.Util.Time.DateTime end_date, int month, String ScaleValue) throws Exception
	public static void scale(TS ts, DateTime start_date, DateTime end_date, int month, string ScaleValue)
	{
		...
		bool DaysInMonth_boolean = false; // If true, then ScaleValue is DaysInMonth
		bool DaysInMonthInverse_boolean = false;
		if (ScaleValue.Equals("DaysInMonth", StringComparison.OrdinalIgnoreCase))
		{
			DaysInMonth_boolean = true;
		}
		else if (ScaleValue.Equals("DaysInMonthInverse", StringComparison.OrdinalIgnoreCase))
		{
		...
		if (month >= 0)
		{
			...
			{
				ts.addToGenesis("Scaled " + start + " to " + end + " (month " + month + ") by " + StringUtil.formatString(ScaleValue_double,"%.6f"));
			}
		}
		else
		{
			...
			{
				ts.addToGenesis("Scaled " + start + " to " + end + " by " + StringUtil.formatString(ScaleValue_double,"%.6f") + ".");
			}
		}
	...
	/// <summary>
	/// Scale the entire time series by a constant value, but only for the specified month. </summary>
	/// <param name="ts"> Time series to scale. </param>
	/// <param name="month"> Month number (1-12) to scale.  Specify -1 to scale all months. </param>
	/// <param name="scale"> Data value to scale time series. </param>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static void scaleByMonth(TS ts, int month, double scale) throws Exception
	...
	/// more time series identifiers strings with those in the time series list. </summary>
	/// <returns> a Vector of matching time series (guaranteed to be non-null but may be
	...
	/// series.  If the input type and name are present int he requested TSID, then the input type and name are checked. </returns>
	/// <param name="tslist"> a list of TS to be checked. </param>
	/// <param name="tsids"> a list of time series identifier strings.  The comparison is made
	...
	/// name are checked for a match.  Each matching time series is returned in the list. </param>
	/// <param name="PropList"> props Properties to control processing, not currently used. </param>
	public static IList<TS> selectTimeSeries(IList<TS> tslist, IList<string> tsids, PropList props)
	{
		IList<TS> v = new List<TS>();
		int tsids_size = 0;
		if (tsids != null)
		{
			tsids_size = tsids.Count;
		}
		int tslist_size = 0;
		if (tslist != null)
		{
			tslist_size = tslist.Count;
		}
		TS ts = null;
		TSIdent tsident = null;
		int j;
		...
		{
			Message.printStatus(2, "", "Checking TSID \"" + tsids[i] + "\"");
			try
			{
				tsident = new TSIdent((string)tsids[i]);
	...
	/// <summary>
	/// Set all the data in the time series to a constant value. </summary>
	/// <param name="value"> Data value to set in the time series. </param>
	/// <param name="ts"> Time series to update. </param>
	...
	/// <summary>
	/// Set the time series data to a constant value. </summary>
	/// <param name="ts"> Time series to update. </param>
	/// <param name="start_date"> Date to start assignment. </param>
	/// <param name="end_date"> Date to stop assignment. </param>
	/// <param name="value"> Data value to set as time series data. </param>
	...
	/// <summary>
	/// Set the entire time series to monthly values.  For example, values[0] is used for any date in January. </summary>
	/// <param name="ts"> Time series to update. </param>
	/// <param name="values"> Data values to set time series data (the first value is for January, the last is for December).
	/// If a value is null then the month value is not modified.  Missing values are expected to be set in the value array. </param>
	...
	/// <param name="start_date"> Date to start assignment. </param>
	/// <param name="end_date"> Date to stop assignment. </param>
	/// <param name="values"> Data values to set in the time series (the first value is for January, the last is for December) or
	/// if null don't set the monthly value.  If setting to missing it is expected that the value has been set to the
	/// missing value for the time series. </param>
	...
	{ // Get valid dates because the ones passed in may have been null...
		...
		if (interval_base == TimeInterval.IRREGULAR)
		{
			...
			for (int i = 0; i < nalltsdata; i++)
			{
				...
				if (value != null)
				{ // If null leave the old value
					if (date.greaterThanOrEqualTo(start))
					{
						tsdata.setDataValue(value.Value);
						// Have to do this manually since TSData are being modified directly to improve performance...
						irrts.setDirty(true);
					}
				}
		...
		else
		{
			...
			for (; date.lessThanOrEqualTo(end); date.addInterval(interval_base, interval_mult))
			{
				...
				if (value != null)
				{
					ts.setDataValue(date, value.Value);
				}
			}
		}
	...
	/// <summary>
	/// Set data by getting values from another time series. </summary>
	/// <param name="dependentTS"> Time series to update. </param>
	/// <param name="independentTS"> Time series to copy from. </param>
	/// <exception cref="Exception"> if an error occurs (usually null input). </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static void setFromTS(TS dependentTS, TS independentTS) throws Exception
	...
	/// The data intervals do not need to be the same (truncation of dates will result if date precision is different). </summary>
	/// <param name="dependentTS"> Time series to update. </param>
	/// <param name="independentTS"> Time series to copy from. </param>
	/// <param name="start_date"> Date to start data transfer (relative to the dependent time series). </param>
	/// <param name="end_date"> Date to stop the data transfer (relative to the dependent time series). </param>
	/// <param name="setWindowStart"> ignoring the year, the remainder indicates the start of the window within each year to set data </param>
	/// <param name="setWindowEnd"> ignoring the year, the remainder indicates the end of the window within each year to set data </param>
	/// <param name="props"> Properties to control the transfer.  Recognized properties are:
	/// <table width=100% cellpadding=10 cellspacing=0 border=2>
	/// <tr>
	/// <td><b>Property</b></td>	<td><b>Description</b></td>	<td><b>Default</b></td>
	/// </tr>
	...
	/// </tr>
	/// </table> </param>
	/// <param name="setDataFlags"> if true, copy the data flags from the independent time series; if false leave the original data flags </param>
	/// <exception cref="Exception"> if an error occurs (usually null input). </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static void setFromTS(TS dependentTS, TS independentTS, RTi.Util.Time.DateTime start_date, RTi.Util.Time.DateTime end_date, RTi.Util.Time.DateTime setWindowStart, RTi.Util.Time.DateTime setWindowEnd, RTi.Util.IO.PropList props, boolean setDataFlags) throws Exception
	public static void setFromTS(TS dependentTS, TS independentTS, DateTime start_date, DateTime end_date, DateTime setWindowStart, DateTime setWindowEnd, PropList props, bool setDataFlags)
	{
		string routine = "TSUtil.setFromTS";
		string message;
		...
		bool transfer_bydate = true; // Default - make dates match in both time series.
		bool setMissing = true; // Default - either this or IgnoreMissing, which is setMissing=false
		bool setOnlyMissingValues = false; // Only processing missing values
		bool doSetFlag = false; // True if the SetFlag property was set - means a new flag is used (not from independent time series)
		string setFlag = null;
		string setFlagDescription = null;
		if (props != null)
		{
			string propVal = props.getValue("TransferData");
			if ((!string.ReferenceEquals(propVal, null)) && propVal.Equals(TRANSFER_SEQUENTIALLY, StringComparison.OrdinalIgnoreCase))
			{
				// Transfer sequentially...
				transfer_bydate = false;
			}
			propVal = props.getValue("HandleMissingHow");
			if (propVal.Equals("SetMissing", StringComparison.OrdinalIgnoreCase) || (string.ReferenceEquals(propVal, null)))
			{
				setMissing = true; // Default
			}
			else if (propVal.Equals("IgnoreMissing", StringComparison.OrdinalIgnoreCase))
			{
				// Ignore missing data (don't transfer)...
				setMissing = false;
			}
			else if (propVal.Equals("SetOnlyMissingValues", StringComparison.OrdinalIgnoreCase))
			{
		...
		bool isMissing; // Whether a data value is missing
		TSData tsdata = new TSData();
		...
		dependentTS.addToGenesis("Set data " + start.ToString() + " to " + end.ToString() + " by using values from " + independentTS.getIdentifierString());
		...
		{
			dependentTS.addToGenesis("Data values were transferred sequentially from start date/time" + handleMissingHowString + ".");
		}
	...
	/// Time zone is NOT considered by this method. </summary>
	/// <param name="oldts"> Time series to shift. </param>
	/// <param name="olddate"> Old date. </param>
	/// <param name="newdate"> New date to shift to.  The shift is therefore applied by moving
	/// the old date to the new date and shifting all other dates accordingly. </param>
	/// <exception cref="RTi.TS.TSException"> if there is a problem with input </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static TS shift(TS oldts, RTi.Util.Time.DateTime olddate, RTi.Util.Time.DateTime newdate) throws Exception
	public static TS shift(TS oldts, DateTime olddate, DateTime newdate)
	{
		...
		//if ( Message.isDebugOn ) {
			Message.printStatus(1, routine, "Shifting " + oldts.getIdentifierString() + " from existing " + olddate + " to new " + newdate);
		//}
		...
		*/		
		...
		{
			Message.printWarning(1, routine, "Time series and output base interval of " + interval_base + " are not recognized.");
			return null;
		}
		...
		double data_value;
		for (; newdate_i.lessThanOrEqualTo(date2) && olddate_i.lessThanOrEqualTo(olddate2); newdate_i.addInterval(interval_base, interval_mult), olddate_i.addInterval(interval_base, interval_mult))
		{
	...
	/// The new value is computed as data[i] = sum(data[interval_offsets[i]]*weights[i]) </summary>
	/// <param name="oldts"> Time series to shift. </param>
	/// <param name="interval_offsets"> Array of whole-interval shifts. </param>
	/// <param name="weights"> Array of double precision weights. </param>
	/// <returns> new time series. </returns>
	/// <exception cref="Exception"> if there is a problem with input </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static TS shiftTimeByInterval(TS oldts, int interval_offsets[], double weights[]) throws TSException
	public static TS shiftTimeByInterval(TS oldts, int[] interval_offsets, double[] weights)
	{
		...
		bool first_time = true; // Indicates to setup in first loop
		// Loop through each date/time in the time series that is being modified (oldts).
		for (DateTime date = new DateTime(date1); date.lessThanOrEqualTo(date2); date.addInterval(interval_base,interval_mult))
		{
			if (first_time)
			{
	...
	/// </ul> </summary>
	/// <param name="ts"> The time series to be modified. </param>
	/// <param name="req_tz"> A requested time zone abbreviation, as recognized by the
	/// RTi.Util.Time.TZ class.  If null, or "" are specified, the original time series is returned. </param>
	/// <param name="from_date"> A DateTime used as a reference to determine daylight savings
	...
	/// supported because standard/daylight shifts will vary in a long time series. </param>
	/// <param name="reset_tz_if_same"> If true and the requested time zone is numerically the
	...
	/// "GMT").  If false and the time zones are numerically equal, use the original time zone. </param>
	/// <returns> the original time series if no shift has occurred, or a new time series with shifted data. </returns>
	/// <exception cref="if"> there is an error shifting the time zone (e.g., trying to shift
	/// hourly data by 30 minutes, or the requested time zone is not recognized). </exception>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static TS shiftTimeZone(TS ts, String req_tz, RTi.Util.Time.DateTime from_date, boolean reset_tz_if_same) throws Exception
	public static TS shiftTimeZone(TS ts, string req_tz, DateTime from_date, bool reset_tz_if_same)
	{
		if ((string.ReferenceEquals(req_tz, null)) || (req_tz.Length == 0))
		{
		...
		else
		{
			...
			for (; date.lessThanOrEqualTo(date_end); date.addInterval(interval_base,interval_mult), date2.addInterval(interval_base,interval_mult))
			{
	...
	/// <param name="ts"> Time series to be subtracted from. </param>
	/// <param name="ts_to_subtract"> Time series to subtract from "ts". </param>
	...
	/// <returns> The subtracted time series. </returns>
	/// <param name="ts"> Time series to be subtracted from. </param>
	/// <param name="ts_to_subtract"> List of time series to subtract from "ts". </param>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static TS subtract(TS ts, java.util.List<TS> ts_to_subtract) throws Exception
	public static TS subtract(TS ts, IList<TS> ts_to_subtract)
	{
	...
	/// <returns> The subtracted time series. </returns>
	/// <param name="ts"> Time series to be subtracted from. </param>
	/// <param name="tsToSubtract"> List of time series to subtract from "ts". </param>
	/// <param name="missingFlag"> See documentation for add(). </param>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static TS subtract(TS ts, java.util.List<TS> tsToSubtract, int missingFlag) throws Exception
	public static TS subtract(TS ts, IList<TS> tsToSubtract, int missingFlag)
	{
	...
	/// <param name="tsToSubtract"> List of time series to subtract from "ts". </param>
	/// <param name="missingFlag"> See documentation for add(). </param>
	/// <param name="subtractStart"> starting date/time for subtract, or null for full period </param>
	/// <param name="subtractEnd"> starting date/time for subtract, or null for full period </param>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static TS subtract(TS ts, java.util.List<TS> tsToSubtract, int missingFlag, RTi.Util.Time.DateTime subtractStart, RTi.Util.Time.DateTime subtractEnd) throws Exception
	public static TS subtract(TS ts, IList<TS> tsToSubtract, int missingFlag, DateTime subtractStart, DateTime subtractEnd)
	{ // Call the main overload routine...
		if (tsToSubtract == null)
		{
			return ts;
		}
		int size = tsToSubtract.Count;
		double[] factor = new double[size];
		for (int i = 0; i < size; i++)
		{
			factor[i] = -1.0;
	...
	/// for those values.  If the start date or end date are null, the start and end dates of the time series are used. </summary>
	/// <returns> The array of data for to time series.  If an error, return null. </returns>
	/// <param name="ts"> Time series to convert data to array format. </param>
	/// <param name="start_date"> Date corresponding to the first date of the returned array. </param>
	/// <param name="end_date"> Date corresponding to the last date of the returned array. </param>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static double[] toArray(TS ts, RTi.Util.Time.DateTime start_date, RTi.Util.Time.DateTime end_date) throws RTi.Util.Time.InvalidTimeIntervalException
	...
	/// dates of the time series are used.  This is a utility routine mainly used by other versions of this routine. </summary>
	/// <returns> The array of data for the time series.  If an error, return null. </returns>
	/// <param name="ts"> Time series to convert data to array format. </param>
	/// <param name="start_date"> Date corresponding to the first date of the returned array. </param>
	/// <param name="end_date"> Date corresponding to the last date of the returned array. </param>
	/// <param name="month_index"> Month of interest (1=Jan, 12=Dec).  If zero, process all months. </param>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static double[] toArray(TS ts, RTi.Util.Time.DateTime start_date, RTi.Util.Time.DateTime end_date, int month_index) throws RTi.Util.Time.InvalidTimeIntervalException
	...
	/// <param name="end_date"> Date corresponding to the last date of the returned array. </param>
	/// <param name="month_index"> Month of interest (1=Jan, 12=Dec).  If zero, process all months. </param>
	/// <param name="includeMissing"> indicate whether missing values should be included in the result. </param>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static double[] toArray(TS ts, RTi.Util.Time.DateTime start_date, RTi.Util.Time.DateTime end_date, int month_index, boolean includeMissing) throws RTi.Util.Time.InvalidTimeIntervalException
	public static double[] toArray(TS ts, DateTime start_date, DateTime end_date, int month_index, bool includeMissing)
	{
		int[] month_indices = null;
		if (month_index != 0)
		{
	...
	/// for those values.  If the start date or end date are null, the start and end
	/// dates of the time series are used.  This is a utility routine mainly used by other versions of this routine. </summary>
	/// <returns> The array of data for the time series.  If an error, return null.  A zero size array may be returned. </returns>
	/// <param name="ts"> Time series to convert data to array format. </param>
	/// <param name="startDate"> Date corresponding to the first date of the returned array. </param>
	/// <param name="endDate"> Date corresponding to the last date of the returned array. </param>
	/// <param name="monthIndices"> Months of interest (1=Jan, 12=Dec).  If null or an empty array, process all months. </param>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static double[] toArray(TS ts, RTi.Util.Time.DateTime startDate, RTi.Util.Time.DateTime endDate, int [] monthIndices) throws RTi.Util.Time.InvalidTimeIntervalException
	...
	/// other versions of this routine. </summary>
	/// <returns> The array of data for the time series.  If an error, return null.  A zero size array may be returned. </returns>
	/// <param name="ts"> Time series to convert data to array format. </param>
	/// <param name="start_date"> Date corresponding to the first date of the returned array. </param>
	/// <param name="end_date"> Date corresponding to the last date of the returned array. </param>
	/// <param name="month_indices"> Months of interest (1=Jan, 12=Dec).  If null or an empty array, process all months. </param>
	/// <param name="includeMissing"> if true, include missing values; if false, do not include missing values </param>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static double[] toArray(TS ts, RTi.Util.Time.DateTime start_date, RTi.Util.Time.DateTime end_date, int [] month_indices, boolean includeMissing) throws RTi.Util.Time.InvalidTimeIntervalException
	public static double[] toArray(TS ts, DateTime start_date, DateTime end_date, int[] month_indices, bool includeMissing)
	{
		return toArray(ts, start_date, end_date, month_indices, includeMissing, true, null, TSToArrayReturnType.DATA_VALUE);
	}
	...
	/// <param name="start_date"> Date corresponding to the first date of the returned array. </param>
	/// <param name="end_date"> Date corresponding to the last date of the returned array. </param>
	/// <param name="includeMonths"> Months of interest (1=Jan, 12=Dec) to include.  If null or an empty array, process all months. </param>
	/// <param name="includeMissing"> if true, include missing values; if false, do not include missing values </param>
	/// <param name="matchOtherNonmissing"> only used if "pairedTS" -s not null;
	...
	/// other time series (this functionality is used to extract data for regression analysis) </param>
	/// <param name="pairedTS"> a second time series used to extract paired (or non-paired) data samples; if null then
	/// the time series will not be checked as a constraint regardless of the "matchOtherNonmissing" value;
	/// this capability is NOT availble for irregular time series </param>
	/// <param name="returnType"> indicates which value should be returned in the array; requesting the date/time will return
	/// DateTime.getYear() for Year interval time series, DateTime.getAbsoluteMonth() for Month interval time series,
	/// DateTime.getAbsoluteDay() for Day interval time series, and will throw an exception for other intervals. </param>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static double[] toArray(TS ts, RTi.Util.Time.DateTime start_date, RTi.Util.Time.DateTime end_date, int [] includeMonths, boolean includeMissing, boolean matchOtherNonmissing, TS pairedTS, TSToArrayReturnType returnType) throws RTi.Util.Time.InvalidTimeIntervalException
	public static double[] toArray(TS ts, DateTime start_date, DateTime end_date, int[] includeMonths, bool includeMissing, bool matchOtherNonmissing, TS pairedTS, TSToArrayReturnType returnType)
	{
		if (pairedTS != null)
		{
			if (!TimeInterval.isRegularInterval(ts.getDataIntervalBase()))
			{
				throw new IrregularTimeSeriesNotSupportedException("Irregular interval time series cannot have data array extracted using paired time series.");
			}
			if (!TSUtil.intervalsMatch(ts, pairedTS))
			{
				throw new UnequalTimeIntervalException("Time series from which to extract data has a different interval than paired time series.");
			}
		}
		// Get valid dates because the ones passed in may have been null...
		...
		if (returnType == TSToArrayReturnType.DATE_TIME)
		{
			...
			if ((interval_mult != 1) || ((interval_base != TimeInterval.YEAR) && (interval_base != TimeInterval.YEAR) && (interval_base != TimeInterval.YEAR)))
			{
				throw new InvalidTimeIntervalException("Interval must be Year, Month, or Day (no multiplier) to return date/time as array.");
			}
		}

		bool[] includeMonthsMask = new bool[12];
	...
	/// record for the time series, use the missing data value from the time series
	/// for those values.  If the start date or end date are null, the start and end
	/// dates of the time series are used. </summary>
	/// <returns> The array of data for the time series.  If an error, return null. </returns>
	/// <param name="ts"> Time series to convert data to array format. </param>
	/// <param name="start_date"> Date corresponding to the first date of the returned array. </param>
	/// <param name="end_date"> Date corresponding to the last date of the returned array. </param>
	/// <param name="monthIndex"> Month of interest (1=Jan, 12=Dec).  If zero, process all months. </param>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static double[] toArrayByMonth(TS ts, RTi.Util.Time.DateTime start_date, RTi.Util.Time.DateTime end_date, int monthIndex) throws RTi.Util.Time.InvalidTimeIntervalException
	...
	/// <param name="end_date"> Date corresponding to the last date of the returned array. </param>
	/// <param name="monthIndex"> Month of interest (1=Jan, 12=Dec).  If zero, process all months. </param>
	/// <param name="includeMissing"> indicate whether missing values should be included. </param>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static double[] toArrayByMonth(TS ts, RTi.Util.Time.DateTime start_date, RTi.Util.Time.DateTime end_date, int monthIndex, boolean includeMissing) throws RTi.Util.Time.InvalidTimeIntervalException
	public static double[] toArrayByMonth(TS ts, DateTime start_date, DateTime end_date, int monthIndex, bool includeMissing)
	{
	...
	/// record for the time series, use the missing data value from the time series
	/// for those values.  If the start date or end date are null, the start and end
	/// dates of the time series are used.  Year, month, day, hour, and minute regular interval time series can be processed. </summary>
	/// <returns> The array of data for the time series.  If an error, return null.  A zero size array may be returned.
	/// An array of TSData is returned to allow the dates corresponding to values to be tracked, for example when
	/// computing statistics like maximum where the date/time of the maximum is of interest. </returns>
	/// <param name="ts"> Time series to convert data to array format. </param>
	/// <param name="startDate"> Date corresponding to the first date of the returned array. </param>
	/// <param name="endDate"> Date corresponding to the last date of the returned array. </param>
	/// <param name="dateTimeRequested"> a date/time matching the precision of the time series interval, for which values
	/// are to be extracted.  For example, for a daily time series, this is the month and day of the year for
	/// which values should be extracted (the other date/time parts are ignored). </param>
	/// <param name="includeMissing"> if true, include missing values; if false, do not include missing values </param>
	/// <exception cref="InvalidTimeIntervalException"> if other than day or month time series are processed. </exception>
	public static TSData[] toArrayForDateTime(TS ts, DateTime startDate, DateTime endDate, DateTime datetimeRequested, bool includeMissing)
	{ // Get month and data for the extraction
		...
		if ((intervalBase != TimeInterval.YEAR) && (intervalBase != TimeInterval.MONTH) && (intervalBase != TimeInterval.DAY) && (intervalBase != TimeInterval.HOUR) && (intervalBase != TimeInterval.MINUTE))
		{
			throw new InvalidTimeIntervalException("Only Year, Month, Day, Hour, and Minute time series can be processed.  Cannot process interval " + ts.getIdentifier().getInterval() + ".");
		}
		/*
		if ( intervalMult != 1 ) {
		...
		else
		{
			...
			bool processValue; // whether to process the data value
			for (; date.lessThanOrEqualTo(end); date.addInterval(intervalBase, intervalMult))
			{
				month = date.getMonth();
				day = date.getDay();
				...
				else if ((intervalBase == TimeInterval.DAY) && (month == monthRequested) && (day == dayRequested))
				{
					// Month and day must agree...
					processValue = true;
				}
				else if ((intervalBase == TimeInterval.HOUR) && (month == monthRequested) && (day == dayRequested) && (hour == hourRequested))
				{
					// Month, day, and hour must agree...
					processValue = true;
				}
				else if ((intervalBase == TimeInterval.MINUTE) && (month == monthRequested) && (day == dayRequested) && (hour == hourRequested) && (minute == minuteRequested))
				{
	...
	/// for those values.  If the start date or end date are null, the start and end
	/// dates of the time series are used. </summary>
	/// <returns> The array of data for to time series. </returns>
	/// <param name="ts"> Time series to convert data to array format. </param>
	/// <param name="start_date"> Date corresponding to the first date of the returned array. </param>
	/// <param name="end_date"> Date corresponding to the last date of the returned array. </param>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static double[] toArrayNoMissing(TS ts, RTi.Util.Time.DateTime start_date, RTi.Util.Time.DateTime end_date) throws RTi.Util.Time.InvalidTimeIntervalException
	...
	/// <summary>
	/// Returns the first editable Time Series in a list </summary>
	/// <param name="tslist"> A list of TS (Time Series)
	/// @return </param>
	public static TS getFirstEditableTS(IList<TS> tslist)
	{
	  int size = 0;
	  if (tslist != null)
	  {
